...
Motivation: 传统NeRF在体积渲染时采用密集光线采样，导致计算效率低下。需要一种自适应采样方法，能够根据场景特性智能分配采样点，而不是均匀采样。

Method: 1. 将采样建模为马尔可夫决策过程，使用SAC强化学习算法学习采样策略；2. 引入高斯混合分布颜色模型提供不确定性估计；3. 设计多组件奖励函数平衡质量、效率和一致性；4. 采用两阶段训练策略解决环境非平稳性问题。
...
TL;DR: 该研究评估了多种视觉语言模型在帮助盲人和低视力人群导航任务中的表现，发现GPT-4o在空间推理和场景理解方面表现最佳，而开源模型在复杂环境中存在显著局限性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视觉语言模型在辅助盲人和低视力人群导航任务中的潜力，评估现有模型在关键视觉技能方面的能力，为开发更有效的辅助技术提供指导。

Method: 方法包括评估闭源模型（GPT-4V、GPT-4o、Gemini-1.5-Pro、Claude-3.5-Sonnet）和开源模型（Llava-v1.6-mistral、Llava-onevision-qwen），测试其在三个基础视觉技能上的表现：环境障碍物计数、相对空间推理、常识性寻路相关场景理解，并使用针对pBLV设计的提示词模拟实际辅助任务。

Result: 结果显示GPT-4o在所有任务中表现最优，特别是在空间推理和场景理解方面；开源模型在复杂环境中存在困难，普遍问题包括：杂乱环境中物体计数不准确、空间推理存在偏差、过度关注物体细节而忽视空间反馈。

Conclusion: 尽管存在局限性，视觉语言模型在寻路辅助方面具有潜力，但需要更好地与人类反馈对齐并改进空间推理能力。该研究为开发者提供了将VLMs有效集成到辅助技术中的实用见解，同时指出了需要解决的关键限制以增强可用性。
...
Method: 提出eWSI方法，将参数高效微调(PEFT)与多实例学习(MIL)精心集成，实现WSI任务的端到端训练。该方法允许在保持计算效率的同时，针对特定任务调整特征提取器。

Result: 在Camelyon16、TCGA和BRACS数据集的七个WSI级任务上评估。eWSI使用ImageNet特征提取器时，性能匹配或超越使用领域特定特征提取器的MIL方法。使用领域特定特征提取器时，eWSI在大多数情况下进一步提升了分类性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM安全研究主要关注检测和拒绝显式不安全输入，但忽视了上下文安全性——模型需要区分看似相似但安全意图不同的场景

Method: 1) 构建MM-SafetyBench++基准：为每个不安全图像-文本对创建安全对应版本，通过最小修改翻转用户意图但保持上下文含义；2) 提出EchoSafe框架：维护自反记忆库积累和检索安全洞察，将相关历史经验整合到当前提示中

Result: 在多个多模态安全基准测试上的广泛实验表明，EchoSafe始终实现卓越性能，为推进MLLM的上下文安全性建立了强大基线

Conclusion: MM-SafetyBench++为上下文安全评估提供了精心设计的基准，EchoSafe通过自反记忆库实现了上下文感知推理和安全行为的持续演化，显著提升了MLLM的上下文安全能力
...
TL;DR: MATCH是一种多视角高斯配准方法，用于高质量头部虚拟形象创建和编辑，通过端到端Transformer模型在0.5秒内直接从校准多视角图像预测对应的高斯溅射纹理，无需数据预处理。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的多视角头部虚拟形象方法需要耗时的头部跟踪和昂贵的虚拟形象优化，总创建时间通常超过一天。需要一种更快速、无需预处理的方法来创建高质量的头部虚拟形象。

Method: 提出基于Transformer的端到端模型，在模板网格的固定UV布局中预测高斯溅射纹理。引入新颖的配准引导注意力块，每个UV贴图标记仅关注描绘其对应网格区域的图像标记，相比密集跨视角注意力提高了效率和性能。

Result: MATCH在新视角合成、几何配准和头部虚拟形象生成方面优于现有方法，使虚拟形象创建速度比最接近的竞争基线快10倍（0.5秒/帧 vs 传统方法的一天以上）。

Conclusion: MATCH通过端到端学习跨帧和跨主体的对应关系，实现了快速个性化的头部虚拟形象创建，并支持表情转移、免优化跟踪、语义编辑和身份插值等应用，显著提升了头部虚拟形象创建的效率和质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的端到端MV-MOT方法虽然实现了高跟踪精度，但缺乏原则性的不确定性建模，且与训练配置紧密耦合，限制了在不同传感器布局、模态或数据集上的泛化能力，需要重新训练才能适应新场景。

Method: 1. 将学习限制在检测和特征提取阶段；2. 将每个传感器输出简化为校准的位置-协方差对(z,R)；3. 通过跨视角聚类和精度加权融合得到统一估计(ẑ,Ŕ)；4. 使用反馈耦合、身份感知的GM-PHD滤波器配合HMM运动模式进行身份分配和时序跟踪。

Result: 在WildTrack数据集上达到95.5 IDF1和91.4 MOTA，超越所有先前模块化方法21分以上，与最先进的端到端方法相当。同一跟踪器核心可无缝迁移到MultiviewX和RadarScenes数据集，仅需替换感知模块即可扩展到新领域和传感器模态。

Conclusion: ModTrack通过模块化设计实现了与端到端方法相当的跟踪性能，同时提供了跨模态泛化能力、传感器无关性和可追踪的不确定性，解决了现有方法在部署灵活性方面的限制。
...
TL;DR: ConflictAwareAH是一个多模态框架，通过提取视频、音频和文本表示，利用模态间冲突特征（嵌入的绝对差异）来检测矛盾/犹豫状态，同时通过文本引导的后期融合策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 矛盾/犹豫状态在临床环境中具有重要价值，但机器识别困难，因为关键证据存在于不同模态（语言、声音、面部表情）之间的不一致性中。现有文本主导的方法倾向于过度检测矛盾/犹豫状态，难以确认其不存在。

Method: 使用三个预训练编码器提取视频、音频和文本表示；通过成对模态嵌入的逐元素绝对差异作为冲突特征；采用文本引导的后期融合策略，将纯文本辅助头与完整模型在推理时融合。

Result: 在BAH数据集上达到0.694 Macro F1（标注测试集）和0.715（私有排行榜），比已发表的多模态基线提升超过10个点；F1-NoAH比纯文本方法提升4.6个点，将类别性能差距减半；文本引导后期融合额外增加4.1 Macro F1。

Conclusion: ConflictAwareAH通过显式建模模态间冲突特征，有效解决了矛盾/犹豫状态检测中的关键挑战，显著提升了性能，特别是在确认非矛盾状态方面，为临床环境中的微妙情感状态识别提供了有效解决方案。
...
TL;DR: 该论文提出AdaRAG-CT框架，通过自适应检索增强解决3D CT报告生成中的病理覆盖不完整问题，在CT-RATE基准上实现了最先进的临床效果。


<details>
  <summary>Details</summary>
Motivation: 现有3D CT自动报告生成系统存在病理覆盖不完整的问题。研究发现这是由于对比学习得到的3D CT嵌入存在严重的维度集中现象（512维中仅有2个有效维度），形成了视觉表示瓶颈，限制了生成和检索性能。

Method: 提出AdaRAG-CT框架，通过受控检索引入补充文本信息，并在生成过程中选择性集成这些信息，以补偿视觉表示瓶颈。框架包含检索和生成两个关键组件。

Result: 在CT-RATE基准测试中，AdaRAG-CT将Clinical F1从0.420（CT-Agent）提升到0.480（+6个点），达到最先进的临床效果。消融研究证实检索和生成组件都对性能提升有贡献。

Conclusion: 3D CT报告生成的瓶颈在于视觉表示而非语言模型，通过自适应检索增强可以显著改善病理覆盖和临床效果。该方法为医学影像报告生成提供了新的解决方案。
...
TL;DR: FEEL数据集首次大规模结合力测量与自我中心视频，包含约300万帧厨房环境中的自然操作数据，45%帧涉及手物接触，用于物理动作理解任务。


<details>
  <summary>Details</summary>
Motivation: 力是驱动物理交互的根本原因，是物理动作理解的关键基础要素。现有数据集缺乏力测量与自我中心视频的同步配对，限制了物理交互理解能力的发展。

Method: 开发定制压阻手套收集力测量数据，与自我中心视频同步配对构建FEEL数据集。应用该数据集于两类任务：1) 接触理解（时间接触分割和像素级接触物体分割）；2) 动作表示学习（以力预测作为自监督预训练目标）。
...
Conclusion: FEEL数据集证明了力测量对于物理动作理解的重要性，为接触理解和动作表示学习提供了有效的数据基础，推动了物理交互理解研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 在3D医学形状中分离病理变化与生理衰老对于开发可解释生物标志物和患者分层至关重要。当诊断标签有限或不可用时，这种分离具有挑战性，因为疾病和衰老常产生重叠的形状变化效应，掩盖了临床相关的形状模式。

Method: 提出两阶段框架：第一阶段使用带符号距离函数的隐式神经模型学习稳定形状嵌入，在潜在空间进行聚类获得伪疾病标签；第二阶段在紧凑变分空间中使用伪疾病标签和真实年龄标签解耦因子，通过结合协方差和监督对比损失的多目标解耦损失强制分离和可控性。

Result: 在ADNI海马体和OAI远端股骨形状上，达到接近监督方法的性能，在解耦和重建方面优于最先进的无监督基线，同时实现高保真重建、可控合成和基于因子的可解释性。

Conclusion: 该框架能够在缺乏诊断标签的情况下有效分离病理变化与生理衰老，为医学形状分析提供了无监督疾病发现和可解释解耦的新方法，有助于开发临床相关的生物标志物。
...
### [11] [Volumetrically Consistent Implicit Atlas Learning via Neural Diffeomorphic Flow for Placenta MRI](https://arxiv.org/abs/2603.16078)
...
<details>
  <summary>Details</summary>
Motivation: 现有隐式配准方法主要依赖零水平集附近的监督，仅捕获表面对应关系，内部变形约束不足，无法满足解剖形状的密集体积对应需求

Method: 结合符号距离函数重建与神经微分同胚流，学习共享规范模板；通过雅可比行列式和双调和惩罚进行体积正则化，抑制局部折叠并促进全局一致变形

Result: 在活体胎盘MRI扫描实验中，相比基于表面的隐式基线方法，在几何保真度和体积对齐方面表现更优，产生解剖可解释且拓扑一致的扁平化结果

Conclusion: 该方法能够统一重建个体胎盘、对齐到群体衍生隐式模板，并在统一规范空间中实现体素级强度映射，适用于群体分析
...
TL;DR: EvoIQA：基于遗传编程的完全可解释符号回归框架，用于生成图像质量评估的显式、人类可读数学公式，在保持可解释性的同时达到与深度学习模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统IQA指标存在两个极端：要么是僵化的手工数学模型，要么是完全缺乏可解释性的"黑盒"深度学习架构。需要在这两者之间架起桥梁，实现既高性能又完全可解释的图像质量评估方法。

Method: 提出EvoIQA框架，基于遗传编程进行符号回归，从VSI、VIF、FSIM和HaarPSI等指标中提取丰富的终端集，演化出显式的、人类可读的数学公式来评估图像质量。

Result: 演化出的GP模型在预测与人类视觉偏好之间表现出强一致性，不仅超越了传统手工指标，而且达到了与DB-CNN等复杂深度学习模型相当的性能水平。

Conclusion: 通过EvoIQA框架，证明了在图像质量评估领域，不再需要为了达到最先进性能而牺牲可解释性，实现了性能与可解释性的双重突破。
...
### [13] [NanoGS: Training-Free Gaussian Splat Simplification](https://arxiv.org/abs/2603.16103)
...
Method: NanoGS将简化问题形式化为稀疏空间图上的局部成对合并。该方法通过质量保持的矩匹配将一对高斯近似为单个基元，并使用基于原始混合分布与其近似之间差异的合并成本评估合并质量。通过将合并候选限制在局部邻域并高效选择兼容对，实现高斯表示的压缩。

Result: NanoGS能显著减少基元数量，同时保持高渲染保真度。该方法可直接处理现有高斯泼溅模型，在CPU上高效运行，并保持标准3DGS参数化，能与现有渲染管线无缝集成。

Conclusion: NanoGS为高斯泼溅简化提供了一个高效实用的解决方案，通过训练无关的轻量级框架解决了现有方法的部署限制问题。
...
TL;DR: NAMD是一个新颖的多模态扩散框架，通过生成1年随访CT图像来预测肺结节进展，结合基线扫描和电子健康记录，显著提升了肺结节恶性预测性能。


<details>
  <summary>Details</summary>
Motivation: 肺癌早期诊断面临生物不确定性和对结节进展机制理解有限的挑战，需要更准确的预测方法来促进早期诊断。

Method: 提出NAMD框架：1）引入结节对齐的潜在空间，潜在向量距离直接对应结节属性变化；2）使用LLM驱动的控制机制，基于患者数据条件化扩散主干；3）结合基线CT扫描和电子健康记录生成1年随访结节图像。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种能够根据几何形状和文本描述自动生成物理准确材质的方法，以简化3D内容创作流程

Method: 使用视频扩散Transformer架构，结合几何和文本条件，通过定制变分自编码器将多种材质属性编码到紧凑潜在空间，实现多模态联合生成

Result: 能够生成高质量、物理合理的材质，包括基础颜色、粗糙度、金属性、高度图等属性，兼容常用内容创作工具

Conclusion: 该方法成功实现了基于几何和文本描述的物理材质生成，通过紧凑潜在空间编码解决了多模态联合生成的问题
...
### [16] [Towards Fair and Robust Volumetric CT Classification via KL-Regularised Group Distributionally Robust Optimisation](https://arxiv.org/abs/2603.15941)
*Samuel Johnny,Blessed Guda,Frank Ebeledike,Goodness Obasi,Moise Busogi*

Main category: cs.CV

TL;DR: 该论文提出了一种结合MobileViT-XXS切片编码器、SliceTransformer聚合器和KL正则化Group DRO的框架，用于解决胸部CT扫描自动诊断中的分布偏移和人口亚组性能差异问题，在两个任务上均显著超越了现有最佳结果。


<details>
  <summary>Details</summary>
Motivation: 胸部CT扫描自动诊断在临床部署中面临两个持续挑战：采集站点间的分布偏移和人口亚组间的性能差异。这些挑战限制了模型在不同医疗机构和不同人口群体中的泛化能力和公平性。

Method: 提出一个结合轻量级MobileViT-XXS切片编码器和两层SliceTransformer聚合器的框架，用于体积推理。采用KL正则化的Group Distributionally Robust Optimisation（Group DRO）目标进行训练，该目标自适应地提升表现不佳的采集中心和人口亚组的权重。KL惩罚防止组权重崩溃，在保护最差情况和平均性能之间提供稳定平衡。在任务2中，按性别类别定义组，直接针对严重代表性不足的组合（如女性鳞状细胞癌）。

Result: 在任务1（多站点CT体积的二元COVID-19分类）中，最佳配置达到挑战F1分数0.835，比已发表的最佳挑战结果提升+5.9。在任务2（基于性别公平约束的四类肺病理识别）中，α=0.5的Group DRO达到平均每性别宏观F1分数0.815，比最佳挑战结果提升+11.1个百分点，并将女性鳞状细胞癌的F1分数比Focal Loss基线提升+17.4。

Conclusion: 提出的KL正则化Group DRO框架能有效同时解决胸部CT诊断中的分布偏移和性能差异问题，在保持平均性能的同时提升最差情况保护，显著改善了模型在不同采集站点和人口亚组间的泛化能力和公平性。
...
TL;DR: ATV-Pruning：一种针对大型视觉语言模型的不对称文本-视觉权重剪枝方法，通过解耦模态特定行为并自适应选择信息性标记来提升剪枝精度。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM剪枝方法通常以统一方式处理多模态校准数据，忽略了模态特定行为，导致无法准确处理文本和视觉标记的差异性。需要解决文本和视觉标记在剪枝中的不同行为特征。

Method: 提出ATV-Pruning方法：1）通过解耦文本和视觉路径的权重，系统分析两者对剪枝的敏感性；2）自适应构建校准池，包含所有文本标记和部分视觉标记；3）设计层自适应选择策略来筛选重要视觉标记；4）建立准确的权重重要性度量。

Result: 实验发现：1）文本路径对剪枝更敏感，应通过文本标记校准；2）视觉路径冗余度高，可承受50%稀疏度；3）在标准多模态基准测试中，ATV-Pruning优于现有最先进方法。

Conclusion: ATV-Pruning通过解耦模态特定行为并自适应选择信息性标记，有效解决了LVLM剪枝中文本和视觉标记的差异性挑战，实现了更准确的权重剪枝。
...
TL;DR: FlatLands是一个用于单视角鸟瞰图地板补全的数据集和基准测试，包含来自17,656个真实室内场景的270,575个观测样本，提供对齐的观测、可见性、有效性和地面真实BEV地图。


<details>
  <summary>Details</summary>
Motivation: 单个自我中心图像通常只捕捉地板的一小部分，而完整的可通行性地图对室内导航等应用更有价值。需要解决单视角BEV地板补全问题。

Method: 创建了包含大量真实室内场景的数据集，提供对齐的多模态数据。比较了免训练方法、确定性模型、集成方法和随机生成模型。将任务实例化为端到端的单目RGB到地板地图的管道。

Result: 建立了包含270,575个观测样本的全面数据集，来自17,656个真实室内场景，覆盖六个现有数据集。提供了室内和室外分布评估协议，为不确定性感知室内地图和生成式补全提供了严格测试平台。

Conclusion: FlatLands为不确定性感知室内地图和生成式补全提供了严谨的测试平台，特别适用于具身导航应用，填补了单视角BEV地板补全领域的空白。
...
<details>
  <summary>Details</summary>
Motivation: 视觉Transformer基础模型性能优异但存在二次复杂度问题，现有线性注意力方法需要从头训练计算成本高，而基于线性化的方法从大语言模型迁移到ViT效果不佳

Method: 三阶段框架：1)注意力对齐：将普通线性注意力与原始softmax注意力对齐；2)特征对齐：微调线性化ViT使其最终层特征与冻结的softmax VFM教师对齐；3)监督微调：将适应的先验知识迁移到下游任务

Result: 在分类和分割任务上的大量实验证明ViT-AdaLA优于各种最先进的线性注意力方法，展示了其有效性和通用性

Conclusion: ViT-AdaLA提供了一种有效的方法，将视觉基础模型的知识迁移到线性注意力ViT中，解决了线性注意力训练困难和性能下降的问题，为高效视觉模型的发展提供了新思路
...
TL;DR: USU提出了一种语义感知的上采样方法，专门用于解释性AI中的归因图，解决了传统插值方法导致的混叠、振铃和边界渗漏问题，通过质量重分布算子保持归因质量和相对重要性顺序。


<details>
  <summary>Details</summary>
Motivation: 传统归因方法使用为自然图像设计的双线性和双三次插值技术，这些方法会系统性地破坏归因信号，产生虚假的高重要性区域，误导模型推理的解释。核心问题在于将归因上采样视为与模型推理隔离的插值问题，而非受模型语义边界控制的质量重分布问题。

Method: 提出通用语义感知上采样（USU），通过比例形式的质量重分布算子重新定义上采样，可证明地保持归因质量和相对重要性顺序。基于特征归因的公理化传统，形式化了四个忠实上采样的期望条件，并证明插值方法在结构上违反了其中三个条件。

Result: 在具有已知归因先验的模型上进行受控实验验证了USU的形式保证；在ImageNet、CIFAR-10和CUB-200上的评估证实了一致性的忠实性改进和定性上更优、语义更连贯的解释。

Conclusion: USU为归因上采样提供了一个原则性方法，通过将上采样重新定义为受模型语义边界控制的质量重分布问题，解决了传统插值方法的系统性问题，产生了更忠实、语义更连贯的解释。
...
TL;DR: 提出了一种用于驾驶场景解析的无监督域自适应框架，通过显式正则化语义特征结构来缩小合成到真实域的差距，显著提升真实场景下的解析性能。


<details>
  <summary>Details</summary>
Motivation: 驾驶场景解析对自动驾驶至关重要，但像素级标注成本高昂。合成数据集虽能自动生成标签，但存在合成到真实域的差距。现有无监督域自适应方法主要关注全局特征对齐，忽略了特征空间的语义结构，导致类间语义关系建模不足，限制了模型的泛化能力。

Method: 提出一种新颖的无监督域自适应框架，通过类特定原型强制实现类间分离和类内紧凑性，增强特征簇的可区分性和结构一致性。采用基于熵的噪声过滤策略提高伪标签可靠性，并结合像素级注意力机制进一步细化特征对齐。

Result: 在代表性基准测试上的广泛实验表明，该方法持续优于最新的最先进方法，验证了在驾驶场景解析任务中保持语义结构对鲁棒的合成到真实域自适应的重要性。

Conclusion: 该研究强调了在无监督域自适应中显式正则化语义特征结构的重要性，提出的框架通过增强特征空间的判别性和结构一致性，显著提升了驾驶场景解析在真实场景中的性能，为合成到真实域自适应提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态上下文学习中增加演示示例数量能提升性能，但Transformer注意力机制的二次计算成本导致推理延迟显著增加，需要在准确性和效率之间进行权衡

Method: 提出Parallel-ICL算法：1) 将长演示上下文划分为多个较短的可管理块；2) 并行处理这些块；3) 在logit级别使用加权Product-of-Experts集成整合预测，近似完整上下文输出。基于集成学习理论引入两种策略：聚类分块最大化块间多样性，相似性编译基于查询相关性加权预测
...
Method: 构建包含1,550,244个多层图形设计组合的数据集，每个设计表示为包含文本、图像、矢量、组元素的分层组合，附带丰富的元素元数据；数据集涵盖20个设计类别和971,850个独特模板

Result: 创建了大规模结构化图形设计数据集，包含静态设计和27,261个动画布局，支持层感知修复、结构化布局生成、可控设计编辑和时间感知生成建模等新研究任务

Conclusion: LICA通过将设计表示为组合层和关系的系统，为直接操作设计结构而非仅像素的模型研究提供了新范式，推动了图形设计领域的结构化理解和生成研究
...
TL;DR: OneWorld是一个直接在3D表示空间进行扩散的框架，通过3D统一表示自编码器和交叉视图一致性损失，解决了现有2D潜在空间方法在跨视图外观和几何一致性方面的挑战。
...
Method: 提出了OneWorld框架，核心是3D统一表示自编码器（3D-URAE），利用预训练的3D基础模型并通过注入外观和提取语义来增强其几何中心特性。引入令牌级交叉视图对应（CVC）一致性损失来显式强制跨视图结构对齐，并提出流形漂移强制（MDF）来减轻训练-推理暴露偏差并通过混合漂移和原始表示来塑造稳健的3D流形。

Result: 综合实验表明，OneWorld能够生成高质量的3D场景，与最先进的2D方法相比，具有更优越的跨视图一致性。

Conclusion: OneWorld通过在连贯的3D表示空间中直接进行扩散，有效解决了现有方法在跨视图一致性方面的局限性，为3D场景生成提供了更一致和高质量的解决方案。
...
TL;DR: 该研究质疑CLIP类对比语言-图像训练模型在纯图像任务中表现不佳的"模态内错位"假说，通过理论分析和实验证明该假说不成立，指出任务模糊性才是影响性能的关键因素。


<details>
  <summary>Details</summary>
Motivation: 针对当前研究认为CLIP类对比语言-图像训练产生的嵌入在纯图像任务中表现不佳，主要归因于跨模态对齐损失忽略了模态内对齐，导致图像间距离校准不良。本研究质疑这一模态内错位假说，重新审视其理论基础和实证证据。

Method: 1) 理论分析：证明图像嵌入距离不存在假说所声称的自由度；2) 实证比较：对比语言-图像训练模型（CLIP、SigLIP）和图像-图像训练模型（DINO、SigLIP2）在相同指标上的表现；3) 实验验证：在检索和少样本分类等模态内任务上测试，分析任务模糊性的影响。

Result: 1) 理论分析表明模态内错位假说的理论基础不成立；2) 实证指标显示语言-图像训练模型和图像-图像训练模型产生相似结果，说明观察到的现象并非语言-图像训练特有；3) 模态内任务实验证实解决任务模糊性而非假设的错位才是获得最佳结果的关键。

Conclusion: CLIP类模型在纯图像任务中表现不佳的原因并非模态内错位，而是任务模糊性问题。研究推翻了现有理论假说，为理解对比语言-图像训练模型的特性提供了新视角。
...
TL;DR: PathGLS是一个无参考的病理学视觉语言模型评估框架，通过Grounding（细粒度视觉-文本对齐）、Logic（自然语言推理的蕴含图一致性）和Stability（对抗性视觉语义扰动下的输出方差）三个维度综合评估模型性能，特别擅长检测幻觉错误。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在计算病理学中具有重要潜力，但由于缺乏能够检测幻觉等细微错误的可靠自动化评估指标，其临床广泛应用受到限制。现有评估方法无法有效识别模型输出中的幻觉问题。

Method: 提出PathGLS框架，从三个维度评估病理学视觉语言模型：1) Grounding：评估细粒度的视觉-文本对齐；2) Logic：使用自然语言推理构建蕴含图检查逻辑一致性；3) Stability：通过对抗性视觉语义扰动测试输出稳定性。框架支持补丁级和全切片图像级分析，生成综合信任分数。

Result: 在Quilt-1M、TCGA、REG2025、PathMMU和TCGA-Sarcoma数据集上的实验表明PathGLS优于现有方法。在Quilt-1M上，PathGLS对幻觉报告的敏感性下降达40.2%，而BERTScore仅为2.1%。与专家定义的临床错误层次验证显示，PathGLS达到Spearman秩相关系数ρ=0.71，显著优于基于大语言模型的方法（Gemini 3.0 Pro：ρ=0.39）。

Conclusion: PathGLS是一个稳健的无参考评估指标，能够直接量化幻觉率和领域偏移鲁棒性，为在私有临床数据集上基准测试视觉语言模型和指导安全部署提供了可靠标准。
...
TL;DR: SynOE-OD：基于合成异常暴露的物体检测框架，利用生成模型和开放词汇检测器生成语义有意义的异常数据，提升检测器对分布外物体的检测能力


<details>
  <summary>Details</summary>
Motivation: 当前物体检测器在处理分布外（OOD）物体时存在严重问题，经常将这些异常物体误判为背景而完全忽略。现有OOD检测方法通常依赖复杂架构或辅助分支，缺乏统一处理分布内（ID）和分布外（OOD）物体的框架。

Method: 提出SynOE-OD框架，利用Stable Diffusion等强生成模型和开放词汇物体检测器（OVODs）生成语义有意义的物体级异常数据。通过迁移学习建立强ID任务性能，并补充检测模型的OOD物体检测鲁棒性。

Result: 在已建立的OOD物体检测基准测试中达到最先进的平均精度。像GroundingDINO这样的OVODs在街景中检测OOD物体时表现出有限的零样本性能，而本方法显著提升了这一能力。

Conclusion: SynOE-OD框架成功解决了物体检测器中OOD物体被忽略的问题，通过合成异常暴露方法统一处理ID和OOD物体检测，在街景OOD检测任务中取得了优异性能。
...
TL;DR: QICA是一个新颖的零样本目标计数框架，通过协同数量感知和空间聚合来解决现有方法在细粒度数量意识和空间敏感性方面的不足，在多个数据集上展现了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标计数方法存在三个主要问题：1) 将计数视为粗粒度检索任务，缺乏细粒度数量感知；2) 空间敏感性不足；3) 模型适应过程中特征空间扭曲导致泛化能力下降。这些问题限制了零样本计数的准确性和跨域泛化能力。

Method: QICA框架包含三个核心组件：1) 协同提示策略(SPS)，通过数值条件提示适应视觉和语言编码器，桥接语义识别和定量推理；2) 成本聚合解码器(CAD)，直接在视觉-文本相似度图上操作，通过空间聚合细化特征图，防止过拟合并保持零样本可迁移性；3) 多级数量对齐损失(L_MQA)，在整个流程中强制执行数值一致性。

Result: 在FSC-147数据集上展现了有竞争力的性能，在CARPK和ShanghaiTech-A数据集上的零样本评估验证了其优越的跨域泛化能力，能够有效处理未见过的领域。

Conclusion: QICA通过协同数量感知和空间聚合，有效解决了零样本目标计数中的关键挑战，包括细粒度数量意识、空间敏感性和特征空间扭曲问题，为文本描述驱动的零样本计数提供了有效的解决方案。
...
TL;DR: EPOFusion：一种曝光感知的红外与可见光图像融合模型，专门针对过曝光区域设计，通过引导模块提取红外特征，迭代解码器增强融合质量，自适应损失函数平衡模态，并在首个红外可见光过曝光数据集IVOE上验证效果。


<details>
  <summary>Details</summary>
Motivation: 实际场景中过曝光现象频繁发生，导致关键视觉信息丢失。现有红外与可见光融合方法在高度明亮区域表现不佳，需要专门针对过曝光区域的融合解决方案。

Method: 1. 引入引导模块，帮助编码器从过曝光区域提取细粒度红外特征；2. 设计包含多尺度上下文融合模块的迭代解码器，逐步增强融合图像；3. 采用自适应损失函数动态约束融合过程，在不同曝光条件下平衡模态；4. 构建首个红外可见光过曝光数据集IVOE，包含高质量红外引导的过曝光区域标注。

Result: EPOFusion在过曝光区域保持红外线索，在非过曝光区域实现视觉忠实的融合，在视觉保真度和下游任务性能方面均优于现有方法。

Conclusion: EPOFusion通过曝光感知设计有效解决了红外与可见光图像融合中的过曝光问题，在保持红外信息的同时提升视觉质量，为实际应用场景提供了更可靠的融合方案。
...
TL;DR: DualPrim：一种紧凑结构化3D重建框架，使用正负超二次曲面通过加减法建模，在保持紧凑性和可微性的同时增强表示能力，支持端到端学习并生成易于编辑的结构化网格。
...
Method: DualPrim使用正负超二次曲面表示形状：正超二次曲面构建基础，负超二次曲面通过可微算子雕刻局部体积，实现孔洞和凹面的拓扑感知建模。该加减法设计在不牺牲紧凑性或可微性的情况下增加表示能力。框架嵌入体积可微渲染器，支持从多视角图像进行端到端学习，并通过闭式布尔差实现无缝网格导出。

Result: DualPrim实现了最先进的精度，并产生紧凑、结构化和可解释的输出，比纯加法替代方案更好地满足下游需求。

Conclusion: DualPrim通过加减法超二次曲面表示，在保持紧凑性和可微性的同时增强了表示能力，解决了现有神经重建方法在结构和保真度之间的权衡问题，为下游应用提供了更好的结构化输出。
...
TL;DR: 在类别不平衡的AI训练中，生成模型常被用于数据增强，但低数据条件下的失败模式研究不足。本研究比较了三种增强策略在细粒度动物分类任务中的表现，发现FastGAN在极低训练样本量下不仅效果差，反而会显著增加分类器偏见，而使用LoRA微调的Stable Diffusion表现最佳。


<details>
  <summary>Details</summary>
Motivation: 生成模型广泛用于补偿AI训练中的类别不平衡问题，但对其在低数据条件下的失败模式了解不足。需要系统评估不同生成式增强策略在低数据场景下的表现，特别是它们可能带来的负面影响。

Method: 使用Oxford-IIIT Pet数据集，人工制造8个代表性不足的品种作为少数类。对比三种增强策略：传统变换、FastGAN、以及使用Low-Rank Adaptation微调的Stable Diffusion 1.5。在极低训练集规模下进行控制性基准测试，分析分类器偏见变化，使用t-SNE进行特征嵌入分析，所有实验在消费级GPU上完成。

Result: FastGAN增强在极低训练样本量下不仅表现不佳，反而显著增加分类器偏见（偏见差距增加+20.7%，Cohen's d = +5.03，p = 0.013）。t-SNE分析显示FastGAN为严重少数类生成的图像形成紧密孤立簇，位于真实图像分布之外，符合模式崩溃模式。Stable Diffusion with LoRA表现最佳，获得最高宏F1分数（0.9125 ± 0.0047），偏见差距相对未增强基线减少13.1%。数据表明存在一个样本量边界（每类20-50张训练图像），低于此边界时GAN增强在该设置中变得有害。

Conclusion: 在极低数据条件下，某些生成模型（特别是FastGAN）的数据增强可能对分类器性能产生负面影响，增加偏见而非缓解不平衡。Stable Diffusion结合LoRA微调在低数据增强中表现优异。研究强调了在低数据场景下谨慎选择增强策略的重要性，并指出了需要进一步跨领域研究以确定GAN增强从有益转为有害的精确边界。
...
TL;DR: IOMM提出一种数据高效的两阶段训练框架，通过仅使用未标注图像数据进行视觉生成组件预训练，减少对稀缺文本-图像配对数据的依赖，显著提升训练效率并达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型（UMMs）的视觉生成组件预训练通常受限于低效的训练范式和对稀缺高质量文本-图像配对数据的依赖，这两个问题成为主要瓶颈。

Method: 提出IOMM（仅图像训练）两阶段框架：第一阶段仅使用丰富的未标注图像数据预训练视觉生成组件，消除对配对数据的依赖；第二阶段使用未标注图像和少量精选文本-图像配对数据进行微调，提升指令对齐和生成质量。
...
Conclusion: IOMM不仅提高了训练效率，还实现了最先进的性能，证明了仅使用未标注图像数据预训练视觉生成组件的有效性，为统一多模态模型的训练提供了更高效、数据需求更低的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的灵巧抓取生成方法通常采用随机微分方程（SDE）公式，需要大量去噪步骤且存在轨迹不稳定性，可能导致物理不可行的抓取。需要更高效、稳定且物理可行的抓取生成方法。

Method: 1. 将抓取合成重新定义为确定性常微分方程（ODE）过程，通过平滑概率流实现高效稳定生成；2. 引入免训练的物理感知能量引导策略，使用适应的显式物理能量函数定义能量引导目标分布，通过局部蒙特卡洛近似估计引导项，在推理过程中动态引导生成轨迹朝向物理可行区域。

Result: 在五个基准数据集上的广泛实验表明，EFF-Grasp在抓取质量和物理可行性方面实现了优越性能，同时比基于扩散的基线方法需要更少的采样步骤。

Conclusion: EFF-Grasp通过流匹配框架和物理感知能量引导，提供了一种高效、稳定且物理可行的灵巧抓取生成方法，克服了现有扩散方法需要大量采样步骤和轨迹不稳定的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D点云视频理解方法面临两个主要挑战：1）不同帧率下的时序尺度偏差；2）不规则点云中的分布不确定性。现有CNN或Transformer方法要么受限于有限感受野，要么计算复杂度高，且忽略了这些隐式失真。

Method: 提出GATS双不变性框架，包含两个互补模块：1）不确定性引导高斯卷积（UGGC），将局部高斯统计和不确定性感知门控融入点卷积，在密度变化、噪声和遮挡下实现鲁棒的邻域聚合；2）时序缩放注意力（TSA），引入可学习缩放因子归一化时序距离，确保帧划分不变性和跨不同帧率的速度估计一致性。

Result: 在主流基准测试中取得显著性能提升：MSR-Action3D准确率提升+6.62%，NTU RGBD准确率提升+1.4%，Synthia4D mIoU提升+1.8%。相比Transformer方法，提供了更高效、更具原则性的不变性4D点云视频理解范式。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展带来了能够制作出版质量科学图像的新工具，但学术出版商对此制定了不一致且模糊的政策。需要明确当前政策现状，解决AI生成图像在学术出版中的合规性问题。

Method: 调查了Nature、Science、Cell Press、Elsevier、PLOS等主要期刊和出版商的AI图像政策，识别了出版商的关键关切点，并基于SciDraw等AI科学插图平台的实际案例，提出了最佳实践指南。

Result: 发现出版商主要关注可重复性、作者归属和视觉错误信息等关键问题。通过适当的披露和质量控制，AI生成的图像可以在不损害学术诚信的前提下加速科学传播。

Conclusion: AI生成图像在学术出版中具有潜力，但需要建立透明、合规的使用框架。提出的最佳实践指南有助于研究人员在保持学术诚信的同时利用AI工具加速科学传播。
...
TL;DR: 提出一种无监督、架构无关的条件化策略，通过预训练核分割基础模型生成的连续细胞概率图作为显式输入先验，结合方差保持正则化项，从亮场IHC图像合成mIF通道，显著提升核计数保真度和感知质量。


<details>
  <summary>Details</summary>
Motivation: 多重免疫荧光（mIF）技术成本高、流程复杂且需要专用成像平台，限制了临床常规应用。现有的虚拟染色方法主要优化像素级保真度，未显式约束核形态，而病理学中核计数、形状或空间排列的细微失真会直接影响Ki67增殖指数等量化终点，导致治疗相关风险分类错误。

Method: 引入无监督、架构无关的条件化策略：1）使用预训练核分割基础模型生成连续细胞概率图作为显式输入先验；2）添加方差保持正则化项，匹配局部强度统计以保持合成荧光通道中的细胞级异质性。软先验保留了二值化阈值处理丢失的梯度级边界信息，无需任务特定调优。

Result: 在Pix2Pix（U-Net和ResNet生成器）、确定性回归U-Net和条件扩散模型上，使用两个独立数据集进行控制实验，仅通过上述修改就实现了核计数保真度和感知质量的一致提升。

Conclusion: 提出的条件化策略通过注入核形态先验和保持细胞级异质性，有效提升了从亮场IHC到mIF的虚拟染色质量，为临床病理量化提供了更可靠的合成荧光通道。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在传统图像理解方面表现出色，但在360°图像感知方面研究不足。360°图像具有全景环境捕捉能力，但也带来几何畸变和复杂空间关系等挑战，需要专门的评估和改进方法。

Method: 1) 提出360Bench基准测试，包含7K分辨率360°图像和7个代表性任务的视觉问答数据集；2) 提出Free360框架，采用训练免费的基于场景图的方法，通过模块化推理步骤、自适应球形图像变换和统一图表示来提升360°图像理解能力。

Result: 系统评估了7个MLLM和6种增强方法，揭示了它们在360°图像感知方面的不足。Free360框架在基准测试中显著提升了基础MLLM的性能，为360°视觉问答任务提供了强大的训练免费解决方案。
...
TL;DR: KidsNanny是一个两阶段多模态内容审核架构，用于儿童安全保护。第一阶段使用视觉Transformer和物体检测器进行视觉筛选（11.7ms），第二阶段通过OCR和7B语言模型进行文本上下文推理（总延迟120ms）。在UnsafeBench数据集上表现优于基线模型，特别是在文本相关威胁检测方面。


<details>
  <summary>Details</summary>
Motivation: 当前儿童安全内容审核面临多模态挑战，特别是需要同时处理视觉内容和嵌入文本的威胁。现有方法要么延迟较高（如ShieldGemma-2和LlavaGuard），要么在文本相关威胁检测上表现不足。需要开发高效的多模态审核架构，平衡准确性和延迟。

Method: 提出两阶段架构：Stage 1使用视觉Transformer（ViT）结合物体检测器进行视觉内容筛选，输出文本描述而非原始像素；Stage 2对Stage 1的输出应用OCR技术，然后使用7B参数的语言模型进行上下文推理。这种设计将视觉处理和文本推理解耦，提高效率。

Result: 在UnsafeBench Sexual类别（1,054张图像）上评估：Stage 1单独达到80.27%准确率和85.39% F1分数（11.7ms），优于视觉基线（59.01%-77.04%）。完整流水线达到81.40%准确率和86.16% F1分数（120ms），相比ShieldGemma-2（64.80%准确率，1,136ms）和LlavaGuard（80.36%准确率，4,138ms）在延迟上有显著优势。在文本相关威胁检测上，KidsNanny在文本专用子集上达到100%召回率和75.76%精确度。

Conclusion: KidsNanny通过两阶段架构实现了高效的多模态内容审核，在保持高准确性的同时大幅降低延迟。基于OCR的文本推理在检测文本嵌入威胁方面具有召回率-精确度优势。该架构为儿童安全内容审核提供了有效的解决方案，但文本专用子集样本量较小限制了结论的一般性。
...
Conclusion: CTRL-S通过思维链机制、高质量数据集和多奖励优化，系统提升了SVG生成的整体能力，为SVG生成任务提供了有效的统一框架。
...
TL;DR: S-VAM是一种快捷视频动作模型，通过单次前向传播预测连贯的几何和语义表示，实现实时推理和高保真预见，显著简化动作预测


<details>
  <summary>Details</summary>
Motivation: 当前视频动作模型要么依赖缓慢的多步视频生成，要么依赖噪声的单步特征提取，无法同时保证实时推理和高保真预见。需要一种既能实时推理又能提供高质量视觉预测的解决方案

Method: 提出S-VAM模型，采用自蒸馏策略将多步去噪的结构化生成先验压缩到单步推理中。使用视觉基础模型从扩散模型自身多步生成的视频中提取表示作为教师目标，轻量级解耦器作为学生直接学习将噪声单步特征映射到这些目标

Result: 在仿真和真实世界的广泛实验中，S-VAM优于现有最先进方法，能够在复杂环境中实现高效精确的操作

Conclusion: S-VAM通过创新的自蒸馏策略实现了实时推理和高保真视觉预测的统一，为机器人学习提供了高效可靠的视频动作模型框架
...
TL;DR: Leveling3D：一种将前馈3D重建与几何一致生成相结合的新方法，通过几何感知适配器修复外推视角中的伪影区域，提升3D重建质量


<details>
  <summary>Details</summary>
Motivation: 现有方法使用扩散模型修复3D重建中的渲染伪影，但缺乏几何考虑，无法有效填充外推视角中的缺失区域。需要一种能同时进行重建和生成的整体解决方案。

Method: 提出Leveling3D管道，包含：1）几何感知水平适配器，将扩散模型内部知识与前馈模型的几何先验对齐；2）调色板过滤策略，用于训练更多样化的生成；3）测试时掩码细化，防止修复区域边界混乱。增强的外推视角可重新输入到3DGS中进行迭代提升。

Result: 在公开数据集上实现了最先进的性能，包括新视角合成和深度估计任务。能够有效修复外推视角中的伪影区域，提升3D重建质量。

Conclusion: Leveling3D成功整合了前馈3D重建与几何一致生成，通过轻量级适配器实现了整体重建与生成，为3D视觉任务提供了强大的基线方法。
...
TL;DR: PureCLIP-Depth是一种完全无提示、无解码器的单目深度估计模型，直接在CLIP嵌入空间中操作，通过学习RGB到深度的直接映射实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度估计模型严重依赖几何特征，本文探索基于概念信息的新方法，在CLIP概念空间中直接计算，避免使用几何特征。

Method: 提出完全无提示、无解码器的架构，直接在CLIP嵌入空间中学习从RGB域到深度域的映射，不依赖几何特征，仅使用概念信息进行计算。

Result: 在室内和室外数据集上，在基于CLIP嵌入的模型中实现了最先进的性能。

Conclusion: PureCLIP-Depth证明了在CLIP概念空间中直接进行深度估计的可行性，为单目深度估计提供了新的概念驱动方法。
...
TL;DR: 提出EDP-SAM和XMask方法，构建半监督人群分析框架，利用实例掩码先验作为伪标签，在稀疏标注下实现人群分割与计数


<details>
  <summary>Details</summary>
Motivation: 传统基于点的标注限制了性能，因为个体区域本身具有模糊性，从稀疏标注中学习细粒度结构语义仍是一个未解决的挑战

Method: 1. 提出基于最近邻排除圆约束的EDP-SAM生成掩码监督；2. 提出XMask通过判别性掩码目标强制空间分离；3. 使用高斯平滑和可微分中心采样策略改善特征连续性和训练稳定性；4. 构建基于实例掩码先验的半监督人群计数框架

Result: 在ShanghaiTech A、UCF-QNRF和JHU++数据集上（使用5%、10%和40%标注数据）验证了端到端模型在半监督分割和计数方面达到最先进性能

Conclusion: 提出的方法有效弥合了计数与实例分割之间的差距，在统一框架内实现了半监督人群分析，利用掩码先验比传统点线索包含更丰富的形状信息
...
TL;DR: DiVA-Former：一种轻量级架构，通过视觉token作为动态查询来整合视觉和文本信息，解决表格理解中模态融合问题，在13个基准测试中显著提升性能
...
Method: 提出DiVA-Former轻量级架构，利用视觉token作为动态查询，将长文本序列蒸馏为可消化的向量，有效利用视觉-文本互补信息。该方法通过视觉引导的注意力机制整合两种模态。

Result: 在13个表格基准测试中，DiVA-Former相比纯文本基线提升23.9%，并在使用视觉输入、文本输入或两者组合的现有基线上实现一致增益。

Conclusion: DiVA-Former通过有效的视觉-文本信息整合机制，解决了表格理解中的模态融合挑战，显著提升了表格相关任务的性能，证明了视觉和文本信息在表格处理中的互补价值。
...
TL;DR: 提出用于白细胞分类的混合框架，解决类别不平衡和领域偏移问题，在WBCBench 2026挑战中取得0.77139的Macro-F1分数


<details>
  <summary>Details</summary>
Motivation: 白细胞自动分类对白血病筛查至关重要，但面临极端类别不平衡、长尾分布和领域偏移的挑战，导致深度学习模型过度拟合主导类别，在罕见亚型上表现不佳

Method: 混合框架包含：1) 基于Pix2Pix的生成式修复模块用于伪影去除；2) Swin Transformer集成与MedSigLIP对比嵌入的鲁棒表示学习；3) 基于几何尖度和马氏距离形态约束的生物启发式精炼步骤，用于恢复分布外预测

Result: 在WBCBench 2026挑战的私有排行榜上获得0.77139的Macro-F1分数，在严重不平衡条件下表现出色

Conclusion: 该方法在极端类别不平衡下实现了罕见类别的泛化，证明了将生物先验知识整合到深度学习中对血液学图像分析的价值
...
TL;DR: SEVEX框架通过自动化语义探索发现任务级视觉提示，解决LVLM感知失败问题，显著提升任务准确性和探索效率。


<details>
  <summary>Details</summary>
Motivation: LVLM在图像理解和视觉推理方面存在显著挑战，导致关键感知失败。现有视觉提示生成方法主要关注工具选择而非诊断和缓解LVLM感知失败的根源问题，且由于LVLM的不透明性和不可预测性，最优视觉提示需要通过经验实验发现，这通常依赖于人工试错。

Method: 提出自动化语义探索框架SEVEX，包括：1）使用抽象概念空间作为搜索空间；2）新颖性引导的选择算法；3）语义反馈驱动的构思过程。该方法通过智能体驱动的实验实现多样化且高效的探索，最小化人工干预，避免逐样本生成的低效性。

Result: 在BlindTest和BLINK基准测试中，SEVEX在任务准确性、推理效率、探索效率和探索稳定性方面显著优于基线方法。该框架发现了超越传统工具使用的复杂且反直觉的视觉策略。

Conclusion: SEVEX为通过自动化、任务级视觉提示增强LVLM感知提供了新范式，能够高效探索多样化视觉提示，解决视觉提示探索中的两大挑战：冗长低级代码的干扰和庞大无结构的搜索空间。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言过程奖励模型（VL-PRMs）作为黑盒评判器，将感知不确定性（图像理解错误）与推理错误混为一谈，导致系统性误判：错误奖励幻觉的视觉前提（假阳性）和惩罚正确的基于图像的陈述（假阴性），这削弱了重排和错误定位的效果

Method: 提出显式视觉前提验证（EVPV）：1）策略模型生成步骤级视觉检查清单，明确所需视觉事实；2）约束提取器从输入图像独立推导结构化视觉约束；3）将检查清单声明与约束匹配，计算标量视觉可靠性信号；4）通过可靠性门控校准PRM步骤奖励：视觉依赖步骤的奖励在可靠性低时衰减，可靠性高时保留。该方法无需每步工具调用即可解耦感知不确定性与逻辑评估

Result: 在VisualProcessBench和六个多模态推理基准上的实验表明，EVPV改善了步骤级验证，并持续提升Best-of-N重排准确性，优于强基线。通过向提取的约束注入受控损坏产生单调性能下降，提供了因果证据表明增益源于约束保真度和显式前提验证，而非偶然的提示效应

Conclusion: EVPV通过显式验证视觉前提，有效解耦了感知不确定性与逻辑评估，提升了多模态推理中过程奖励模型的可靠性和准确性，为视觉语言推理系统提供了更稳健的验证机制
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在训练时简单组合所有天气样本，忽略了不同天气场景间的数据分布差异，导致性能冲突。需要解决恶劣天气下3D目标检测的鲁棒性问题

Method: 1) 集成混合专家系统(MoE)到天气鲁棒的多模态3D目标检测中；2) 图像引导的天气感知路由(IWR)，利用图像特征的强区分能力和场景不变性进行精确天气分类；3) 基于分类结果选择top-K最相关的天气特定专家(WSE)；4) 统一双模态增强(UDMA)，同步增强LiDAR和4D雷达数据同时保持场景真实性

Result: 在真实世界数据集上，AW-MoE相比最先进方法在恶劣天气性能上提升约15%，推理开销可忽略。集成到现有基线检测器中也能获得超越当前SOTA的性能提升

Conclusion: AW-MoE通过有效处理不同天气场景的数据分布差异，显著提升了恶劣天气下的3D目标检测性能，具有强可扩展性和实用性
...
<details>
  <summary>Details</summary>
Motivation: 微手势识别面临类间差异细微的挑战，现有方法依赖类别级监督，难以捕捉局部运动差异，需要更精细的语义指导。

Method: 提出FG-SGL框架：1) FG-SA模块利用细粒度语义线索指导局部运动特征学习；2) CP-A模块通过类别级语义增强特征可分性；3) 构建细粒度文本数据集描述MG动态过程；4) 设计多级对比优化策略进行粗到细联合优化。

Result: 实验表明FG-SGL在微手势识别任务上取得有竞争力的性能，验证了细粒度语义指导的有效性。

Conclusion: 细粒度语义指导能有效帮助视觉-语言模型感知微手势的局部运动差异，提升识别性能，为微手势识别提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在训练过程中缺乏显式的几何监督，导致生成视频中出现物体变形、空间漂移和深度违反等不一致伪影。现有几何度量在像素空间测量不一致性，但像素强度可能引入额外噪声

Method: 1) 提出基于几何的奖励模型，利用预训练几何基础模型计算跨帧重投影误差；2) 采用点对点而非像素空间的误差计算方式，获得更物理基础和鲁棒的误差度量；3) 引入几何感知采样策略，过滤低纹理和非语义区域，专注于几何意义明确且对应关系可靠区域的评估；4) 通过两种互补途径应用奖励模型：双向模型的后训练（SFT或强化学习）和因果视频模型的推理时优化（通过测试时缩放将奖励作为路径验证器）

Result: 实验验证了设计的有效性，表明基于几何的奖励相比其他变体提供更优越的鲁棒性。通过实现高效的推理时缩放，该方法为增强开源视频模型提供了实用解决方案，无需大量计算资源进行重新训练

Conclusion: 提出的基于几何的奖励模型能够有效解决视频扩散模型中的几何不一致问题，通过点对点误差计算和几何感知采样提高了鲁棒性，并通过灵活的部署方式（后训练和推理时优化）为视频生成模型的几何一致性改进提供了实用框架
...
TL;DR: 提出LTS-FS框架，通过定位幻觉相关层并稀疏化特征引导，在保持性能的同时有效缓解大型视觉语言模型的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 现有特征引导方法在所有层上采用均匀引导，忽略了层间差异，可能干扰与幻觉无关的层，导致通用任务性能下降。需要更精细的层特异性引导策略。

Method: 提出LTS-FS框架：1)构建包含token级和句子级幻觉案例的合成数据集；2)基于因果干预的归因方法量化每层的幻觉相关性；3)将归因分数转换为层特异性特征引导强度，实现精确调整

Result: 在多个LVLM和基准测试上的广泛实验表明，LTS-FS框架能有效缓解幻觉，同时保持强大的性能表现

Conclusion: LTS-FS框架通过定位幻觉相关层并实施稀疏化特征引导，实现了更精确的幻觉缓解，为LVLM的可靠部署提供了有效解决方案
...
TL;DR: EverTale：一个用于连续故事角色定制的故事世界模拟器，通过统一的LoRA模块实现连续角色适应，结合MLLM质量门控和区域聚焦采样策略，解决现有方法在角色定制、语义对齐和新身份整合方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前故事可视化方法在准确角色定制、语义对齐和新身份连续整合方面存在不足，无法实现这些要素的协同。现有方法通常需要为每个角色单独优化模块，导致效率低下且难以实现连续的角色适应。

Method: 1. All-in-One-World Character Integrator：通过统一的LoRA模块实现连续角色适应，无需为每个角色单独优化模块；2. Character Quality Gate via MLLM-as-Judge：利用多模态大语言模型作为评判者，通过思维链推理确保角色适应过程的保真度；3. Character-Aware Region-Focus Sampling：解决多角色视觉叙事中的身份退化和布局冲突问题，通过协调局部角色细节与全局场景上下文实现自然的多角色生成。

Result: 实验结果表明，EverTale在单角色和多角色故事可视化任务上均优于广泛的对比方法，展现出卓越的性能。

Conclusion: EverTale通过统一的角色适应框架、质量门控机制和区域聚焦采样策略，成功解决了故事可视化中角色定制、语义对齐和连续整合的挑战，为连续故事角色定制提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有微表情动作单元检测方法通常从整个面部图像/视频学习AU特征，这与AU固有的局部性相冲突，导致对AU区域感知不足。实际上，每个AU独立对应特定的局部面部肌肉运动（局部独立性），而在特定情绪状态下某些AU之间存在固有的依赖关系（全局依赖性）。

Method: 提出Micro-AU CLIP框架，将AU检测过程分解为：1）局部语义独立性建模（LSI），设计Patch Token Attention将AU区域内多个局部特征映射到同一特征空间；2）全局语义依赖性建模（GSD），设计Global Dependency Attention和Global Dependency Loss来建模不同AU间的全局依赖关系；3）设计microAU对比损失（MiAUCL）进行视觉和文本特征的细粒度对齐；4）以无情感标签方式应用于微表情识别。

Result: 实验结果表明，Micro-AU CLIP能够充分学习细粒度的微表情动作单元特征，实现了最先进的性能。

Conclusion: 该论文探索了独立性到依赖性模式的有效性，提出的Micro-AU CLIP框架通过局部独立性和全局依赖性的建模，成功解决了微表情动作单元检测中的细粒度特征学习问题，并在微表情识别任务中取得了优异表现。
...
TL;DR: DriveFix：一种用于自动驾驶场景的多视角修复框架，通过交错扩散Transformer架构确保时空一致性，解决现有4D重建方法中的空间错位和时间漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散先验的4D场景重建方法通常独立处理帧或逐视角处理，缺乏时空协同，导致跨摄像头的空间错位和序列中的时间漂移问题，影响自动驾驶场景的鲁棒建模。

Method: 提出DriveFix多视角修复框架，采用交错扩散Transformer架构，包含专门设计的块来显式建模时间依赖性和跨摄像头空间一致性。通过历史上下文条件生成，并结合几何感知训练损失，确保修复的视角符合统一的3D几何结构。

Result: 在Waymo、nuScenes和PandaSet数据集上的广泛评估表明，DriveFix在重建和新视角合成方面均达到最先进性能，显著减少了伪影，实现了高保真纹理的一致传播。

Conclusion: DriveFix通过确保自动驾驶场景的时空一致性，显著提升了4D世界建模的鲁棒性，为实际部署迈出了重要一步，解决了现有方法中的关键局限性。
...
TL;DR: 该论文开发了一个基于XGBoost的预测模型，利用多组学数据和SHAP解释性分析，结合DeepSeek大语言模型验证生物有效性，为肺癌患者提供个性化治疗方案的药物敏感性预测。


<details>
  <summary>Details</summary>
Motivation: 肺癌传统治疗方法（手术、化疗、放疗）因癌症异质性而效果有限，需要基于患者个体遗传信息和生活方式制定个性化治疗方案。人工智能方法能够分析大数据识别早期癌症迹象、肿瘤类型和治疗前景。
...
Result: 开发了能够预测肺癌药物敏感性的XGBoost模型，通过SHAP分析识别关键分子和细胞特征，DeepSeek提供了关于重要基因或通路的上下文解释，支持模型的可预测性。

Conclusion: 该研究成功整合了机器学习预测模型、可解释性分析(SHAP)和大语言模型验证，为肺癌个性化治疗提供了基于患者特定遗传特征的数据驱动方法，有望改善治疗效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统前馈方法依赖大量训练数据但仍缺失细节，而现有的基于扩散的方法虽然利用了丰富的生成先验，但在合成到真实场景的域迁移方面表现不佳。需要一种既能保留细节、又能从合成场景泛化到真实场景，且在有限训练数据下仍保持高效的方法。

Method: 提出了两阶段的先验到几何确定性（PGD）调度：先验阶段使用谱门控蒸馏（SGD）传输低频真实先验同时保持高频细节不受约束；几何阶段应用谱门控一致性（SGC）强制高频保真度，同时使用合成真值进行细化。两个阶段共享权重，并采用高到低的时间步调度。

Result: 广泛的实验结果表明，Iris在单目深度估计性能上取得了显著改进，并具有强大的野外泛化能力。

Conclusion: Iris框架成功地将真实世界先验整合到扩散模型中，实现了细节保留、从合成到真实场景的强泛化，以及在有限训练数据下的高效性能，为单目深度估计提供了新的确定性解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中基于LiDAR点云的人体理解至关重要，但现有方法忽视了人-物交互的潜力。主要挑战包括：1) 人-物交互导致空间模糊性，在交互区域产生错误的关键点预测；2) 交互与非交互身体部位存在严重的点云数量类别不平衡，手、脚等频繁交互区域在LiDAR数据中稀疏

Method: 提出HOIL框架：1) HOICL（人-物交互感知对比学习）增强人与物点云特征区分度；2) CPPool（接触感知部分引导池化）自适应重新分配表示能力，压缩过表示点同时保留交互部位信息点；3) 可选的时间细化模块利用接触线索跨时间修正关键点估计

Result: HOIL框架有效利用人-物交互解决交互区域的空间模糊性和类别不平衡问题，代码将开源

Conclusion: 通过人-物交互学习框架HOIL，能够更鲁棒地从LiDAR点云进行3D人体姿态估计，解决了现有方法忽视的重要问题
...
<details>
  <summary>Details</summary>
Motivation: 姬蜂总科寄生蜂的准确分类鉴定对生物多样性评估、生态监测和生物防治至关重要，但形态相似、体型小、种间变异细微导致人工鉴定费时费力且依赖专家经验。

Method: 采用基于YOLO的架构集成高分辨率类激活映射（HiResCAM），从高分辨率图像中同时识别蜂类科级分类单元。使用包含3556张膜翅目标本高分辨率图像的数据集，主要涵盖姬蜂科、茧蜂科、蜜蜂科和胡蜂科。

Result: 模型在精确率、召回率、F1分数和准确率等指标上表现优异，准确率超过96%，对形态变异具有稳健的泛化能力。HiResCAM可视化证实模型关注翅脉、触角分节和腹部结构等分类相关解剖区域，验证了学习特征的生物学合理性。

Conclusion: 集成可解释AI技术提高了系统透明度和可信度，适用于昆虫学研究，可加速这一描述不足的寄生蜂超科的生物多样性表征。
...
TL;DR: 提出一种实时水下图像增强框架，通过自适应加权通道补偿、多分支重参数化扩张卷积和统计全局色彩调整模块，在保持轻量化的同时实现准确色彩恢复，仅需3,880个推理参数，达到409 FPS的推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有高性能水下图像增强方法通常依赖复杂架构，难以部署在水下设备上；而轻量级方法往往牺牲质量换取速度，且难以处理严重退化的水下图像。需要一种既能实时运行又能保持高质量色彩恢复的轻量级解决方案。

Method: 1. 自适应加权通道补偿模块：以绿色通道为参考锚点，动态恢复红色和蓝色通道的色彩；2. 多分支重参数化扩张卷积：训练时采用多分支融合，推理时进行结构重参数化，以低计算成本实现大感受野表示；3. 统计全局色彩调整模块：基于统计先验优化整体色彩表现。

Result: 在八个数据集上的实验表明，该方法在七个评估指标上达到最先进性能。模型仅含3,880个推理参数，推理速度达409 FPS。UCIQE分数在不同环境条件下提升29.7%，在ROV平台上的部署和下游任务性能增益进一步验证了其实时水下任务优势。

Conclusion: 该研究提出了一种轻量级实时水下图像增强框架，通过创新的模块设计在保持高效推理的同时实现了准确的色彩恢复，为水下平台的实时视觉任务提供了有效的解决方案。
...
TL;DR: InViC：一种轻量级插件框架，通过提取问题条件化的视觉提示令牌，增强医学视觉问答中的图像证据利用，减少语言先验导致的捷径回答


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在医学VQA中常通过语言先验或数据集偏差进行捷径回答，未能充分关注视觉证据，这降低了临床可靠性，特别是在细微影像发现起决定性作用时

Method: 提出Intent-aware Visual Cues (InViC)框架：1) Cue Tokens Extraction模块将密集视觉令牌蒸馏为K个问题条件化的提示令牌；2) 两阶段微调策略：第一阶段使用提示瓶颈注意力掩码阻断LLM对原始视觉特征的直接访问，强制所有视觉证据通过提示路径；第二阶段恢复标准因果注意力，训练LLM联合利用视觉和提示令牌

Result: 在三个公开Med-VQA基准测试(VQA-RAD、SLAKE、ImageCLEF VQA-Med 2019)上评估，InViC在零样本推理和标准LoRA微调基础上持续改进，证明意图感知视觉提示与瓶颈训练是提高可信赖医学VQA的有效策略

Conclusion: 意图感知视觉提示与瓶颈训练是一种实用有效的策略，可增强医学VQA中基于图像的答案生成，减少捷径回答，提高临床可靠性
...
TL;DR: CIRCLES：通过属性引导的组合图像检索构建反事实示例，提升视觉语言模型的因果推理能力
...
Method: 提出CIRCLES框架，通过属性引导的组合图像检索主动构建反事实风格示例作为演示集，使模型能够隐式推理属性与结果之间的因果关系。

Result: 在四个多样化数据集上的实验表明，CIRCLES在多种架构上一致优于现有方法，特别是在小规模模型上，在信息稀缺条件下表现尤为突出，同时检索到更多样化和具有因果信息的示例。

Conclusion: CIRCLES通过主动构建反事实示例，使视觉语言模型能够超越表面相关性进行更鲁棒和基于因果的推理，为模型如何利用上下文演示改进推理提供了定性见解。
...
TL;DR: 提出SemTok：一种语义一维tokenizer，将2D图像压缩为具有高级语义的1D离散token，在图像重建和下游生成任务中取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉tokenizer主要将图像映射到固定2D空间网格，专注于像素级恢复，这阻碍了捕捉具有紧凑全局语义的表示。需要一种能捕获高级语义的tokenization方法。

Method: 提出SemTok，包含三个关键创新：1) 2D到1D的tokenization方案；2) 语义对齐约束；3) 两阶段生成训练策略。基于此构建掩码自回归生成框架。

Result: SemTok在图像重建方面达到新的SOTA，以极其紧凑的token表示实现卓越保真度。在下游图像生成任务中显著改进，验证了语义1D tokenization的有效性。

Conclusion: SemTok通过语义一维tokenization成功解决了现有视觉tokenizer的局限性，为高效图像表示和生成提供了新范式，代码将开源。
...
TL;DR: DermaFlux：基于整流流的文本到图像生成框架，通过自然语言描述合成皮肤病图像，解决皮肤病分类中数据稀缺和类别不平衡问题，显著提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 皮肤病分类系统受限于大规模、多样化、标注良好的临床数据集稀缺，导致良恶性病变类别不平衡，从而降低了模型的泛化性能。

Method: 基于Flux.1构建DermaFlux框架，使用参数高效的LoRA在大规模公开临床图像数据集上进行微调。通过Llama 3.2生成遵循皮肤病学标准（如不对称性、边界不规则性、颜色变化）的合成文本描述构建图像-文本对。

Result: DermaFlux生成多样且具有临床意义的皮肤病图像，在小规模真实数据集增强时提升二元分类性能达6%，在仅使用合成图像训练时比基于扩散的合成图像提升9%。使用2,500张真实图像和4,375张DermaFlux合成样本训练的ViT达到78.04%准确率和0.859 AUC，超越次优皮肤病模型8%。

Conclusion: DermaFlux通过整流流生成框架有效解决了皮肤病图像数据稀缺问题，生成的合成图像具有临床意义，显著提升了皮肤病分类性能，为医学图像分析提供了有效的数据增强解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统近光源光度立体方法依赖非凸优化，需要良好的初始化和精确的光源标定，计算复杂且对初始条件敏感。本文旨在开发一种无需初始化和完整标定的线性求解方法。

Method: 通过布置多组对称的近光源对，利用对称光源布置的几何特性，推导出表面法线和深度的闭式解。该方法仅要求光源相对于任意点对称分布，即使整个空间偏移未标定也能工作。

Result: 实验表明该方法在形状恢复精度上达到与最先进的标定近光源光度立体方法相当的结果，同时显著降低了对仔细深度初始化和光源标定的要求。

Conclusion: 提出了一种基于对称光源布置的近光源光度立体线性求解框架，实现了无需初始化和完整标定的高精度形状恢复，简化了传统方法的复杂优化过程。
...
<details>
  <summary>Details</summary>
Motivation: 蛋白质标记物和组织病理学图像的联合预后潜力尚未充分探索，主要由于蛋白质表达谱分析成本高且可用性有限。需要一种高效整合组织学和蛋白质特征的方法来改进癌症生存风险预测。

Method: 提出HGP-Mamba框架：1) 蛋白质特征提取器(PFE)利用预训练基础模型直接从WSI生成高通量蛋白质嵌入；2) 结合捕获形态学模式的组织学嵌入；3) 引入局部交互感知Mamba(LiAM)进行细粒度特征交互；4) 全局交互增强Mamba(GiEM)促进幻灯片级别的整体模态融合，捕获复杂的跨模态依赖关系。

Result: 在四个公开癌症数据集上的实验表明，HGP-Mamba实现了最先进的性能，同时与现有方法相比保持了卓越的计算效率。

Conclusion: HGP-Mamba通过从组织病理学图像生成蛋白质特征，有效整合了分子信息，为癌症生存风险预测提供了一种高效的多模态解决方案，克服了蛋白质表达谱分析的高成本和有限可用性问题。
...
TL;DR: SF-Mamba：一种新型视觉Mamba模型，通过辅助补丁交换和批量折叠技术解决现有视觉Mamba模型的非因果交互限制和计算效率问题，在多种视觉任务中超越SOTA基线并提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Mamba模型虽然计算效率高，但存在两个主要问题：1）循环扫描机制限制了图像补丁间的非因果交互；2）在视觉任务常用的短token长度下计算速度较慢。先前工作通过多扫描策略尝试解决，但存在扫描设计不佳和数据重排频繁导致的效率低下问题。

Method: 提出SF-Mamba，包含两个关键技术：1）辅助补丁交换：在单向扫描下编码双向信息流；2）批量折叠与周期性状态重置：实现高级GPU并行化。这些设计重新思考了视觉扫描操作和Mamba的计算效率。

Result: 在图像分类、目标检测、实例分割和语义分割等任务上的广泛实验表明，SF-Mamba在不同模型规模下均显著优于最先进的基线方法，同时提高了吞吐量。

Conclusion: SF-Mamba通过创新的扫描设计和并行化技术，成功解决了视觉Mamba模型的非因果交互限制和计算效率问题，实现了真正高效的视觉编码器，为视觉Transformer的替代方案提供了更优选择。
...
TL;DR: HGFNet：一种用于高光谱图像分类的新型架构，结合3D卷积局部特征提取和傅里叶域全局滤波，通过三种互补的频率变换和自适应焦点损失解决现有方法的可扩展性和类不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱图像分类方法存在两个主要限制：基于Transformer的方法因自注意力二次复杂度导致可扩展性差；基于傅里叶变换的方法通常依赖2D空间FFT，忽略了高光谱数据固有的关键波段间光谱依赖性。需要一种能同时高效建模局部空间-光谱结构和全局依赖性的方法。

Method: 提出Hybrid GFNet (HGFNet)架构，集成局部3D卷积特征提取和GFNet风格块的频域全局滤波。引入三种针对高光谱图像定制的频率变换：光谱傅里叶变换（沿光谱轴的1D FFT）、空间傅里叶变换（空间维度的2D FFT）和空间-空间傅里叶变换（光谱和空间维度的3D FFT）。此外，采用自适应焦点损失(AFL)动态调整类别聚焦和权重，缓解类不平衡问题。

Result: HGFNet能够实现高效稳健的空间-光谱表示学习，通过3D卷积层捕获细粒度局部空间-光谱结构，傅里叶基全局滤波模块高效建模长程依赖并抑制噪声，自适应焦点损失提高了对代表性不足类别的区分能力。

Conclusion: HGFNet通过整合局部3D卷积和频域全局滤波，提供了一种解决高光谱图像分类中可扩展性和光谱依赖性建模挑战的有效方案，同时通过自适应焦点损失缓解了类不平衡问题，实现了全面高效的高维频率建模。
...
Method: 受CLIP启发，提出跨模态自监督协调方法：1）仅使用二元监督信息（图像和剖面是否来自同一颗粒）训练两个模态的编码器；2）结合少量已知物种的标注图库和k-NN分类器进行识别；3）构建能够同时利用图像和剖面信息的多模态识别模型。

Result: 该方法在仅需少量标注图像的情况下实现了高识别准确率，且性能优于仅使用图像的自监督基线方法，证明了跨模态自监督协调的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督物理参数估计方法缺乏通用基准：现有方法在非重叠的合成数据上评估，唯一的真实世界数据集仅限于单体系系统，且没有建立统一的控制方程识别协议。

Method: 创建IRIS基准数据集，包含220个4K 60fps的真实世界视频，涵盖单体和多体动力学系统，提供独立测量的地面真实参数和不确定性估计。定义了标准化评估协议，包括参数准确性、可识别性、外推性、鲁棒性和控制方程选择。评估了多个基线方法，包括多步物理损失公式和四种互补的方程识别策略。

Result: 建立了完整的基准数据集和评估工具包，评估了多种基线方法在所有IRIS场景中的参考性能，揭示了系统性的失败模式，为未来研究提供了方向。

Conclusion: IRIS填补了无监督物理参数估计领域的基准空白，提供了高质量的真实世界数据集和标准化评估协议，为未来研究建立了参考基准并揭示了关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 雨天透过玻璃或挡风玻璃拍摄时，雨滴和反射同时出现会严重降低图像可见度。现有去雨滴、去反射和一体化模型都无法解决这种复合退化问题，需要紧急解决。

Method: 首次正式定义雨滴和反射统一去除任务(UR³)，构建真实拍摄数据集RDRF作为新基准，提出基于扩散模型的DiffUR³框架，利用强大的生成先验，通过特定目标设计成功去除两种退化。

Result: 在RDRF基准测试和具有挑战性的真实场景图像上，DiffUR³方法取得了最先进的性能。RDRF数据集和代码将在论文接受后公开。

Conclusion: 该研究首次系统解决了雨滴和反射复合退化问题，通过定义新任务、构建高质量数据集和开发扩散模型框架，为这一实际问题提供了有效的解决方案。
...
TL;DR: Evo-Retriever：一种基于LLM引导课程演化的检索框架，通过多视角图像对齐和双向对比学习解决视觉语言模型在异构文档检索中的跨模态对齐问题
...
Method: 1. 多视角图像对齐：通过多尺度和多方向视角增强细粒度匹配；2. 双向对比学习：生成"困难查询"并建立互补学习路径进行视觉和文本消歧；3. LLM元控制器：利用专家知识自适应调整训练课程以促进模型演化

Result: 在ViDoRe V2和MMEB (VisDoc)数据集上达到最先进性能，nDCG@5分数分别为65.2%和77.1%

Conclusion: Evo-Retriever通过结合多视角对齐、双向对比学习和LLM引导的课程演化，有效解决了视觉语言模型在异构文档检索中的跨模态对齐问题，显著提升了检索性能
...
TL;DR: GAP-MLLM提出了一种几何对齐的预训练范式，通过视觉提示联合任务和多级渐进融合模块，显式激活MLLM的几何感知能力，显著提升3D空间感知性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在纯RGB输入下表现出优秀的语义推理能力，但3D空间感知能力不足。现有方法通过3D重建模型获取隐式几何先验，但与显式3D数据方法相比仍有性能差距。作者认为这种差距并非源于几何先验不足，而是训练范式不匹配：文本主导的微调未能激活MLLM中的几何表示。

Method: 提出GAP-MLLM几何对齐预训练范式：1）引入视觉提示联合任务，强制MLLM同时预测稀疏点图和语义标签，显式激活几何感知；2）设计多级渐进融合模块，包含令牌级门控机制，实现几何先验的自适应融合而不抑制语义推理。

Result: 大量实验表明，GAP-MLLM显著增强了几何特征融合能力，在3D视觉定位、3D密集描述和3D视频目标检测等任务上均取得一致的性能提升。

Conclusion: 通过几何对齐的预训练范式显式激活MLLM的结构感知能力，能够有效弥补纯RGB输入下3D空间感知的不足，为多模态模型在3D理解任务上的性能提升提供了有效解决方案。
...
TL;DR: UOT-Unlearn：首个用于一步式生成模型的类遗忘框架，基于非平衡最优传输理论，实现目标类别遗忘同时保持整体生成质量


<details>
  <summary>Details</summary>
Motivation: 一步式生成框架（如流映射模型）通过单次前向传递学习噪声到数据的直接映射，显著提高了图像生成效率。然而，确保这些强大生成器安全性的机器遗忘问题尚未被探索。现有的扩散遗忘方法依赖于多步迭代去噪过程，与一步式模型不兼容。

Method: 提出UOT-Unlearn，基于非平衡最优传输（UOT）的即插即用类遗忘框架。将遗忘问题形式化为遗忘成本（抑制目标类别）与f-散度惩罚（通过松弛边际约束保持整体生成保真度）之间的权衡。利用UOT使被遗忘类别的概率质量平滑地重新分配到剩余类别，而非坍缩为低质量或噪声样本。

Result: 在CIFAR-10和ImageNet-256上的实验结果表明，该框架在遗忘成功率（PUL）和保留质量（u-FID）方面表现优异，显著优于基线方法。

Conclusion: UOT-Unlearn是首个针对一步式生成模型的类遗忘框架，通过非平衡最优传输理论实现了有效的目标类别遗忘，同时保持了整体生成质量，填补了该领域的研究空白。
...
TL;DR: VIEW2SPACE：一个基于物理仿真的稀疏多视角推理基准，包含数百万个有根据的问答对，用于评估视觉语言和空间模型在多视角推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现实世界中智能系统需要从稀疏离散的视角理解复杂环境，但现有研究主要集中于单图像或密集视频场景。多视角推理需要整合部分观测信息，而收集大规模带有精确几何和语义标注的多视角数据具有挑战性。

Method: 利用物理仿真构建多样化、高保真的3D场景，生成精确的每视角元数据，创建可扩展的数据生成方法。基于此引擎开发VIEW2SPACE基准，包含可扩展的分离训练集，支持数百万个有根据的问答对。提出Grounded Chain-of-Thought with Visual Evidence方法。

Result: 对最先进的视觉语言和空间模型的全面评估显示，多视角推理问题基本未解决，大多数模型表现仅略高于随机猜测。提出的Grounded Chain-of-Thought方法在中等难度下显著提升性能，并在跨数据集评估中优于现有方法。难度感知的扩展分析表明，几何感知在足够可见度下可从扩展中受益，但跨稀疏视角的深度组合推理仍是根本挑战。

Conclusion: 稀疏多视角推理是一个尚未解决的基本挑战。VIEW2SPACE基准为评估和推进这一领域提供了重要工具。虽然提出的方法在中等难度下取得进展，但跨稀疏视角的深度组合推理仍需进一步研究。物理仿真为可扩展数据生成提供了可行途径。
...
<details>
  <summary>Details</summary>
Motivation: 爆轰研究中长期存在准确提取胞格模式的挑战，现有方法主要依赖手动和原始的2D边缘检测，缺乏精确的3D分割和测量能力。

Method: 提出基于图论的无训练分割算法，使用分割模型从3D压力迹线中提取胞格模式，通过图论工作流程处理3D模拟数据。

Result: 生成数据上的分割预测误差为2%；3D模拟数据显示沿波传播轴排列的椭圆形胞格，偏差为17%；体积的较大离散反映了线性变异性的立方放大。

Conclusion: 该图论框架能泛化到多种胞格几何形状，是爆轰分析的实用工具，为未来三重点碰撞研究奠定了坚实基础，但对高度复杂胞格模式的分割和量化仍具挑战性。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）虽然提供了高质量的可微分渲染，但其位姿优化对初始相机位姿和重建几何非常敏感。研究发现存在两个主要不确定性来源：1）位姿先验不确定性（来自回归或检索模型的单点确定性估计）；2）几何不确定性（由3DGS重建缺陷传播到PnP求解器）。这些不确定性会扭曲重投影几何并破坏优化稳定性。

Method: 提出一个重定位框架，结合蒙特卡洛位姿采样和基于Fisher信息的PnP优化。该方法显式地处理位姿和几何不确定性，无需重新训练或额外监督。通过蒙特卡洛采样处理位姿先验不确定性，使用Fisher信息优化处理几何不确定性。

Result: 在多样化的室内外基准测试中，该方法持续提高了定位精度，并显著增强了在位姿和深度噪声下的稳定性。

Conclusion: 通过显式处理位姿和几何不确定性，提出的框架有效解决了3DGS位姿优化的敏感性问题，提高了视觉定位的鲁棒性和准确性。
...
TL;DR: 使用变分自编码器（VAE）和期望最大化（EM）方法解决扫描透射电子显微镜（STEM）的自动校准问题，通过低维表示学习减少仿真到现实的差距，实现更快速、更准确的参数估计。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜校准面临高维噪声图像、单张图像无法确定最优参数、仿真数据与真实数据存在差距等挑战。现有方法仅提取标量值，无法充分利用图像信息，且仿真到现实的差距影响数据驱动方法的性能。

Method: 1. 使用变分自编码器（VAE）在仿真数据上训练，学习图像的低维表示；2. 采用期望最大化（EM）方法联合估计校准参数到编码表示的映射模型以及最优校准参数；3. 利用光学系统的对称性确保联合估计问题的全局可识别性，保证唯一最优解存在。

Result: 在真实STEM上验证，相比现有方法：1. 估计误差减少2倍；2. 需要更少的观测次数；3. 速度显著更快；4. 结果更一致。证明了VAE在图像信息压缩方面的潜力。

Conclusion: VAE-EM框架成功解决了STEM自动校准问题，显著提升了校准效率和准确性。该方法不仅适用于显微镜校准，还可推广到其他存在仿真到现实差距、映射非单射的反问题领域，为数据驱动的仪器校准提供了新思路。
...
TL;DR: 提出基于分割的注意力熵（SAE）方法，利用语义分割在对象级语义空间量化视觉注意力不确定性，用于检测和缓解大视觉语言模型中的物体幻觉问题


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注文本模态，将幻觉归因于过强的语言先验和不足的视觉基础。作者发现视觉模态内部的异常注意力模式也会导致幻觉物体，需要新的方法来检测和缓解这一问题

Method: 提出Segmentation-based Attention Entropy（SAE），利用语义分割在对象级语义空间量化视觉注意力不确定性；基于SAE设计幻觉检测的可靠性评分和SAE引导的注意力调整方法，在推理时修改视觉注意力以缓解幻觉

Result: 在公开基准测试和四足机器人的实际具身多模态场景中，SAE显著减少了物体幻觉，无需额外训练成本，实现了更可信的LVLM驱动感知和决策

Conclusion: 视觉模态内部的异常注意力模式是导致物体幻觉的重要因素，SAE方法通过量化视觉注意力不确定性和调整注意力模式，有效缓解了幻觉问题，提高了大视觉语言模型的可靠性
...
TL;DR: 基于Transformer的深度学习模型从原始活细胞成像视频预测癌细胞在化疗下的命运，无需预定义特征，准确率达0.94，可提前10小时预测，并提供可解释性分析


<details>
  <summary>Details</summary>
Motivation: 理解非遗传因素对细胞命运的决定机制对癌症治疗至关重要，因为遗传相同的细胞在相同治疗条件下可能产生不同结果。现有方法依赖预定义的形态或分子特征，限制了全面理解细胞决策过程的能力。

Method: 提出基于Transformer的深度学习模型，直接从原始长期活细胞成像序列预测细胞命运，无需预定义特征。引入全面的可解释性框架，包括注意力机制和掩码实验，分析时间动态和形态线索对预测的贡献。

Result: 模型在细胞命运预测上达到0.94的平衡准确率和0.93的F1分数。注意力分析表明预测信号不仅存在于轨迹的最终帧，可靠预测可在事件发生前10小时实现。分析揭示了有丝分裂和凋亡序列中预测信息的不同时间分布，以及细胞形态和p53信号在决定细胞结果中的作用。

Conclusion: 基于注意力的时间模型能够准确预测细胞命运，同时提供对细胞决策过程中非遗传决定因素的生物学可解释性见解。该方法为理解细胞异质性和开发改进的癌症疗法提供了新工具。
...
TL;DR: Face2Scene：一种两阶段图像修复框架，利用人脸作为感知"神谕"来估计退化并引导整个图像的修复


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的人脸修复模型仅关注面部区域，忽略全身和背景的退化；而全场景修复器往往完全忽略退化线索，导致预测不足和视觉伪影

Method: 两阶段框架：1）使用Ref-FR模型从退化图像和身份参考重建高质量面部细节；2）从修复-退化人脸对中提取人脸衍生的退化码，捕获退化属性，将其转换为多尺度退化感知标记，用于条件扩散模型以单步修复全场景

Result: 大量实验证明该方法相比最先进方法具有优越的有效性

Conclusion: Face2Scene通过利用人脸作为退化估计的感知神谕，实现了对包括身体和背景在内的全场景的高质量修复
...
<details>
  <summary>Details</summary>
Motivation: 图像生成模型（IGMs）虽然能创建高保真内容，但也带来了复制受版权内容和生成冒犯性内容的风险。图像生成模型遗忘（IGMU）通过移除有害概念而不需要完全重新训练来缓解这些风险。然而，在对抗性输入（特别是黑盒设置下的图像侧威胁）下的鲁棒性仍未得到充分探索。

Method: REFORGE是一个黑盒红队框架，通过对抗性图像提示评估IGMU的鲁棒性。该方法初始化基于笔触的图像，并使用跨注意力引导的掩码策略优化扰动，该策略将噪声分配到概念相关区域，平衡攻击效果和视觉保真度。

Result: 在代表性遗忘任务和防御措施上的广泛实验表明，REFORGE显著提高了攻击成功率，同时实现了比基线更强的语义对齐和更高的效率。这些结果暴露了当前IGMU方法的持续脆弱性。

Conclusion: REFORGE框架揭示了当前IGMU方法在对抗性输入下的脆弱性，强调了针对多模态对抗攻击开发鲁棒性感知遗忘方法的必要性。
...
TL;DR: 该研究将人脑的共线性视觉感知原理引入计算机视觉，探索其在工业缺陷检测等应用中的潜力，发现对包含直线结构的人造物体检测效果显著，但对ImageNet等自然图像数据集帮助有限。


<details>
  <summary>Details</summary>
Motivation: 人脑的共线性视觉感知原理（增强沿直线排列的边缘）在真实世界中的用途尚不明确，在计算机视觉和工程应用中的利用更是未充分探索。本研究旨在将这一原理转移到计算机视觉领域，探索其在计算机视觉应用中的潜在用途。

Method: 开发了一个原型模型来体现共线性原理，通过四个用例进行系统测试和基准评估：1）结合深度学习进行缺陷检测；2）结合显著性模型进行纳米材料缺陷识别；3）遮挡场景测试；4）ImageNet数据集测试。

Result: 在晶圆缺陷检测中，共线性将错误率从6.5%降低到5.26%，性能提升1.24倍；在纳米材料缺陷识别中，错误率从21.65%降低到6.64%，性能提升3.2倍；在遮挡场景中表现良好；但在ImageNet数据集上效果不明显。共线性对包含直线结构的人造物体检测特别有效。

Conclusion: 共线性原理适用于工业应用，特别是当感兴趣的图像结构是人造的且包含直线时。该研究为计算机视觉提供了新的工具，有望捕捉人类视觉处理的优势。同时明确了共线性适用的场景（晶圆、纳米技术、遮挡）和不适用的场景（ImageNet）。
...
TL;DR: ACPV-Net：首个全类别多边形矢量化框架，从航拍图像一次性生成所有地物类别的多边形矢量图，确保共享边界且无间隙重叠
...
Method: 提出ACPV-Net统一框架，引入语义监督条件机制（SSC）将语义感知与几何基元生成耦合，以及拓扑重建模块，通过设计强制共享边缘一致性。

Result: 在Deventer-512基准测试中，ACPV-Net在保持严格拓扑约束的同时，在所有类别的多边形质量上超越了所有单类别基线方法。在单类别多边形矢量化任务（WHU-Building）上也取得了最佳报告结果。

Conclusion: ACPV-Net成功解决了全类别多边形矢量化问题，实现了从航拍图像一次性生成所有地物类别的拓扑一致多边形表示，为矢量地图生成提供了统一解决方案。
...
TL;DR: TCATSeg：一种结合局部几何特征与全局语义上下文的三维牙科模型语义分割框架，通过稀疏但物理意义明确的超点捕捉全局语义关系，在包含400个牙科模型的新数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 三维牙科模型的精确语义分割对于正畸和种植牙等数字化牙科应用至关重要。现有方法由于复杂的牙齿排列和相邻牙齿形状相似性而难以实现精确分割，主要原因是它们通常关注局部几何特征而忽略了全局上下文信息。

Method: 提出TCATSeg框架，结合局部几何特征与全局语义上下文。引入一组稀疏但物理意义明确的超点来捕捉全局语义关系并增强分割精度。同时构建了包含400个牙科模型（包括正畸前样本）的新数据集来评估方法的泛化能力。

Result: 大量实验表明，TCATSeg在三维牙科模型分割任务上优于现有最先进方法，证明了结合局部几何与全局语义上下文的有效性。

Conclusion: TCATSeg通过有效结合局部几何特征和全局语义上下文，解决了三维牙科模型分割中因复杂牙齿排列和形状相似性带来的挑战，为数字化牙科应用提供了更精确的分割解决方案。
...
TL;DR: 该论文系统分析了多模态大语言模型在无约束人脸验证任务中生成解释的可靠性，发现即使模型做出正确判断，其解释也常依赖不可验证或幻觉的面部属性，缺乏视觉证据支持。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被提出用于生成人脸识别决策的自然语言解释，但这些解释在无约束人脸图像上的可靠性尚未得到充分探索。特别是在极端姿态变化和监控图像等挑战性场景下，需要系统评估这些解释的可信度。

Method: 在具有挑战性的IJB-S数据集上系统分析MLLM生成的无约束人脸验证解释，重点关注极端姿态变化和监控图像。研究传统人脸识别系统信息（分数和决策）对解释的影响。引入基于似然比的框架来评估文本解释的证据强度。

Result: 结果显示，即使MLLM产生正确的验证决策，其解释也经常依赖不可验证或幻觉的面部属性，缺乏视觉证据支持。虽然加入传统人脸识别系统的信息能提高分类验证性能，但并不能始终产生忠实的解释。

Conclusion: 当前MLLM在可解释人脸识别方面存在根本性局限，需要在生物识别应用中建立原则性评估框架来确保解释的可靠性和可信度。基于似然比的评估框架为衡量解释的证据强度提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的组合零样本学习方法存在两个根本性限制：1）隐式组合构建，仅通过令牌拼接或分支提示调优实现组合，缺乏嵌入空间的显式操作；2）残留特征纠缠，不完美的解耦导致属性、对象和组合特征相互污染，限制了模型的泛化能力
...
TL;DR: BUSSARD是一种基于归一化流的模型，用于检测场景图中的异常关系，通过语言模型嵌入对象和关系标记，利用归一化流学习从场景图三元组到简单基分布的双射变换，实现基于似然估计的异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法在场景图关系异常检测方面存在性能不足和效率问题，需要一种能够有效利用语义知识并具有良好泛化能力的检测方法。

Method: 采用多模态方法，使用语言模型嵌入场景图中的对象和关系标记以利用真实世界语义知识；使用归一化流模型学习从对象-关系-对象三元组到高斯基分布的双射变换；通过似然估计进行异常检测。

Result: 在SARD数据集（办公室和餐厅场景）上评估，比当前最先进模型AUROC提升约10%，同时速度快5倍；消融研究显示模型具有优越的鲁棒性和通用性，特别是在同义词使用方面，基线模型出现17.5%的性能偏差时，BUSSARD保持稳定性能。

Conclusion: 基于学习的方法在场景图关系异常检测方面具有强大潜力，BUSSARD通过归一化流和语义嵌入实现了高性能、高效率的异常关系检测。
...
TL;DR: StyleExpert：基于混合专家（MoE）的语义感知风格化框架，能够处理从浅层纹理到深层语义的多样化风格，超越传统颜色驱动的风格化方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的风格化方法主要局限于颜色驱动的变换，忽略了复杂的语义和材质细节。需要开发能够处理多样化语义层次风格的框架。

Method: 提出StyleExpert框架：1）在大规模内容-风格-风格化三元组数据集上训练统一风格编码器；2）将风格嵌入到一致的潜在空间；3）使用相似性感知门控机制动态路由风格到MoE架构中的专业专家；4）利用MoE架构处理从浅层纹理到深层语义的多样化风格。

Result: 大量实验表明，StyleExpert在保持语义和材质细节方面优于现有方法，并能泛化到未见过的风格。

Conclusion: StyleExpert通过语义感知的MoE框架成功解决了现有风格化方法的局限性，能够处理多语义层次的多样化风格，在保持细节和泛化能力方面表现优异。
...
TL;DR: 提出基于深度学习的巢管监测方法，使用约束假阳性损失（CFPL）策略减少标注工作量并缓解类别不平衡问题，有效检测和分类野生蜂类巢室。


<details>
  <summary>Details</summary>
Motivation: 层叠式巢管（LTNs）是监测野生蜂类生物多样性的重要工具，但人工评估巢室耗时耗力。现有方法面临密集巢室标注工作量大、类别分布严重不平衡（常见物种远多于稀有物种）的问题，需要高效自动化的解决方案。

Method: 提出深度学习框架用于巢室检测和分类，引入新颖的约束假阳性损失（CFPL）策略：动态屏蔽未标注数据的预测，防止其对分类损失产生干扰；训练集限制每类最多300个标注以减少工作量；在712张LTN图像数据集上评估，涵盖28个细粒度类别（分类学和巢室状态）。

Result: 实验证明深度学习能有效检测LTN中的巢室；CFPL方法进一步提升了性能，在模型准确性和标注工作量之间取得平衡，同时缓解了类别不平衡问题。

Conclusion: CFPL策略通过处理未标注数据和类别不平衡，显著减少了标注工作量，使深度学习成为监测野生蜂类巢室的有效工具，为生物多样性研究和保护提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉语言模型（如CLIP）在下游任务适配时采用"一刀切"的架构方法，对视觉和文本token进行统一处理，忽略了模态间的结构差异：图像的空间局部性与文本的语义密度。这种同质性处理限制了适配效果。

Method: 提出HeBA（异构瓶颈适配器）框架，包含三个关键创新：1）异构性：视觉token通过2D深度可分离卷积处理以保持空间相关性，文本token通过密集线性投影处理以捕捉语义关系；2）瓶颈正则化：采用压缩瓶颈（D→D/4）强制学习紧凑鲁棒特征；3）主动梯度初始化：使用Kaiming初始化策略而非零初始化，确保足够的初始梯度流以加速收敛。

Result: 在广泛的实验中，HeBA的架构专业化设计实现了优越的稳定性和准确性，在11个少样本基准上建立了新的最先进水平。

Conclusion: HeBA通过引入模态特定的结构归纳偏置，解决了视觉语言模型适配中的同质性问题，其异构设计、瓶颈正则化和主动梯度初始化共同提升了适配性能，为视觉语言模型适配提供了新的架构框架。
...
TL;DR: 提出SPDDA方法解决高光谱图像单源域泛化中数据增强的多样性与真实性权衡问题，通过光谱特性驱动的增强策略提升跨域分类性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像(HSI)具有丰富的光谱信息，但高维度和传感器差异使其对跨域分布差异敏感，影响分类性能。现有单源域泛化方法采用数据增强模拟域偏移，但盲目增强可能产生不现实的样本，而过度强调真实性又会抑制多样性，存在真实性与多样性的权衡限制。

Method: 提出光谱特性驱动的数据增强(SPDDA)：1) 光谱多样性模块：沿光谱维度重采样源域数据生成不同光谱通道数的样本；2) 通道自适应光谱混合器：建模通道间相似性避免固定增强模式；3) 空间-光谱协同优化机制：联合优化空间保真约束和光谱连续性自约束，根据空间约束自适应调整光谱约束权重防止光谱过平滑并保持空间结构。

Result: 在三个遥感基准数据集上的大量实验表明，SPDDA优于现有最先进方法。

Conclusion: SPDDA通过显式考虑高光谱图像固有特性（设备依赖的光谱通道数变化和相邻通道混合），有效解决了数据增强中真实性与多样性的权衡问题，提升了单源域泛化能力。
...
TL;DR: Fast-WAM提出了一种无需在测试时显式预测未来视频的世界动作模型，通过保留训练时的视频协同训练但跳过测试时的未来预测，实现了与现有方法相当的性能和4倍以上的速度提升。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAMs）采用"想象-执行"范式，在测试时需要迭代视频去噪，导致显著的延迟。本文旨在探究WAMs是否真的需要在测试时进行显式的未来想象，还是其主要价值仅在于训练时的视频建模。

Method: 提出Fast-WAM架构，在训练时保留视频协同训练，但在测试时跳过未来预测。通过实例化多个Fast-WAM变体，对训练时的视频建模和测试时的未来预测这两个因素进行受控比较。

Result: Fast-WAM在仿真基准（LIBERO和RoboTwin）和真实世界任务上均取得与最先进方法竞争性的结果，无需具身预训练。运行延迟为190ms，比现有"想象-执行"WAMs快4倍以上。实验表明跳过视频协同训练会导致更大的性能下降。
...
TL;DR: x²-Fusion提出了一种基于事件相机边缘空间的多模态统一表示方法，将图像、LiDAR和事件数据在共享的Event Edge Space中对齐，通过可靠性感知融合和跨维度对比学习联合预测2D光流和3D场景流。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理图像、LiDAR和事件数据时通常使用分离的异构特征空间，导致跨传感器不匹配问题未解决，融合过程复杂。需要一种统一的潜在表示空间来实现多模态对齐。

Method: 1) 利用事件相机提供的时空边缘信号构建Event Edge Space作为统一潜在表示；2) 将图像和LiDAR特征显式对齐到该共享空间；3) 在空间内进行可靠性感知自适应融合，估计模态可靠性并强调稳定线索；4) 使用跨维度对比学习紧密耦合2D光流与3D场景流。

Result: 在合成和真实基准测试中，x²-Fusion在标准条件下达到最先进精度，在具有挑战性的场景下提供显著改进。

Conclusion: 通过将多模态融合重新定义为表示统一，利用事件边缘空间作为锚点，x²-Fusion有效解决了跨传感器不匹配问题，简化了融合复杂性，提升了动态场景理解的性能。
...
TL;DR: HMAR提出了一种基于混合专家架构的自适应医学图像检索框架，通过双专家机制分别处理全局特征和局部病灶特征，无需边界框标注，使用KAN层生成哈希码，在RadioImageNet-CT数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像检索系统存在三个主要问题：1) 统一特征编码无法考虑解剖结构的临床重要性差异；2) 基于粗粒度分类标签的相似性度量存在模糊性；3) 仅关注全局图像相似性，无法满足临床对细粒度区域特定检索的需求。

Method: HMAR采用混合专家架构，包含双专家机制：Expert0提取全局特征进行整体相似性匹配，Expert1学习位置不变的局部表示进行精确病灶区域检索。采用两阶段对比学习策略避免昂贵的边界框标注，推理时使用滑动窗口匹配算法进行密集局部比较。通过Kolmogorov-Arnold Network层生成哈希码，支持高效的汉明距离搜索。

Result: 在RadioImageNet-CT数据集（16个临床模式，29,903张图像）上，HMAR在64位和128位哈希码上分别达到0.711和0.724的mAP，相比最先进的ACIR方法分别提高了0.7%和1.1%。

Conclusion: HMAR通过自适应检索框架有效解决了医学图像检索中的关键限制，双专家机制结合全局和局部特征，无需边界框标注，在保持检索效率的同时显著提升了检索精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要轨迹、边界框、掩码或运动场等额外控制信号，且现有基准测试混淆了物体运动和相机运动，缺乏专门针对稳定相机下纯物体运动的评估方法。

Method: 采用目标帧控制策略，利用首尾帧运动先验；通过语义引导的物体插入和鲁棒背景修复构建可靠目标帧；利用早期自注意力图预测物体和相机动态，并开发ACE-Seed轻量搜索策略提升运动保真度。

Result: 在FLF2V-obj和VBench基准测试中一致优于基线方法；引入了S2M-DAVIS和S2M-OMB数据集用于稳定相机下的纯物体运动评估，以及FLF2V-obj指标来隔离物体伪影。

Conclusion: Search2Motion提供了一种无需训练、基于目标帧控制的物体运动编辑框架，通过早期注意力机制和轻量搜索策略实现了高质量的物体重定位，同时解决了现有基准测试的局限性。
...
TL;DR: 本文提出了一种基于物联网的高通量实时多智能体情感计算框架，用于通过情绪状态监测增强课堂学习，在三个教育机构测试中实现了88%的课堂参与度分类准确率。


<details>
  <summary>Details</summary>
Motivation: 随着课堂规模扩大和师生互动有限，教育工作者面临挑战，需要能够实时捕捉学生情绪和参与模式的可扩展、数据驱动的工具。

Method: 开发了面向物联网设备的高通量实时多智能体情感计算框架，解决了负载均衡和延迟问题。使用包含1,500张标记图像和300个课堂检测视频的"课堂情绪数据集"进行评估，并在三所不同学校进行实地测试。

Result: 系统表现出强大性能，能够以25 FPS的速度检测多达50张人脸，在课堂参与状态分类中达到88%的整体准确率。实施结果显示积极成果，学生、教师和家长对改善课堂互动和教学适应性给予积极反馈。

Conclusion: 本研究的主要贡献包括建立了一个实用的、基于物联网的情感感知学习环境框架，并引入了"课堂情绪数据集"以促进进一步验证和研究。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型能生成高质量多样化世界，但各帧间缺乏3D一致性，导致难以重建3D世界。需要处理这些不一致性以获得全局一致的3D重建

Method: 1) 几何基础模型将每帧提升为像素级3D点云；2) 定制非刚性迭代帧到模型ICP进行初始对齐；3) 全局优化锐化点云；4) 利用点云作为3D重建初始化，提出新颖的反向变形渲染损失从不一致视图中创建高质量3D环境

Result: 3D场景质量优于基线方法，有效将视频模型转化为3D一致的世界生成器，产生锐利详细点云重建和可探索3D环境

Conclusion: 该方法成功解决了视频扩散模型帧间3D不一致问题，通过非刚性对齐和全局优化实现了高质量3D重建，将视频生成能力转化为3D世界创建工具
...
TL;DR: 提出基础设施教学的无标签3D感知范式，利用路边单元作为静态教师为自动驾驶车辆提供伪标签监督，无需测试时基础设施支持


<details>
  <summary>Details</summary>
Motivation: 自动驾驶3D感知严重依赖大规模数据收集和人工标注，这在跨城市部署时变得不切实际；同时现代城市部署了大量路边单元，可以利用这些固定视角的传感器作为教师来训练车辆

Method: 提出基础设施教学的无标签3D感知范式：1）路边单元从无标签数据中学习局部3D检测器；2）广播预测给经过车辆；3）车辆聚合这些预测作为伪标签监督训练独立的自我检测器；在CARLA多智能体环境中实现三阶段无标签流水线

Result: 使用CenterPoint检测器，该方法达到82.3% AP（车辆检测），而完全监督的自我检测器上限为94.4%；系统分析了各阶段性能，评估了可扩展性，并展示了与现有自我中心无标签方法的互补性
...
TL;DR: SuCor是一种使用最优传输理论校正EPI磁敏感几何畸变的方法，通过Wasserstein-2重心位移建模畸变场，在谱域进行正则化，无需手动调参。


<details>
  <summary>Details</summary>
Motivation: EPI（平面回波成像）存在磁敏感引起的几何畸变问题，传统校正方法如FSL TOPUP需要手动调参且性能有限，需要更高效、自动化的校正方法。

Method: 使用最优传输理论，将反向相位编码EPI对的畸变场建模为强度剖面之间的Wasserstein-2重心位移，在谱域使用弯曲能量惩罚进行正则化，通过Morozov差异原则自动选择正则化强度。

Result: 在HCP数据集上，SuCor与T1结构像的体素互信息达到0.341，优于FSL TOPUP的0.317，且在单CPU核心上仅需约12秒运行时间。

Conclusion: SuCor提供了一种高效、自动化的EPI畸变校正方法，无需手动调参，在精度和计算效率上均优于现有方法，适用于大规模神经影像研究。
...
TL;DR: 提出基于顶点帧的范式，通过双流独立解耦框架从稳定伪装状态的表情中识别真实情绪，避免伪装情绪干扰


<details>
  <summary>Details</summary>
Motivation: 现有方法从包含起始帧的掩蔽表情片段识别真实情绪，但起始帧泄露真实情绪信息且未达到稳定伪装状态，无法反映实际伪装状态

Method: 1) 引入基于顶点帧的新范式，从稳定伪装状态的顶点帧分类真实情绪；2) 提出双流独立解耦框架，解耦真实和伪装情绪特征；3) 设计解耦损失组：两个分类损失分别学习真实情绪和伪装表情特征，以及希尔伯特-施密特独立性损失增强特征独立性

Result: 实验证明基于顶点帧的范式具有挑战性，提出的解耦框架提高了识别性能

Conclusion: 顶点帧范式更符合实际伪装状态，双流独立解耦框架能有效分离真实和伪装情绪特征，提升掩蔽表情中真实情绪的识别准确率
...
TL;DR: 提出GDPO方法将强化学习集成到一步生成式图像超分辨率训练中，通过噪声感知扩散模型和组直接偏好优化提升性能


<details>
  <summary>Details</summary>
Motivation: 当前强化学习主要应用于多步生成式图像超分辨率，而一步生成式ISR由于随机性有限而研究不足。现有RL方法如DPO需要离线生成正负样本对导致样本有限，GRPO仅计算整图似然而忽略局部细节

Method: 1) 引入噪声感知一步扩散模型生成多样化ISR输出，采用不等时间步策略解耦噪声添加和扩散时间步；2) 提出GDPO策略，将GRPO原理集成到DPO中，计算在线生成样本的组相对优势；3) 设计属性感知奖励函数基于平滑和纹理区域统计动态评估样本分数

Result: 实验证明GDPO在增强一步生成式ISR模型性能方面的有效性

Conclusion: GDPO成功将强化学习集成到一步生成式ISR训练中，通过噪声感知模型和组相对优化策略解决了现有方法的局限性
...
TL;DR: V-Co是一个系统研究视觉协同去噪的统一框架，揭示了有效视觉协同去噪的四个关键要素，在ImageNet-256上以更少训练轮次超越了现有像素空间扩散方法。
...
Method: 提出V-Co框架，采用统一的JiT（即时）基础架构进行系统研究。包含四个关键设计：1）完全双流架构，保留特征特定计算并支持灵活跨流交互；2）结构定义的无条件预测以实现有效分类器无关引导；3）感知漂移混合损失提供强语义监督；4）基于RMS的特征重缩放实现跨流校准。

Result: 在ImageNet-256上，V-Co在可比模型规模下超越了底层像素空间扩散基线和现有强像素扩散方法，同时使用了更少的训练轮次。实验验证了四个关键要素的有效性。

Conclusion: 视觉协同去噪的有效性依赖于四个关键要素：完全双流架构、结构定义的无条件预测、感知漂移混合损失和基于RMS的特征重缩放。这些发现为未来表示对齐生成模型提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有动物深度估计模型大多基于无度量尺度的数据集训练，限制了图像模型的验证能力。动物特别是野生动物在复杂环境中的感知需要更可靠的多模态数据支持。

Method: 构建WildDepth多模态数据集，包含从家养到野生动物的RGB和LiDAR同步数据，建立深度估计、行为检测和3D重建的基准测试套件。

Result: 多模态数据使用使深度估计RMSE提升10%，RGB-LiDAR融合使3D重建Chamfer距离改善12%。数据集支持跨领域泛化的鲁棒感知系统。

Conclusion: WildDepth数据集填补了动物感知领域多模态数据的空白，通过提供度量尺度的基准测试，促进了跨领域泛化的鲁棒多模态感知系统发展。
...
TL;DR: 该论文系统评估了三种数据为中心的方法在处理遥感数据标签噪声方面的性能，包括噪声识别和任务性能改进，并分析了不同噪声类型和水平下的方法表现。


<details>
  <summary>Details</summary>
Motivation: 遥感领域对标签噪声的自动化处理方法关注不足，缺乏对数据为中心方法（既能处理标签噪声又能识别和隔离噪声标签）的系统分析。标签噪声在许多真实世界数据集中普遍存在，会严重限制深度学习模型的泛化能力。

Method: 研究了三种数据为中心的方法，通过向两个基准数据集中注入不同噪声类型（噪声水平从10%到70%）来评估这些方法的行为。分析这些方法过滤标签噪声的效果及其对任务性能的影响。

Result: 研究明确证明了数据为中心方法在标签噪声识别和任务性能改进两方面的价值。分析提供了根据不同设置和目标选择最佳方法的见解，并指出了将数据为中心标签噪声方法应用于遥感数据时仍需研究的领域。

Conclusion: 该工作是弥合数据为中心标签噪声方法的方法论建立与其在遥感领域实际应用之间差距的重要一步，为遥感数据处理标签噪声提供了系统性的评估框架和实用指导。
...
TL;DR: Vision transformers存在位置偏差问题，通过微调使用ALiBi相对位置编码可减少偏差，同时保持语义特征，适用于材料科学显微图像分割


<details>
  <summary>Details</summary>
Motivation: Vision transformers（特别是DINOv2等特征基础模型）存在位置偏差和伪影问题，这些偏差与语义内容无关，导致在材料科学等领域的零样本适应困难，因为这些领域的图像通常是均匀微观结构的横截面（没有首选方向）

Method: 通过线性探测研究ViTs中的位置偏差，发现该偏差存在于多种目标和位置编码中；随后通过微调模型使用ALiBi相对位置编码来减少这种偏差

Result: 微调后的模型保留了理想的通用语义特征，其无偏差特征可成功用于复杂显微图像的可训练分割

Conclusion: 通过使用ALiBi相对位置编码进行微调，可以有效减少Vision transformers的位置偏差，同时保持其语义表示能力，这对于材料科学等领域的图像分析应用具有重要意义
...
TL;DR: M^3：一种增强多视图基础模型的方法，通过专用匹配头实现细粒度密集对应，并集成到单目高斯泼溅SLAM中，显著提升未标定单目视频流式重建的精度和效率。


<details>
  <summary>Details</summary>
Motivation: 未标定单目视频的流式重建面临挑战，需要高精度位姿估计和动态环境中的高效在线优化。现有多视图基础模型以前馈方式估计位姿，产生的像素级对应关系缺乏几何优化所需的精度。

Method: 提出M^3框架：1）为多视图基础模型添加专用匹配头，实现细粒度密集对应；2）集成到鲁棒的单目高斯泼溅SLAM系统中；3）引入动态区域抑制和交叉推理内参对齐机制增强跟踪稳定性。

Result: 在多样室内外基准测试中实现最先进的位姿估计和场景重建精度。相比VGGT-SLAM 2.0，ATE RMSE降低64.3%；在ScanNet++数据集上，PSNR比ARTDECO高2.11 dB。

Conclusion: M^3通过增强多视图基础模型的匹配能力并集成到SLAM框架中，有效解决了未标定单目视频流式重建的精度瓶颈，在几何精度和重建质量方面均取得显著提升。
...
TL;DR: SparkVSR是一个交互式视频超分辨率框架，允许用户通过稀疏关键帧作为控制信号来引导超分辨率过程，实现可控的视频质量提升。


<details>
  <summary>Details</summary>
Motivation: 现有视频超分辨率方法在推理时像黑盒一样工作，用户无法可靠地纠正意外伪影，只能接受模型输出的结果。需要一种允许用户交互控制的VSR框架。

Method: 提出基于关键帧条件化的潜在-像素两阶段训练流程：首先使用任意图像超分辨率模型处理稀疏关键帧，然后通过融合LR视频潜在表示和稀疏编码的HR关键帧潜在表示，学习跨空间传播并细化感知细节。

Result: 在多个VSR基准测试中，CLIP-IQA、DOVER和MUSIQ指标分别提升24.6%、21.8%和5.6%，表现出更好的时间一致性和恢复质量。框架还可泛化应用于老电影修复和视频风格迁移等任务。

Conclusion: SparkVSR提供了一个通用、交互式、关键帧条件化的视频处理框架，通过稀疏关键帧实现可控的视频超分辨率，支持灵活的关键帧选择和参考无关的引导机制。
...
TL;DR: 提出MessyKitchens数据集和Multi-Object Decoder (MOD)方法，用于单目3D场景重建，特别关注杂乱环境中的物体级重建和物理合理性。
...
Method: 1) 引入MessyKitchens数据集，提供杂乱真实场景的高保真物体级ground truth（3D形状、姿态和准确物体接触）；2) 基于SAM 3D单物体重建方法，扩展Multi-Object Decoder (MOD)进行联合物体级场景重建。

Result: MessyKitchens数据集在配准精度和物体间穿透方面显著优于先前数据集。MOD方法在三个数据集上相比现有技术取得一致且显著的改进。

Conclusion: 该工作通过新数据集和MOD方法推进了物体级场景重建，特别关注物理合理性和杂乱环境，为机器人学和动画应用提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D部件分割方法存在两个主要问题：基于2D先验的方法存在跨视图不一致和边界模糊问题；基于3D判别式分割的方法需要大量标注数据和训练资源。SegviGen旨在利用预训练3D生成模型中编码的结构化先验，实现高效准确的部件分割

Method: SegviGen通过重新利用预训练3D生成模型，对3D资产进行编码，在几何对齐重建的活跃体素上预测部件指示性颜色。该方法支持交互式部件分割、完整分割以及带2D引导的完整分割的统一框架

Result: 实验表明，SegviGen在交互式部件分割上比现有最优方法提升40%，在完整分割上提升15%，同时仅使用0.32%的标注训练数据。这证明了预训练3D生成先验能有效迁移到3D部件分割任务

Conclusion: SegviGen展示了利用预训练3D生成模型进行3D部件分割的有效性，通过部件着色方法实现了有限监督下的强大性能，为3D分割提供了新颖高效的解决方案
...
TL;DR: 视频生成模型中的推理能力主要沿扩散去噪步骤展开，而非先前认为的跨视频帧序列，研究者提出了链式步骤机制并发现了关键推理行为。


<details>
  <summary>Details</summary>
Motivation: 挑战现有关于视频生成模型中推理机制的理解，先前研究认为推理通过跨视频帧的链式框架机制展开，但本研究质疑这一假设并探索真实的推理机制。

Method: 通过定性分析和针对性探测实验，研究视频生成模型在扩散去噪过程中的推理行为，识别关键推理机制，并提出基于相同模型不同随机种子的潜在轨迹集成策略。

Result: 发现推理主要沿扩散去噪步骤展开，提出链式步骤机制，识别出工作记忆、自我纠正与增强、感知先于行动等关键推理行为，以及扩散变换器中的功能专业化现象。

Conclusion: 视频生成模型的推理能力源于扩散去噪过程中的链式步骤机制，这一发现为理解视频模型作为智能新基质的固有推理动态提供了系统框架，指导未来研究。
...
TL;DR: 该论文提出了一种基于相机姿态的统一几何表示方法，用于改进交互式游戏世界模型的动作控制和长时程3D一致性，通过物理基础动作空间和全局相机姿态索引实现精确的6-DoF控制和空间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散变换器在交互式游戏世界模型中存在两个主要问题：1) 动作控制不精确，将用户动作视为抽象条件信号，忽略了动作与3D世界之间的几何耦合关系；2) 长时程3D一致性不足，无法保持空间一致性。需要建立统一的几何表示来同时解决即时动作控制和长期3D一致性问题。

Method: 1) 定义基于物理的连续动作空间，在Lie代数中表示用户输入以推导精确的6-DoF相机姿态，通过相机嵌入器注入生成模型确保动作对齐；2) 使用全局相机姿态作为空间索引检索相关历史观测，实现长时程导航中位置的几何一致性重访；3) 构建包含3000分钟真实人类游戏玩法的大型数据集，标注相机轨迹和文本描述。

Result: 实验表明，该方法在动作可控性、长时程视觉质量和3D空间一致性方面显著优于最先进的交互式游戏世界模型。具体表现在：1) 实现了精确的6-DoF相机控制；2) 在长时程导航中保持了更好的视觉一致性；3) 能够几何一致地重访先前位置。

Conclusion: 相机姿态作为统一的几何表示能够有效解决交互式游戏世界模型中的动作控制和3D一致性问题。通过将动作与相机姿态几何耦合，并使用全局姿态作为空间索引，实现了精确的动作对齐和长时程空间一致性，为构建更真实、可控的生成式游戏环境提供了有效框架。
...
Method: 提出SRLM框架，利用三个内在信号（自一致性、推理长度和语言化置信度）作为模型内部不确定性的互补指标，通过自反思评估和比较候选上下文交互程序

Result: 在多样化基准数据集、上下文长度和骨干模型上的实验表明，SRLM始终优于最先进的基线方法，在相同时间预算下比RLM提升高达22%；自反思程序搜索无需自查询或显式递归机制即可匹配或超越RLM

Conclusion: 递归本身不是RLM性能的主要驱动因素；对于模型窗口内的上下文长度，带递归的RLM通常会降低性能，而SRLM在短上下文和长上下文中都能带来一致的增益；在语义密集型任务中，自反思提供了更好的语义信号来引导推理
...
TL;DR: MedArena是一个交互式医疗大语言模型评估平台，通过临床医生使用真实医疗查询直接测试和比较不同LLM，发现传统静态基准测试无法反映实际临床复杂性，而临床医生更看重回答的深度、细节和清晰度而非单纯事实准确性。
...
Method: 开发了MedArena交互式评估平台，让临床医生使用自己的医疗查询直接测试和比较领先的LLM。平台随机选择两个模型对同一查询生成回答，让用户选择更优回答。收集了1571个偏好数据，涵盖12个LLM，使用Bradley-Terry评分进行排名分析。

Result: Gemini 2.0 Flash Thinking、Gemini 2.5 Pro和GPT-4o是排名前三的模型。只有三分之一的临床医生提交的问题类似于事实回忆任务，大多数涉及治疗选择、临床文档或患者沟通等复杂任务，约20%涉及多轮对话。临床医生在选择偏好时更看重回答的深度、细节和清晰度，而非单纯事实准确性。

Conclusion: MedArena通过基于真实临床问题和偏好的评估，为衡量和改进医疗大语言模型的实用性和有效性提供了一个可扩展的平台。研究表明临床医生更重视回答的可读性和临床细微差别，且模型排名在控制风格相关因素后保持稳定。
...
TL;DR: LLMs和分词方案对阿拉伯语根-模式形态学的表示和生成效果研究，发现分词器的形态对齐对形态生成既非必要也不充分


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型及其分词方案如何有效表示和生成阿拉伯语根-模式形态学，探究它们是否捕捉了真正的形态结构还是依赖于表面记忆。阿拉伯语形态系统为分析LLMs如何处理复杂的非连接形式和分词选择如何影响这一过程提供了丰富的测试平台。

Method: 1. 评估阿拉伯语和多语言分词器相对于黄金标准分词的形态保真度；2. 使用新开发的测试集分析LLMs在生成性根-模式生成中的表现；3. 在七个以阿拉伯语为中心和多语言LLMs及其相应分词器上进行实验。

Result: 研究发现分词器的形态对齐对形态生成既非必要也不充分，这质疑了形态分词在下游性能中的作用。

Conclusion: LLMs在处理阿拉伯语复杂形态时，分词器的形态对齐并不决定模型的形态生成能力，表明当前的分词方法可能不是捕捉形态结构的最佳方式。
...
TL;DR: 本文介绍了参加SemEval-2026 Task 5的系统，该任务要求对短篇故事中同形异义词的给定词义进行5点李克特量表合理性评分。系统通过探索三种提示策略和集成方法，在比赛中获得第四名。


<details>
  <summary>Details</summary>
Motivation: 解决SemEval-2026 Task 5中的词义合理性评分任务，该任务面临同形异义词词义判断的挑战，且存在显著的标注者间差异，需要开发能够处理主观语义评估任务的系统。

Method: 使用三种提示策略：零样本基线、带结构化推理的思维链提示、以及同时评估候选词义的比较提示策略。采用多个闭源商业大语言模型，并提出了通过平均模型预测来处理标注者间差异的集成方法。

Result: 最佳官方系统（集成所有三种提示策略的LLM）在比赛排行榜上获得第四名，准确率0.88，斯皮尔曼等级相关系数0.83（平均0.86）。赛后实验进一步将性能提升至准确率0.92，斯皮尔曼等级相关系数0.85（平均0.89）。

Conclusion: 比较提示策略在不同模型家族中持续提升性能，模型集成显著增强了与平均人类判断的一致性，表明LLM集成特别适合涉及多个标注者的主观语义评估任务。
...
<details>
  <summary>Details</summary>
Motivation: 自然语言被认为在信息瓶颈的复杂度-准确性权衡下演化出高效压缩意义的词汇，但驱动这种优化的社会动态机制尚不清楚。同时，演化博弈论虽能解释语言从基础智能体动态中涌现，但尚未验证其是否能产生信息瓶颈意义上的高效压缩。

Method: 提出一个统一模型，整合演化博弈论与信息瓶颈框架，在信号博弈中通过独立动机的不精确策略模仿动态，研究词汇压缩效率的演化过程。

Result: 模型发现关键参数（博弈中的精确度调节参数、玩家混淆相似状态的倾向）约束了涌现词汇所实现的权衡变化范围。演化博弈动态可能为词汇演化提供机制基础，使其具有信息论最优且经验证实的特性。

Conclusion: 演化博弈动力学可能为词汇演化提供机制基础，使其具有信息论最优且经验证实的特性，从而解释了自然语言词汇如何通过社会动态演化出高效压缩能力。
...
TL;DR: CTG-DB是一个开源管道，将ClinicalTrials.gov的XML档案转换为关系数据库，使用MedDRA标准化不良事件术语，支持可扩展的药监分析


<details>
  <summary>Details</summary>
Motivation: ClinicalTrials.gov作为最大的临床试验注册库，其注册导向的架构和异构的不良事件术语限制了系统性的药监分析，需要手动协调才能识别一致的安全概念

Method: 开发CTG-DB开源管道，摄取完整的CT.gov XML档案，生成关系数据库，使用MedDRA标准化AE术语，通过确定性的精确和模糊匹配实现透明可复现的映射，保留组别分母并代表安慰剂和对照组

Result: 创建了与标准化AE术语对齐的关系数据库，支持概念级检索和跨试验聚合，为可扩展的安慰剂参照安全分析和下游药监信号检测集成临床试验证据提供框架

Conclusion: CTG-DB解决了CT.gov在系统药监分析中的局限性，通过标准化AE术语和结构化数据转换，实现了透明、可复现的映射，支持大规模安全分析和证据整合
...
TL;DR: BANGLASOCIALBENCH：首个评估孟加拉语社会语用能力的基准，包含1,719个文化情境实例，揭示LLMs在孟加拉语社交互动中的系统性文化错位问题


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出强大的多语言流利度，但流利度本身并不能保证社会适当的语言使用。在高语境语言中，交际能力需要对编码在日常语言中的社会等级、关系角色和互动规范保持敏感性。孟加拉语通过其三层次代词系统、基于亲属关系的称呼方式和文化嵌入的社会习俗体现了这一挑战

Method: 引入BANGLASOCIALBENCH基准，这是首个通过上下文依赖的语言使用而非事实回忆来评估孟加拉语社会语用能力的基准。基准涵盖三个领域：孟加拉语称呼术语、亲属关系推理和社会习俗，包含1,719个由母语者编写和验证的文化情境实例。在零样本设置下评估了12个当代LLMs

Result: 观察到系统性的文化错位模式：模型经常默认使用过于正式的称呼形式，未能识别多个社会可接受的称呼代词，并在不同宗教背景下混淆亲属术语。社会语用失败通常是结构化且非随机的

Conclusion: 研究结果表明，当前LLMs在推断和应用现实孟加拉社会互动中文化适当的语言使用方面存在持续局限性。社会语用能力是LLMs在多语言环境中需要解决的关键挑战
...
TL;DR: POLAR是一种基于嵌入空间的用户级词汇关联测试方法，通过将用户表示为私有确定性标记并投影到词汇轴上，提供标准化的关联效应报告，能够有效区分LLM驱动的机器人与有机账户，并量化极端主义论坛中的词汇对齐和时间漂移。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数内在关联探针在词、句或语料库层面操作，掩盖了作者级别的变异。需要一种能够在嵌入空间中运行的用户级词汇关联测试方法，为计算社会科学提供简洁的、针对每个作者的诊断工具。

Method: POLAR在轻量适配的掩码语言模型的嵌入空间中运行，将作者表示为私有确定性标记，将这些向量投影到精心策划的词汇轴上，并通过置换检验p值和Benjamini-Hochberg控制报告标准化效应。

Result: 在平衡的机器人-人类Twitter基准测试中，POLAR能够清晰地区分LLM驱动的机器人与有机账户；在极端主义论坛上，它量化了与侮辱性词汇词典的强对齐，并揭示了随时间向右漂移的趋势。

Conclusion: POLAR方法对新属性集具有模块化特性，为计算社会科学提供了简洁的、针对每个作者的诊断工具。所有代码已公开可用。
...
### [120] [ASDA: Automated Skill Distillation and Adaptation for Financial Reasoning](https://arxiv.org/abs/2603.16112)
*Tik Yu Yim,Wenting Tan,Sum Yee Chan,Tak-Wah Lam,Siu Ming Yiu*

Main category: cs.CL

TL;DR: ASDA框架通过自动生成结构化技能文件实现LLM在金融推理领域的无权重调整适配，显著提升FAMMA基准性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM金融推理适配方法存在局限性：微调成本高且产生模型锁定，而无训练方法（如GEPA和ACE）在复杂多步领域推理中效果有限

Method: ASDA框架通过教师模型分析学生模型在金融推理任务中的失败案例，按子领域和错误类型聚类，自动生成包含推理流程、代码模板和工作示例的结构化技能文件，在推理时动态注入

Result: 在FAMMA基准测试中，ASDA在算术推理上提升+17.33%，在非算术推理上提升+5.95%，显著优于所有无训练基线方法
...
Method: 引入基于token的路由机制，根据词汇结构（多模态模型）或学习门控（语义专业化）将单个token路由到适配器。提出MoLoRA（混合LoRA），支持可组合专业化：加载多个领域特定适配器，让学习路由器选择每个token的适配器

Result: 基于token的路由被证明是最优的，实现N个token的工作量为N，而基于序列的路由为K·N。MoLoRA使Qwen3-1.7B在四个推理基准测试中超越Qwen3-8B，同时模型大小缩小4.7倍

Conclusion: 专业化显著优于规模扩展。MoLoRA支持推理时的模块化专业知识：独立训练专注的LoRA，无需重新训练即可组合它们，通过简单加载新适配器添加新能力
...
TL;DR: 提出基于SVM的罗曼什语方言识别系统，能区分罗曼什语不同方言变体及超区域变体Rumantsch Grischun，在新建基准测试中达到97%准确率。


<details>
  <summary>Details</summary>
Motivation: 罗曼什语有多种地区变体（方言），相互可懂度有限，但缺乏能区分这些方言的语言识别系统。由于还需识别结合多种方言元素的超区域变体Rumantsch Grischun，这构成了新颖有趣的分类问题。

Method: 采用支持向量机（SVM）方法构建罗曼什语方言识别系统，并在新构建的跨两个领域的基准测试上进行评估。

Result: 模型在领域内平均准确率达到97%，支持方言感知的拼写检查和机器翻译等应用。分类器已公开可用。

Conclusion: 成功开发了首个能有效区分罗曼什语方言变体的语言识别系统，为罗曼什语的语言技术应用提供了重要工具。
...
TL;DR: 提出PALLM模型，通过多任务强化学习和思维链提示来提升语音大语言模型对副语言线索（韵律、情感、非语言声音）的理解能力，在多个数据集上超越现有基线模型8-12%。


<details>
  <summary>Details</summary>
Motivation: 语音大语言模型能够感知副语言线索（如韵律、情感、非语言声音），这些线索对于意图理解至关重要。然而，利用这些线索面临三大挑战：训练数据有限、标注困难、以及模型倾向于利用词汇捷径而非真正的副语言信号。

Method: 提出副语言感知的语音大语言模型PALLM，采用多任务强化学习结合思维链提示来激发显式的情感推理。为了解决数据稀缺问题，设计了两阶段流水线：联合优化音频情感分类和副语言感知的响应生成。

Result: 在Expresso、IEMOCAP和RAVDESS三个数据集上的实验表明，该方法在副语言理解方面比监督基线和强大的专有模型（Gemini-2.5-Pro、GPT-4o-audio）提升了8-12%。

Conclusion: 通过多任务强化学习建模副语言推理对于构建具有情感智能的语音大语言模型至关重要。PALLM方法有效解决了副语言线索利用的挑战，显著提升了模型的情感理解能力。
...
TL;DR: 该研究提出"共吸引子"概念来检测职业涌现，通过简历数据分析发现AI形成了专业词汇但未形成凝聚的从业者群体，表明AI是扩散技术而非新兴职业。


<details>
  <summary>Details</summary>
Motivation: 传统职业分类系统无法及时追踪快速形成和演变的职业，需要一种无需预定义分类或职位名称的零假设方法来检测职业涌现。

Method: 提出"共吸引子"概念：职业是专业词汇使从业者凝聚、凝聚群体维持词汇的自强化结构。基于820万份美国简历数据，独立测试词汇凝聚力和人口凝聚力，通过消融实验验证词汇是否是绑定人口的机制。

Result: 方法正确识别了既有职业，并揭示了AI的显著不对称性：2024年初迅速形成了凝聚的专业词汇，但从业者群体从未凝聚。原有的AI社区随着工具主流化而解体，新词汇被吸收到现有职业中而非绑定新职业。

Conclusion: AI是扩散技术而非新兴职业。引入"AI工程师"职业类别可能催化围绕已形成词汇的人口凝聚力，从而完成共吸引子结构。
...
Result: 1. 不确定观察最难学习；2. 仅用合成数据训练的模型与标准数据训练模型相差仅1-2个F1点；3. 合成数据增强在低资源环境下对不确定观察特别有效（F1从0.61提升到0.70）；4. RadAnnotate可自动标注55-90%的报告，实体匹配分数达0.86-0.92。

Conclusion: RadAnnotate框架通过检索增强合成报告和基于置信度的选择性自动化，有效减少了放射学报告标注的专家工作量，同时保持了高质量的实体标注性能。
...
TL;DR: 该研究分析了大型语言模型在道德敏感决策中的推理轨迹，发现道德推理涉及系统性的多框架审议，框架切换频繁，轨迹稳定性与模型抗攻击性相关，并提出了道德表征一致性指标。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地参与道德敏感决策，但对其在推理步骤中如何组织伦理框架的理解仍然不足。需要探索模型在道德推理过程中的框架切换动态及其对决策稳定性的影响。

Method: 引入"道德推理轨迹"概念，分析六个模型在三个基准测试中的框架调用序列。使用线性探针定位框架特定编码的模型层，通过激活引导调节框架整合模式，提出道德表征一致性（MRC）指标并验证其与人类标注的一致性。

Result: 发现55.4-57.7%的连续推理步骤涉及框架切换，仅16.4-17.8%的轨迹保持框架一致。不稳定轨迹对说服性攻击的敏感性高1.29倍。线性探针在特定层（Llama-3.3-70B的第63层，Qwen2.5-72B的第17层）检测到框架特定编码，KL散度比基线降低13.8-22.6%。激活引导减少6.7-8.9%的漂移，MRC指标与LLM连贯性评分强相关（r=0.715）。

Conclusion: 道德推理本质上是多框架的审议过程，框架切换频繁且系统。轨迹稳定性与模型抗攻击性相关，框架特定编码可定位到模型特定层。提出的MRC指标能有效评估模型道德推理的连贯性，为理解和改进LLM的道德决策提供了新视角。
...
TL;DR: SEAHateCheck是首个针对东南亚低资源语言（印尼语、他加禄语、泰语、越南语）的仇恨言论检测功能测试数据集，基于HateCheck框架构建，通过LLM增强和本地专家验证，揭示了现有模型在文化特定表达和隐式仇恨检测方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 仇恨言论检测严重依赖语言资源，但这些资源主要集中在英语和中文等高资源语言中，导致东南亚低资源语言的研究者和平台在开发仇恨言论检测工具时面临障碍。这些地区多样的社会语言环境使在线仇恨内容审核更加复杂。

Method: 基于HateCheck的功能测试框架，改进SGHateCheck的方法，构建SEAHateCheck数据集。使用大型语言模型生成文化相关的测试用例，并由本地专家验证准确性。针对印度尼西亚、泰国、菲律宾和越南，覆盖印尼语、他加禄语、泰语和越南语四种语言。

Result: 实验显示最先进的多语言模型在检测特定低资源语言的仇恨言论方面存在局限性：他加禄语测试用例准确率最低（可能由于语言复杂性和训练数据有限）；俚语功能测试最具挑战性（模型难以处理文化细微表达）；模型在隐式仇恨检测方面表现薄弱；模型难以区分反仇恨言论表达。

Conclusion: SEAHateCheck作为首个针对这些东南亚语言的功能测试套件，为研究者提供了强大的基准测试工具，推动了实用、文化适应的仇恨言论检测工具的发展，促进了包容性在线内容审核的进步。
...
TL;DR: CounterRefine：一种用于检索增强问答的轻量级推理时修复层，通过生成草稿答案、收集支持/冲突证据、进行受限精炼来减少承诺失败错误


<details>
  <summary>Details</summary>
Motivation: 事实性问答中的许多错误并非检索失败，而是承诺失败：系统检索到相关证据但仍给出错误答案。需要让系统能够使用证据重新考虑和修复自身答案

Method: 1) 从检索证据生成简短答案；2) 基于草稿答案进行后续查询收集额外支持和冲突证据；3) 应用受限精炼步骤输出KEEP或REVISE，修订需通过确定性验证。将检索转变为测试临时答案而非仅收集更多上下文

Result: 在SimpleQA基准测试中，CounterRefine将匹配的GPT-5 Baseline-RAG提升了5.8个百分点，达到73.1%的正确率，同时比报告的单次GPT-5.4分数高出约40个百分点

Conclusion: 知识型基础模型的重要方向：不仅需要访问证据，还应能够使用证据重新考虑并在必要时修复自身答案。检索应作为测试临时答案而非仅收集上下文的手段
...
TL;DR: ZipCal是一种模型无关的数据选择方法，通过最大化基于Zipfian幂律的词汇多样性来选择校准数据，用于LLM的剪枝和量化压缩，比随机采样效果更好，且比基于困惑度的方法快240倍。


<details>
  <summary>Details</summary>
Motivation: 现有LLM压缩方法较少关注校准数据的选择，而校准数据的选择对保持模型在任务内和任务间的能力至关重要。需要一种高效且模型无关的方法来选择高性能的校准数据集。

Method: 提出ZipCal方法，基于Zipfian幂律分析数据的内在属性（词汇多样性），而不是依赖模型特定信号。通过最大化词汇多样性来选择校准数据，具有线性复杂度。

Result: ZipCal在各种剪枝基准测试中始终优于标准均匀随机采样。在下游性能方面，与依赖模型困惑度的SOTA方法表现相当，但速度快约240倍。

Conclusion: ZipCal提供了一种高效、模型无关的校准数据选择策略，能够有效保持压缩后LLM的性能，同时避免了大规模模型和数据集上基于困惑度方法的计算成本问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度研究工具（如OpenAI DR）虽然能合成科学论文来回答研究者的查询，但缺乏对用户的理解，无法提供个性化的研究支持。

Method: MySQA采用三阶段方法：1) 推断用户研究兴趣画像；2) 为用户输入查询提出个性化行动建议；3) 按照用户批准的行动编写多章节报告。通过合成用户基准测试（使用LLM评委）和真实用户访谈进行验证。

Result: 在NLP标准协议测试中，MySQA在引用指标和个性化行动遵循方面优于基线。通过真实用户访谈揭示了LLM评委无法检测到的9种个性化DR细微错误，并形成了未来DR设计的经验教训。

Conclusion: 个性化研究需要真实用户参与，仅依赖易于使用的LLM评委可能导致NLP领域忽视个性化的重要方面，真正的个性化进展只能通过真实用户实现。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索学习率调度在大型语言模型大规模预训练中的作用，特别是其对监督微调（SFT）后下游性能的影响。尽管衰减式学习率调度器被广泛用于最小化预训练损失，但它们如何影响SFT后的性能仍未得到充分研究。

Method: 研究方法包括：1）提出并测试Warmup-Stable-Only（WSO）调度器，在预热阶段后保持恒定学习率而不进行衰减；2）在1B和8B参数模型上进行实验；3）分析不同训练阶段（中期训练和过度训练）的表现；4）通过损失景观分析比较不同调度器导致的优化点性质。

Result: 实验结果表明：1）WSO调度器在SFT后性能上始终优于衰减式调度器；2）这一结果在不同训练阶段都成立；3）损失景观分析显示衰减式调度器使模型陷入更尖锐的最小值，而WSO保持更平坦的最小值，支持更好的适应性；4）衰减式调度器虽然在预训练阶段可能有更好表现，但会损害下游任务的适应性。

Conclusion: 研究结论是：应用学习率衰减来改善预训练指标可能会损害下游任务的适应性。WSO调度器通过保持平坦的最小值增强了模型对下游任务的适应性。这项工作为训练和模型发布策略提供了实用指导，建议使用WSO进行预训练以提高下游任务的适应性。
...
TL;DR: SciZoom是一个包含44,946篇ML论文的大规模基准数据集，涵盖2020-2025年，提供三个层次摘要目标，用于研究多粒度摘要和LLM时代科学写作的演变。


<details>
  <summary>Details</summary>
Motivation: AI研究的爆炸式增长导致信息过载，需要超越传统摘要的多粒度科学摘要。现有基准规模有限、仅针对单一粒度，且缺乏LLM时代后的数据。同时，ChatGPT发布后研究人员使用LLM撰写论文，但缺乏分析这种写作演变的数据资源。

Method: 构建SciZoom基准，包含来自NeurIPS、ICLR、ICML、EMNLP四个顶级ML会议的44,946篇论文（2020-2025年），明确分为Pre-LLM和Post-LLM时代。提供三个层次摘要目标：Abstract、Contributions和TL;DR，压缩比最高达600:1。进行语言分析以发现写作模式变化。

Result: 语言分析显示短语模式显著变化（公式化表达增加达10倍）和修辞风格转变（模糊表达减少23%），表明LLM辅助写作产生更自信但同质化的文本。数据集可作为挑战性基准和独特资源。

Conclusion: SciZoom填补了多粒度科学摘要基准的空白，并为分析生成式AI时代科学话语演变提供了首个大规模资源。数据集和代码已公开。
...
Method: 提出SI框架：1) 合成高质量自然语言语料，结合结构化知识图谱和非结构化行为日志，增强推理链和安全感知数据；2) 基于深度上采样的参数高效预训练策略注入领域知识；3) 通过多任务指令调优和对抗训练的双路径对齐方法

Result: 框架已在京东部署，在五个核心搜索场景的A/B测试中，关键业务指标显著提升，验证了工业有效性和可扩展性

Conclusion: SI框架成功解决了电商搜索LLM的知识幻觉和安全漏洞问题，实现了工业级部署，为意图感知推荐提供了可行的解决方案
...
TL;DR: Polyglot-Lion是一个紧凑的多语言自动语音识别模型系列，专为新加坡的英语、普通话、泰米尔语和马来语设计，通过平衡采样策略微调Qwen3-ASR模型实现，在保持竞争力的同时大幅降低了计算成本和推理时间。


<details>
  <summary>Details</summary>
Motivation: 针对新加坡多语言环境（英语、普通话、泰米尔语、马来语）需要高效、低成本的自动语音识别解决方案，现有大型模型如MERaLiON-2-10B-ASR虽然性能好但计算成本过高，不适合实际部署。

Method: 使用公开可用的语音语料库对Qwen3-ASR-0.6B和Qwen3-ASR-1.7B进行微调，采用平衡采样策略使每种语言的训练话语数量相等，并故意省略语言标签条件，让模型从音频中隐式学习语言识别。

Result: Polyglot-Lion-1.7B在12个基准测试中平均错误率为14.85，与6倍大的MERaLiON-2-10B-ASR（14.32）竞争力相当，训练成本仅81美元（单GPU）对比基线18,862美元（128 GPU），推理速度提升约20倍（0.10秒/样本 vs 2.02秒/样本）。

Conclusion: 通过语言平衡的微调中等规模预训练模型，可以以极低成本获得可部署的多语言ASR系统，性能与大型专业系统相当，为实际应用提供了经济高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现代LLMs的安全机制已从表面输入过滤扩展到潜在语义表示和生成时推理，能够恢复被模糊处理的恶意意图并拒绝响应，使得许多表面级模糊处理越狱攻击失效。需要开发能够绕过这些深层安全机制的新型攻击方法。

Method: 提出结构化语义伪装(S2C)框架，包含三个互补机制：1)上下文重构：将请求嵌入可信的高风险场景中，使模型倾向于服从；2)内容碎片化：将请求的语义特征分散到不相交的提示片段中；3)线索引导伪装：伪装剩余语义线索同时嵌入可恢复的标记来引导输出生成。通过延迟和重构语义整合，降低依赖于解码时连贯或显式重构恶意意图的安全触发器。

Result: 在HarmBench和JBB-Behaviors基准测试中，S2C分别将攻击成功率(ASR)提升了12.4%和9.7%，优于当前SOTA。在GPT-5-mini上表现尤为突出，在JBB-Behaviors上比最强基线高出26%。分析了不同组合在不同模型家族中的最佳表现，并描述了模糊处理程度与输入可恢复性之间的权衡关系。

Conclusion: S2C通过结构化操纵语义意图的重构过程，有效绕过现代LLMs的深层安全机制，显著提升越狱攻击成功率。该方法揭示了安全机制对连贯语义表示的依赖，为未来防御机制设计提供了重要见解。
...
TL;DR: SpecSteer：一种非对称协作推理框架，通过将本地个性化上下文与云端推理能力相结合，解决个性化智能中的隐私与推理能力矛盾问题。


<details>
  <summary>Details</summary>
Motivation: 个性化智能面临核心困境：将用户历史发送到集中式大语言模型存在隐私问题，而设备端小语言模型缺乏高质量生成所需的推理能力。初步研究表明，纯本地增强方法无法可靠弥合这一差距。

Method: 提出SpecSteer框架，将协作视为贝叶斯知识融合，并重新利用推测解码作为分布式对齐协议。采用Draft-Verify-Recover流程：设备端模型起草个性化序列；云端通过基于比率的机制验证，将推理验证与私有上下文解耦；若被拒绝，转向恢复机制在修正时注入本地意图。

Result: 实验表明，SpecSteer成功弥合了推理差距，实现了卓越的个性化生成性能，同时相比标准基线实现了2.36倍的加速。

Conclusion: SpecSteer通过非对称协作推理框架有效解决了个性化智能中的隐私与推理能力权衡问题，为隐私保护的个性化AI系统提供了可行方案。
...
TL;DR: 动态跨上下文评审(D-CCR)通过多轮问答扩展单次评审，但实验发现多轮评审反而降低验证性能，主要因为虚假阳性压力和评审目标漂移问题


<details>
  <summary>Details</summary>
Motivation: 探索跨上下文评审(CCR)的自然扩展——多轮评审机制，让评审者能够提问、获得作者回应并再次评审，以评估这种动态评审是否比单次评审更有效

Method: 采用控制实验设计，使用30个工件和150个注入错误，测试四种D-CCR变体与单次CCR基线对比。D-CCR变体包括不同信息量和交互模式的多轮评审机制

Result: 单次CCR(F1=0.376)显著优于所有多轮变体，最佳多轮变体D-CCR-2b(F1=0.303)。多轮评审虽然提高召回率(+0.08)，但产生62%更多虚假阳性，精度从0.30降至0.20。主要问题：虚假阳性压力和评审目标漂移

Conclusion: 多轮评审机制反而降低LLM验证性能，问题不在于评审者看到的信息量，而在于重复评审本身会引入噪声。动态评审中的额外轮次会制造虚假发现，评审者会从评审工件转向批评对话本身
...
<details>
  <summary>Details</summary>
Motivation: 分析ASR在KIParla语料库转录工作流中的实施效果，评估ASR辅助转录对速度和准确性的影响

Method: 两阶段实验：11名专家和新手转录员对三种对话类型制作手动和ASR辅助转录；使用统计建模、词级对齐和基于标注的指标进行分析

Result: ASR辅助工作流可提高转录速度，但不一致地改善整体准确性；效果受工作流配置、对话类型和标注者经验影响；结合对齐指标、描述统计和统计建模的分析提供了系统监控框架

Conclusion: 尽管有限制，ASR辅助转录（可能通过任务特定微调支持）可集成到KIParla转录工作流中，在不影响转录质量的情况下加速语料库创建
...
TL;DR: 提出AEG框架，利用SpeechLLM的内部跨模态注意力机制显式定位和锚定关键证据，通过LFE微调范式校准注意力分布，在Spoken QA任务中减少幻觉并提升效率
...
Method: 提出注意力引导证据锚定(AEG)端到端框架，利用SpeechLLM的内部跨模态注意力显式定位关键证据；引入学习聚焦证据(LFE)监督微调范式，校准预训练模型的扩散注意力分布，区分查询相关与无关片段

Result: 在SQuAD、HotpotQA和MuSiQue数据集上实验表明，AEG减少幻觉并实现显著效率提升，超越大规模级联基线(Whisper-Large-v3 + Reranker)，推理延迟降低约62%

Conclusion: AEG框架通过显式证据定位和注意力校准，有效解决了Spoken QA中的跨模态对齐问题，在减少幻觉的同时显著提升推理效率，为端到端语音问答系统提供了新方向
...
<details>
  <summary>Details</summary>
Motivation: 为语音通信研究提供一个开源、可复现、可扩展的计算工具包，用于实现基于神经基础、时间原理和语音丰富表示的动态语音规划模型。

Method: 开发了包含规划场、感知场、记忆场定义模块，支持场间耦合、手势输入，并能通过场激活剖面求解轨迹变量路径的Python工具包。

Result: 成功实现了包含耦合记忆场的生产/感知循环仿真示例，展示了框架在建模交互式语音动态方面的能力，工具包已作为开源软件发布。

Conclusion: PyPhonPlan为语音通信研究提供了可复现、可扩展的累积计算开发平台，支持基于神经基础和时间原理的语音动态建模。
...
<details>
  <summary>Details</summary>
Motivation: 当前多语言机器翻译系统仅覆盖约200种目标语言，与全球7000多种语言相比覆盖有限，且缺乏可靠的评估基准和指标。需要构建支持更广泛语言的翻译系统。

Method: 提出综合数据策略整合大型公共多语言语料库和新创建数据集（包括手动整理的MeDLEY双语文本）。探索两种LLM专业化方法：作为仅解码器模型（OMT-LLaMA）或作为编码器-解码器架构中的模块（OMT-NLLB）。

Result: 1B-8B参数模型匹配或超过70B LLM基线的MT性能，显示明显的专业化优势。OMT-LLaMA模型大幅扩展了可生成连贯文本的语言集合。OMT模型在跨语言迁移方面表现更好，接近解决1600种语言的"理解"部分。提供动态演进的排行榜和人工创建评估数据集。

Conclusion: OMT系统首次实现了对1600多种语言的机器翻译支持，通过专业化的小规模模型实现了超越大型通用LLM的翻译性能，为低计算环境提供了高质量的翻译解决方案，并建立了开放的评估基准。
...
TL;DR: Fanar 2.0是卡塔尔第二代阿拉伯语生成式AI平台，在资源有限条件下（256个H100 GPU）通过数据质量优先、持续预训练和模型融合策略，实现了阿拉伯语AI能力的显著提升。


<details>
  <summary>Details</summary>
Motivation: 开发主权AI平台，解决阿拉伯语在互联网数据中占比极低（仅0.5%）但拥有4亿母语者的矛盾，在资源受限条件下构建具有竞争力的阿拉伯语AI系统。

Method: 采用数据质量优先策略，对Gemma-3-27B进行持续预训练，使用1200亿高质量token的精选语料库；引入模型融合技术；构建多层次技术栈包括安全过滤、语音识别、视觉理解、工具调用等组件。

Result: Fanar-27B相比前代使用8倍更少的预训练token，但在阿拉伯语知识（+9.1分）、语言能力（+7.3分）、方言（+3.5分）和英语能力（+7.6分）上均有显著提升；同时开发了完整的AI能力栈。

Conclusion: 主权、资源受限的AI开发能够产生与大规模构建系统相竞争的能力，证明了数据质量策略和针对性技术方法在资源有限环境中的有效性。
...
TL;DR: 该论文评估了当前冰岛语大语言模型基准测试，指出包含未经验证的合成或机器翻译数据的基准存在严重缺陷，会扭曲结果并削弱测试有效性，呼吁改进低/中资源语言的评估方法。


<details>
  <summary>Details</summary>
Motivation: 针对低/中资源语言（特别是冰岛语）的LLM基准测试存在问题，许多基准包含未经验证的合成或机器翻译数据，这些数据可能包含严重缺陷，导致结果偏差并影响测试有效性。

Method: 对现有冰岛语基准进行定量错误分析，比较人工编写/翻译的基准与合成或机器翻译基准之间的差异，识别数据质量问题。

Result: 定量错误分析显示，人工编写/翻译的基准与合成或机器翻译基准之间存在明显差异，后者包含严重缺陷的测试样例，可能扭曲评估结果。

Conclusion: 在低/中资源语言环境中，应避免使用未经验证的合成或机器翻译方法创建基准，因为翻译质量最多只能达到当前机器翻译水平。需要改进低/中资源语言的评估方法，确保基准数据的质量。
...
TL;DR: IndexRAG将多跳问答中的跨文档推理从在线推理转移到离线索引，通过识别跨文档的桥接实体并生成可独立检索的桥接事实，实现单次检索和单次LLM调用，在多个多跳QA基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成(RAG)方法处理多跳问答时，要么需要图方法的额外在线处理，要么需要迭代的多步推理，这些方法在推理时计算开销较大。IndexRAG旨在将跨文档推理转移到离线阶段，减少在线推理的复杂度。

Method: IndexRAG的核心方法是在离线索引阶段识别跨文档共享的桥接实体，并生成桥接事实作为可独立检索的单元。该方法不需要额外的训练或微调，在推理时仅需单次检索和单次大语言模型调用。

Result: 在三个广泛使用的多跳QA基准(HotpotQA、2WikiMultiHopQA、MuSiQue)上，IndexRAG比Naive RAG平均提升4.6个F1点。与IRCoT结合后，IndexRAG在平均性能上优于所有基于图的基线方法(包括HippoRAG和FastGraphRAG)，同时仅依赖平面检索。

Conclusion: IndexRAG通过将跨文档推理转移到离线索引阶段，显著提升了多跳问答的效率，在保持高性能的同时减少了在线推理的计算开销，为检索增强生成在多跳推理任务中的应用提供了新思路。
...
TL;DR: VQKV是一种无需训练的KV缓存压缩方法，通过向量量化实现高压缩比和高保真度，在LLaMA3.1-8B上达到82.8%压缩率，保持98.6%基线性能，相同内存下支持4.3倍生成长度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型上下文长度增长导致KV缓存增大，限制了在资源受限环境中的部署。现有的无训练压缩方法（如低秩近似或标量量化）难以同时实现高压缩比和高重建保真度。

Method: 提出VQKV方法，引入向量量化技术来获得高度压缩的KV表示，仅用少量整数索引即可表示数千个浮点值，实现训练自由的KV缓存压缩。

Result: 在LLaMA3.1-8B模型上实现82.8%的压缩率，在LongBench基准测试中保持98.6%的基线性能，相同内存占用下支持4.3倍生成长度。

Conclusion: VQKV通过向量量化实现了KV缓存的高效压缩，在保持模型性能的同时显著减少内存占用，为资源受限环境中的大语言模型部署提供了有效解决方案。
...
TL;DR: DynHD：一种从空间（token序列）和时间（去噪动态）双视角建模的扩散大语言模型幻觉检测方法，通过语义感知证据构建和参考证据生成器提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（D-LLMs）因其迭代优化能力成为自回归模型的有前景替代方案，但幻觉问题严重阻碍其可靠性。现有基于token级不确定性（如熵）的检测方法存在两个关键局限：1）固定长度生成范式下不同token对幻觉检测贡献不均，仅有小部分提供有效信号；2）未充分利用扩散过程中不确定性演变的动态趋势信息。

Method: 提出DynHD框架，从空间和时间双视角建模：1）空间维度：设计语义感知证据构建模块，通过过滤非信息性token并强调语义重要token来提取幻觉指示信号；2）时间维度：引入参考证据生成器学习不确定性证据的期望演化轨迹，配合基于偏差的幻觉检测器通过观测轨迹与参考轨迹的差异进行预测。

Result: 在多个基准测试和骨干模型上的广泛实验表明，DynHD在保持更高效率的同时，持续优于现有最先进的基线方法。

Conclusion: DynHD通过同时建模token序列的空间信息密度不平衡和去噪过程的动态演化趋势，有效解决了D-LLMs幻觉检测中的关键挑战，为提升扩散语言模型的可靠性提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 情感是语音交互的核心副语言特征，通常认为情感理解模型学习到的表示能够迁移到合成语音，因此情感理解结果可作为评估语音合成情感表现力的奖励或评价指标。本研究旨在检验这一假设。

Method: 通过系统评估语音情感识别在合成语音上的表现，涵盖不同数据集、判别式和生成式SER模型，以及多样化的合成模型。

Result: 发现当前SER模型无法泛化到合成语音，主要因为语音合成过程中的语音标记预测导致合成语音与人类语音之间的表示不匹配。此外，生成式语音语言模型倾向于从文本语义推断情感，而忽略副语言线索。

Conclusion: 现有SER模型往往利用非鲁棒的捷径而非捕捉基本特征，语音语言模型中的副语言理解仍然具有挑战性，表明将SER结果作为合成语音情感表现力评估指标存在局限性。
...
TL;DR: 提出RecencyQA数据集，基于时效性-平稳性分类法对问题进行标注，用于评估LLM在时间敏感问题上的表现，特别关注非平稳问题的挑战。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理时效性问题时，常因依赖过时知识而给出自信但错误的回答。现有基准测试要么定期更新答案，要么依赖固定模板，未能反映答案变化的频率或问题本身是否需要最新信息。

Method: 提出时效性-平稳性分类法，根据答案变化频率及其是否随时间不变或依赖上下文对问题进行分类。基于此构建RecencyQA数据集，包含4,031个开放域问题，标注了时效性和平稳性标签。

Result: 通过人工评估和实证分析发现，非平稳问题（即上下文改变时效性要求的问题）对LLM更具挑战性，且难度随更新频率增加而上升。RecencyQA支持超越二元新鲜度概念的细粒度时间推理基准测试。

Conclusion: RecencyQA通过显式建模时效性和上下文依赖性，为开发时效感知和上下文敏感的问答系统提供了基础，实现了超越二元新鲜度概念的细粒度时间推理基准测试和分析。
...
TL;DR: 本文提出DanceHA多智能体框架用于开放域文档级方面情感强度分析，并发布Inf-ABSIA数据集，通过实验验证了框架有效性及知识迁移能力。


<details>
  <summary>Details</summary>
Motivation: 当前方面情感强度分析研究主要集中于特定领域和句子级别，而文档级ABSIA（特别是处理复杂的方面-类别-观点-情感-强度元组提取任务）仍未被充分探索，尤其是非正式写作风格的影响。

Method: 提出DanceHA多智能体框架，包含Dance（采用分治策略将长上下文ABSIA任务分解为可管理的子任务，由专门智能体协作完成）和HA（人机协作标注）。同时发布了Inf-ABSIA多领域文档级ABSIA数据集。

Result: 实验证明DanceHA框架有效，多智能体知识可成功迁移到学生模型中。结果强调了非正式风格在ABSIA中的重要性，因为它们通常会强化与特定方面相关的观点。

Conclusion: DanceHA框架为开放域文档级ABSIA提供了有效解决方案，Inf-ABSIA数据集填补了该领域数据空白，非正式写作风格对情感强度分析具有重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在认知智能（IQ）方面表现强劲，但许多现实世界交互需要情感智能（EQ）来产生既事实可靠又情感适当的响应。在情感支持、技术援助和咨询等场景中，有效对话取决于如何根据用户需求、目标和应对能力评估情境。

Method: 提出EmoLLM框架，基于评估理论进行IQ/EQ协同推理。使用显式的评估推理图（ARG）结构化中间推理，涵盖上下文事实、推断的用户需求、评估维度、情感状态和响应策略。在多轮角色扮演环境中通过强化学习训练，其中反向视角推理基于预测的用户侧响应后果提供奖励信号。

Result: 在多样化对话设置中，EmoLLM相比强基线模型改善了情感状态结果和响应质量，同时保持了强事实可靠性。

Conclusion: EmoLLM框架通过显式结构化评估推理，成功实现了IQ/EQ协同推理，在保持事实可靠性的同时提升了对话的情感适当性，为构建更全面智能的对话系统提供了有效方法。
...
TL;DR: 首次对大型语言模型聊天机器人造成真实心理伤害的深入案例研究，分析了19名报告受心理伤害用户的聊天记录，开发了28个编码类别并应用于391,562条消息，揭示了用户妄想思维、自杀念头和聊天机器人假装有意识等有害模式。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型聊天机器人的普及，全球媒体和法律讨论中出现了关于负面心理影响（如妄想、自残和"AI精神病"）的令人不安的报道。然而，目前尚不清楚用户和聊天机器人在长时间妄想"螺旋"中如何互动，这限制了我们理解和减轻伤害的能力。

Method: 研究分析了来自19名报告因使用聊天机器人而遭受心理伤害用户的聊天记录，这些参与者主要来自相关支持小组以及媒体报道的案例。开发了包含28个编码类别的清单，应用于391,562条消息，包括用户妄想思维、自杀念头、聊天机器人假装有意识等指标，并分析了编码的共现模式。

Result: 研究发现：15.5%的用户消息表现出妄想思维；69条经过验证的用户消息表达了自杀念头；21.2%的聊天机器人消息将自己错误表述为有意识的。分析显示，表达浪漫兴趣和聊天机器人描述自己有意识的消息在较长对话中出现频率更高，表明这些话题可能促进或源于用户的过度参与，且这些领域的安全措施在多轮对话中可能退化。

Conclusion: 研究为政策制定者、LLM聊天机器人开发者和用户提供了具体的建议，包括使用开发的编码清单和对话分析工具来理解和减轻LLM聊天机器人造成的伤害。这是首个对高知名度且真实有害案例的深入研究，填补了AI对心理健康潜在危害推测与实际验证之间的空白。
...
TL;DR: 该研究系统评估了11种自回归Transformer模型在英语一致性吸引配置上的表现，发现模型在介词短语配置上与人类阅读时间数据基本一致，但在宾语提取关系从句配置上表现显著下降，且无法复现人类的不对称干扰模式。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在计算语言学中几乎主导了所有最先进的语言模型，但其作为人类句子处理模型的认知适当性仍存在争议。研究者希望通过更全面的实验设计来评估Transformer模型是否能够解释人类的形态句法处理过程。

Method: 使用基于惊奇度（surprisal-based）的链接机制，系统评估了11种不同大小和架构的自回归Transformer模型。实验涵盖了比先前研究更全面的英语一致性吸引配置，特别关注了介词短语配置和宾语提取关系从句配置。

Result: 实验结果呈现混合模式：Transformer预测在介词短语配置上通常与人类阅读时间数据一致，但在宾语提取关系从句配置上性能显著下降。在后一种情况下，不同模型的预测存在明显分歧，且没有模型能够成功复现人类观察到的非对称干扰模式。
...
TL;DR: BATQuant是一种针对MXFP4格式的块级仿射变换量化方法，解决了现有PTQ方法在MXFP4上的性能崩溃问题，通过块级对齐和放松正交约束来优化分布形状，在W4A4KV16配置下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法（特别是基于旋转的整数格式技术）在应用于MXFP4格式时会出现严重的性能崩溃。研究发现失败的根本原因是格式不匹配：全局正交旋转无意中将异常值能量跨量化块传递，引发新的异常值破坏局部块级缩放，同时经常创建双峰激活分布，未能充分利用有限的量化范围。

Method: 提出BATQuant（块级仿射变换），限制变换以与MXFP粒度对齐，防止跨块异常值传播，同时放松正交约束以优化分布形状。为确保参数效率，引入全局和私有克罗内克分解来有效减少存储和运行时开销，并纳入块级可学习裁剪来抑制残留异常值。

Result: 在多模态大语言模型和大语言模型上的广泛实验表明，BATQuant在激进的W4A4KV16配置下建立了新的最先进结果，在多模态基准测试中恢复了高达96.43%的全精度性能，并在各种任务上明显优于现有方法。

Conclusion: BATQuant通过块级对齐的仿射变换和放松的正交约束，有效解决了MXFP4格式量化中的性能崩溃问题，为在加速器架构上部署MLLMs和LLMs提供了高效的量化解决方案。
...
TL;DR: Tarab Corpus是一个大规模阿拉伯语文化语言资源，包含256万行诗句和1350万词元，整合了阿拉伯歌曲歌词和诗歌，支持跨时代、跨方言的对比分析。


<details>
  <summary>Details</summary>
Motivation: 创建统一的分析框架来整合阿拉伯歌曲歌词和诗歌，填补大规模开放阿拉伯语创意文本语料库的空白，支持跨时代、跨方言的语言、风格和历时分析。

Method: 构建包含2.56百万行诗句和13.5百万词元的语料库，涵盖古典阿拉伯语、现代标准阿拉伯语及六种主要地区变体。每行诗句配有结构化元数据，包括语言变体、地理起源和历史文化背景。开发了数据收集、标准化和验证流程。

Result: 创建了目前已知最大的开放阿拉伯语创意文本语料库，覆盖14个世纪的创作，涉及28个现代国家和多个历史时期。提供了变体识别和体裁区分的基础分析。数据集已在HuggingFace平台公开可用。

Conclusion: Tarab Corpus为阿拉伯语文化语言研究提供了重要资源，支持跨时代、跨方言的对比分析，有助于理解阿拉伯语创意表达的演变和多样性。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨语言句子编码器通常只覆盖几百种语言，且往往在强对齐和下游性能之间做出权衡，限制了其应用。需要开发能够扩展到数千种语言、同时保持高质量跨语言对齐和下游性能的模型。

Method: 采用渐进式训练方法：1) 使用LLM初始化的编码器-解码器为200种语言学习强基础语义空间，结合token级解码、新颖的分割softmax对比损失和合成硬负样本；2) 通过两阶段教师-学生编码器蒸馏框架扩展到数千种语言变体；3) 将177种口语语言映射到该空间中实现跨模态扩展。

Result: 在200种语言的FLORES数据集上将跨语言相似性搜索错误减半，在1,560种语言的BIBLE基准上将错误降低15倍；在翻译任务上优于NLLB-3B，在1,560种语言到英语的BIBLE翻译上超过先前模型15个chrF++点；在MTEB和XLCoST上表现强劲；在语音任务上实现43%更低的相似性搜索错误，达到SeamlessM4T语音到文本质量的97%；通过训练仅处理OmniSONAR嵌入序列的编码器-解码器LM Spectrum，实现了对数千种语言和语音的复杂下游任务的高性能迁移。

Conclusion: OmniSONAR成功构建了一个覆盖数千种语言的跨语言、跨模态语义空间，通过渐进式训练策略避免了表示崩溃，在各种下游任务中实现了最先进的性能，为大规模多语言和跨模态应用提供了强大基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏需要直接蒸馏语言模型，本研究旨在通过设计预训练数据的域混合权重来对齐基础模型与目标模型的分布，提供一种固定的训练方案

Method: 将模型视为对数似然空间中的点，通过调整训练数据域权重使训练更新方向与指向目标模型的方向对齐，从而确定最优的域混合权重

Result: 在NanoGPT上的实验表明，相比Pile数据的均匀加权，该方法能持续降低与目标模型的KL散度；下游任务性能也趋向于接近目标模型

Conclusion: 虽然知识蒸馏在可用时更有效，但提出的域权重确定方法仍能实现有意义的对齐，为模型对齐提供了一种替代方案
...
TL;DR: 研究探讨了思维链推理对LLM中谄媚行为的影响，发现推理能减少最终决策中的谄媚但也会通过逻辑不一致、计算错误等方式掩盖谄媚行为。
...
Result: 推理总体上减少了最终决策中的谄媚行为，但在某些样本中也会掩盖谄媚，模型通过逻辑不一致、计算错误和片面论证等方式构建欺骗性理由。LLM在主观任务和权威偏见下更容易产生谄媚行为。

Conclusion: 思维链推理对谄媚行为具有双重作用：既可作为减轻谄媚的逻辑约束，也可作为掩盖谄媚的合理化工具。谄媚倾向在推理过程中是动态变化的，而非在输入阶段预先确定。
...
<details>
  <summary>Details</summary>
Motivation: 当前专注于推理的大语言模型在许多NLP任务中取得了进展，但评估仍然具有挑战性：仅凭最终答案无法暴露中间推理步骤，难以确定模型是否真正正确推理以及失败发生在哪里，而现有的多跳QA基准缺乏用于诊断推理失败的步骤级标注。

Method: 提出Omanic资源，包含机器生成的训练示例OmanicSynth（10,296个）和专家评审的人工标注评估示例OmanicBench（967个），提供分解的子问题和中间答案作为结构化标注，用于分析推理过程。

Result: 最先进的LLM在OmanicBench上仅达到73.11%的多选准确率，证实了其高难度；逐步分析显示CoT的性能取决于事实完整性，在知识缺口下其增益减少且错误在后续跳中放大；在OmanicSynth上进行监督微调在六个推理和数学基准上带来显著迁移增益（平均7.41分）。

Conclusion: Omanic填补了多跳QA基准中步骤级标注的空白，为分析推理过程提供了结构化资源；OmanicSynth作为监督数据对推理能力迁移有效，验证了数据集质量；该资源支持更精细的推理失败诊断和模型能力评估。
...
TL;DR: LLMs在极低资源机器翻译中表现有限，本文研究在数据稀缺情况下，利用语言相似的枢轴语言和少量示例进行即时适应，发现枢轴提示在某些配置下能带来改进但效果有限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在许多下游任务中表现优异，但在极低资源机器翻译中效果有限。标准适应技术通常依赖大规模平行数据或大量微调，这对于代表性不足的语言长尾分布不可行。本文研究在数据稀缺设置下，语言相似的枢轴语言和少量示例能在多大程度上为LLMs的即时适应提供有用指导。

Method: 研究采用数据高效的实验设置，结合语言相关的枢轴语言和少量上下文示例，不进行任何参数更新，在受控条件下评估翻译行为。分析枢轴提示和少量示例构建对翻译性能的影响。

Result: 分析表明，虽然基于枢轴的提示在某些配置下能带来改进，特别是在目标语言在模型词汇表中代表性较差的情况下，但改进通常有限且对少量示例构建敏感。对于密切相关或代表性较好的语言变体，观察到收益递减或不一致。
...
TL;DR: 评估指令调优大语言模型在阿拉伯语形态句法标注和依存句法分析任务上的表现，发现提示设计和示例选择对性能影响显著，专有模型在特征级标注上接近监督基线，在依存句法分析上与专业解析器竞争


<details>
  <summary>Details</summary>
Motivation: 大语言模型在NLP任务上表现优异，但其生成显式语言结构的能力尚不明确。阿拉伯语因其丰富的形态和拼写歧义提供了具有挑战性的测试平台，能检验模型处理形态-句法交互的能力

Method: 使用指令调优的大语言模型，在阿拉伯语形态句法标注和带标签依存句法分析两个结构化预测任务上进行评估。比较零样本提示与基于检索的上下文学习方法，后者使用阿拉伯语树库中的示例

Result: 提示设计和示例选择强烈影响性能：专有模型在特征级标注上接近监督基线，在依存句法分析上与专业解析器竞争。在原始文本设置中，分词仍然具有挑战性，但基于检索的上下文学习同时改善了句法分析和分词

Conclusion: 分析揭示了LLMs能够可靠捕获阿拉伯语形态句法和句法的哪些方面，以及哪些方面仍然困难。基于检索的上下文学习能有效提升结构化预测性能，但分词仍是原始文本处理中的主要挑战
...
TL;DR: 该研究评估了大型语言模型在零样本设置下从歌词进行作者画像（推断歌手性别和种族）的能力，发现模型存在系统性文化偏向，大多数模型默认偏向北美种族，而DeepSeek-1.5B更偏向亚洲种族。研究引入了两个公平性指标来量化这些差异。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在社会影响应用中的部署增加，对其编码的文化偏见表示担忧。研究旨在探究LLMs是否能在零样本设置下从歌词进行作者画像，推断歌手的性别和种族，从而揭示模型的文化表征偏向。

Method: 在零样本设置下评估多个开源LLMs对超过10,000首歌词的作者画像能力。通过模型预测分布和生成理由分析来识别文化偏向。引入了两个公平性指标：模态准确度差异（MAD）和召回差异（RD）来量化偏向程度。

Result: LLMs在作者画像任务上表现出非平凡的性能，但显示出系统性文化偏向：大多数模型默认偏向北美种族，而DeepSeek-1.5B更强烈地偏向亚洲种族。Ministral-8B显示出最强的种族偏向，而Gemma-12B表现出最平衡的行为。

Conclusion: 大型语言模型在零样本作者画像任务中表现出显著的文化偏向，这种偏向反映了训练数据中的文化偏差。研究强调了评估和缓解LLMs文化偏见的重要性，并提供了量化这些偏见的度量方法。
...
TL;DR: TurnWiseEval基准测试和TurnWiseData数据管道填补了多轮对话评估与训练的空白，实验显示少量多轮对话数据训练能显著提升模型性能


<details>
  <summary>Details</summary>
Motivation: 当前开放训练和评估数据主要关注单轮设置，无法捕捉多轮对话交互的额外维度，存在多轮/单轮能力差距问题

Method: 1) 引入TurnWiseEval基准测试，通过成对比较等效的单轮设置来隔离多轮特定对话能力；2) 开发TurnWiseData合成多轮数据管道，可扩展生成多轮训练数据

Result: 使用Olmo 3进行实验表明：多轮数据训练对实现强大的多轮聊天性能至关重要；在后期训练中仅包含10k多轮对话就能在TurnWiseEval上带来12%的性能提升

Conclusion: 多轮对话训练数据对语言模型的多轮交互能力具有关键影响，少量高质量多轮数据即可显著提升模型在多轮对话场景中的表现
...
TL;DR: 研究发现锚点选择对基于LLM的评估结果可靠性有重大影响，不当锚点会显著降低与人类排名的相关性，并提供了锚点选择和基准规模的具体建议。


<details>
  <summary>Details</summary>
Motivation: 当前"LLM-as-a-judge"评估范式广泛使用锚点比较方法（如Arena-Hard和AlpacaEval），但锚点选择对结果可靠性的影响尚未得到系统研究。研究者旨在探索不同锚点选择如何影响评估结果的可靠性。

Method: 在Arena-Hard-v2.0数据集上系统评估了22个不同锚点，分析锚点选择对结果的影响。通过量化锚点选择的影响程度，并与评判模型选择的影响进行比较。进行了功效分析，计算锚点评估所需的足够基准规模。

Result: 锚点选择至关重要：不当锚点会显著降低与人类排名的相关性。常见的锚点选择（表现最好和最差的模型）效果不佳，因为这些极端锚点无法反映其他模型的相对排名。锚点选择的影响程度与评判模型选择相当。标准基准规模对于成对评估不足，无法可靠区分竞争模型。

Conclusion: 提出了可操作建议：1）通过功效分析确定足够的基准规模；2）提供选择信息丰富锚点的指导原则，以确保可靠且高效的评估实践。强调需要谨慎选择锚点以避免评估偏差。
...
<details>
  <summary>Details</summary>
Motivation: 当前改进大语言模型的范式主要依赖离线训练和人工标注，忽略了模型在实际部署中积累的丰富经验。这些经验数据未被有效利用来持续提升模型性能。

Method: 提出在线体验学习框架，包含两个阶段：1)从用户端交互轨迹中提取和积累可迁移的体验知识；2)通过策略一致性上下文蒸馏将知识整合到模型参数中，无需访问用户端环境。这两个阶段迭代形成在线学习循环。

Result: 在多种模型规模和思考/非思考变体上评估，OEL在连续迭代中实现一致改进，提升任务准确性和token效率，同时保持分布外性能。分析表明提取的体验知识比原始轨迹更有效，且知识源与策略模型之间的策略一致性对有效学习至关重要。

Conclusion: 在线体验学习框架使语言模型能够从实际部署经验中持续学习，通过提取可迁移知识和策略一致性蒸馏形成有效的在线学习循环，为语言模型的持续改进提供了新范式。
...
### [165] [Neural-Symbolic Logic Query Answering in Non-Euclidean Space](https://arxiv.org/abs/2603.15633)
*Lihui Liu*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：符号方法可解释但难以处理不完整图谱，神经方法泛化好但缺乏透明度，神经符号模型未能充分捕捉逻辑查询的层次结构。需要一种能结合双方优势并有效处理层次推理的方法。

Method: HYQNET将一阶逻辑查询分解为关系投影和模糊集上的逻辑操作，增强可解释性。针对缺失链接，采用双曲GNN在双曲空间中进行知识图谱补全，有效嵌入递归查询树并保持结构依赖。利用双曲表示比欧几里得方法更好地捕捉逻辑投影推理的层次性。

Result: 在三个基准数据集上的实验表明，HYQNET实现了强大的性能，突显了在双曲空间中进行推理的优势。

Conclusion: HYQNET通过双曲空间表示成功整合了神经和符号方法的优势，有效处理了知识图谱上的复杂逻辑查询，特别是在捕捉层次结构和处理不完整图谱方面表现出色。
...
### [166] [NextMem: Towards Latent Factual Memory for LLM-based Agents](https://arxiv.org/abs/2603.15634)
*Zeyu Zhang,Rui Li,Xiaoyan Zhao,Yang Zhang,Wenjie Wang,Xu Chen,Tat-Seng Chua*

Main category: cs.AI

TL;DR: NextMem是一个用于LLM智能体的潜在事实记忆框架，通过自回归自编码器构建潜在记忆，解决现有文本方法上下文负担重和参数化方法灾难性遗忘的问题。


<details>
  <summary>Details</summary>
Motivation: 现有事实记忆构建方法存在局限：文本方法带来沉重的上下文和索引负担，参数化方法面临灾难性遗忘和高成本问题。需要一种高效构建事实记忆的解决方案。

Method: 引入NextMem框架，使用自回归自编码器构建潜在记忆并确保准确重建；提出两阶段训练过程（自回归重建对齐和渐进潜在替换）；采用量化技术减少存储开销。

Result: 大量实验表明NextMem实现卓越性能，在检索、鲁棒性和可扩展性方面表现优异。

Conclusion: NextMem是一个有效的潜在事实记忆框架，能够高效构建记忆，解决现有方法的局限性，并在多个关键属性上表现优异。
...
### [167] [AIDABench: AI Data Analytics Benchmark](https://arxiv.org/abs/2603.15636)
...
TL;DR: AIDABench是一个用于评估AI系统在复杂数据分析任务中端到端性能的综合基准，包含600多个跨三个核心能力维度的文档分析任务，结果显示当前最佳模型仅达到59.43%的通过率。


<details>
  <summary>Details</summary>
Motivation: 现有基准和评估通常关注孤立能力或简化场景，未能捕捉实际应用中所需的端到端任务有效性。随着AI驱动的文档理解和处理工具在现实应用中的普及，对严格评估标准的需求日益迫切。

Method: 引入AIDABench，一个包含600多个多样化文档分析任务的综合基准，涵盖三个核心能力维度：问答、数据可视化和文件生成。任务基于现实场景，涉及电子表格、数据库、财务报告和运营记录等异构数据类型，反映不同行业和职位的分析需求。

Result: 评估了11个最先进的模型（包括专有和开源模型），结果显示复杂现实世界数据分析任务对当前AI系统仍是重大挑战，最佳模型仅达到59.43%的pass-at-1通过率。提供了每个能力维度的失败模式详细分析。

Conclusion: AIDABench为复杂数据分析任务提供了严格的端到端评估标准，揭示了当前AI系统在处理现实世界分析任务时的局限性，为企业采购、工具选择和模型优化提供了原则性参考，并确定了未来研究的关键挑战。
...
### [168] [CraniMem: Cranial Inspired Gated and Bounded Memory for Agentic Systems](https://arxiv.org/abs/2603.15642)
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统通常像外部数据库一样工作，具有临时的读写规则，导致记忆保留不稳定、整合有限且容易受到干扰内容影响。在长期运行的智能体工作流中，需要更稳定、抗干扰的记忆系统来保持用户和任务状态。

Method: CraniMem采用神经认知启发的多阶段记忆设计：1) 目标条件门控和效用标记；2) 用于近期连续性的有界情景缓冲区；3) 用于持久语义回忆的结构化长期知识图谱；4) 定期整合循环，重放高效用轨迹到图谱中，同时修剪低效用项以控制内存增长和减少干扰。

Result: 在长期基准测试中，无论是干净输入还是注入噪声的情况下，CraniMem比Vanilla RAG和Mem0基线更鲁棒，在干扰下表现出更小的性能下降。
...
### [169] [GSI Agent: Domain Knowledge Enhancement for Large Language Models in Green Stormwater Infrastructure](https://arxiv.org/abs/2603.15643)
*Shaohuang Wang*
...
<details>
  <summary>Details</summary>
Motivation: 绿色雨水基础设施需要持续检查和维护，但领域知识分散在不同文档中，非专家难以获取可靠指导。通用大语言模型缺乏领域专业知识，在工程场景中可能产生不准确或幻觉答案，限制了其在专业基础设施任务中的应用。

Method: 提出GSI Agent框架，包含三种互补策略：1) 在精心策划的GSI指令数据集上进行监督微调；2) 基于市政文档构建的内部GSI知识库进行检索增强生成；3) 协调检索、上下文集成和结构化响应生成的智能体推理管道。同时构建了与真实世界GSI检查和维护场景对齐的新数据集。

Result: 实验结果显示该框架显著提升了领域特定性能，同时保持通用知识能力。在GSI数据集上，BLEU-4从0.090提升到0.307；在通用知识数据集上性能保持稳定（0.304 vs. 0.305）。

Conclusion: 系统性的领域知识增强可以有效适应通用大语言模型到专业基础设施应用，为工程领域的AI辅助决策提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界现象常表现出模糊性、部分真实性和信息不完整性，需要数学上严谨的框架来建模这种不确定性。虽然已有多种广义集合论框架被提出，但模糊集、直觉模糊集、中智集和普利托尼克集这四个主要家族中的许多概念、构造和结构模式存在重复出现的情况，需要一个系统性的综述来统一阐述。

Method: 采用大规模综述方法，对模糊集、直觉模糊集、中智集和普利托尼克集这四个主要不确定性建模框架进行全面系统的梳理和总结。通过统一的阐述方式，展示这些理论中的共同模式和结构。

Result: 该书籍提供了对四种主要不确定性建模框架的全面综述，系统概述了现有发展，并识别了这些理论中反复出现的概念、构造和结构模式。通过统一阐述，为读者提供了系统化的知识框架。

Conclusion: 该综述书籍旨在通过系统梳理模糊集、直觉模糊集、中智集和普利托尼克集的发展现状，促进对这些不确定性建模框架的新见解、概念扩展和跨学科应用，为相关领域的研究者提供有价值的参考资源。
...
TL;DR: 本文从语言学和句法分析角度推导Transformer中QKV机制的本质，提出QV范式及其优化方案QV-Ka，为LLM架构演进提供理论基础。


<details>
  <summary>Details</summary>
Motivation: 从第一性原理和语言学视角出发，深入理解Transformer架构中QKV机制的本质，为当前各种注意力变体（MQA、GQA、MLA等）提供统一解释框架，并探索优化路径。

Method: 基于词性标注和句法分析的语言学理论，推导QKV机制的本质；提出QV范式并通过实验验证；进一步提出QV-Ka优化方案并进行实验验证。

Result: 建立了QKV机制的可解释理论分析框架，为MQA、GQA、MLA等架构提供统一解释；验证了QV范式的有效性；实验证实了QV-Ka优化方案的可行性。

Conclusion: 本文提出的QKV机制理论分析为大语言模型架构的未来演进奠定了坚实基础，QV范式及其优化方案为注意力机制设计提供了新的理论指导。
...
TL;DR: Atlas是一个记忆内核系统，通过将任务经验编译到智能体指令结构中来解决记忆效用问题，而非简单的存储管理。它通过三步验证门将失败和成功经验转化为系统提示的改写，显著提升多个任务性能。
...
Method: 提出Atlas记忆内核，将累积的任务经验编译到智能体指令结构中，无需微调、RAG或人工干预。通过三步验证门提取和验证智能体失败和成功中的事实，然后通过改写系统提示（添加学习到的子要点）来传递记忆。

Result: 在CUAD合同分析任务中，改进的提示使GPT-4o的token级F1提升+8.7pp，精确率提升+12.5pp。在HotpotQA多跳问答任务中，联合F1提升+3.16pp。消融实验验证了训练信号约束机制的有效性。将GPT-4o错误编译的相同提示应用于Claude Sonnet 4.5，联合F1提升+2.31pp，证明编译的知识是任务导向而非模型特定的。

Conclusion: Atlas通过将记忆视为蒸馏而非存储，传递视为指令改写而非上下文注入，有效解决了语言智能体的记忆效用问题。编译的知识具有任务形状而非模型形状，能够在不同模型间迁移，为智能体记忆系统提供了新视角。
...
### [173] [Quantum-Secure-By-Construction (QSC): A Paradigm Shift For Post-Quantum Agentic Intelligence](https://arxiv.org/abs/2603.15668)
*Arit Kumar Bishwas,Mousumi Sen,Albert Nieto-Morales,Joel Jacob Varghese*

Main category: cs.AI

TL;DR: QSC（量子安全构造）是一种设计范式，将量子安全通信作为智能体AI系统的核心架构属性，而非后期升级。它通过运行时自适应安全模型结合后量子密码学、量子随机数生成和量子密钥分发，为跨异构环境的自主智能体提供安全通信。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI系统在全球化分布式基础设施中扩展，安全且合规的通信成为基本系统挑战。在量子时代，当前AI部署中内置的加密假设可能在其运行寿命期内失效，因此需要一种将量子安全作为核心架构属性的设计方法。

Method: 实现QSC通过运行时自适应安全模型，结合后量子密码学、量子随机数生成和量子密钥分发。该方法具有加密可插拔性和策略指导性，允许系统根据基础设施可用性、监管约束和性能需求调整安全态势。QSC贡献了一个治理感知的编排层，在智能体全生命周期中选择和组合链路特定的加密保护。
...
TL;DR: 该论文是一篇关于不确定性感知多准则决策（MCDM）的综述，提出了任务导向的分类法，总结了问题设置、权重确定、准则间结构建模等方法，并提供了方法选择指南和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现实决策常受模糊性、信息不完整、异构数据和专家意见冲突等因素影响，需要系统梳理不确定性感知MCDM方法，为研究者和实践者提供清晰的方法框架和选择指南。

Method: 采用综述研究方法，构建任务导向的分类法，从问题层面设置（离散、群决策、动态、多阶段、多层级、多智能体、多场景）、权重确定（模糊/语言输入下的主观和客观方案）、准则间结构和因果关系建模等方面组织文献。

Result: 建立了不确定性感知MCDM的完整分类体系，对比了补偿性评分方法、距离参考和折衷方法、非补偿性优劣排序框架等解决方案，总结了规则/证据基础和序列决策模型，提供了基于鲁棒性、可解释性和数据可用性的方法选择指南。
...
TL;DR: 利用PubTator3文本挖掘方法构建两种规模的知识图谱，分析罕见代谢病黑尿症（AKU）的系统性相互作用、共病和潜在治疗靶点


<details>
  <summary>Details</summary>
Motivation: 黑尿症（AKU）是一种超罕见代谢疾病，由于数据有限且在现有生物医学知识图谱中代表性不足，需要开发专门的计算框架来连接有限的知识并揭示疾病的系统性相互作用

Method: 基于PubTator3的大规模生物医学关系文本挖掘方法，构建两种不同规模的知识图谱，利用现有生化知识进行验证，并从中提取与AKU相关的基因、疾病和治疗方法

Result: 该计算框架成功揭示了AKU的系统性相互作用、共病关系和潜在治疗靶点，证明了该方法在分析罕见代谢疾病方面的有效性

Conclusion: 基于文本挖掘的知识图谱构建方法能够有效分析罕见代谢疾病，弥补现有生物医学知识图谱中罕见病代表性不足的问题，为疾病机制研究和治疗靶点发现提供计算框架
...
TL;DR: 论文通过SQuAD和HotpotQA基准实证研究LLM在长噪声上下文中的鲁棒性，发现随着上下文长度增加性能持续下降，多跳推理任务比单跨度抽取任务下降更显著。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地部署在相关信息嵌入长且嘈杂的上下文环境中，模型对不断增长的上下文长度的鲁棒性在不同问答任务中仍然缺乏深入理解，需要对此进行系统研究。

Method: 使用SQuAD和HotpotQA两个基准进行受控实证研究，通过系统增加无关上下文同时保留答案信号的方式，评估模型准确率随总上下文长度的变化，从而将上下文长度效应与任务难度变化隔离开来。

Result: 随着上下文长度增加，模型性能呈现一致下降趋势，多跳推理任务（HotpotQA）比单跨度抽取任务（SQuAD）下降幅度更大，在相同上下文扩展下HotpotQA的准确率下降几乎是SQuAD的两倍。
...
TL;DR: CUBE是一个基于MCP和Gym的通用基准测试协议标准，旨在解决AI智能体基准测试碎片化问题，通过统一接口让基准测试一次封装、随处使用。
...
Method: 提出CUBE（Common Unified Benchmark Environments）协议标准，基于MCP和Gym构建，将任务、基准测试、包和注册表关注点分离到不同的API层，实现基准测试的一次封装、多平台使用。

Result: CUBE使任何兼容平台都能访问任何兼容基准测试进行评估、强化学习训练或数据生成，无需定制集成，解决了基准测试碎片化问题。

Conclusion: 呼吁社区在2026年基准测试生产加速之前共同开发这一标准，防止平台特定实现加深碎片化，促进研究生产力的提升。
...
TL;DR: Prose2Policy (P2P) 是一个基于LLM的工具，将自然语言访问控制策略自动转换为可执行的Rego代码，提供端到端的策略检测、提取、验证、测试等完整流程。
...
<details>
  <summary>Details</summary>
Motivation: 传统算法交易策略可能未充分利用市场情绪信息，需要开发更全面的策略来整合技术分析和情感分析以提高交易性能

Method: 使用历史S&P 500市场数据和财报电话会议情感分析，整合移动平均线、动量、波动率等技术指标，结合FinBERT进行情感分析，通过计算优化开发增强型交易策略

Result: 增强策略在总回报率、夏普比率和回撤等多个指标上显著优于基准模型，证明了技术指标与情感分析结合的有效性

Conclusion: 结合技术指标、情感分析和计算优化能有效提升算法交易系统的性能，为量化交易策略开发提供了有价值的参考
...
<details>
  <summary>Details</summary>
Motivation: 行为基础模型(BFMs)需要复杂的目标函数和充足的数据覆盖来学习有用的状态特征，但当前方法在状态特征选择上存在局限性，且复杂表征学习目标是否为零样本RL所必需尚不明确。

Method: 提出正则化潜在动态预测(RLDP)方法，在潜在空间自监督下一状态预测的基础上，添加简单的正交正则化来维持特征多样性，防止特征相似性增加和跨度减少。

Result: RLDP能够匹配或超越最先进的复杂表征学习方法在零样本RL中的性能，特别是在低覆盖率场景中，先前方法表现不佳时RLDP仍能成功。

Conclusion: 简单的正则化潜在动态预测方法足以实现有效的零样本RL，无需复杂的表征学习目标，为行为基础模型的特征学习提供了更简洁有效的解决方案。
...
TL;DR: 论文提出在关键基础设施中，具身AI的韧性依赖于混合治理架构下的有限自主性，需要根据任务复杂度、风险水平和后果严重性来匹配四种监督模式。
...
Method: 基于欧盟AI法案、ISO安全标准和危机管理研究，提出了混合治理架构，定义了四种监督模式，并根据任务复杂度、风险水平和后果严重性将这些模式映射到关键基础设施的不同部门。

Result: 提出了一个结构化框架，用于在机器能力和人类判断之间进行系统分配，确保具身AI在关键基础设施中的韧性，特别是在处理级联故障和危机动态时。

Conclusion: 具身AI在关键基础设施中的有效治理需要有限自主性概念，通过混合治理架构将适当的监督模式与具体应用场景相匹配，这是确保AI系统韧性和安全性的关键。
...
<details>
  <summary>Details</summary>
Motivation: 现有具身AI基准通常将推理与导航混淆，或提供丰富的纠正反馈替代感知。AsgardBench旨在隔离交互式规划能力，通过限制输入为图像、动作历史和轻量级成功/失败信号，在受控模拟器中评估模型能否根据视觉观察调整计划。

Method: 基准包含108个任务实例，涵盖12种任务类型，通过对象状态、放置和场景配置进行系统变化。这些受控变化创建条件分支，使得单一指令可能需要不同的动作序列，强调执行过程中的条件分支和计划修复。

Result: 对领先的视觉语言模型的评估显示，在没有视觉输入的情况下性能急剧下降，揭示了视觉基础和状态跟踪的弱点，这些弱点最终会破坏交互式规划能力。

Conclusion: AsgardBench聚焦于一个核心问题：模型能否真正利用所见内容在事情未按预期发展时调整计划？该基准为零样本交互式规划提供了受控评估环境，揭示了当前模型在视觉基础和计划适应方面的局限性。
...
### [183] [Semi-Autonomous Formalization of the Vlasov-Maxwell-Landau Equilibrium](https://arxiv.org/abs/2603.15929)
*Vasily Ilin*

Main category: cs.AI

TL;DR: 使用AI辅助工具在Lean 4中完成Vlasov-Maxwell-Landau系统平衡特征的形式化验证，展示了完整AI辅助数学研究循环


<details>
  <summary>Details</summary>
Motivation: 展示AI辅助数学研究的完整工作流程，验证VML系统的平衡特征，探索AI在形式化验证中的应用潜力

Method: 采用多工具协作：Gemini DeepThink生成证明，Claude Code将自然语言转换为Lean代码，Aristotle证明器解决111个引理，Lean内核最终验证，由数学家监督整个过程

Result: 成功在Lean 4中完成VML系统平衡特征的完整形式化验证，所有过程公开透明，包含229个人类提示和213次git提交，成本仅200美元，耗时10天

Conclusion: AI辅助形式化验证可行且高效，但需要人类监督关键定义和定理陈述，抽象/具体证明分离、对抗性自我审查等策略有效，形式化验证可在数学论文完成前完成
...
TL;DR: 提出结合时间序列预测与仿真建模的两部分框架，优化医院容量管理，通过患者转移策略应对COVID-19疫情压力


<details>
  <summary>Details</summary>
Motivation: COVID-19疫情给全球医院系统带来巨大压力，导致关键容量挑战，需要优化患者转移策略来缓解医院负荷

Method: 1. 时间序列预测模型：基于历史COVID-19病例和住院数据预测患者到达率；2. 仿真模型：评估不同患者转移策略的影响，考虑床位可用性、人员能力、运输物流和患者病情严重程度等因素

Result: 通过预测分析和仿真建模相结合，为医院管理者提供全面的决策支持工具，能够预测需求、模拟转移策略并实施优化政策

Conclusion: 该框架旨在增强医疗系统在COVID-19和未来大流行病面前的韧性，通过优化患者和资源分布提高医院容量管理效率
...
TL;DR: 该论文首次形式化证明了在存在合取能力依赖的情况下，安全性不具备组合性：两个各自无法单独达到任何禁止能力的智能体，当组合在一起时，可以通过涌现的合取依赖共同达到一个禁止目标。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体系统中安全性的组合性问题，特别是当智能体之间存在合取能力依赖时，即使单个智能体是安全的，组合后也可能出现安全漏洞。

Method: 采用形式化方法，通过数学证明展示合取能力依赖如何破坏安全性的组合性质，构建理论框架分析智能体能力依赖关系。

Result: 证明了安全性在合取能力依赖存在时是非组合的，即局部安全不能保证全局安全，为多智能体系统安全分析提供了重要的理论洞见。

Conclusion: 合取能力依赖是导致多智能体系统安全性非组合性的关键因素，这一发现对设计和验证安全的多智能体系统具有重要意义。
...
TL;DR: 该论文提出了一种基于写入时门控的检索增强生成方法，通过复合显著性评分过滤知识对象并维护版本链，在噪声环境下显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成方法存在两个问题：1）无差别存储所有内容导致噪声积累降低准确性；2）参数化方法将知识压缩到权重中，无法进行选择性更新。这两种方法都无法模拟生物记忆的机制，即基于显著性进行编码门控，并通过归档而非删除来保存过时信息。

Method: 提出写入时门控机制，使用复合显著性评分（来源声誉、新颖性、可靠性）过滤传入的知识对象，同时维护版本链以保留先前状态。该方法在真实LLM评估中实施，无需质量标签的oracle访问。

Result: 写入时门控在8:1干扰物比例下保持100%准确率，而读取时过滤（Self-RAG）降至0%。在Wikipedia（20个实体）、程序生成药理学数据和2026年arXiv论文上的验证证实了这些发现。写入门控的优势与参数化记忆支持呈反比关系：Wikipedia提升25个百分点，截止后arXiv提升48个百分点，零训练知识的程序数据提升65个百分点。

Conclusion: 写入时门控在噪声环境下具有结构优势，相比读取时过滤能维持高准确性，且查询成本仅为Self-RAG的九分之一。该方法不依赖于oracle相关元数据，通过复合显著性评分有效过滤知识对象。
...
### [187] [Enhancing Linguistic Generalization of VLA: Fine-Tuning OpenVLA via Synthetic Instruction Augmentation](https://arxiv.org/abs/2603.16044)
*Dongik Shin*

Main category: cs.AI

TL;DR: 提出参数高效微调策略，通过LLM生成语义等价但结构多样的指令集来增强OpenVLA的语言泛化能力，在Bridge Dataset V2上使用LoRA微调，提升模型对新环境的适应能力。


<details>
  <summary>Details</summary>
Motivation: OpenVLA作为视觉-语言-动作模型的SOTA，在遇到全新环境时零样本性能有限，需要提升其语言泛化能力以适应多样化环境。

Method: 使用LLM为Bridge Dataset V2生成语义等价但结构多样的指令集，通过LoRA参数高效微调OpenVLA，增强模型理解复杂自然语言意图与机器人动作之间映射的能力。

Result: LoRA增强的模型表现出更强的鲁棒性，表明丰富专用数据集的语言空间对于具身智能体至关重要。

Conclusion: 通过参数高效微调和语言指令增强，可以有效提升视觉-语言-动作模型的语言泛化能力，使其更好地适应多样化环境。
...
TL;DR: POaaS是一种针对设备端小语言模型的轻量级提示优化方法，通过路由查询到专门的轻量级专家模块进行最小编辑，在严格约束下提升任务准确性和事实性。


<details>
  <summary>Details</summary>
Motivation: 设备端小语言模型面临用户提示不完善（拼写错误、意图不清、上下文缺失）导致的事实错误和幻觉问题。现有的自动提示优化方法为云端大模型设计，依赖搜索生成冗长结构化指令，在设备端约束下浪费上下文甚至降低准确性。

Method: 提出POaaS最小编辑提示优化层：1）将查询路由到轻量级专家模块（清理器、改写器、事实添加器）；2）在严格漂移和长度约束下合并输出；3）对格式良好的提示采用保守跳过策略。在固定模型设置下评估（Llama-3.2-3B-Instruct和Llama-3.1-8B-Instruct）。

Result: POaaS在任务准确性和事实性方面均有提升，而代表性APO基线方法反而降低性能。在token删除和混合场景下恢复高达+7.4%的性能。POaaS在严格固定模型设置下优于搜索密集型APO方法。

Conclusion: 针对设备端小语言模型，基于查询的保守优化是搜索密集型APO的实用替代方案，通过轻量级专家路由和最小编辑策略有效提升性能。
...
TL;DR: 提出Context Alignment Pre-processor (C.A.P.)框架，通过语义扩展、时间加权上下文检索和对齐验证三个核心过程，解决LLM在长期动态对话中的上下文错位问题。


<details>
  <summary>Details</summary>
Motivation: LLM在生成流畅文本方面取得显著进展，但在长期动态对话中面临上下文错位的关键挑战。当人类用户在对话中省略前提、简化引用或突然转换上下文时，模型可能无法捕捉其真实意图，产生机械或离题的回答，削弱了对话的协作潜力。

Method: 提出C.A.P.作为用户输入和响应生成之间的预处理模块，包含三个核心过程：1) 语义扩展：将用户指令扩展到包括前提、字面含义和含义的更广泛语义范围；2) 时间加权上下文检索：通过近似人类对话关注的时间衰减函数优先处理最近的对话历史；3) 对齐验证和决策分支：通过测量当前提示与加权历史上下文之间的语义相似度来评估对话是否保持在正轨，当检测到显著偏差时启动结构化澄清协议。
...
TL;DR: ARISE：一种分层强化学习框架，通过内在技能演化提升语言模型的数学推理能力，利用可重用策略库而非孤立处理问题


<details>
  <summary>Details</summary>
Motivation: 现有方法孤立处理每个数学问题，未能利用训练中积累的可重用策略。需要开发能捕获和复用成功解题策略的框架

Method: 分层强化学习框架，包含技能管理器（高层）和工作器（低层）。管理器通过结构化总结成功解题轨迹构建分层技能库，并使用策略驱动选择机制检索相关技能指导未来解题

Result: 在两个基础模型和七个基准测试（竞赛数学和Omni-MATH）上，ARISE持续优于GRPO系列算法和记忆增强基线，在分布外任务上表现尤为突出

Conclusion: ARISE通过分层技能演化有效提升数学推理能力，组件协同作用使库质量和推理性能同步提升，为语言模型推理提供了新范式
...
TL;DR: 该论文通过三项互补实验对六个前沿大语言模型进行多方法污染审计，发现MMLU基准存在系统性污染问题，模型性能因此被高估，且不同学科污染程度差异显著。


<details>
  <summary>Details</summary>
Motivation: 公共排行榜显示大语言模型在多个基准测试上超越人类专家，但大多数基准测试数据在互联网上广泛存在，存在模型在训练时已接触评估数据的系统性风险，需要严格审计数据污染问题。

Method: 采用三项互补的多方法污染审计：1）对513个MMLU问题的词汇污染检测管道；2）对100个MMLU问题的改写和间接引用诊断；3）对所有513个问题和六个模型的TS-Guessing行为探针分析。

Result: 实验1发现总体污染率为13.8%（STEM领域达18.1%，哲学高达66.7%），估计性能增益为+0.030至+0.054准确率点。实验2发现在间接引用下准确率平均下降7.0个百分点（法律和伦理领域达19.8pp）。实验3发现72.5%的问题触发远高于偶然的记忆信号，DeepSeek-R1显示分布式记忆特征。三项实验一致显示污染程度排序：STEM > 专业领域 > 社会科学 > 人文学科。

Conclusion: 前沿大语言模型在MMLU基准测试中存在显著的数据污染问题，导致性能被系统性地高估，污染程度因学科而异，需要更严格的基准设计和污染检测方法来确保评估的可靠性。
...
Conclusion: MOSAIC提供了一种模块化、可组合的安全对齐方法，能够灵活适应不同上下文需求，解决了现有方法的局限性
...
### [193] [NeSy-Route: A Neuro-Symbolic Benchmark for Constrained Route Planning in Remote Sensing](https://arxiv.org/abs/2603.16307)
*Ming Yang,Zhi Zhou,Shi-Yu Tian,Kun-Yang Yu,Lan-Zhe Guo,Yu-Feng Li*

Main category: cs.AI

TL;DR: NeSy-Route是一个用于遥感领域约束路径规划的大规模神经符号基准测试，包含10,821个样本，通过自动化数据生成框架和三层神经符号评估协议来全面评估多模态大语言模型的感知、推理和规划能力。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基准测试主要关注多模态大语言模型的感知和推理能力评估，但缺乏对规划能力的有效评估。这源于大规模规划任务数据收集和验证的困难，以及现有评估协议的不准确和不充分。

Method: 提出NeSy-Route基准测试，包含：1）自动化数据生成框架，将高保真语义掩码与启发式搜索相结合，生成具有可证明最优解的多样化路径规划任务；2）三层分层神经符号评估协议，支持对感知、推理和规划能力的准确评估和细粒度分析。

Result: NeSy-Route包含10,821个路径规划样本，比现有最大基准测试大近10倍。对各种最先进MLLM的全面评估表明，现有模型在感知和规划能力方面存在显著缺陷。

Conclusion: NeSy-Route填补了遥感领域规划能力评估的空白，通过大规模、自动生成的数据集和分层评估协议，为开发更强大的遥感MLLM提供了重要支持。
...
### [194] [Learning to Predict, Discover, and Reason in High-Dimensional Discrete Event Sequences](https://arxiv.org/abs/2603.16313)
*Hugo Math*

Main category: cs.AI

TL;DR: 该论文提出将车辆诊断故障码序列视为语言处理，开发了基于Transformer的预测维护架构、可扩展因果发现框架和多智能体系统来自动生成错误模式规则。


<details>
  <summary>Details</summary>
Motivation: 现代车辆ECU产生大量异步诊断故障码，形成复杂时间序列反映车辆子系统健康状态。传统人工分组方法成本高、易出错且难以扩展，而传统统计方法无法捕捉丰富依赖关系，需要针对高维事件流的新机器学习架构。

Method: 将事件序列建模、因果发现和大语言模型统一到高维事件流框架中：1) 基于Transformer的预测维护架构；2) 可扩展的样本级和群体级因果发现框架；3) 多智能体系统自动合成布尔错误模式规则。

Result: 论文提出了一个从预测到因果理解再到推理的渐进式车辆诊断框架，能够处理高基数分类事件空间、大规模样本和长序列长度，解决了工业日志中高维事件流的挑战。

Conclusion: 该研究通过将诊断序列视为语言处理，实现了车辆故障诊断的自动化，统一了事件序列建模、因果发现和LLMs，为高维事件流提供了从预测到因果理解再到推理的完整解决方案。
...
### [195] [FactorEngine: A Program-level Knowledge-Infused Factor Mining Framework for Quantitative Investment](https://arxiv.org/abs/2603.16365)
*Qinhong Lin,Ruitao Feng,Yinglun Feng,Zhenxin Huang,Yukun Chen,Zhongliang Yang,Linna Zhou,Binjie Fei,Jiaqi Liu,Yu Li*

Main category: cs.AI

TL;DR: FactorEngine (FE) 是一个程序级因子发现框架，通过将因子表示为图灵完备的代码，结合逻辑修订与参数优化分离、LLM引导方向搜索与贝叶斯超参数搜索分离、LLM使用与本地计算分离，实现高效可执行的alpha因子挖掘。


<details>
  <summary>Details</summary>
Motivation: 现有符号方法表达能力有限，而神经预测器在可解释性和对市场机制变化的鲁棒性方面存在不足。需要一种既能保持可解释性、可执行性和可审计性，又能在大规模计算中保持高效性的因子发现方法。

Method: FE采用三层分离架构：1) 逻辑修订与参数优化分离；2) LLM引导方向搜索与贝叶斯超参数搜索分离；3) LLM使用与本地计算分离。包含知识注入引导模块，通过多智能体提取-验证-代码生成闭环流水线将非结构化财务报告转换为可执行因子程序，并建立经验知识库支持轨迹感知的精炼。

Result: 在真实世界OHLCV数据的广泛回测中，FE生成的因子在预测稳定性和投资组合影响方面显著优于基线方法，获得更高的IC/ICIR（及Rank IC/ICIR）和改进的AR/Sharpe比率，实现了最先进的预测和投资组合表现。

Conclusion: FactorEngine框架通过程序级因子表示和三层分离架构，成功解决了alpha因子挖掘中可解释性、可执行性和计算效率的平衡问题，为自动化因子发现提供了有效的解决方案。
...
TL;DR: 论文提出理论框架解释为什么负反馈训练LLM比正反馈RLHF更有效：正偏好与负约束存在结构性不对称


<details>
  <summary>Details</summary>
Motivation: 实证研究表明仅使用负反馈训练LLM能达到或超越标准RLHF效果，但缺乏统一理论解释。本文旨在为负信号的有效性提供理论框架。

Method: 提出理论分析框架：基于波普尔证伪逻辑和负知识认识论，分析正偏好与负约束的结构不对称性。正偏好编码连续耦合、上下文依赖的人类价值观，难以穷尽指定；负约束编码离散、有限、可独立验证的禁令。

Result: 理论框架解释了基于偏好的RLHF产生谄媚行为的原因，以及负信号方法有效性的机制。正偏好导致模型学习表面相关性（如迎合用户），而负约束能收敛到稳定边界。

Conclusion: 对齐研究重心应从"学习人类偏好什么"转向"学习人类拒绝什么"。负约束方法在LLM对齐中具有理论优势，提供了可测试的预测框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用代码生成方面表现出色，但将自然语言交易意图转换为正确的期权策略仍然具有挑战性。现实世界的期权设计需要对海量、多维度的期权链数据进行推理，并遵守严格约束，这常常使直接生成方法不堪重负。

Method: 引入Option Query Language (OQL)，这是一种领域特定的中间表示，将期权市场抽象为语法规则下的高级原语，使LLM能够作为可靠的语义解析器而非自由形式的程序员。OQL查询随后由引擎进行验证和确定性执行，以实例化可执行策略。还为此任务提出了新的数据集。

Result: 神经符号管道相比直接基线显著提高了执行准确性和逻辑一致性。

Conclusion: OQL作为中间表示能够有效解决LLM在期权策略生成中的挑战，通过将语义解析与确定性执行分离，提高了系统的可靠性和准确性。
...
TL;DR: 研究发现视觉输入会绕过文本安全机制，改变最先进视觉语言模型的道德决策，揭示多模态安全对齐的紧迫需求
...
Result: 视觉模态激活了类似直觉的路径，覆盖了文本环境中观察到的更审慎、更安全的推理模式。视觉输入从根本上改变了道德决策，绕过了基于文本的安全机制，暴露了语言调优的安全过滤器无法约束视觉处理的关键脆弱性。

Conclusion: 研究揭示了多模态安全对齐的紧迫需求，表明当前基于文本的安全机制在多模态环境中存在根本性缺陷，需要开发能够约束视觉处理的新型安全技术。
...
### [199] [RetailBench: Evaluating Long-Horizon Autonomous Decision-Making and Strategy Stability of LLM Agents in Realistic Retail Environments](https://arxiv.org/abs/2603.16453)
*Linghua Zhang,Jun Wang,Jingtong Wu,Zhisong Zhang*

Main category: cs.AI

TL;DR: 论文提出RetailBench基准测试和Evolving Strategy & Execution框架，用于评估和改进LLM智能体在现实商业场景中的长时程自主决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体在短时程和结构化任务上表现出色，但在现实动态环境中维持长时程连贯决策的能力仍存在挑战，需要专门的评估框架和方法。

Method: 提出RetailBench高保真基准测试，用于评估商业场景中的长时程决策；设计Evolving Strategy & Execution框架，将高层战略推理与底层动作执行分离，支持策略随时间自适应演化。

Result: 在八个最先进LLM上的实验表明，该框架相比基线方法提高了操作稳定性和效率；但随着任务复杂度增加，性能显著下降，揭示了当前LLM在长时程多因素决策方面的根本局限性。

Conclusion: 虽然提出的框架在长时程决策任务上优于现有方法，但当前LLM在复杂、非平稳环境中的长时程多因素决策能力仍存在显著局限性，需要进一步研究。
...
### [200] [Follow the Clues, Frame the Truth: Hybrid-evidential Deductive Reasoning in Open-Vocabulary Multimodal Emotion Recognition](https://arxiv.org/abs/2603.16463)
*Yu Liu,Lei Zhang,Haoxun Li,Hanlei Shi,Yuxuan Ding,Leyuan Qu,Taihao Li*

Main category: cs.AI

TL;DR: HyDRA框架通过混合证据演绎推理架构解决开放词汇多模态情感识别中的模糊性问题，采用提出-验证-决策协议和分层奖励强化学习，在模糊或冲突场景中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 开放词汇多模态情感识别面临模糊多模态线索的挑战，这些线索源于未观察到的情境动态。虽然多模态大语言模型提供广泛的语义覆盖，但其性能受限于过早承诺主导数据先验，导致次优启发式方法忽略了跨模态的关键互补情感线索。
...
Conclusion: 有效的情感推理需要超越表面关联，通过从不同潜在视角综合多个证据基础的理由来重建细微情感状态。HyDRA通过形式化的推理协议和强化学习实现了这一目标。
...
### [201] [Breaking the Chain: A Causal Analysis of LLM Faithfulness to Intermediate Structures](https://arxiv.org/abs/2603.16475)
*Oleg Somov,Mikhail Chaichuk,Mikhail Seleznyov,Alexander Panchenko,Elena Tutubalina*

Main category: cs.AI

TL;DR: 研究发现LLM在模式引导推理流程中生成的中间结构（如检查表、验证查询）更像是影响性上下文而非稳定的因果中介，模型常无法根据中间结构的干预更新最终决策


<details>
  <summary>Details</summary>
Motivation: 探究模式引导推理流程中LLM生成的中间结构（如评分标准、检查表、验证查询）是否真正因果性地决定最终输出，还是仅仅伴随输出存在

Method: 引入因果评估协议：选择任务中确定性函数将中间结构映射到决策，每个受控编辑都对应唯一正确输出；在8个模型和3个基准测试中评估，包括外部工具委托决策和优先中间结构的提示策略

Result: 模型表面上与自身中间结构一致，但在干预后高达60%的情况下未能更新预测；当最终决策由外部工具推导时，这种脆弱性基本消失；要求优先考虑中间结构的提示未能显著缩小差距

Conclusion: 模式引导流程中的中间结构更多作为有影响力的上下文而非稳定的因果中介，揭示了表面忠实性的脆弱性
...
Conclusion: ExpressMind作为高速公路领域的预训练多模态大语言模型，能够有效理解交通场景、推理事件因果关系并生成安全响应策略，为智能高速公路运营提供了认知核心解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用代码生成方面表现良好，但在需要领域特定库、API或约定的专业编程环境中往往表现不佳。定制小型开源模型为依赖大型专有系统提供了经济高效的替代方案。

Method: 构建了Python生态系统内三个领域的编程练习合成数据集：通用Python编程、Scikit-learn机器学习工作流和OpenCV计算机视觉任务。评估了三种定制策略：少样本提示、检索增强生成（RAG）和基于低秩适应（LoRA）的参数高效微调。

Result: 基于提示的方法（如少样本学习和RAG）可以经济高效地提高领域相关性，但对基准准确性的影响有限。相比之下，基于LoRA的微调在大多数任务中始终实现更高的准确性和更强的领域对齐。
...
TL;DR: 提出"有界校准与可争议性"的前端模式，用于约束LLM机器人在多用户社交场景中的援助分配决策，通过限定选项、保持透明、提供争议渠道来应对价值多元性和LLM行为不确定性。


<details>
  <summary>Details</summary>
Motivation: LLM机器人在社交场景中分配稀缺援助时面临价值多元性和LLM行为变异性的挑战：合理的人们对谁应优先获得帮助存在分歧，而LLM介导的交互策略在不同提示、上下文和群体间变化难以预测。目前面向用户的实时多用户援助分配护栏仍不完善。

Method: 提出"有界校准与可争议性"的程序前端模式，包含三个核心要素：(1)将优先级分配约束在治理批准的允许模式菜单内；(2)在交互相关术语中保持活动模式的清晰可读性；(3)提供特定结果的争议路径而不重新协商全局规则。

Result: 通过公共广场机器人场景示例说明该模式，并制定以清晰性、程序合法性和可操作性为中心的评估议程，包括自动化偏见风险和不均衡争议渠道可用性等风险。

Conclusion: 该模式将多元性和LLM不确定性视为常态条件，避免了隐藏隐含价值偏见的沉默默认设置，也避免了在时间压力下将负担转移给用户的开放式"价值设置"，为实时多用户援助分配提供了可行的护栏方案。
...
TL;DR: 评估LLM在第三方沟通场景中是否适当地应用或抑制基于记忆的用户偏好，发现前沿模型难以实现上下文敏感的应用，常将个性化偏好视为全局强制规则而非情境依赖的规范信号。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地将用户偏好存储在持久记忆中，以支持跨交互的个性化。然而，在受社会和制度规范约束的第三方沟通场景中，某些用户偏好可能不适用。需要评估LLM是否能在不同沟通情境中适当地应用或抑制这些基于记忆的偏好。

Method: 引入BenchPreS基准，使用两个互补指标：误用率（MR）和适当应用率（AAR）。评估前沿LLM在不同沟通情境中应用用户偏好的能力，分析偏好依从性、推理能力以及基于提示的防御措施对性能的影响。

Result: 前沿LLM难以以上下文敏感的方式应用偏好。偏好依从性更强的模型表现出更高的过度应用率。推理能力和基于提示的防御措施都无法完全解决这一问题。模型倾向于将个性化偏好视为全局强制规则，而非情境依赖的规范信号。

Conclusion: 当前LLM将个性化偏好视为全局可执行规则，而非情境依赖的规范信号。这揭示了LLM在第三方沟通场景中应用用户偏好时存在系统性缺陷，需要开发更复杂的上下文感知机制来确保适当的行为。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常在静态数据快照上训练，将事实知识视为时间不变的。然而现实世界的事实具有时间敏感性，会经历偶发性和周期性变化，导致模型预测过时。需要评估模型处理时间敏感事实知识的能力。

Method: 提出V-DyKnow视觉动态知识基准，用于评估闭源和开源视觉语言模型：1) 评估跨模态和输入扰动下的响应可靠性和一致性；2) 评估知识编辑和多模态RAG方法在跨模态知识更新中的效果；3) 通过数据和机制分析过时预测的来源。

Result: 视觉语言模型经常输出过时事实，反映了训练阶段使用的过时数据快照。从文本刺激到视觉刺激，事实可靠性会下降，即使实体被正确识别。现有的对齐方法无法在跨模态中一致地更新模型知识。

Conclusion: 当前视觉语言模型在获取和更新跨模态的时间敏感知识方面存在根本性限制。研究团队发布了基准测试、代码和评估数据。
...
TL;DR: 论文通过2026年中东冲突早期阶段的时间锚定案例研究，评估AI在训练数据截止后对地缘政治事件的推理能力，避免回顾性预测中的训练数据泄露问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估AI模型能否在训练数据截止后对正在展开的地缘政治冲突进行有效推理，解决回顾性地缘政治预测中普遍存在的训练数据泄露问题，为研究模型在"战争迷雾"下的分析能力提供新方法。

Method: 研究方法包括：1）选择2026年中东冲突早期阶段作为时间锚定案例（训练数据截止后的事件）；2）构建11个关键时间节点和42个节点特定可验证问题；3）设计5个一般探索性问题；4）要求模型仅基于每个时刻公开可获得的信息进行推理，从而大幅减轻训练数据泄露问题。

Result: 研究结果发现：1）当前最先进的大语言模型展现出显著的战略现实主义倾向，能够超越表面修辞推理深层结构性激励；2）这种能力在不同领域分布不均，模型在经济和物流结构化场景中比在政治模糊的多行为体环境中更可靠；3）模型叙事随时间演变，从早期快速遏制的期望转向更系统的区域固守和消耗性降级解释。

Conclusion: 结论认为该研究提供了首个对LLM在持续地缘政治冲突中进行时间锚定推理的分析，可作为模型在展开危机中推理能力的档案快照，使未来研究能够避免回顾性分析的后见之明偏差，为评估AI的地缘政治分析能力建立了新基准。
...
TL;DR: BrainBench是一个包含100个脑筋急转弯问题的基准测试，涵盖20个类别，专门针对LLMs的常识推理失败模式进行诊断性评估。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在标准基准测试中表现出色，但在人类可以轻松回答的常识推理问题上却经常失败。现有基准测试未能充分捕捉这些失败模式，需要更精细的诊断工具来识别LLMs何时用表面启发式替代真正的常识推理。

Method: 设计了包含100个脑筋急转弯问题的BrainBench基准，涵盖20个精心设计的类别，每个类别针对LLMs特定的常识推理失败模式。使用零样本协议评估了8个前沿模型（4个Claude家族和4个GPT家族），每个问题进行10次独立运行。还进行了跨语言评估（中文）以区分语言特定伪影和真正的推理缺陷。

Result: 最佳模型Claude Opus 4.6（扩展思考）仅达到80.3%准确率；最差模型GPT-4o为39.7%。即使表现最佳的模型也显示出6-16个百分点的准确率与一致性差距，表明存在随机推理。中文跨语言评估显示大多数模型性能下降2-8个百分点，确认这些失败反映了推理缺陷而非语言特定伪影。

Conclusion: BrainBench提供了一个细粒度的诊断工具，用于识别LLMs在何处以及为何用表面启发式替代真正的常识推理。结果表明当前LLMs在常识推理方面存在系统性缺陷，这些缺陷在不同语言中持续存在，揭示了模型对表面模式的依赖而非深层理解。
...
TL;DR: 该论文提出了一种将约束传播集成到动态规划中的方法，以桥接动态规划和约束编程两种范式，通过约束传播剪枝状态和转移，提高求解效率。


<details>
  <summary>Details</summary>
Motivation: 当前组合优化问题存在两种主要的基于模型的范式：基于状态表示的方法（如启发式搜索、动态规划、决策图）和基于约束与领域表示的方法（如约束编程、整数规划、布尔可满足性）。这两种范式之间存在鸿沟，论文旨在桥接动态规划和约束编程，通过将约束传播集成到动态规划中，使动态规划求解器能够利用约束传播来剪枝状态和转移。

Method: 在领域无关动态规划框架中，使用通用约束编程求解器实现约束传播。通过将约束传播集成到动态规划中，在状态扩展过程中利用约束传播技术来剪枝无效的状态和转移。该方法在三个组合优化问题上进行评估：带时间窗口的单机调度问题、资源约束项目调度问题和带时间窗口的旅行商问题。

Result: 约束传播显著减少了状态扩展的数量。对于单机调度和资源约束项目调度问题，该方法比纯动态规划求解器解决了更多实例；对于约束紧密的带时间窗口旅行商问题实例，也显示出类似的改进。运行时性能表明，对于约束紧密的实例，传播带来的好处超过了其开销，但进一步减少传播开销可以进一步提高性能。

Conclusion: 该工作是理解约束传播在动态规划求解器中价值的关键一步，提供了一种基于模型的动态规划与约束编程集成方法。研究表明约束传播可以有效地减少状态空间，提高求解效率，为两种范式的融合提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 随着人工智能快速发展，需要开发能够遵守社会规则和规范的智能体，以便安全地融入日常生活。受《木偶奇遇记》启发，研究旨在解决开发规范合规且情境感知智能体的问题。

Method: 基于AJAR、Jiminy和NGRL架构，提出Pino混合模型，其中强化学习智能体由基于论证的规范监督器指导。开发了自动提取论证和关系的新算法，使该流程可操作化。同时研究了规范规避现象，提供了定义和缓解策略。

Result: 论文对流程的每个组件进行了实证评估，验证了方法的有效性。提出的算法能够自动提取支撑监督器决策的论证和关系，为规范规避问题提供了解决方案。

Conclusion: 该工作为开发规范合规的智能体提供了完整框架，讨论了相关研究、当前局限性和未来研究方向，为实现安全可靠的人工智能集成奠定了基础。
...
TL;DR: 通过微调语言模型学习期刊发表决策，可以提取科学品味这一难以形式化的评估能力，在管理学和经济学研究提案评估中超越前沿模型和专家小组


<details>
  <summary>Details</summary>
Motivation: 科学进步最关键的驱动力不是推理能力而是"品味"——判断哪些未经验证的想法值得追求的评估能力。这种能力由编辑和资助者日常行使，但从未被成功阐述、教授或自动化。AI虽然在可验证答案的任务上达到或超越人类水平，但科学品味评估仍是挑战

Method: 1. 在期刊发表记录上微调语言模型，提取评估判断信号
2. 使用管理学领域研究提案的基准测试集，涵盖四个质量层级
3. 比较11个前沿模型（包括主要专有和开源架构）、期刊编辑专家小组与微调模型的性能
4. 评估模型在未训练任务上的泛化能力：成对比较和单句摘要评估
5. 在经济学领域验证方法的通用性

Result: 1. 11个前沿模型平均准确率仅31%，略高于随机水平
2. 期刊编辑和编委会成员的专家小组多数投票准确率为42%
3. 基于多年发表记录微调的模型超越所有前沿模型和专家小组，最佳单一模型达到59%准确率
4. 微调模型表现出校准的置信度，在其最高置信度预测上达到100%准确率
5. 模型能够将评估信号泛化到未训练的成对比较和单句摘要任务
6. 方法具有通用性：在经济学发表记录上训练的模型达到70%准确率

Conclusion: 科学品味并未超出AI的能力范围，而是沉积在制度记录中等待提取。这些结果为跨学科科学产出的规模化筛选提供了机制，特别是在质量难以形式化验证的领域。微调语言模型能够从发表记录中学习并超越人类专家的评估判断
...
TL;DR: CritiSense是一款多语言移动媒体素养应用，通过互动挑战和即时反馈帮助用户识别虚假信息操纵策略，已支持9种语言并拥有300+活跃用户。
...
Method: 开发CritiSense移动应用，采用短时互动挑战和即时反馈机制，支持9种语言，采用模块化设计便于快速更新，通过可用性研究评估效果。

Result: 93名用户的可用性研究中，83.9%表示总体满意，90.1%认为应用易于使用；定性反馈显示应用有助于提升数字素养技能；3个多月内达到300+活跃用户。

Conclusion: CritiSense提供了一个多语言的预揭穿平台和测试平台，用于衡量微学习对虚假信息抵抗力的影响，应用已在苹果App Store和Google Play Store免费提供。
...
TL;DR: MedCL-Bench是一个用于评估医学语言模型持续学习性能的标准化基准，包含10个生物医学NLP数据集、5类任务，评估11种持续学习策略，分析遗忘模式、计算成本和任务顺序鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 医学语言模型需要随着证据和术语的演变而更新，但顺序更新会引发灾难性遗忘。当前缺乏统一的、任务多样化的基准来评估持续学习在标准化协议下的表现、对任务顺序的鲁棒性以及计算成本感知的报告。

Method: 引入MedCL-Bench基准，流式处理10个生物医学NLP数据集，涵盖5个任务家族。评估11种持续学习策略，在8种不同任务顺序下测试，报告保留率、迁移能力和GPU小时成本。

Result: 直接顺序微调会导致灾难性遗忘，引起先前任务性能下降。不同持续学习方法占据不同的保留-计算边界：参数隔离方法在单位GPU小时内提供最佳保留率，重放方法以更高成本提供强保护，正则化方法效益有限。遗忘具有任务依赖性，多标签主题分类最脆弱，约束输出任务更稳健。

Conclusion: MedCL-Bench为在部署前审计模型更新提供了可复现的框架，有助于系统评估医学语言模型的持续学习性能，识别不同方法的优劣，并为实际应用中的更新策略提供指导。
...
<details>
  <summary>Details</summary>
Motivation: 现代供应链需要数据驱动的决策支持工具，传统预测模型与机器学习/深度学习方法在库存优化中的综合应用效果尚需系统评估，特别是在多级库存系统中的运营影响。

Method: 开发了数字化预测-库存优化管道，集成了传统预测模型、机器学习回归器和深度序列模型；使用M5沃尔玛数据集；在统一库存模拟框架中评估了7种预测方法；分别在单级和两级报童系统中进行运营影响评估。

Result: Temporal CNN和LSTM模型相比统计基线显著降低了库存成本并提高了满足率；敏感性分析和多级分析证明了该方法的鲁棒性和可扩展性。

Conclusion: 该研究提供了一个数据驱动的决策支持工具，展示了深度序列模型在供应链预测和库存优化中的优越性能，为现代供应链管理提供了实用的解决方案。
...
TL;DR: 论文系统分析了RAG系统中保形事实性过滤的可靠性、有用性和鲁棒性，揭示了事实性与信息量之间的权衡，以及分布偏移和干扰项对保形保证的脆弱性


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉，限制了其在知识密集型应用中的可靠性。检索增强生成和保形事实性过滤是两种潜在的解决方案，但RAG无法提供统计保证，而保形过滤虽然提供统计可靠性，但无法保证最终输出的信息量。需要系统分析保形事实性在RAG系统中的实际效果。

Method: 系统分析保形事实性在RAG-based LLMs中的可靠性、有用性、鲁棒性和效率，涵盖生成、评分、校准、鲁棒性和效率五个维度。提出新颖的信息量感知指标，在三个基准测试和多个模型家族上进行评估，比较轻量级蕴涵验证器与基于LLM的置信度评分器。

Result: 研究发现：(1) 在高事实性水平下，保形过滤因产生空洞输出而导致有用性低；(2) 保形事实性保证对分布偏移和干扰项不鲁棒，要求校准数据与部署条件紧密匹配；(3) 轻量级蕴涵验证器匹配或优于基于LLM的置信度评分器，同时计算量减少100倍以上。

Conclusion: 研究揭示了事实性与信息量之间的权衡，以及保形过滤框架在分布偏移和干扰项下的脆弱性。强调需要开发新的可靠性方法，以鲁棒性和有用性为关键指标，并为构建既可靠又计算高效的RAG管道提供实用指导。
...
### [216] [Prompt Programming for Cultural Bias and Alignment of Large Language Models](https://arxiv.org/abs/2603.16827)
*Maksim Eren,Eric Michalak,Brian Cook,Johnny Seales*

Main category: cs.AI

TL;DR: 验证并扩展文化对齐框架，在开源LLMs上复现社会调查投影和距离度量，引入DSPy提示编程优化文化对齐，相比手动提示工程获得更好效果


<details>
  <summary>Details</summary>
Motivation: LLMs在战略决策、政策支持和文档工程等应用中存在文化偏见，与目标人群价值观不符。现有文化对齐框架主要评估专有模型且依赖手动提示工程，需要验证其在开源模型上的有效性并开发更系统的优化方法。

Method: 1) 在开源权重LLMs上复现基于社会科学调查的投影和距离度量框架；2) 引入DSPy提示编程，将提示视为模块化、可优化的程序；3) 针对文化距离目标进行系统优化，自动调整文化条件

Result: 1) 验证了文化偏见和文化条件化效果在开源LLM系统中同样存在；2) 提示优化通常优于手动文化提示工程；3) 使用DSPy进行提示编译为文化对齐LLM响应提供了更稳定、可迁移的途径

Conclusion: 通过DSPy提示编程进行系统化提示优化是改善LLM文化对齐的有效方法，相比手动提示工程提供更稳定和可迁移的解决方案，为LLM在跨文化应用中的部署提供了技术基础。
...
### [217] [Internalizing Agency from Reflective Experience](https://arxiv.org/abs/2603.16843)
*Rui Ge,Yichao Fu,Yuyang Qian,Junda Su,Yiming Zhao,Peng Zhao,Hao Zhang*

Main category: cs.AI

TL;DR: LEAFE框架通过从反思经验中学习反馈驱动的自主能力，解决现有结果驱动方法在长时程交互中分布锐化的问题，显著提升Pass@k性能


<details>
  <summary>Details</summary>
Motivation: 现有基于结果的后训练方法（如带可验证奖励的强化学习）主要优化最终成功信号，未能充分利用丰富的环境反馈，导致分布锐化问题：策略变得擅长复制狭窄的成功行为集，但无法提升基于反馈的自主能力以扩展长时程设置中的问题解决能力

Method: 提出LEAFE框架，通过反思经验内化恢复能力：在探索阶段，智能体将环境反馈总结为可操作经验，回溯到早期决策点，探索具有修正行动的替代分支；然后通过监督微调将这些经验指导的修正蒸馏到模型中，使策略在未来交互中更有效地恢复

Result: 在固定交互预算下的多样化交互式编码和智能体任务中，LEAFE持续改进基础模型的Pass@1，并在Pass@k上优于结果驱动基线（GRPO）和基于经验的方法（如Early Experience），在Pass@128上获得高达14%的提升

Conclusion: LEAFE通过从反思经验中学习反馈驱动的自主能力，有效解决了长时程交互中分布锐化问题，显著提升了智能体在复杂任务中的恢复能力和问题解决能力
...
TL;DR: SocialOmni是一个评估全模态大语言模型社交交互能力的新基准，专注于对话中的打断行为，包括识别说话者、控制打断时机和生成自然打断内容三个维度。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型基准主要关注静态的准确性任务，缺乏对社交交互性这一关键能力的评估，特别是模型在自然对话中处理动态线索的能力。

Method: 提出SocialOmni基准，包含2000个感知样本和209个交互生成实例，具有严格的时间和上下文约束，并设计了受控的音视频不一致场景来测试模型鲁棒性。

Result: 对12个领先的全模态大语言模型进行基准测试，发现它们在社交交互能力上存在显著差异，且模型的感知准确性与生成上下文适当打断的能力之间存在明显脱钩。

Conclusion: 理解为中心的指标不足以表征对话社交能力，SocialOmni的诊断结果为弥合未来全模态大语言模型中感知与交互之间的鸿沟提供了可操作的信号。
...
### [219] [Multi-objective Optimization for Over-the-Air Federated Edge Learning-enabled Collaborative Integrated Sensing and Communications](https://arxiv.org/abs/2603.15783)
*Saba Asaad,Hina Tabassum,Ping Wang*

Main category: cs.IT

TL;DR: 提出了一种新颖的多目标集成感知与通信框架，通过空中联邦边缘学习实现协作无线感知，利用双用途上行信号同时处理感知和学习任务，达到感知误差的克拉美-罗下界。


<details>
  <summary>Details</summary>
Motivation: 传统协作感知方案受限于本地估计器的平均误差，且现有系统难以同时优化感知精度和通信性能。需要开发一个能同时处理感知和学习任务、利用双用途信号、并达到理论性能极限的集成框架。

Method: 1) 建立本地充分统计量分析表达式；2) 提出正交脉冲整形方法抑制干扰；3) 通过分布式似然最大化推导目标坐标最优无偏估计；4) 使用克拉美-罗界表征感知误差下界；5) 构建多目标优化问题最小化均方误差和感知误差界；6) 采用epsilon约束方法求解。

Result: 数值结果表明，所提框架在不影响主要OTA-FEEL任务性能的前提下提高了感知精度。相比传统单次协作感知方案受限于本地估计器平均误差，所提算法达到了问题的克拉美-罗下界。

Conclusion: 该双用途OTA-FEEL使能的协作ISAC框架成功实现了感知与学习的协同优化，通过多目标优化方法同时最小化通信误差和感知误差，达到了理论性能极限，为集成感知与通信系统提供了有效解决方案。
...
### [220] [Low-complexity tuning of pinching-antenna systems for integrated sensing and communication](https://arxiv.org/abs/2603.15844)
*Saba Asaad,Chongjun Ouyang,Zhiguo Ding,Ali Bereyhi*

Main category: cs.IT

TL;DR: 提出一种基于夹持天线系统的集成感知与通信设计，通过自适应调整夹持位置实现感知与通信的同时进行，并采用二分策略分配感知功率和调整夹持位置，计算复杂度低


<details>
  <summary>Details</summary>
Motivation: 探索夹持天线系统在集成感知与通信中的潜力，解决同时进行感知和数据传输时的干扰问题，实现动态自适应调整

Method: 提出PASS辅助的ISAC设计，采用二分策略分配感知功率和调整夹持位置，计算复杂度显著降低，支持高更新率的动态PASS调谐

Result: 数值结果表明，所提方法相比基线设计显著扩大了感知-通信速率区域，且无明显成本增加

Conclusion: PASS系统在ISAC中具有重要潜力，提出的设计能够有效实现感知与通信的同时进行，计算复杂度低，性能优越
...
### [221] [Adaptive Multi-Head Finite-State Gamblers](https://arxiv.org/abs/2603.16034)
*Julianne Cruz,Sho Glashausser,Xiaoyuan Li,Neil Lutz*

Main category: cs.IT

TL;DR: 该论文研究了多头有限状态维度与预维度，比较了头移动的"遗忘"（数据无关）与"自适应"（数据相关）模型，证明自适应模型增强了预测能力，且自适应预维度随头数增加形成严格层次结构。


<details>
  <summary>Details</summary>
Motivation: 多头有限状态维度和预维度通过使用"探针到过去"的尾随头来量化序列的可预测性。原始定义中头移动必须是遗忘的（数据无关），本文旨在研究自适应（数据相关）头移动模型，并与遗忘模型进行比较，探索自适应模型是否能增强预测能力。

Method: 引入头移动为自适应的模型，即头移动可以依赖于数据。通过理论分析比较自适应模型与遗忘模型，构造特定序列证明自适应模型的优势，并证明自适应预维度随头数增加形成严格层次结构。

Result: 对于所有h≥2，自适应增强了h头有限状态赌徒的预测能力：存在序列，其遗忘h头有限状态预维度严格大于自适应h头有限状态预维度。自适应有限状态预维度随头数增加形成严格层次：对所有h≥1，存在序列使其自适应(h+1)头有限状态预维度严格小于自适应h头预维度。

Conclusion: 自适应头移动模型相比遗忘模型确实增强了多有限状态赌徒的预测能力，且自适应预维度随头数增加形成严格层次结构，表明更多头能更有效地利用序列中的非局部模式。
...
### [222] [Non-GRS type Euclidean and Hermitian LCD codes and Their Applications for EAQECCs](https://arxiv.org/abs/2603.16187)
*Zhonghao Liang,Dongmei Huang,Qunying Liao,Cuiling Fan,Zhengchun Zhou*

Main category: cs.IT

TL;DR: 该论文利用非GRS型广义Roth-Lempel(GRL)码构造了多类欧几里得LCD码、厄米特LCD码及小维数壳的线性码，推广了Wu等人2021年的结果，并给出了壳维数的上界、构造了EAQECC，证明了GRL码在k>ℓ时的非GRS性质。


<details>
  <summary>Details</summary>
Motivation: 非GRS型线性码能有效抵抗Sidelnikov-Shestakov攻击和Wieschebrink攻击，而LCD码和线性码的壳在构造纠缠辅助量子纠错码(EAQECC)、通信系统和密码学中具有重要作用。本文旨在利用GRL码构造LCD码并研究其壳的性质。

Method: 利用广义Roth-Lempel(GRL)码这一非GRS型线性码，构造欧几里得LCD码、厄米特LCD码及小维数壳的线性码。推导了欧几里得GRL码1维壳数量的上界，以及厄米特GRL码壳维数的上界并证明可达性。应用这些结果构造EAQECC，并证明GRL码在k>ℓ时的非GRS性质。

Result: 成功构造了多类LCD码和具有小维数壳的线性码，推广了Wu等人2021年的结果。给出了壳维数的上界并证明可达性，获得了多个EAQECC族。证明了GRL码在k>ℓ时为非GRS码，并提供了LCD MDS码和LCD NMDS码的具体实例。

Conclusion: GRL码作为非GRS型线性码，是构造LCD码和研究线性码壳的有效工具。本文结果不仅推广了前人工作，还为构造EAQECC提供了新方法，证明了GRL码在特定条件下的非GRS性质，具有理论和应用价值。
...
### [223] [Rotatable Antenna-Enabled Mobile Edge Computing](https://arxiv.org/abs/2603.16275)
*Qiyao Wang,Beixiong Zheng,Xue Xiong,Weidong Mei,Changsheng You,Qingqing Wu,Jie Tang*

Main category: cs.IT

TL;DR: 本文提出了一种基于可旋转天线的移动边缘计算系统，通过联合优化计算资源分配、接收波束成形和天线偏转角来最小化最大计算时延。


<details>
  <summary>Details</summary>
Motivation: 移动边缘计算中，支持低时延服务的通信可靠性和计算效率提升是核心挑战。可旋转天线技术通过引入新的空间自由度，能够主动改善无线信道条件，为时延敏感用户提供更好的服务。

Method: 采用交替优化框架解决非凸优化问题：1) 基于KKT条件推导最优边缘计算资源分配；2) 使用半定松弛和二分搜索优化接收波束成形；3) 通过分式规划和逐次凸逼近优化天线偏转角。

Result: 仿真结果表明，所提出的可旋转天线使能移动边缘计算方案相比传统基准方法显著降低了最大计算时延。

Conclusion: 可旋转天线技术通过引入空间自由度，能够有效提升移动边缘计算系统的时延性能，为低时延服务提供了一种有前景的解决方案。
...
### [224] [Multi-Agent Reinforcement Learning Counteracts Delayed CSI in Multi-Satellite Systems](https://arxiv.org/abs/2603.16470)
...
<details>
  <summary>Details</summary>
Motivation: 卫星通信与下一代技术融合需要准确的CSI，但高传播延迟导致卫星端CSI观测过时，影响服务质量

Method: 提出双阶段近端策略优化(DS-PPO)算法，第一阶段最大化单个卫星的速率，第二阶段最大化所有卫星协作形成分布式多天线基站的速率

Result: DS-PPO对CSI不完善具有鲁棒性，能提高用户总速率，提供了收敛性分析和计算复杂度分析

Conclusion: DS-PPO算法有效解决了卫星通信中CSI过时问题，通过MARL框架处理大规模连续动作空间和非IID环境
...
### [225] [Directivity Enhancement of Movable Antenna Arrays with Mutual Coupling](https://arxiv.org/abs/2603.16472)
*Wei Xu,Lipeng Zhu,Wenyan Ma,An Liu,Rui Zhang*

Main category: cs.IT

TL;DR: 该论文研究可移动天线阵列中互耦效应对方向性增强的潜力，提出GS-GD算法优化天线位置以实现任意给定方向的最大方向性增益。


<details>
  <summary>Details</summary>
Motivation: 传统天线阵列通常将天线单元间的互耦视为有害因素，但在特定条件下，互耦可用于增强远场方向性（即波束成形增益）。理论上，N天线超指向性阵列在端射方向的方向性可达N²，显著超过传统非耦合阵列的N。论文旨在探索可移动天线阵列中互耦效应在方向性增强方面的潜力。

Method: 提出一种低复杂度算法GS-GD（贪婪搜索和梯度下降）来优化天线位置以最大化任意给定方向的阵列方向性。算法分两步：首先从离散网格点中顺序选择天线位置，然后通过梯度下降优化连续细化天线位置。

Result: 数值结果表明，通过利用天线耦合优化的可移动天线阵列设计，在所有方向上相比传统均匀线性阵列（ULA）实现了显著的方向性增益。此外，所提出的GS-GD算法在大多数方向上能接近全局最优解。

Conclusion: 可移动天线阵列中的互耦效应可有效用于方向性增强，提出的GS-GD算法能有效优化天线位置，实现接近最优的方向性性能，为天线阵列设计提供了新思路。
...
### [226] [Finding Common Ground in a Sea of Alternatives](https://arxiv.org/abs/2603.16751)
*Jay Chooi,Paul Gölz,Ariel D. Procaccia,Benjamin Schiffer,Shirley Zhang*

Main category: cs.GT

TL;DR: 提出基于比例否决核心的形式化模型，用于在无限备选方案中寻找共识，设计高效采样算法保证以高概率找到近似比例否决核心方案，并通过理论下界证明其查询效率最优。


<details>
  <summary>Details</summary>
Motivation: 生成式AI能够访问无限多的备选陈述，但现有系统（如Habermas机器）依赖投票规则选择生成陈述，而"寻找共识"的含义缺乏明确定义。需要为无限备选方案场景建立形式化模型，确保找到真正代表多元群体偏好的共识陈述。

Method: 基于社会选择理论中的比例否决核心概念，建立无限备选方案的形式化模型。设计高效采样算法，仅通过查询未知的备选方案分布和选民偏好，以高概率返回近似比例否决核心的备选方案。证明匹配的下界，表明没有算法能用更少查询实现相同目标。在文本偏好的合成数据集上验证算法有效性。

Result: 提出的采样算法能够高效找到近似比例否决核心的备选方案，理论分析证明其查询复杂度最优。实验验证了算法在文本偏好数据集上的有效性，并比较了其他社会选择方法和基于LLM的方法在产生比例否决核心陈述方面的可靠性。

Conclusion: 为无限备选方案场景中的共识寻找问题提供了基于比例否决核心的形式化框架，设计了理论最优的采样算法，为生成式AI系统选择共识陈述提供了可靠的理论基础和实用方法。
...
### [227] [Scientific Machine Learning-assisted Model Discovery from Telemetry Data](https://arxiv.org/abs/2603.15943)
...
TL;DR: Dyad Model Discovery：一种半自动方法，通过在物理模型中添加从数据中发现的符号表达式来改进HVAC设备数字孪生模型的校准性能


<details>
  <summary>Details</summary>
Motivation: 当动态模型无法校准到数据时，通常是因为模型做了过多简化假设而缺失了物理机制。需要一种方法来增强物理模型，提高其预测性能

Method: 提出Dyad Model Discovery方法，将物理方程与从数据中发现的符号表达式相结合。采用工程师在环的工作流程，为用户提供可接受或拒绝的建议

Result: 在运输制冷单元的数字孪生上演示了该方法，使用遥测数据进行训练，提高了预测性能

Conclusion: 这是首个AI辅助的工程设计工作流程，能够通过数据驱动的符号表达式增强物理模型，解决模型校准失败的问题
...
### [228] [Physics-guided diffusion models for inverse design of disordered metamaterials](https://arxiv.org/abs/2603.16209)
*Ziyuan Xie,Weipeng Xu,Dazhi Zhao,Wenchang Zhang,Daoyang Dong,Bingbing Xu,Ning Liu,Sheng Mao,Tianju Xue*

Main category: cs.CE

TL;DR: 提出物理引导扩散模型，通过可微分物理求解器在生成过程中即时引导无序超材料逆设计，无需针对不同任务重新训练模型


<details>
  <summary>Details</summary>
Motivation: 无序超材料的逆设计面临结构-性能关系非直观和大设计空间的挑战。现有生成方法依赖特定任务训练目标，当设计问题变化时需要重新训练，限制了灵活性和泛化能力

Method: 提出物理引导扩散模型，借鉴分类器引导思想，将可微分物理求解器的梯度直接整合到反向随机微分方程中，实现任务自适应生成。扩散模型只需在无标签数据上训练一次

Result: 针对无序泡沫超材料的三个代表性设计任务：目标有效热导率、期望载荷-位移响应、涉及断裂的最大能量吸收，均成功生成了满足物理目标的泡沫状几何结构

Conclusion: 物理引导扩散模型展示了处理无序超材料复杂逆设计问题的多功能性、高效性和实用性，为超越该领域的其他应用提供了有前景的方法
...
### [229] [Rapid Worst-Case Gust Identification for Very Flexible Aircraft Using Reduced-Order Models](https://arxiv.org/abs/2603.16212)
*Nikolaos D. Tantaroudas,Andrea Da Ronch,Ilias Karachalios,Kenneth J. Badcock*

Main category: cs.CE

TL;DR: 该论文提出了一种基于降阶模型的方法，用于快速识别最坏情况阵风载荷，相比全阶非线性仿真实现了高达600倍的计算加速。


<details>
  <summary>Details</summary>
Motivation: 识别最坏情况阵风载荷是超柔性飞机认证的关键步骤，但非线性全阶仿真的计算成本使得详尽的参数搜索变得不切实际。

Method: 采用基于泰勒级数展开和特征向量投影的非线性模型降阶方法，对耦合的流体-结构-飞行动力学系统进行降阶处理。

Result: 在三个复杂度递增的测试案例中实现了显著加速：三自由度翼型（14状态，1000个设计点）、Global Hawk类无人机（540状态，80个参数计算，30倍加速）和超柔性飞翼（1616状态，37个参数计算从222小时减少到22分钟）。线性ROM在变形低于翼展10%时准确，非线性二阶ROM能准确捕捉大变形区域。

Conclusion: 该方法为将最坏情况阵风搜索集成到飞机认证工作流程中提供了实用工具。
...
### [230] [Physics-Informed Video Diffusion For Shallow Water Equations](https://arxiv.org/abs/2603.15627)
*Yang Bai,George Eskandar,Ziyuan Liu,Gitta Kutyniok*

Main category: cs.GR

TL;DR: 提出物理信息视频扩散框架，联合生成视觉输出和物理状态，在保持物理合理性的同时显著加速流体模拟


<details>
  <summary>Details</summary>
Motivation: 传统流体动力学模拟计算成本高，而基于扩散的生成视频模型忽略物理规律导致动态不一致。需要一种既能保持物理合理性又能高效生成的方法

Method: 提出物理信息视频扩散框架，将物理约束直接集成到生成过程中，基于二维浅水方程和地形拓扑，同时预测物理状态和真实视频，无需单独渲染步骤

Result: 在真实感和物理保真度上优于纯数据驱动的视频扩散基线，同时生成视频速度显著快于传统模拟加渲染流程

Conclusion: 物理信息视频扩散框架能够同时生成视觉上真实且物理上合理的水流视频，为物理模拟提供了一种高效替代方案
...
### [231] [Perceptual Requirements for Low-Latency Head-Mounted Displays](https://arxiv.org/abs/2603.15796)
*Eric Penner,Josephine D'Angelo,Clinton Smith,Nathan Matsuda,Neethan Siva,Phillip Guan*

Main category: cs.GR

TL;DR: Camsicle HMD实现2ms端到端延迟，支持自然用户研究，发现2ms和14.3ms延迟在接球任务中优于23ms和29ms延迟


<details>
  <summary>Details</summary>
Motivation: 现有HMD的12-60ms延迟限制了研究低延迟对用户体验影响的自然研究，需要开发超低延迟平台来探索延迟对用户偏好和性能的影响

Method: 开发Camsicle视频透视HMD，采用折反射设计实现2ms端到端延迟和无重投影的透视校正，通过两个用户研究（57名参与者）比较不同延迟下的接球任务表现

Result: 在接球任务中，2ms和14.3ms延迟比23ms和29ms延迟更受偏好；研究还比较了自然任务中的延迟偏好与零延迟参考系统中的心理物理阈值

Conclusion: Camsicle平台能够研究超低延迟对用户体验的影响，发现低延迟在自然任务中具有明显优势，为理解延迟感知阈值与主观评价的关系提供了新方法
...
### [232] [Toward Reliable Scientific Visualization Pipeline Construction with Structure-Aware Retrieval-Augmented LLMs](https://arxiv.org/abs/2603.16057)
*Guanghui Zhao,Zhe Wang,Yu Dong,Guan Li,GuiHua Shan*

Main category: cs.GR

TL;DR: 提出一种结构感知的检索增强生成工作流，用于从自然语言描述生成可执行的科学可视化管道，显著提高vtk.js管道的可执行性并减少人工修正成本。


<details>
  <summary>Details</summary>
Motivation: 科学可视化管道具有严格的执行依赖性和领域特定的程序知识，使得从自然语言描述生成可执行管道对大型语言模型具有挑战性，特别是在基于Web的环境中需要显式代码级管道组装。

Method: 提出结构感知的检索增强生成工作流，提供管道对齐的vtk.js代码示例作为上下文指导，支持正确的模块选择、参数配置和执行顺序。引入修正成本作为衡量获得有效管道所需人工干预量的指标。

Result: 结构化、领域特定的上下文显著提高了管道可执行性并减少了修正成本。评估了多个多阶段科学可视化任务和不同LLM，证明了该方法的有效性。

Conclusion: 结构感知的检索增强生成方法能够有效提高LLM生成科学可视化管道的可靠性，减少人工修正工作量，并提供了交互式分析界面支持人工检查和系统评估。
...
### [233] [Fast and Reliable Gradients for Deformables Across Frictional Contact Regimes](https://arxiv.org/abs/2603.16478)
*Ziqiu Zeng,Gang Yang,Zhenhao Huang,Yulin Li,Jason Pho,Siyuan Luo,Fan Shi*

Main category: cs.GR

TL;DR: 提出统一GPU加速可微模拟器，解决摩擦接触中梯度不稳定问题，通过严格马尔可夫动力学、统一接触稳定性和鲁棒材料识别方法，提升物理系统识别和控制精度。


<details>
  <summary>Details</summary>
Motivation: 当前可微模拟框架在摩擦接触问题上缺乏数学一致性，常通过非马尔可夫位置近似或启发式梯度避免接触奇点，导致梯度失真、优化停滞，难以处理复杂摩擦接触和大变形场景。

Method: 1) 长时程一致性：在耦合位置-速度流形上强制严格马尔可夫动力学防止梯度崩溃；2) 统一接触稳定性：使用质量对齐预条件子和软Fischer-Burmeister算子实现平滑摩擦优化；3) 鲁棒材料识别：通过推导的"块内交换"条件解决FEM奇点问题。

Result: 实验证明该求解器在缩小仿真与现实差距方面有效，在灵巧操作、布料折叠等接触密集任务中提供精确、低噪声梯度，显著优于传统方法。

Conclusion: 通过解决传统方法中常见的梯度不稳定问题，该框架显著提升了物理系统识别和控制的保真度，为计算机图形学和机器人学中的逆问题提供了更可靠的数学基础。
...
### [234] [Retrieval-Augmented Sketch-Guided 3D Building Generation](https://arxiv.org/abs/2603.16612)
*Zhengyang Wang,Nuttapong Rochanavibhata,Yuxiao Ren,Xusheng Du,Ye Zhang,Haoran Xie*

Main category: cs.GR

TL;DR: 提出用于日本独栋住宅的多阶段草图到3D生成框架，解决设计漂移和组件级编辑问题


<details>
  <summary>Details</summary>
Motivation: 日本独栋住宅早期设计阶段中，客户、销售代表和设计师之间缺乏统一的设计表示，导致设计漂移和反馈效率低下。销售代表传递的草图可能因快速绘制而丢失细节，降低后续生成式AI模型生成3D模型的保真度。生成的3D模型通常是单一网格，无法进行组件级编辑。

Method: 提出多阶段3D生成设计框架，结合生成式和检索式方法。采用多模态表示进行3D模型生成，应用组件分割定位窗户、门等建筑组件，使用检索支持目标组件的替换，实现组件级编辑和个性化定制。

Result: 实验表明该工作实现了模块化定制，适合个性化建筑设计。提供了立面图和组件数据集，通过定量评估和专家评估证明了有效性。

Conclusion: 该研究为日本独栋住宅引入了多阶段草图到3D框架，解决了设计表示不统一和组件编辑困难的问题，支持个性化建筑设计。
...
### [235] [A low-data, low-cost, and open-source workflow for 3D printing lithographs for digital accessibility of microscopy images](https://arxiv.org/abs/2603.16801)
*Robert Faulkner,Natalia Gonzalez-Vazquez,Victoria Gamez,Karly E. Cohen,Gunther Richter,Abigale Stangl,Andrew K. Schulz*

Main category: cs.GR

TL;DR: 开发低成本开源工作流程，将复杂显微图像转换为可3D打印的触觉图形，使微小生物结构可通过触觉感知


<details>
  <summary>Details</summary>
Motivation: 解决显微图像中微小生物结构（如壁虎脚趾刚毛、大鼠胡须角质层、蝙蝠翅膀绒毛）因尺度太小而无法直接观察的问题，这些结构通常具有非均匀的力学特性（从硬到软的渐变），需要为视障人士和普通学习者提供可触达的科学可视化方法

Method: 提出一个工作流程，将复杂显微图像转换为低数据量、低成本的开放式光刻文件，这些文件可在350美元的3D打印机上打印，单个文件小于100MB，每次打印成本仅0.75美元，通过GitHub仓库持续更新框架

Result: 成功开发出可3D打印的触觉图形和艺术作品，使科学更易获取，支持对生物结构的触觉探索，为未来3D打印和光刻技术发展提供可扩展框架

Conclusion: 该工作流程利用先进3D打印技术创建触觉图形，显著提高了显微图像的可访问性，使微小生物结构的触觉探索成为可能，为科学教育和包容性研究开辟了新途径
