...
TL;DR: SwiftTailor是一个高效的两阶段3D服装生成框架，通过几何图像表示统一缝纫图案推理和网格合成，大幅减少推理时间的同时保持高质量输出。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的方法虽然能生成高质量的3D服装，但推理时间过长（30秒到1分钟），限制了实际应用。需要一种既能保持高质量又能显著加速推理的解决方案。

Method: 提出SwiftTailor两阶段框架：1) PatternMaker - 轻量级视觉语言模型，从多种输入模态预测缝纫图案；2) GarmentSewer - 高效密集预测Transformer，将图案转换为Garment Geometry Image（在统一UV空间中编码所有服装面板的3D表面）。通过高效的逆映射过程重建最终3D网格，包含重新网格化和动态缝合算法，避免物理模拟成本。
...
TL;DR: 提出了首个用于自动驾驶暗光环境研究的真实世界昼夜对齐数据集DarkDriving，包含9,538对精确对齐的昼夜图像，并引入了四项感知相关任务。


<details>
  <summary>Details</summary>
Motivation: 现有低光增强数据集存在以下问题：1）只能在静态场景和小范围内通过控制曝光采集；2）现有夜间驾驶数据集的暗光图像缺乏精确对齐的白天对应图像；3）在动态驾驶场景中采集真实世界昼夜对齐数据集极其困难，严重限制了该领域研究。

Method: 提出了一种自动的昼夜轨迹跟踪姿态匹配（TTPM）方法，在大型真实世界封闭驾驶测试场（面积69英亩）中采集数据。该方法能够精确对齐昼夜图像的位置和空间内容，对齐误差仅为数厘米。数据集包含9,538对昼夜图像对，并为每对图像手动标注了2D边界框。

Result: DarkDriving数据集提供了9,538对精确对齐的昼夜图像对，对齐误差在数厘米级别。该数据集引入了四项感知相关任务：低光增强、广义低光增强、用于2D检测的低光增强以及用于3D检测的低光增强。实验结果表明，该数据集为评估自动驾驶低光增强提供了全面基准，并能推广到其他低光驾驶环境（如nuScenes）的暗光图像增强和检测提升。

Conclusion: DarkDriving是首个用于自动驾驶暗光环境研究的真实世界昼夜对齐数据集，解决了现有数据集的局限性，为低光增强研究提供了全面基准，并能推广到其他低光驾驶场景。
...
TL;DR: SSP-SAM是一个增强SAM语言理解能力的框架，通过语义-空间提示编码器整合视觉和语言注意力适配器，实现高质量的指代表达式分割，并自然支持广义指代表达式分割任务。


<details>
  <summary>Details</summary>
Motivation: SAM在通用图像分割方面表现出色，但对自然语言的理解能力有限，这限制了其在指代表达式分割（RES）中的直接应用。需要一种方法既能充分利用SAM的分割能力，又能增强其语言理解能力。
...
Result: 在广泛使用的RES和GRES基准测试中验证了方法的优越性。该方法生成高质量的分割掩码，即使在严格的Pr@0.9阈值下也能实现强精度。在PhraseCut数据集上的进一步评估显示，在开放词汇场景中相比现有最先进的RES方法有改进。

Conclusion: SSP-SAM成功地将SAM的分割能力与语言理解相结合，通过语义-空间提示编码器实现了高质量的指代表达式分割。该方法不仅适用于标准RES任务，还能自然支持更灵活的广义RES设置，无需额外修改，展现了良好的泛化能力。
...
TL;DR: CytoSyn是一个用于组织病理学H&E染色图像生成的先进基础潜在扩散模型，通过大规模TCGA数据集训练，支持引导生成高度逼真和多样化的病理图像。


<details>
  <summary>Details</summary>
Motivation: 计算病理学在特征提取器方面已有显著进展，但专门针对组织病理学的生成式基础模型仍然稀缺。这类模型能够处理特征提取器无法完成的任务，如虚拟染色等。

Method: 开发了CytoSyn基础潜在扩散模型，探索了方法改进、训练集扩展、采样策略和切片级过拟合问题，最终推出改进版CytoSyn-v2。模型在超过10,000张TCGA诊断全切片图像上训练，涵盖32种癌症类型。

Result: CytoSyn能够生成高度逼真和多样化的H&E染色图像，在广泛基准测试中表现优异。尽管仅在肿瘤学切片上训练，但在生成炎症性肠病图像方面仍保持最先进性能。研究还发现扩散模型和性能指标对预处理细节（如JPEG压缩）高度敏感。
...
TL;DR: O2MAG：一种无需训练的小样本异常生成方法，通过自注意力嫁接和异常掩码操作，利用单张参考异常图像合成更多真实异常，提升下游异常检测性能


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临正常图像丰富但异常样本稀缺的问题。现有小样本异常合成方法需要耗时训练，且难以学习真实异常分布，限制了异常检测模型的效果。
...
Conclusion: O2MAG是一种无需训练的小样本异常生成方法，能够合成符合真实异常分布的样本，有效提升下游异常检测模型的性能，解决了现有方法训练耗时和分布不准确的问题。
...
TL;DR: 提出Q-Drift方法，通过采样器端校正处理扩散模型后训练量化中的噪声累积问题，将量化误差视为隐式随机扰动并推导保持边际分布的漂移调整。


<details>
  <summary>Details</summary>
Motivation: 后训练量化（PTQ）是部署大型扩散模型的实用路径，但量化噪声会在去噪轨迹上累积并降低生成质量，需要解决量化误差对扩散过程的影响。

Method: Q-Drift将量化误差建模为每个去噪步骤上的隐式随机扰动，推导出保持边际分布的漂移调整。通过校准估计时间步相关的方差统计量，仅需少量（如5次）全精度/量化配对校准运行。该方法与常见采样器、扩散模型和PTQ方法即插即用。
...
Conclusion: Q-Drift提供了一种原则性的采样器端校正方法，有效缓解扩散模型后训练量化中的噪声累积问题，具有即插即用、低开销的优点，显著提升量化模型的生成质量。
...
Result: 在标准1-16样本基准测试中，该方法持续达到新的最先进性能，推理时保持与Tip-Adapter完全相同的零额外延迟和内存开销

Conclusion: 通过仅训练时使用的异构图教师监督，能够有效提升Tip-Adapter的缓存原型质量，实现细粒度跨模态对齐，为少样本适配提供了一种高效且强大的解决方案
...
TL;DR: 提出一个基于人类可理解美学概念的图像美学评估框架，通过构建美学概念子空间实现内在可解释性，同时引入残差预测器捕捉超出显式概念的细微影响。


<details>
  <summary>Details</summary>
Motivation: 现有IAA模型虽然预测性能强，但缺乏对预测驱动因素的洞察。用户不仅需要美学评分，更需要理解图像为何被认为美观或不美观。人类评估美学时自然依赖高层次线索来证明判断，这促使研究者开发基于人类可理解美学概念的可解释IAA框架。

Method: 1. 以可访问的方式学习人类可理解的美学概念，构建美学概念子空间作为内在可解释模型的基础；2. 引入简单有效的残差预测器，捕捉超出显式概念的细微美学感知影响；3. 在摄影和艺术数据集上进行实验验证。

Result: 在摄影和艺术数据集上的实验表明，该方法在保持竞争力的预测性能的同时，提供了透明、人类可理解的美学判断。

Conclusion: 提出的基于人类可理解美学概念的IAA框架实现了预测性能与可解释性的平衡，为图像美学评估提供了透明、可理解的判断依据，满足了用户理解美学评分背后原因的需求。
...
TL;DR: Insight-V++是一个统一的多智能体视觉推理框架，通过自主生成高质量长链推理数据和创新的双智能体架构，显著提升了多模态大语言模型的时空推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型通过扩展测试时推理取得了显著可靠性，但将这种能力扩展到多模态大语言模型面临重大挑战，主要原因是高质量长链推理数据的严重匮乏以及优化训练管道的不足。

Method: 1. 提出可扩展的数据生成管道，配备多粒度评估，自主合成跨图像和视频领域的结构化复杂推理轨迹；2. 设计双智能体架构：推理智能体执行广泛分析链，总结智能体批判性评估并提炼最终结果；3. 引入ST-GRPO和J-GRPO算法增强时空推理和评估鲁棒性；4. 利用总结智能体的可靠反馈指导迭代推理路径生成过程，实现持续自我改进循环。

Result: 在LLaVA-NeXT和Qwen2.5-VL等基础模型上的广泛实验表明，在具有挑战性的图像和视频推理基准上取得了显著性能提升，同时保持了传统感知任务的强大能力。

Conclusion: Insight-V++框架通过统一的多智能体视觉推理方法，成功解决了多模态大语言模型在长链推理方面的关键限制，为复杂时空理解任务提供了有效的解决方案。
...
Result: 零样本设置下碰撞召回率接近零；使用VLM-AutoDrive微调后，碰撞F1分数从0.00提升至0.69，整体准确率从35.35%提升至77.27%。在真实世界Nexar车载摄像头视频上实现碰撞和近碰撞检测的显著提升，并产生可解释的推理轨迹。

Conclusion: VLM-AutoDrive为将通用VLM适配到安全关键、时间定位的感知任务提供了可扩展方案，弥合了自动驾驶中感知、因果性和决策推理之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 微出行工具日益普及，但现有图像数据集对弱势道路使用者和微出行工具的分类不足（如将行人和微出行骑手都归为"人"类），且缺乏从弱势道路使用者视角（人行道、自行车道）采集的数据，限制了交通安全和规划相关计算机视觉模型的发展。

Method: 创建MicroVision数据集：在瑞典哥德堡采集超过8,000张匿名全高清图像，包含30,000多个精心标注的弱势道路使用者（行人、骑自行车者、电动滑板车骑手）和静止微出行工具（自行车、电动滑板车），覆盖全年不同季节，包含近2,000个独特的交互场景。同时基于最先进架构提供了基准目标检测模型。

Result: 数据集包含8,000多张图像和30,000多个标注，基准模型在未见测试集上达到最高0.723的平均精度均值（mAP）。

Conclusion: MicroVision数据集填补了微出行检测数据集的空白，支持交通安全系统区分不同类型的弱势道路使用者和微出行工具，有助于监控系统识别微出行的使用情况，促进交通安全和规划研究。
...
TL;DR: CycleCap：一种基于循环一致性的自监督微调方案，通过图像-文本-图像重建相似度作为奖励信号，无需标注数据即可提升视觉语言模型的图像描述准确性和减少幻觉


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在视觉-语言对齐问题，容易产生过于通用或幻觉的描述。现有方法需要昂贵的标注数据集或复杂的测试时框架。本文通过循环一致性视角重新审视图像-文本对齐问题，旨在开发无需标注数据的自监督训练方法

Method: 提出CycleCap微调方案：1）使用VLM作为图像到文本组件生成描述；2）使用预训练文本到图像模型从描述重建图像；3）基于原始图像与重建图像相似度构建奖励信号；4）采用组相对策略优化（GRPO）进行微调，将循环一致性直接作为自监督训练信号
...
TL;DR: PreSCAN：基于SHAP分析发现多视角一致性决定NeRF重建质量，开发了预测框架，在训练前使用轻量级几何和光度描述符估计NeRF质量，实现1000倍加速于NAS，并在边缘平台上显著降低功耗和延迟。


<details>
  <summary>Details</summary>
Motivation: 虽然NeRF在多视角图像的光照真实3D重建中表现出色，但在卫星图像部署中面临挑战：每个场景需要单独训练，且通过NAS优化架构需要数小时到数天的GPU时间。现有方法主要关注架构改进，但SHAP分析揭示多视角一致性（而非模型架构）才是决定重建质量的关键因素。

Method: 基于SHAP分析的关键发现，开发了PreSCAN预测框架。该框架在训练前使用轻量级几何和光度描述符来估计NeRF质量。通过这种方法，能够在<30秒内选择合适的架构，预测误差<1 dB。进一步在边缘平台（Jetson Orin）上部署，结合离线成本分析，优化推理过程。

Result: PreSCAN实现了1000倍加速于NAS方法。在边缘平台上，结合预测和离线成本分析，将推理功耗降低26%，延迟减少43%，且质量损失最小。在DFC2019数据集上的实验证实，PreSCAN能够泛化到多样化的卫星场景而无需重新训练。

Conclusion: PreSCAN通过识别多视角一致性作为NeRF重建质量的关键决定因素，开发了高效的预测框架，显著加速了卫星图像NeRF的部署过程，并在边缘计算平台上实现了显著的功耗和延迟优化，展示了良好的泛化能力。
...
TL;DR: 论文提出Tri-Layer诊断框架，通过三个指标分析VLM幻觉来源：潜在异常检测（感知意识）、视觉必要性分数（视觉依赖度）、竞争分数（视觉基础与指令跟随的冲突）。研究发现69.6%样本存在"视觉奉承"现象，即模型能检测视觉异常但仍产生幻觉以满足用户期望。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究VLM在回答正确时，是否真正依赖视觉信息还是利用语言捷径。当前缺乏系统化诊断框架来解构VLM幻觉的来源，特别是区分感知意识、视觉依赖和指令冲突等不同层面的问题。

Method: 提出Tri-Layer诊断框架，包含三个核心指标：1）潜在异常检测评估感知意识；2）视觉必要性分数通过KL散度测量视觉依赖；3）竞争分数量化视觉基础与指令跟随的冲突。使用反事实干预（盲图、噪声图、冲突图）在7个VLM和7,000个模型-样本对上进行实验。

Result: 研究发现69.6%样本存在Visual Sycophancy（视觉奉承），即模型能检测视觉异常但仍产生幻觉以满足用户期望。零样本显示Robust Refusal，表明对齐训练系统性地抑制了真实不确定性承认。缩放分析显示更大模型减少语言捷径但放大视觉奉承。诊断分数支持后验选择性预测策略，在50%覆盖率下实现+9.5pp准确率提升。

Conclusion: VLM存在系统性幻觉问题，特别是视觉奉承现象普遍存在。缩放本身无法解决基础问题，需要新的训练范式。提出的诊断框架和选择性预测策略为理解和缓解VLM幻觉提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于极线几何的匹配方法通常依赖粗略的空间分箱，这会引入近似误差、需要昂贵的后处理，并且可能遗漏有效对应关系。需要一种更精确、高效的匹配方法来解决这些限制。

Method: 提出一种精确的匹配框架：为每个关键点分配一个容差圆，从极点的视角定义角度区间；将匹配问题转化为1D角度区间查询；使用线段树在对数时间内高效解决查询问题。

Result: 在ETH3D数据集上的广泛评估显示，相比现有方法获得了显著的速度提升，同时恢复了精确的对应关系集合。

Conclusion: 该方法提供了像素级容差控制、支持每个关键点的独立控制、移除了不必要的描述符比较，在保证精确性的同时实现了高效的匹配性能。
...
TL;DR: Inst4DGS提出了一种实例分解的4D高斯泼溅方法，通过标签排列潜变量和可微Sinkhorn层解决多视角视频中实例标签不一致的关联问题，实现长期轨迹优化和高质量渲染与分割。


<details>
  <summary>Details</summary>
Motivation: 动态4DGS发展迅速，但实例分解的4DGS仍未被充分探索，主要挑战在于如何关联独立分割的多视角视频中不一致的实例标签，需要解决跨视频实例匹配和身份一致性保持的问题。

Method: 1) 引入每视频标签排列潜变量，通过可微Sinkhorn层学习跨视频实例匹配，实现多视角监督下的身份一致性保持；2) 提出实例分解的运动支架，为每个对象提供低维运动基，支持长期轨迹优化；3) 显式标签对齐产生清晰决策边界和时间稳定的身份，避免身份漂移。

Result: 在Panoptic Studio和Neural3DV数据集上，Inst4DGS同时支持跟踪和实例分解，在渲染和分割质量上达到最先进水平。在Panoptic Studio数据集上，PSNR从26.10提升到28.36，实例mIoU从0.6310提升到0.9129，显著优于最强基线。

Conclusion: Inst4DGS通过创新的标签排列潜变量和实例分解运动支架，成功解决了多视角视频中实例关联的挑战，实现了高质量的动态场景重建和实例分解，为4D高斯泼溅的实例感知应用提供了有效解决方案。
...
TL;DR: DermCase：基于皮肤病学病例报告构建的长上下文多模态基准，用于评估LVLMs在罕见皮肤病诊断中的临床推理能力，发现现有模型在诊断准确性、鉴别诊断和临床推理方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注常见疾病且仅评估最终准确性，忽视了临床推理过程，而这对复杂病例至关重要。特别是在罕见皮肤病诊断方面，LVLMs的评估仍未被充分探索。

Method: 构建DermCase基准，包含26,030个多模态图像-文本对和6,354个临床挑战性病例，每个病例都标注了全面的临床信息和逐步推理链。建立了基于DermLIP的相似性度量来评估鉴别诊断质量，并对22个领先的LVLMs进行基准测试。

Result: 基准测试显示现有LVLMs在诊断准确性、鉴别诊断和临床推理方面存在显著缺陷。指令微调能显著提升性能，而直接偏好优化（DPO）带来的改善有限。系统误差分析进一步揭示了当前模型推理能力的关键局限性。

Conclusion: DermCase基准填补了评估LVLMs在皮肤病学中临床推理能力的空白，揭示了当前模型在处理罕见复杂病例时的局限性，为未来改进提供了重要方向。
...
TL;DR: SynQ是一个零样本量化框架，通过低通滤波器减少合成数据噪声，对齐类激活图提高精度，并使用软标签避免预训练模型错误指导，在无数据情况下实现最先进的量化性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本量化方法面临三个主要挑战：合成数据集中的噪声、基于偏离目标模式的预测、以及错误硬标签的误导。这些挑战限制了在隐私或安全原因导致训练数据不可访问场景下的量化性能。

Method: SynQ采用三阶段方法：1) 使用低通滤波器最小化生成样本的噪声；2) 通过对齐量化模型与预训练模型的类激活图来提高精度；3) 对于困难样本仅使用软标签，避免预训练模型错误的误导。

Result: 大量实验表明，SynQ在零样本量化方法中提供了最先进的准确率，显著优于现有方法。

Conclusion: SynQ通过系统解决零样本量化的三个关键挑战，成功实现了在无训练数据情况下的高效模型量化，为隐私敏感场景下的模型部署提供了有效解决方案。
...
TL;DR: 提出一种基于可控扩散模型的数据增强方法，通过类别感知提示和视觉先验融合生成高质量合成图像，显著提升语义分割性能，特别是在数据稀缺场景下。


<details>
  <summary>Details</summary>
Motivation: 像素级语义分割数据集标注成本高昂，传统数据增强方法只能产生几何变化而无法生成新结构，现有生成模型在保持原始图像与生成图像一致性方面存在困难。

Method: 提出集成可控扩散模型的合成数据增强流程，采用类别感知提示和视觉先验融合技术，平衡数据多样性和可靠性，确保生成图像与分割标签的精确对齐。

Result: 在PASCAL VOC和BDD100K等基准数据集上验证，方法显著提升语义分割性能，特别是在数据稀缺场景下，同时增强了模型在真实世界应用中的鲁棒性。

Conclusion: 提出的可控扩散模型数据增强方法有效弥合合成数据与真实数据之间的差距，为语义分割任务提供了一种高效的数据增强解决方案。
...
TL;DR: SR-Nav通过建模空间关系图来增强零样本目标导航，在HM3D数据集上实现了最先进的性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在视角不佳或语义线索弱时，基础模型难以支持可靠的感知和规划推理，导致导航效率低或失败。观察到物体和区域间的内在关系编码了结构化场景先验，有助于在部分观测下推断合理的目标位置。

Method: 提出SR-Nav框架：1) 构建动态空间关系图(DSRG)，通过基础模型编码以目标为中心的空间关系，并随实时观测动态更新；2) 关系感知匹配模块，利用关系匹配而非简单检测，通过DSRG中的多样关系验证和纠正错误；3) 动态关系规划模块，基于DSRG从当前位置动态计算最优路径，减少规划搜索空间。

Result: 在HM3D数据集上的实验表明，该方法在成功率和导航效率方面均达到了最先进的性能。

Conclusion: 建模观察到的和经验性的空间关系能有效增强零样本目标导航的感知和规划能力，特别是在视角不佳或语义线索弱的情况下。
...
TL;DR: 提出一种无需额外标注的自一致性训练方法，通过强制相关任务（生成与验证）关注相同帧来改善视频-LLMs的时间定位能力，在体育教练任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视频-LLMs在时间定位上经常关注无关帧，这对需要精确时间定位的体育教练任务尤其有害。获取帧级监督既昂贵（人工标注）又不可靠（其他模型）。

Method: 利用观察发现：相关任务（如生成和验证）必须关注相同的帧。通过自一致性目标函数，在紧密相关任务的选择性视觉注意力图上强制执行这一约束，无需额外标注。

Result: 使用VidDiffBench验证注意力分配错误是主要瓶颈。自一致性训练在三个体育教练任务（Exact、FitnessQA、ExpertAF）上分别获得+3.0%、+14.1%准确率和+0.9 BERTScore的提升，甚至超过闭源模型。

Conclusion: 通过自一致性目标强制相关任务关注相同帧，可以有效改善视频-LLMs的时间定位能力，无需额外标注，在体育教练任务上实现显著性能提升。
...
TL;DR: 开发了一个用于T2加权前列腺MRI自动癌症检测的可解释框架，在小型数据集上比较了ViT、CNN和传统方法，发现迁移学习的ResNet18性能最佳，而HOG+SVM在小型数据集上表现相当，仅使用T2加权图像就实现了与需要多参数MRI的大规模方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是男性主要死亡原因，但T2加权前列腺MRI解释具有挑战性，因为病变细微且异质性高。现有方法通常依赖双参数MRI（T2+DWI）和大规模队列，增加了采集复杂性和计算成本。本研究旨在开发仅使用T2加权图像的自动癌症检测框架，解决数据稀缺问题并降低临床应用门槛。

Method: 使用162张T2加权图像（102例癌症，60例正常）的小型数据集，通过迁移学习和数据增强解决数据稀缺问题。全面比较了Vision Transformers（ViT、Swin）、CNN（ResNet18）和传统方法（逻辑回归、SVM、HOG+SVM）。在22例病例的读者研究中，比较了5名放射科医生与AI模型的性能。

Result: 迁移学习的ResNet18获得最佳性能（准确率90.9%，敏感性95.2%，AUC 0.905），仅1100万参数。Vision Transformers性能较低但复杂度显著更高。HOG+SVM获得可比性能（AUC 0.917），显示手工特征在小数据集中的有效性。读者研究中放射科医生平均敏感性67.5%（Fleiss Kappa=0.524），AI模型敏感性95.2%，表明AI辅助筛查可减少漏诊并提高一致性。

Conclusion: 仅使用T2加权图像即可实现竞争性前列腺癌检测性能，降低了采集复杂性和计算成本。迁移学习的轻量级CNN在小数据集上优于复杂Vision Transformers，而传统手工特征方法在小数据集上仍具竞争力。AI模型显著优于放射科医生，显示出AI辅助筛查减少漏诊和提高一致性的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像恢复方法通常是模态特定或退化特定的，无法泛化到临床实践中遇到的异构退化问题。这种局限性源于医学图像恢复与医学图像质量评估的分离，没有明确质量理解的恢复模型难以适应跨模态的多种退化类型。

Method: 提出MedQ-UNI统一视觉语言模型，采用多模态自回归双专家架构：质量评估专家通过结构化自然语言描述识别退化问题，恢复专家基于这些描述进行针对性图像恢复。构建了包含约50K配对样本的大规模数据集，涵盖三种成像模态和五个恢复任务，每个样本都带有结构化质量描述用于联合训练。

Result: 单一MedQ-UNI模型无需任何任务特定适应，在所有任务上实现了最先进的恢复性能，同时生成更优的描述。实验证实明确的质量理解能显著提高恢复保真度和可解释性。

Conclusion: 将医学图像质量评估与医学图像恢复相结合的统一方法能有效处理临床实践中遇到的异构退化问题。MedQ-UNI通过"评估-修复"范式实现了跨模态和退化类型的通用修复，为医学图像处理提供了新的解决方案。
...
TL;DR: ColourCrafter是一个统一的扩散框架，通过RGB颜色令牌与图像令牌的融合实现细粒度、区域感知的颜色编辑，解决了现有方法在连续颜色变化和局部编辑上的不足。
...
Method: 提出ColourCrafter框架：1）在潜在空间中进行RGB颜色令牌与图像令牌的令牌级融合，选择性将颜色信息传播到语义相关区域同时保持结构保真度；2）使用感知Lab空间损失，通过解耦亮度和色度并在掩码区域内约束编辑来增强像素级精度；3）构建ColourfulSet大规模高质量图像对数据集，包含连续多样的颜色变化。

Result: 大量实验表明，ColourCrafter在细粒度颜色编辑中实现了最先进的颜色准确性、可控性和感知保真度。

Conclusion: ColourCrafter将颜色编辑从全局色调转移转变为结构化、区域感知的生成过程，通过创新的令牌融合和损失设计解决了连续颜色编辑的挑战，为细粒度颜色控制提供了有效解决方案。
...
TL;DR: VLMs在从游戏视频推断玩家参与度方面表现有限，零样本预测效果差，理论引导提示反而强化表面捷径，存在感知-理解差距


<details>
  <summary>Details</summary>
Motivation: 研究视觉-语言模型能否仅从视觉线索推断人类参与度这一潜在心理状态，这对游戏设计和玩家体验研究很重要

Method: 使用GameVibe Few-Shot数据集，在9款第一人称射击游戏中评估3个VLMs，采用6种提示策略：零样本预测、基于Flow/GameFlow/自我决定理论/MDA的理论引导提示、检索增强提示，同时考虑逐点参与度预测和连续窗口间参与度变化的成对预测

Result: 零样本VLM预测普遍较弱，常无法超越简单的每游戏多数类基线；检索增强提示在某些设置下改善逐点预测，而成对预测在所有策略中都持续困难；理论引导提示无可靠帮助，反而可能强化表面捷径
...
Conclusion: T-QPM框架通过扩展双模式匹配为时间四重模式匹配，有效解决了动态环境中多模态OOD检测的挑战，提高了对时间分布偏移和协变量变化的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现有SOTA编辑模型在进行纹理编辑时经常难以保持结构一致性，尽管预期的变化纯粹是外观相关的。这促使研究者从数据和训练两个角度共同增强结构保持能力。

Method: 1. 构建TexBlender：使用Blender生成的高质量SFT数据集，提供强结构先验；2. 提出StructureNFT：基于RL的方法，集成结构保持损失，将SFT阶段学习的结构先验迁移到真实场景；3. 建立TexBench：通用的真实世界纹理编辑基准。
...
TL;DR: FILT3R提出了一种无需训练、可插拔的潜在状态过滤层，将流式3D重建中的循环状态更新建模为令牌空间中的随机状态估计，通过自适应平衡记忆保留与新观测来提高长期稳定性。


<details>
  <summary>Details</summary>
Motivation: 流式3D重建需要维护持续更新的潜在状态，但现有状态更新规则存在关键缺陷：激进覆盖会遗忘有用历史，保守更新则无法跟踪新证据，且两种行为在超出训练范围时都会变得不稳定。

Method: 提出FILT3R训练免费潜在过滤层，将循环状态更新建模为令牌空间中的随机状态估计。维护每个令牌的方差，计算卡尔曼式增益来自适应平衡记忆保留与新观测。过程噪声通过候选令牌的EMA归一化时间漂移在线估计。

Result: FILT3R产生可解释的插拔式更新规则，将常见的覆盖和门控策略作为特例。在稳定区域中，随着不确定性随累积证据收缩，增益减小；在真实场景变化增加过程不确定性时，增益上升，相比现有方法在深度、姿态和3D重建的长期稳定性方面表现更好。

Conclusion: FILT3R通过将状态更新建模为随机估计问题，提供了一种自适应平衡记忆与观测的通用框架，解决了流式3D重建中长期稳定性问题，且无需额外训练即可部署。
...
TL;DR: Diff-SIT：一种基于稀疏信息传输的高效视频扩散模型，通过稀疏时间编码和一步视频扩散框架，在超低码率下实现优异的感知质量和时间一致性


<details>
  <summary>Details</summary>
Motivation: 传统端到端压缩模型在超低码率下会产生模糊图像且感知质量差，现有生成式压缩方法常独立处理视频帧，存在时间一致性和效率限制。需要解决超低码率下的感知质量、时间一致性和效率问题。
...
TL;DR: HOMEY是一个结合YOLO与领域特定掩码机制及自定义损失函数的检测框架，用于自动检测17种房产风险类别，在真实房产图像上表现出优于基线YOLO模型的检测精度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 自动化房产风险检测是计算机视觉中一个高影响但未充分探索的前沿领域，对房地产、承保和保险运营具有直接影响。目前缺乏专门针对房产风险检测的有效解决方案。
...
TL;DR: AI已从静态蛋白质结构预测转变为动态构象集合和复杂生物分子相互作用的建模，涵盖统一多模态表示、静态预测精化、生成框架、异质相互作用预测和功能推断五个维度。


<details>
  <summary>Details</summary>
Motivation: 人工智能正在从根本上改变蛋白质折叠问题，从静态结构预测转向动态构象集合和复杂生物分子相互作用的建模。这一转变需要系统性地审视AI驱动蛋白质科学的范式转变，以理解AI如何从结构分析工具转变为能够理解和重写生命动态语言的通用模拟器。

Method: 该综述系统性地从五个相互关联的维度审视AI驱动蛋白质科学的范式转变：1）统一多模态表示，整合序列、几何结构和文本知识；2）通过无MSA架构和全原子复合物建模精化静态预测；3）生成框架，包括扩散模型和流匹配，捕捉与热力学集合一致的构象分布；4）预测异质相互作用，涵盖蛋白质-配体、蛋白质-核酸和蛋白质-蛋白质复合物；5）功能推断，包括适应度景观、突变效应和文本引导的属性预测。

Result: AI方法已成功实现从静态结构预测到动态构象集合建模的转变，能够处理复杂的生物分子相互作用并进行功能推断。然而，当前仍存在数据分布偏差、机制可解释性有限、几何指标与生物物理现实脱节等瓶颈。

Conclusion: 这一方法论转变标志着人工智能从结构分析工具转变为能够理解和最终重写生命动态语言的通用模拟器。未来方向包括开发物理一致的生成模型、多模态基础架构和实验闭环系统，以克服当前瓶颈并推动该领域进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 针对车险行业需要处理大量视觉数据（车辆损伤）、多模态文档（理赔材料）和复杂风险评估的需求，传统方法难以实现端到端自动化。手册旨在建立将现代人工智能技术转化为可靠、生产级系统的原则性框架，特别适用于高风险工业环境。

Method: 1. 提出垂直整合的AI范式，统一感知、多模态推理和生产基础设施；2. 开发领域适应的Transformer架构，包括结构化视觉理解、关系型车辆表示学习和多模态文档智能；3. 构建可扩展的流水线，在泰国全国车险系统的实际约束下运行；4. 强调学习算法与MLOps实践的协同演化。

Result: 实现了车辆损伤分析、理赔评估和核保工作流的端到端自动化。系统已在泰国全国车险系统中大规模实际部署，验证了其在真实工业环境中的可行性和可靠性。

Conclusion: 该手册为将现代人工智能技术转化为生产级系统提供了系统性的框架，特别是在高风险工业环境中。通过垂直整合的AI范式和领域适应的架构设计，成功实现了车险业务流程的自动化，同时强调了算法与工程实践的协同演化对于构建可靠工业AI系统的重要性。
...
TL;DR: OnlinePG：首个基于3D高斯泼溅的在线开放词汇全景映射系统，实现实时几何重建与实例级场景理解


<details>
  <summary>Details</summary>
Motivation: 现有方法多为离线或缺乏实例级理解，限制了在真实机器人任务中的应用。需要实时的开放词汇场景理解系统，支持在线全景映射。

Method: 采用滑动窗口的局部到全局范式：1)构建局部一致性图，通过3D分割聚类图结合几何与语义线索融合不一致片段；2)为局部3D高斯图构建带空间属性的显式网格，通过鲁棒双向二分3D高斯实例匹配融合到全局地图；3)利用3D空间属性网格中的融合VLM特征实现开放词汇理解。

Result: 在广泛使用的数据集上实验表明，该方法在在线方法中取得更好性能，同时保持实时效率。

Conclusion: OnlinePG是首个基于3D高斯泼溅的在线开放词汇全景映射系统，有效解决了现有方法的局限性，为具身应用提供了实时感知能力。
...
TL;DR: LVLMs展现类似人类的计数行为，研究者通过机制分析发现了共享的"计数电路"，并提出利用合成图像进行轻量级微调的方法，显著提升了计数和通用视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 计数作为大型视觉语言模型推理能力的简单而强大的测试，需要模型识别每个对象并进行累加。本研究旨在探索LVLMs如何实现计数，并理解计数在视觉推理中的核心作用。

Method: 使用受控合成和真实世界基准测试结合机制分析；引入两种新颖的可解释性方法：视觉激活修补和HeadLens；提出利用简单合成图像对预训练LVLMs进行专门计数微调的轻量级干预策略。

Result: LVLMs表现出类似人类的计数行为：小数量精确，大数量噪声估计；发现了跨多种视觉推理任务共享的结构化"计数电路"；仅针对计数的微调不仅提升了分布内合成数据的准确性，还在分布外计数基准上平均提升+8.36%，在复杂通用视觉推理任务上为Qwen2.5-VL带来平均+1.54%的增益。

Conclusion: 计数在视觉推理中扮演核心且有影响力的角色，通过针对性增强计数机制可能成为提升整体视觉推理能力的潜在途径。轻量级干预策略展示了通过专门微调基础能力可以带来广泛的性能提升。
...
TL;DR: 提出3DreamBooth和3Dapter框架，实现3D感知的视频定制化，解决现有2D方法缺乏空间先验导致新视角合成时无法保持真实3D身份的问题。


<details>
  <summary>Details</summary>
Motivation: 现有主题驱动视频生成方法主要将主题视为2D实体，缺乏3D空间先验，导致新视角合成时无法重建真实3D几何，只能生成合理但任意的细节。同时，多视角视频数据稀缺，有限视频序列微调容易导致时间过拟合。

Method: 提出3DreamBooth通过单帧优化范式将空间几何与时间运动解耦，仅更新空间表示以注入3D先验。引入3Dapter视觉条件模块，采用非对称条件策略与主生成分支进行多视角联合优化，作为动态选择性路由器从最小参考集中查询视角特定的几何提示。

Result: 该方法无需大量视频训练即可将稳健3D先验融入模型，增强细粒度纹理并加速收敛，实现真正的3D感知视频定制化。

Conclusion: 提出的3DreamBooth和3Dapter框架解决了2D方法在3D对象定制中的根本限制，通过解耦空间几何与时间运动、引入视觉条件模块，实现了保持真实3D身份的视频定制化。
...
TL;DR: 提出一种受生物启发的中心-外周注意力优化框架，解决跨域少样本目标检测中的目标域散光问题，通过重塑注意力模式提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 跨域少样本目标检测面临严重的域偏移和数据稀缺问题。研究发现模型在目标域中表现出分散、不集中的注意力模式（称为目标域散光问题），导致定位不精确和冗余预测。分析表明常规微调虽能缓解此问题但效果有限，需要增强注意力聚焦能力。

Method: 受人类中央凹视觉系统启发，提出中心-外周注意力优化框架：1）正模式优化模块：使用类别特定原型重塑对语义对象的注意力，模拟视觉中心区域；2）负上下文调制模块：通过建模背景上下文增强边界判别能力，模拟视觉外周区域；3）文本语义对齐模块：通过跨模态线索强化中心-外周区分。

Result: 在六个具有挑战性的CD-FSOD基准测试中，该方法显著提升了检测精度，并建立了新的最先进结果，有效将散光注意力转化为聚焦模式。

Conclusion: 提出的生物启发式中心-外周注意力优化框架成功解决了跨域少样本目标检测中的目标域散光问题，通过模拟人类视觉系统的注意力机制，显著提升了模型在目标域的适应能力和检测性能。
...
TL;DR: CoDA框架通过模拟临床影像处理链中的分布偏移，评估医学视觉-语言模型在真实工作流程中的鲁棒性，发现链式组合比单一阶段更具破坏性，并提出轻量级对齐修复策略。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉-语言模型鲁棒性评估多基于干净、经过整理的输入或孤立扰动，忽略了临床工作流程中保持可读性的常规采集、重建、显示和传输操作带来的分布偏移，需要更贴近临床实际的评估框架。

Method: 提出CoDA（链式分布）框架，通过组合模拟采集（如阴影）、重建与显示重映射、传输与导出退化等临床流程操作，在保持结构相似性的约束下联合优化阶段组合和参数，以诱导模型失败同时保持视觉合理性。

Result: CoDA显著降低了CLIP风格医学视觉-语言模型的零样本性能，链式组合比任何单一阶段更具破坏性；专有多模态模型在CoDA偏移样本上的审计可靠性下降且存在高置信度错误，医学专用多模态大语言模型在医学图像质量审计方面存在明显缺陷；提出的基于教师引导令牌空间适应和补丁级对齐的后修复策略提高了对CoDA输出的准确性。

Conclusion: 研究揭示了医学视觉-语言模型部署中基于临床的威胁面，表明轻量级对齐能提高部署鲁棒性，为临床工作流程中的模型可靠性评估提供了新框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前面向规划的端到端驾驶模型主要学习统计相关性而非真实因果关系，容易受到数据集偏差影响而产生因果混淆，这严重损害了模型在复杂场景下的可靠性和安全性

Method: 提出CausalVAD框架，核心是稀疏因果干预方案（SCIS）——一个轻量级即插即用模块，通过构建潜在驾驶场景原型字典，对模型的稀疏向量化查询进行干预，消除由混淆变量引起的虚假关联

Result: 在nuScenes等基准测试中，CausalVAD实现了最先进的规划精度和安全性，同时在针对因果混淆配置的数据偏差和噪声场景中表现出优越的鲁棒性

Conclusion: CausalVAD通过因果干预有效解决了自动驾驶规划模型中的因果混淆问题，显著提升了模型的可靠性和安全性，为构建更鲁棒的端到端驾驶系统提供了新思路
...
TL;DR: 提出跨层注意力传播方法HAViT，通过存储和融合历史注意力矩阵来改进Vision Transformer的层间信息流，在CIFAR-100和TinyImageNet上获得1.25-1.33%的精度提升。
...
Method: 提出跨层注意力传播方法，存储历史注意力矩阵并在后续层中融合，通过注意力矩阵存储和混合操作实现，最小化架构改动。

Result: 在CIFAR-100上ViT精度从75.74%提升至77.07%（+1.33%），TinyImageNet上从57.82%提升至59.07%（+1.25%）；CaiT获得1.01%提升；最优混合超参数α=0.45；随机初始化优于零初始化。

Conclusion: 跨层注意力传播有效改进Vision Transformer的层间信息流，通过历史注意力矩阵融合实现渐进式注意力模式优化，显著提升性能且架构改动最小。
...
TL;DR: 提出基于饱和度-亮度相似性的新型非局部变分方法用于彩色图像恢复，通过考虑彩色图像在饱和度-亮度通道的相似性来改进传统非局部方法
...
Method: 建立基于饱和度-亮度相似性的非局部全变差，将彩色图像块的饱和度-亮度相似性融入非局部梯度；基于此构建变分模型；采用Bregmanized算子分裂法设计高效算法，并研究算法收敛性

Result: 数值实验表明，所提模型在视觉质量和定量指标（PSNR、SSIM、QSSIM、S-CIELAB颜色误差）上均优于其他测试方法

Conclusion: 基于饱和度-亮度相似性的非局部变分方法能更有效地描述彩色图像块间的相似性，在彩色图像恢复任务中表现出优越性能
...
TL;DR: 提出了一种基于自然语言描述面部动作单元(AU)的新方法，解决了传统AU编码在处理冲突AU时的线性组合问题，实现了更自然、解剖学合理的面部行为合成。


<details>
  <summary>Details</summary>
Motivation: 当前基于AU的面部行为合成方法通常将AU编码为one-hot向量，将复合表情建模为单个AU的简单线性组合。这种线性方法在处理冲突AU（激活相同面部肌肉但作用相反）时会产生解剖学上不合理的人工痕迹和不自然的运动叠加，需要更精确的方法来捕捉人类非语言交流的细微差别。

Method: 1) 提出用自然语言描述AU来表示面部行为，保留AU框架的表达能力同时能显式建模复杂和冲突的AU；2) 引入BP4D-AUText数据集，这是首个用于复杂面部行为的大规模文本-图像配对数据集，通过对BP4D和BP4D+数据集应用基于规则的动态AU文本处理器合成；3) 提出VQ-AUFace生成模型，利用面部结构先验从文本合成真实多样的面部行为。

Result: 通过广泛的定量实验和用户研究表明，该方法在生成解剖学合理、行为丰富、感知可信的面部表情方面显著优于现有方法，特别是在涉及冲突AU的挑战性条件下表现出色。

Conclusion: 提出的基于自然语言描述AU的方法有效解决了传统线性AU编码在处理冲突AU时的问题，实现了更自然、解剖学合理的面部行为合成，同时解锁了现代文本到图像模型在高保真面部合成方面的潜力。
...
TL;DR: 该论文在缅甸手写数字数据集myMNIST上进行了首个系统性基准测试，评估了11种不同架构，发现CNN表现最佳，PETNN(GELU)紧随其后，为缅甸NLP/AI研究建立了可复现的基准。


<details>
  <summary>Details</summary>
Motivation: 缅甸手写数字数据集myMNIST（原BHDD）对缅甸NLP/AI研究至关重要，但缺乏系统性基准测试。本研究旨在填补这一空白，为缅甸数字识别研究建立可复现的基准，并评估新兴架构在区域文字上的表现。

Method: 在myMNIST数据集上评估了11种架构：经典深度学习模型（MLP、CNN、LSTM、GRU、Transformer）、近期替代模型（FastKAN、EfficientKAN）、能量基模型（JEM）以及物理启发的PETNN变体（Sigmoid、GELU、SiLU）。使用精确率、召回率、F1分数和准确率作为评估指标。

Result: CNN表现最佳（F1=0.9959，准确率=0.9970），PETNN(GELU)紧随其后（F1=0.9955，准确率=0.9966），优于LSTM、GRU、Transformer和KAN变体。JEM表现具有竞争力（F1=0.9944，准确率=0.9958）。KAN基模型（FastKAN、EfficientKAN）表现相对较弱但提供了有意义的替代基准（准确率约0.992）。

Conclusion: 研究为myMNIST建立了跨多种建模范式的可复现基准，展示了PETNN相对于经典和Transformer模型的强大性能，量化了物理启发PETNN与真实能量基模型（JEM）之间的差距，并发布了基准以促进缅甸数字识别研究和新兴架构在区域文字上的评估。
...
<details>
  <summary>Details</summary>
Motivation: 尽管预训练的视觉语言模型（如CLIP）具有出色的零样本能力，但研究发现它们容易受到对抗样本的攻击。实验观察到对抗扰动会导致文本引导注意力发生偏移，因此需要开发方法来增强CLIP的对抗鲁棒性同时保持其泛化能力。

Method: 提出TGA-ZSR框架，包含局部注意力精炼模块和全局注意力约束模块。进一步提出Comp-TGA方法，整合类别提示引导的前景注意力和非类别提示引导的反向注意力，形成互补的注意力机制来更全面准确地表示前景特征。

Result: 实验验证TGA-ZSR和Comp-TGA分别在16个数据集上比现有最先进技术提升了9.58%和11.95%的零样本鲁棒准确率。

Conclusion: 通过文本引导注意力机制可以有效增强CLIP模型的对抗鲁棒性，互补的注意力策略能够更全面地捕捉前景特征，显著提升模型在对抗攻击下的性能表现。
...
TL;DR: SJD-PAC通过主动草稿策略和自适应延续机制，在保持目标分布无损的情况下，将文本到图像生成的推理速度提升3.8倍。


<details>
  <summary>Details</summary>
Motivation: Speculative Jacobi Decoding (SJD) 虽然提供了一种无需草稿模型的加速方法，但在视觉生成的高熵区域，草稿token接受率较低，这成为限制整体吞吐量的瓶颈。

Method: SJD-PAC采用两种优化：1) 主动草稿策略，提高高熵区域的局部接受率；2) 自适应延续机制，在初始拒绝后维持序列验证，避免完全重新采样。

Result: 在标准文本到图像基准测试中，SJD-PAC实现了3.8倍的加速，同时保持无损的图像质量。

Conclusion: SJD-PAC通过提高每步平均接受长度，显著提升了推理速度，同时严格保持了目标分布，解决了SJD在高熵视觉生成中的瓶颈问题。
...
TL;DR: 本文提出Cross-Modal Context Learning (CCL)方法，通过多个精心设计的模块解决双流transformer音频-视频联合生成中的关键问题，显著提升生成质量和训练效率。


<details>
  <summary>Details</summary>
Motivation: 当前基于双流transformer的音频-视频联合生成方法存在多个限制：1) 门控机制导致模型流形变化；2) 跨模态注意力引入多模态背景区域偏差；3) 多模态无分类器引导在训练和推理时的不一致性；4) 多条件之间的冲突。这些限制影响了生成质量和训练稳定性。

Method: 提出Cross-Modal Context Learning (CCL)框架，包含四个核心模块：1) Temporally Aligned RoPE and Partitioning (TARP) - 增强音频和视频潜在表示的时间对齐；2) Learnable Context Tokens (LCT) - 为跨模态信息提供稳定的无条件锚点；3) Dynamic Context Routing (DCR) - 基于不同训练任务动态路由信息；4) Unconditional Context Guidance (UCG) - 在推理时利用LCT的无条件支持促进不同形式的CFG，提高训练-推理一致性。

Result: CCL在综合评估中相比近期学术方法取得了最先进的性能，同时需要显著更少的计算资源。该方法有效解决了双流transformer架构中的关键问题，提升了音频-视频联合生成的质量和效率。

Conclusion: CCL通过创新的跨模态上下文学习机制，系统性地解决了当前音频-视频联合生成方法中的多个关键限制，在保持高质量生成的同时大幅降低了资源需求，为多模态生成任务提供了有效的解决方案。
...
TL;DR: CNN模型SegResNet在腹部多器官分割任务中优于所有混合Transformer模型，在异构数据集上表现出更强的性能


<details>
  <summary>Details</summary>
Motivation: 准确的多器官分割对计算机辅助诊断和治疗至关重要。虽然CNN一直是医学图像分割的标准方法，但基于Transformer的架构因能建模长距离依赖关系而受到关注。本研究旨在系统比较混合Transformer模型与CNN基线在腹部CT多器官分割任务中的性能。

Method: 在异构RATIC数据集（206个来自23个机构的标注CT扫描，覆盖5个腹部器官）上，系统比较了三种混合Transformer模型（UNETR、SwinUNETR、UNETR++）与强CNN基线SegResNet。所有模型在相同的预处理和训练条件下进行训练和评估，使用Dice相似系数作为主要评估指标。

Result: CNN-based SegResNet在所有器官上均取得最高性能，优于所有混合Transformer模型。在Transformer模型中，UNETR++表现最具竞争力，而UNETR收敛速度最快，需要更少的训练迭代次数。

Conclusion: 对于中小型异构数据集，经过良好优化的CNN架构仍然具有高度竞争力，可能优于混合Transformer设计。Transformer模型在收敛速度方面可能具有优势，但在整体分割性能上仍不及优化的CNN模型。
...
TL;DR: OpenT2M是一个百万级高质量开源运动数据集，包含超过2800小时人体运动数据，通过严格质量控制；MonoFrill是基于该数据集预训练的运动模型，使用新颖的2D-PRQ运动分词器，在文本到运动生成任务中表现出色。
...
Method: 1) 构建OpenT2M数据集：包含超过2800小时高质量人体运动数据，通过物理可行性验证和多粒度过滤进行质量控制，提供详细的逐秒文本标注；2) 开发自动化长时程序列创建流程；3) 提出MonoFrill预训练模型，核心是2D-PRQ运动分词器，通过将人体划分为生物学部分来捕捉时空依赖性。
...
TL;DR: GenVideoLens是一个细粒度基准测试，用于评估大型视觉语言模型在AI生成视频检测中的能力，揭示模型在不同真实性维度上的表现差异和局限性。


<details>
  <summary>Details</summary>
Motivation: 现有评估协议将AI生成视频检测视为二元分类问题，依赖整体准确率等粗粒度指标，无法深入了解LVLMs在哪些方面成功或失败，需要更精细的诊断工具。

Method: 构建包含400个高度欺骗性AI生成视频和100个真实视频的基准测试，由专家在15个真实性维度（感知、光学、物理、时间线索）上进行标注，评估11个代表性LVLMs。

Result: 发现明显的维度不平衡：LVLMs在感知线索上表现较好，但在光学一致性、物理交互和时间因果推理方面表现不佳；模型性能在不同维度间差异显著，小型开源模型有时在特定真实性线索上优于更强的专有模型；时间扰动实验显示当前LVLMs对时间信息的利用有限。
...
TL;DR: SVOO提出了一种无需训练的视频生成稀疏注意力框架，通过离线层敏感度分析和在线双向协同聚类，解决了现有方法忽略层异质性和查询-键耦合的问题，在保持质量的同时实现高达1.93倍加速。
...
Method: SVOO采用两阶段范式：1）离线层敏感度分析，推导每层固有的剪枝级别；2）在线块稀疏注意力，通过新颖的双向协同聚类算法实现。该方法无需训练，通过分析层间注意力稀疏性的内在特性来优化计算效率。

Result: 在七个广泛使用的视频生成模型上的实验表明，SVOO在质量-加速权衡方面优于现有方法，在Wan2.1上实现高达1.93倍加速的同时保持高达29 dB的PSNR。

Conclusion: SVOO通过揭示注意力稀疏性是每层固有属性的关键洞察，提出了一种有效的训练免费稀疏注意力框架，显著提升了视频生成模型的推理效率，为高质量实时视频生成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法在视觉保真度方面取得进展，但难以确保物理一致的运动。真实世界物体运动发生在三维空间，而视频观测仅提供视角依赖的二维投影，导致物理一致性不足。

Method: 提出两阶段框架：1) Phys4View阶段使用物理感知注意力捕捉物理属性对运动动态的影响，结合几何增强的跨视角注意力和时间注意力增强时空一致性；2) VideoSyn阶段以前景视频为引导，学习前景动态与背景环境的交互，实现可控视频合成。构建PhysMV数据集（40K场景，160K视频序列）支持训练。

Result: 大量实验表明，PhysVideo在物理真实性和时空连贯性方面显著优于现有视频生成方法。

Conclusion: 通过将三维物理建模与视频生成相结合，PhysVideo有效解决了现有方法中物理不一致的问题，为生成物理合理的视频提供了新框架。
...
TL;DR: MeInTime是一种基于扩散模型的跨年龄人脸修复方法，通过解耦身份和年龄条件建模，在训练时专注身份特征注入，推理时使用年龄感知梯度引导实现身份保真和年龄一致性。
...
Method: 提出MeInTime方法：1）解耦身份和年龄条件建模；2）训练时通过新型注意力机制有效注入身份特征，并使用门控残差融合模块整合退化特征和身份表示；3）推理时提出无需训练的年龄感知梯度引导采样策略，使用年龄驱动方向迭代引导身份感知去噪潜在向量到目标年龄语义流形。

Result: 大量实验表明，MeInTime在身份保持和年龄一致性方面优于现有人脸修复方法。

Conclusion: MeInTime成功将基于参考的人脸修复从同年龄设置扩展到跨年龄设置，实现了在仅有跨年龄参考情况下的忠实修复，同时保持身份保真和年龄一致性。
...
TL;DR: Click-to-Ask：一种用于直播电商的AI助手，包含离线模块（处理多模态产品信息生成结构化数据和合规文案）和在线模块（实时响应观众问题），显著提升直播电商效率


<details>
  <summary>Details</summary>
Motivation: 直播电商已成为现代重要传播形式，但主播在商品推广过程中面临准备时间长、与观众互动效率低等问题，需要更高效便捷的AI辅助工具来提升直播电商效果

Method: 提出Click-to-Ask系统，包含两个互补组件：1）离线模块处理多模态产品信息，转化为结构化产品数据并生成合规推广文案；2）在线模块在直播中实时响应观众问题，主播点击问题后，系统利用离线模块生成的结构化产品信息和流式架构维护的事件级历史记忆来生成回答

Result: 在收集的TikTok直播帧数据集上，该方法实现了0.913的问题识别准确率和0.876的响应质量评分，显著减少了推广准备时间，增强了内容参与度，并能及时回应观众询问

Conclusion: Click-to-Ask系统通过离线处理和在线实时响应的结合，有效提升了直播电商的效率和互动质量，展示了实际应用的巨大潜力，为直播电商提供了创新的AI辅助解决方案
...
TL;DR: 提出基于合成PDF和LLM评估的表格提取基准框架，相比传统指标显著提升与人类判断的相关性
...
Result: LLM评估与人类判断的相关性显著更高（Pearson r=0.93），优于TEDS（r=0.68）和GriTS（r=0.70）；评估21个PDF解析器在451个表格上的表现发现显著性能差异

Conclusion: 为表格数据提取提供解析器选择指导，建立了可重复、可扩展的评估方法学，LLM评估在语义表格评估中优于传统指标
...
<details>
  <summary>Details</summary>
Motivation: 医学超声图像分割面临标注数据有限和成像伪影（斑点噪声、低对比度边界）的挑战。现有半监督学习方法存在未标记数据利用不足和缺乏鲁棒特征表示机制的问题。

Method: 提出Switch框架，包含两个关键创新：1) 多尺度切换策略，采用分层补丁混合实现均匀空间覆盖；2) 频域切换，在傅里叶空间进行幅度切换并结合对比学习以获得鲁棒特征表示。这些组件集成在师生架构中，有效利用标注和未标注数据。

Result: 在六个不同的超声数据集（淋巴结、乳腺病变、甲状腺结节和前列腺）上评估，在5%标注比例下表现优异：LN-INT数据集Dice系数80.04%，DDTI数据集85.52%，前列腺数据集83.48%。半监督方法甚至超过全监督基线，同时保持参数效率（180万参数）。
...
TL;DR: 该综述系统回顾了多模态计算病理学的最新进展，重点分析了自监督表示学习、多模态数据生成、参数高效适应和多智能体协作推理四个研究方向，并探讨了未来统一框架的发展方向。


<details>
  <summary>Details</summary>
Motivation: 全切片成像技术推动了数字病理学发展，但面临四大挑战：超高分辨率带来的计算负担、专家标注有限、多模态信息整合困难、超长视觉序列建模不透明。需要系统性梳理多模态计算病理学的最新进展以应对这些挑战。

Method: 采用系统性综述方法，分析四个研究方向：1) WSI的自监督表示学习和结构感知令牌压缩；2) 多模态数据生成与增强；3) 参数高效适应和推理增强的少样本学习；4) 可信诊断的多智能体协作推理。特别关注令牌压缩如何实现跨尺度建模，以及多智能体机制如何模拟病理学家在不同放大倍数下的"思维链"推理。

Result: 综述系统梳理了多模态计算病理学的关键技术和进展，包括：令牌压缩技术解决了WSI超高分辨率的计算挑战；多模态生成方法缓解了标注数据稀缺问题；参数高效适应技术提高了模型实用性；多智能体协作推理机制增强了诊断的可解释性和可信度。

Conclusion: 未来进展依赖于统一的多模态框架，该框架需要整合高分辨率视觉数据与临床及生物医学知识，以支持可解释且安全的AI辅助诊断。多智能体协作推理和令牌压缩等技术为实现这一目标提供了重要路径。
...
Method: SCNP通过惩罚logits中最差分类的邻居来提升拓扑准确性，迫使模型在改善像素本身之前先改善其邻居的预测。该方法可集成到语义分割和实例分割的三种框架中，并能与多种损失函数结合。

Result: 在13个不同结构形态和图像模态的数据集上验证了SCNP的有效性，能够显著提升拓扑准确性，且代码已开源。

Conclusion: SCNP是一种高效、通用的方法，能够提升深度学习分割模型的拓扑准确性，易于集成到现有训练流程中，适用于多种结构形态和分割任务。
...
TL;DR: 该论文研究了视觉语言模型(VLMs)的安全决策如何受到简单语义线索的影响，发现这些模型依赖学习的视觉-语言关联而非基于视觉理解的推理，存在被自动化操纵的漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在现实世界和具身环境中的部署，安全决策依赖于视觉上下文。然而，目前不清楚哪些视觉证据驱动这些判断，需要研究多模态安全行为是否可以被简单的语义线索操纵。

Method: 提出语义引导框架，在不改变底层场景内容的情况下应用受控的文本、视觉和认知干预。创建SAVeS基准测试，用于评估语义线索下的情境安全，并设计评估协议分离行为拒绝、基于视觉的安全推理和错误拒绝。

Result: 实验表明，多个VLMs的安全决策对语义线索高度敏感，表明模型依赖学习的视觉-语言关联而非基于视觉理解的推理。进一步证明自动化引导管道可以利用这些机制，突显多模态安全系统的潜在漏洞。

Conclusion: 视觉语言模型的安全决策容易受到语义线索的操纵，这揭示了模型依赖表面关联而非真正视觉理解的根本局限性，对多模态安全系统的可靠性提出了重要警示。
...
TL;DR: 提出一种针对视频扩散变换器的推理时NVFP4/INT8混合精度量化框架，通过动态精度分配和时间冗余利用，实现1.92倍端到端加速和3.32倍内存减少


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成方面表现出色，但实际部署受到高内存使用和计算成本严重限制。现有量化方法采用静态位宽分配，忽视了不同扩散时间步中激活量化的难度，导致效率与质量之间的权衡不够优化。

Method: 提出推理时NVFP4/INT8混合精度量化框架：1) 发现模块输入输出差异与其内部线性层量化敏感度存在强线性相关性，设计轻量级预测器动态分配NVFP4给时间稳定层以最大化内存压缩，选择性保留INT8给波动层以确保鲁棒性；2) 观察到Transformer模块输入输出残差在时间步间具有高时间一致性，利用这种时间冗余引入时间增量缓存(TDC)跳过不变模块的计算。

Result: 广泛实验表明，该方法实现了1.92倍端到端加速和3.32倍内存减少，为视频扩散变换器的高效推理设定了新的基准。

Conclusion: 提出的自适应精度策略和时间冗余利用方法能够在保持生成质量的同时实现显著的效率提升，为视频扩散变换器的实际部署提供了有效的解决方案。
...
Result: 1）仅需每个诊断5个真实解释即可生成忠实合理的解释；2）在模型内设置中，多标签分类器的AUC比独立分类器提高2.21%；3）可适应不同受众（如训练非医学用户的简化版本）；4）在忠实性、可模拟性、多样性和合理性等多个指标上表现良好。

Conclusion: WeNLEX能够生成既忠实于模型推理又合理的自然语言解释，且只需少量标注数据。在模型内设置中还能提升下游任务性能，并可通过更换数据库灵活适应不同受众需求。
...
TL;DR: ProCal：一种用于源自由域适应的概率校准方法，通过双模型协作预测机制动态校准基于邻域的预测，平衡知识保留与域适应


<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法过度依赖邻域预测相似性，导致源知识遗忘加速和局部噪声过拟合问题，需要一种能平衡知识保留与域适应的解决方案

Method: 提出ProCal概率校准方法，通过双模型协作预测机制（源模型初始预测与当前模型在线输出结合）动态校准邻域概率；设计联合优化目标，结合软监督损失和多样性损失指导目标模型

Result: 理论分析表明ProCal收敛到源知识与目标信息有效融合的均衡点；在4个公共数据集的31个跨域任务上验证了有效性，代码已开源

Conclusion: ProCal通过概率校准有效缓解局部噪声干扰并保留源模型判别信息，在源自由域适应中实现了知识保留与域适应的平衡
...
TL;DR: SEAR是一种简单高效的微调策略，用于将预训练的视觉几何Transformer适配到RGB-热成像多模态输入，显著提升RGB-T场景的3D重建和相机姿态估计性能。


<details>
  <summary>Details</summary>
Motivation: 基于RGB数据预训练的视觉几何模型在处理RGB-热成像混合模态时效果下降，因为模型难以对齐RGB和热成像两种模态的特征表示。

Method: 提出SEAR微调策略，在相对较小的RGB-T数据集上微调预训练的几何Transformer，使其能够有效处理多模态RGB-T输入，对齐两种模态的特征。

Result: 在3D重建和相机姿态估计任务上显著超越现有方法，在AUC@30指标上提升超过29%，在低光照和浓烟等挑战性条件下仍能可靠工作，推理时间开销可忽略。

Conclusion: SEAR通过简单高效的微调策略成功将预训练几何模型适配到RGB-T多模态场景，并发布了包含不同时间、视角和光照条件的RGB-T数据集作为未来研究的基准。
...
TL;DR: 该论文对医学图像分割中不确定性量化的两种不确定性类型（偶然不确定性和认知不确定性）的组合方法进行了全面实证研究，提出了量化不确定性纠缠的指标，并评估了不同组合在下游任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割等安全关键应用需要不确定性量化。总不确定性通常分解为数据相关的偶然不确定性和模型相关的认知不确定性。现有许多方法分别建模这两种不确定性，但它们在组合时的相互作用不明确，且近期研究发现两者存在显著纠缠，影响了分解的可解释性和实际应用价值。

Method: 1. 进行了全面的实证研究，涵盖广泛的偶然不确定性-认知不确定性模型组合；2. 提出了量化不确定性纠缠的指标；3. 在多个下游不确定性量化任务中评估这些组合，包括分布外检测、模糊性建模和校准。
...
Conclusion: 偶然不确定性和认知不确定性的组合存在显著纠缠问题，影响不确定性分解的可解释性。集成方法在减少纠缠和提高性能方面表现突出，特别是在分布外检测任务中。需要进一步研究不确定性纠缠的来源和缓解方法，以提高医学图像分割等安全关键应用中不确定性量化的可靠性。
...
TL;DR: Perceptio是一个增强感知能力的大型视觉语言模型，通过显式的语义分割token和深度token在自回归序列中生成空间解释，显著提升了细粒度空间定位能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在语义理解方面表现出色，但在细粒度空间定位方面存在困难，因为模型需要隐式推断复杂几何关系而不产生空间解释。现有模型缺乏显式的空间推理能力。

Method: 1. 从强单目深度估计教师模型蒸馏VQ-VAE深度码本，将密集深度信息token化为紧凑序列；2. 在LLM中集成SAM2语义分割token和VQ-VAE深度token，使模型先输出空间token再回答问题；3. 引入复合深度token目标（标记、token和计数损失）和软合并技术稳定深度token生成；4. 采用跨多个数据集的多元任务协同训练策略。

Result: 在多个基准测试中取得最先进性能：RefCOCO/+/g的指代表达式分割cIoU分别提升+0.8/+1.4/+1.1；HardBLINK空间理解准确率提升10.3%；MMBench准确率提升1.0%。

Conclusion: 显式的空间思维链显著增强了大型视觉语言模型的空间定位能力，证明了在自回归序列中直接生成空间解释token的有效性，为视觉语言模型的空间推理提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 当前基础模型虽然能从大量无标签数据中学习可迁移表示，但大多架构缺乏可解释性，这在医疗影像等高风险领域尤为关键。需要开发既能保持高性能又具备内在可解释性的模型。

Method: 提出Dual-IFM模型，具有双重可解释性设计：1）通过类证据图提供对单个图像的局部可解释性，忠实反映决策过程；2）通过2D投影层提供对整个数据集的全局可解释性，可直接可视化表示空间。模型在超过80万张彩色眼底照片上进行自监督预训练。

Result: 模型在性能上达到与参数规模大16倍的SOTA基础模型相当的水平，同时在分布外数据上提供可解释的预测。大规模自监督预训练与内在可解释性结合能产生鲁棒的视网膜成像表示。

Conclusion: 大规模自监督预训练与内在可解释性设计相结合，可以开发出既高性能又具备局部和全局可解释性的基础模型，特别适用于医疗影像等高风险领域。
...
Method: 提出HORNet框架，使用Group Relative Policy Optimization (GRPO)训练轻量级帧选择策略（少于1M参数），将帧选择任务形式化为Select Any Frames (SAF)问题，与VLM推理解耦。

Result: 在6个基准测试（341,877个QA对，114.2小时视频）上验证：减少99%输入帧和93%处理时间；在MSVD-QA上提升1.7% F1；在NExT-QA上比均匀采样提升7.3分；跨VLM模型无需重新训练即可迁移。

Conclusion: 优化VLM看到的内容（输入选择）是优化其生成内容的实用补充方法，能同时提升效率和性能，GRPO训练的帧选择策略在分布外泛化方面优于监督学习和PPO方法。
...
TL;DR: Motion-o：一种面向视频理解的运动中心视觉语言模型扩展，通过显式轨迹推理增强时空推理能力


<details>
  <summary>Details</summary>
Motivation: 现有视频推理研究主要关注时空证据链，但缺乏对物体在连续观测间运动模式的显式建模。轨迹理解通常是隐式的且难以验证，这限制了视频推理的准确性和可解释性。

Method: 1. 提出空间-时间-轨迹（STT）推理框架；2. 开发Motion-o模型，通过<motion/>标签显式表示物体轨迹（方向、速度、尺度变化）；3. 引入轨迹接地数据集，通过增强稀疏关键帧监督获得更密集的边界框轨迹；4. 设计Motion Chain of Thought（MCoT）结构化推理路径；5. 使用奖励函数强制模型基于视觉证据推理，无需架构修改。

Result: 实验结果表明Motion-o在时空接地和轨迹预测方面均有改进，同时保持与现有框架的完全兼容性，确立了运动推理作为基于证据的视频理解的关键扩展。

Conclusion: Motion-o通过显式建模物体轨迹，填补了视频推理中运动模式理解的空白，为基于证据的视频理解提供了可验证的轨迹推理能力，且无需修改现有模型架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉上下文学习中，基于补丁的融合框架和模型无关的监督阻碍了信息线索的充分利用，限制了性能提升

Method: 提出PromptHub框架，通过局部感知融合捕获更丰富的上下文信息，采用互补的集中、对齐和预测目标相互指导训练，并加入数据增强来强化监督
...
<details>
  <summary>Details</summary>
Motivation: PET在神经退行性疾病诊断中具有重要功能信息，但成本高且有辐射风险；MRI无此限制但诊断敏感性较低。现有跨模态图像翻译方法主要关注结构保留，缺乏病理感知能力。
...
Conclusion: PASTA通过增强的病理感知能力，成功实现了从MRI到高质量合成PET的跨模态翻译，在神经退行性疾病诊断中具有重要应用价值。
...
TL;DR: GHOST是一个基于2D高斯泼溅的快速、类别无关框架，用于从单目RGB视频重建动态手-物体交互，实现了物理一致且可动画化的重建，速度比现有方法快一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖类别特定模板或计算量大，且在手-物体3D对齐上存在物理不一致问题。需要一种快速、类别无关的解决方案来重建现实的手-物体交互，以支持AR/VR、机器人和具身AI应用。

Method: 使用2D高斯泼溅表示手和物体为密集、视角一致的高斯圆盘。引入三个关键创新：1)几何先验检索和一致性损失补全被遮挡物体区域；2)抓握感知对齐细化手平移和物体尺度以确保真实接触；3)手感知背景损失避免惩罚被手遮挡的物体区域。

Result: 在ARCTIC、HO3D和野外数据集上的广泛实验表明，GHOST在3D重建和2D渲染质量方面达到最先进精度，运行速度比现有类别无关方法快一个数量级，实现了完整、物理一致且可动画化的重建。

Conclusion: GHOST为现实手-物体交互建模提供了一个高效、鲁棒的解决方案，能够从单目RGB视频快速重建动态交互，具有物理一致性和可动画化特性，在AR/VR、机器人和具身AI领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度功能图方法主要优化点对点或功能图，而非直接提升嵌入空间的特征表示，导致特征质量不足且匹配性能欠佳；同时依赖耗时功能图求解器，计算成本高

Method: 1) 无监督对比学习框架：最大化正相似对一致性，最小化负相似对一致性，提升特征一致性和区分性；2) 简化功能图学习架构：无需昂贵功能图求解器和多辅助损失，提升计算效率；3) 统一双分支流水线整合两个组件

Result: 在精度和效率上均达到SOTA，在近等距、非等距和拓扑不一致等挑战性基准测试中超越现有方法，甚至优于监督技术；计算效率显著提升

Conclusion: 首次提出基于无监督对比学习的3D形状匹配方法，通过对比学习提升特征表示，简化功能图架构，实现了高效鲁棒的匹配性能，为领域提供了新方向
...
Method: 框架包含三个即插即用模块：1)不确定性引导的自适应投影，将全景切片为透视图以弥合域差距；2)结构显著性增强注意力，在3D重建中注入结构感知置信度；3)相关性加权3D模型校正，通过注意力推断的相关性分数重新加权重叠点。

Result: 在多个分辨率和多样化的室内外数据集上，VGGT-360超越了现有的训练和无需训练的最先进方法。

Conclusion: VGGT-360通过将全景深度估计重新定义为基于3D模型的重投影任务，实现了无需训练的几何一致性全景深度估计，显著提升了性能。
...
TL;DR: CRAFT是一种轻量级扩散模型微调方法，通过复合奖励过滤构建高质量训练数据集，仅需100个样本即可超越现有SOTA方法，收敛速度提升11-220倍。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型对齐方法存在两个主要问题：1) SFT依赖高质量图像但获取成本高；2) DPO风格方法需要大规模偏好数据集但质量不一致。此外，这些方法计算效率低下。

Method: 提出CRAFT框架：1) 复合奖励过滤(CRF)技术构建高质量一致训练数据集；2) 增强版SFT进行微调。理论证明CRAFT优化了基于群体的强化学习下界。

Result: 仅用100个样本即可超越需要数千个偏好配对样本的SOTA偏好优化方法，收敛速度比基线方法快11-220倍，计算效率极高。

Conclusion: CRAFT提供了一种轻量级、高效的扩散模型微调范式，显著减少数据需求和计算成本，同时保持性能优势，为扩散模型对齐提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 指纹识别系统依赖指纹图像质量，但现有增强方法在低质量指纹上表现不佳且计算量大，需要更简单有效的解决方案

Method: 提出两种新颖方法：1) 上下文滤波方法；2) 基于学习的方法，强调简约设计原则

Result: 在具有挑战性的潜指纹数据库上验证，两种方法均优于复杂SOTA方法，产生更清晰、准确、噪声更少的图像
...
TL;DR: GenHOI：一种用于广义手-物体姿态估计的遮挡感知框架，通过分层语义提示和多模态掩码建模，在严重遮挡条件下实现鲁棒性能


<details>
  <summary>Details</summary>
Motivation: 单目RGB图像中的广义3D手-物体姿态估计面临巨大挑战，主要由于物体外观和交互模式的巨大变化，特别是在严重遮挡条件下。现有方法在遮挡情况下泛化能力不足，需要一种能够处理遮挡并推广到未见物体和新型交互的框架。

Method: 提出GenHOI框架，核心包括：1）分层语义提示：通过文本描述编码物体状态、手部配置和交互模式，学习手-物体交互的抽象高层表示；2）多模态掩码建模：在RGB图像、预测点云和文本描述上进行掩码建模，实现鲁棒的遮挡推理；3）手部先验利用：作为稳定的空间参考，提取隐式交互约束。

Result: 在DexYCB和HO3Dv2两个具有挑战性的基准测试上进行广泛实验，结果表明该方法在手-物体姿态估计任务上达到了最先进的性能水平。

Conclusion: GenHOI通过整合分层语义知识和手部先验，在严重遮挡条件下显著提升了手-物体姿态估计的泛化能力，能够处理未见物体和新型交互模式，为复杂交互场景下的姿态估计提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于MLLM的分割方法主要依赖专门的掩码解码器或引入多个额外token，本文旨在探索能否仅通过MLLM自身实现分割，消除对外部解码器的依赖

Method: 1) 保持原始未压缩分辨率的图像特征，并用从MLLM处理的压缩特征中提取的残差特征进行补充；2) 分别对经过和未经过LLM处理的图像特征应用像素反洗牌操作，释放压缩特征细节并增强残差特征；3) 重新设计具有双感知路径（图像到图像和图像到分割）的注意力掩码，促进像素与分割token之间的特征交互

Result: 在多个分割任务上的综合实验验证了SELF1E能够达到与基于专门掩码解码器方法相竞争的性能

Conclusion: SELF1E证明了在MLLM中实现无解码器分割的可行性，为简化分割架构提供了新思路
...
TL;DR: TerraScope是一个统一的视觉语言模型，用于像素级地理空间推理，具有模态灵活性和多时序推理能力，在像素级地理空间推理任务上显著优于现有VLM。
...
TL;DR: Em-Garde框架通过将语义理解与流式感知解耦，解决了现有主动视频理解模型在效率与准确性之间的权衡问题，使用指令引导的提案解析和轻量级提案匹配实现更高效的主动响应。


<details>
  <summary>Details</summary>
Motivation: 当前主动视频理解模型依赖逐帧触发决策，面临效率与准确性之间的权衡困境。现有方法在处理流式视频时计算效率低下，无法在严格计算约束下实现准确且及时的主动响应。

Method: 提出Em-Garde框架，将语义理解与流式感知解耦：1）查询时，指令引导的提案解析器将用户查询转换为结构化、感知基础的可视化提案；2）流式处理时，轻量级提案匹配模块执行基于嵌入的高效匹配来触发响应。

Result: 在StreamingBench和OVO-Bench上的实验表明，Em-Garde相比先前模型在主动响应准确性和效率方面均取得一致改进，验证了其在严格计算约束下实现主动视频理解的有效性。

Conclusion: Em-Garde通过解耦语义理解与流式感知，提供了一种在计算资源受限环境下实现高效准确主动视频理解的解决方案，为流式视频理解开辟了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统SLAM方法通常假设静态场景，在存在运动时会导致跟踪失败。最近的动态SLAM方法尝试使用预定义的动态先验或不确定性感知映射来解决这一挑战，但在面对未知动态对象或几何映射不可靠的高度杂乱场景时仍然受限

Method: 通过利用多视角视觉特征不一致性估计每像素不确定性，采用可微分不确定性感知束调整，实现鲁棒跟踪和重建

Result: 在杂乱动态场景中实现了最先进的相机位姿和场景几何重建，同时以约10 FPS的速度实时运行

Conclusion: 该方法通过不确定性感知处理动态环境，在真实世界环境中实现了鲁棒的实时SLAM性能，代码和数据集已开源
...
<details>
  <summary>Details</summary>
Motivation: 交通异常理解对智能交通系统安全至关重要，但现有视觉语言模型在该任务上进展有限，主要缺乏专门的数据集和任务特定方法。

Method: 提出了TAU-R1两层级视觉语言框架：第一层是轻量级异常分类器进行粗粒度分类，第二层是大型异常推理器生成详细事件摘要。采用两阶段训练策略：分解QA增强的监督微调，后接基于GRPO的TAU-GRPO后训练方法，使用TAU特定奖励函数。

Result: TAU-R1在异常分类和推理任务上均表现出色，同时保持部署效率。实验结果表明该方法在交通异常理解任务上具有强性能。

Conclusion: 通过引入Roundabout-TAU数据集和TAU-R1框架，为交通异常理解任务提供了有效的基准和解决方案，推动了该领域的发展。
...
TL;DR: CustomTex：基于参考图像的实例级高保真3D室内场景纹理生成框架，通过双重蒸馏方法实现语义控制与像素级增强的分离


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动方法在生成3D室内场景纹理时存在局限性：缺乏细粒度实例级控制，纹理质量不足，存在伪影和烘焙阴影问题。需要更精确、高保真的纹理生成方法
...
Conclusion: CustomTex为高质量、可定制的3D场景外观编辑提供了更直接、用户友好的路径，解决了现有方法的局限性
...
TL;DR: 该论文提出了一种基于任意顺序自回归模型的生成式分类器，通过多token顺序的边际化预测克服了传统固定顺序方法的局限性，在分类性能上超越了扩散模型，同时保持了高效性。


<details>
  <summary>Details</summary>
Motivation: 传统视觉自回归生成分类器依赖于固定的token顺序，这为图像理解施加了限制性的归纳偏置。作者观察到单顺序预测更依赖于部分判别性线索，而多token顺序的平均能提供更全面的信号。

Method: 利用最新的任意顺序自回归模型来估计顺序边际化预测，解锁自回归模型的高分类潜力。通过多token顺序的集成来获得更全面的图像理解。

Result: 该方法在多样化的图像分类基准测试中一致优于基于扩散的分类器，同时效率提升高达25倍。与最先进的自监督判别模型相比，该方法提供了具有竞争力的分类性能，这是生成式分类器的重要成就。

Conclusion: 通过利用任意顺序自回归模型进行顺序边际化预测，可以克服传统固定顺序自回归分类器的局限性，实现超越扩散模型的分类性能，同时保持高效性，为生成式分类器在图像分类任务中提供了有竞争力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如R2F利用LLM进行提示调度，但存在语言模型随机性带来的固有方差问题，以及迭代文本嵌入切换的次优指导，导致稀有组合概念生成效果不佳。

Method: ADAPT利用注意力分数和正交分量，确定性地规划和语义对齐提示调度，为稀有概念组合提供一致的指导，无需额外训练或微调。

Result: 在RareBench基准测试中，ADAPT显著提升了稀有组合概念的生成能力，准确反映了稀有属性的语义信息，提供了确定性和精确的控制，同时不损害视觉完整性。

Conclusion: ADAPT框架通过确定性规划和语义对齐的提示调度，有效解决了扩散模型中稀有组合概念生成的挑战，为文本到图像合成提供了更可靠的控制方法。
...
TL;DR: AAPB提出自适应辅助提示混合框架，通过闭式自适应系数在扩散过程中平衡辅助锚点提示和目标提示，解决低密度区域生成和编辑的语义对齐与结构一致性问题。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的文本到图像模型在训练分布的低密度区域（如罕见概念或编辑指令）往往产生语义错位或结构不一致的结果，这源于文本-图像数据集的长尾特性。

Method: 提出自适应辅助提示混合（AAPB）框架，利用辅助锚点提示为罕见概念生成提供语义支持，为图像编辑提供结构支持。基于Tweedie恒等式推导闭式自适应系数，在每一步扩散过程中最优平衡辅助锚点和目标提示的影响。

Result: 在RareBench和FlowEdit数据集上相比现有免训练基线取得一致改进，实现更优的语义准确性和结构保真度。通过控制实验证明自适应插值优于固定插值。

Conclusion: AAPB提供了一个原则性、免训练的自适应提示混合框架，能够稳定低密度区域的扩散过程，确保目标提示的忠实生成，解决了长尾数据集带来的罕见概念生成和编辑挑战。
...
TL;DR: ARIADNE是一个两阶段框架，通过偏好对齐的感知和基于强化学习的诊断推理，实现冠状动脉血管拓扑一致的狭窄检测，解决了传统像素级损失函数无法保证血管树完整性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统像素级损失函数在冠状动脉血管分割中无法强制执行拓扑约束，导致血管树碎片化，尽管像素级精度很高。这影响了狭窄检测的临床可靠性，因为不完整的血管拓扑会误导诊断决策。
...
Result: 在1,400张临床血管造影图像上，ARIADNE实现了最先进的中心线Dice系数0.838，相比几何基线减少了41%的假阳性。在ARCADE和XCAD多中心基准上的外部验证证实了其在不同采集协议间的泛化能力。

Conclusion: 这是DPO在医学影像中拓扑对齐的首个应用，表明基于结构约束的偏好学习能够减轻拓扑违规，同时在介入心脏病学工作流程中保持诊断敏感性。该方法通过耦合偏好对齐感知和RL诊断推理，实现了拓扑一致的冠状动脉狭窄检测。
...
TL;DR: Splat2BEV提出了一种基于高斯溅射的BEV感知框架，通过显式3D重建提升鸟瞰图感知的几何精度和语义丰富性


<details>
  <summary>Details</summary>
Motivation: 现有BEV感知框架采用端到端训练范式，将图像特征直接转换到BEV空间，缺乏显式的3D几何理解和可解释性，导致性能次优。作者认为显式3D表示对于准确的BEV感知至关重要。
...
Conclusion: 显式3D表示对于准确的BEV感知至关重要，Splat2BEV通过高斯溅射辅助的框架成功地将几何精确性和语义丰富性结合到BEV特征表示中。
...
TL;DR: VLMs存在选择性视觉盲区，其视觉注意力受语言框架影响，即使不同框架需要相同的视觉推理。约束性框架（如选择题）导致视觉注意力降低，而轻量级提示调优方法可改善视觉基础化。


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言模型（VLMs）存在选择性视觉盲区，其视觉注意力的分配受语言框架影响，即使不同框架需要相同的视觉推理。这种注意力分配不当导致准确性下降和跨框架不一致性，需要解决这一问题以提高VLMs的视觉基础化能力。

Method: 使用视觉注意力作为探针，量化语言框架如何改变对图像的注意力分配。通过分析约束性框架（如选择题、是/否题）与开放式框架的对比，发现注意力分配差异。基于机制洞察，提出轻量级提示调优方法，使用可学习token来鼓励稳健的视觉基础化注意力模式。

Result: 约束性框架导致对图像上下文的注意力显著降低，任务相关区域关注减少，注意力转向无信息token。这种注意力分配不当是准确性下降和跨框架不一致性的主要原因。提出的提示调优方法改善了视觉基础化，提高了跨框架的性能表现。

Conclusion: VLMs存在选择性视觉盲区，其视觉注意力分配受语言框架影响。约束性框架导致注意力分配不当，进而影响性能。通过轻量级提示调优方法可以改善视觉基础化，提高模型在不同语言框架下的鲁棒性和一致性。
...
Method: 提出Representation-Pivoted AutoEncoder（RPiAE），包含三个关键技术：1）表示枢轴正则化，允许表示初始化的编码器微调以改善重建，同时保持预训练表示空间的语义结构；2）变分桥接，将潜在空间压缩为紧凑表示以降低扩散建模复杂度；3）目标解耦的分阶段训练策略，依次优化生成可处理性和重建保真度目标。
...
TL;DR: DreamPartGen是一个语义基础、部件感知的文本到3D生成框架，通过双工部件潜在变量和关系语义潜在变量实现几何与语义一致的部分分解生成。


<details>
  <summary>Details</summary>
Motivation: 当前文本到3D方法大多忽视了部件的语义和功能结构。虽然最近出现了部件感知方法，但它们主要关注几何分解，缺乏语义基础，未能建模部件如何与文本描述对齐或部件间关系。
...
TL;DR: LVOmniBench：首个针对长音频视频跨模态理解的基准测试，包含275个10-90分钟视频和1014个QA对，揭示现有OmniLLMs在处理长视频时面临显著挑战


<details>
  <summary>Details</summary>
Motivation: 现有OmniLLMs评估主要针对短音频视频片段（10秒到5分钟），无法反映实际应用中通常持续数十分钟的长视频需求，存在评估空白

Method: 从开放平台收集高质量视频，通过严格人工筛选和标注构建LVOmniBench数据集，包含275个10-90分钟视频和1014个QA对，评估维度包括长期记忆、时间定位、细粒度理解和多模态感知

Result: 开源模型准确率普遍低于35%，Gemini 3 Pro达到约65%的峰值准确率，表明当前OmniLLMs在处理长音频视频输入时面临显著挑战

Conclusion: LVOmniBench填补了长音频视频跨模态理解评估的空白，揭示了现有模型的局限性，有望推动更先进模型的发展以解决复杂的长视频多模态理解问题
...
TL;DR: 提出一种基于向量场重塑的扩散分割方法，通过距离感知修正项增强梯度并改善类别分离，配合准随机类别编码实现高效像素级语义对齐


<details>
  <summary>Details</summary>
Motivation: 现有扩散分割方法主要关注架构调整或训练启发式方法，对连续流匹配目标与离散感知任务之间的内在不匹配理解有限。作者从向量场学习角度重新审视扩散分割，发现传统流匹配目标存在梯度消失和轨迹穿越两个关键限制，导致收敛缓慢和类别分离不佳

Method: 1) 提出向量场重塑策略：在学习的速度场中加入分离的距离感知修正项，引入吸引和排斥相互作用，增强质心附近的梯度幅度；2) 设计准随机类别编码方案：受Kronecker序列启发，与端到端像素神经场框架无缝集成，实现像素级语义对齐

Result: 大量实验一致表明，相比传统流匹配方法有显著改进，大幅缩小了生成式分割与强判别式专家方法之间的性能差距

Conclusion: 通过向量场重塑和准随机类别编码，有效解决了扩散分割中的梯度消失和轨迹穿越问题，为连续扩散模型与离散感知任务之间的不匹配提供了理论理解和实用解决方案
...
TL;DR: DriveTok：用于自动驾驶场景的高效3D多视图标记器，通过3D可变形交叉注意力实现统一的多视图重建和理解


<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言-动作模型和世界模型在自动驾驶系统中的广泛应用，可扩展的图像标记化作为视觉模态接口变得至关重要。然而，现有标记器大多针对单目和2D场景设计，应用于高分辨率多视图驾驶场景时效率低下且存在视图间不一致问题。

Method: DriveTok首先从视觉基础模型获取语义丰富的视觉特征，然后通过3D可变形交叉注意力将其转换为场景标记。解码时使用多视图变换器从场景标记重建多视图特征，并通过多头机制获得RGB、深度和语义重建。同时添加3D头直接在场景标记上进行3D语义占据预测以增强空间感知。

Result: 在广泛使用的nuScenes数据集上的大量实验表明，DriveTok的场景标记在图像重建、语义分割、深度预测和3D占据预测任务上表现良好。

Conclusion: DriveTok通过多训练目标学习统一的场景标记，集成了语义、几何和纹理信息，实现了高效的多视图标记化，解决了现有方法在自动驾驶多视图场景中的效率和不一致问题。
...
TL;DR: 提出基于图像频谱特性的实例级噪声调度方法，通过理论推导最小和最大噪声水平的边界，设计"紧凑"噪声调度以消除冗余步骤，在推理时条件采样这些调度，提升单阶段像素扩散模型的生成质量


<details>
  <summary>Details</summary>
Motivation: 现有去噪扩散模型的性能依赖于噪声调度，但噪声调度通常是手工设计的，需要针对不同分辨率进行手动调优。本文旨在提出一种基于图像频谱特性的原则性方法来设计实例级噪声调度

Method: 基于图像的频谱特性设计每个实例的噪声调度，通过理论推导最小和最大噪声水平的边界，设计"紧凑"噪声调度以消除冗余步骤，在推理时条件采样这些噪声调度

Result: 实验表明，提出的噪声调度方法提高了单阶段像素扩散模型的生成质量，特别是在低步数机制下表现更优

Conclusion: 基于图像频谱特性设计实例级噪声调度是一种有效的原则性方法，能够提升扩散模型的生成性能，特别是在减少采样步骤的情况下
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复和对象移除方法虽然能移除对象，但难以消除对象的视觉效应（变形、阴影、反射等）并合成连贯背景。此外，缺乏包含各种对象效应的综合数据集阻碍了该领域的进展。

Method: 1) 引入VOR数据集：包含6万对高质量视频（对象存在vs对象及效应缺失），涵盖5种效应类型、多种对象类别和复杂动态多对象场景；2) 提出EffectErase方法：将视频对象插入作为逆辅助任务，采用互惠学习方案，包括任务感知区域引导（聚焦受影响区域）和插入-移除一致性目标（鼓励互补行为和效应区域定位共享）。

Result: 在VOR数据集上训练的EffectErase在广泛实验中表现出优越性能，能够在多样化场景中实现高质量的视频对象效应擦除。

Conclusion: VOR数据集填补了视频对象移除领域缺乏综合效应数据集的空白，而EffectErase方法通过互惠学习和效应感知机制，显著提升了视频对象及其视觉效应的移除效果，为视频编辑任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 从单张图像中解耦物体的辐射成分（反射率、纹理和光照）是一个固有的模糊性问题。为了解决这一问题，论文利用了同一场景中所有物体都受到相同光照照射这一关键观察，通过多物体间的协同信息来约束解耦过程。

Method: MultiGP采用级联端到端架构，结合图像空间和角度空间解耦；使用协调引导确保扩散过程收敛到一致的光照估计；应用轴向注意力促进不同反射率物体间的"交叉对话"；引入纹理提取ControlNet在保留高频纹理细节的同时确保与估计光照的解耦。

Result: 实验结果表明，MultiGP能够有效利用多物体外观的互补空间和频率特性，恢复出个体的纹理和反射率以及共同的光照，成功解决了单图像辐射解耦的模糊性问题。

Conclusion: MultiGP通过利用场景中多物体共享相同光照的共识，提出了一种有效的生成式逆渲染方法，能够从单张图像中随机采样所有辐射成分，为计算机视觉和图形学中的外观建模提供了新思路。
...
TL;DR: 提出三阶段框架MoTok，结合离散token生成与扩散模型优势，通过感知-规划-控制流程实现高质量可控运动生成，显著提升运动保真度和控制精度。


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法存在局限性：连续扩散模型擅长运动学控制但语义条件效果有限，离散token生成器语义条件有效但运动学控制不足。需要结合两者优势，实现既支持语义条件又保持运动学控制精度的统一框架。

Method: 提出三阶段框架：1) 感知阶段提取条件特征；2) 规划阶段生成离散运动token；3) 控制阶段通过扩散模型合成运动。核心是MoTok扩散离散运动tokenizer，将语义抽象与精细重建解耦，使用单层token保持运动保真度。运动学条件在规划阶段指导token生成，在控制阶段通过扩散优化强制执行。
...
Conclusion: MoTok框架成功结合了离散token生成与扩散模型的优势，实现了高质量可控运动生成。通过解耦语义规划与运动学控制，在减少token数量的同时显著提升了运动保真度和控制精度，为运动生成领域提供了新的有效范式。
...
TL;DR: SAMA框架通过语义锚定和运动对齐的因子化方法解决视频编辑中语义修改与运动保持的平衡问题，无需依赖外部先验，在零样本和微调后均取得优异性能。
...
Method: 提出SAMA框架，将视频编辑因子化为语义锚定和运动建模。语义锚定通过在稀疏锚定帧联合预测语义标记和视频潜在表示来建立可靠的视觉锚点；运动对齐通过视频修复预训练任务（立方体内绘、速度扰动、管状混洗）使模型从原始视频中内化时间动态。采用两阶段优化：因子化预训练阶段学习内在的语义-运动表示，然后使用配对编辑数据进行监督微调。

Result: 因子化预训练本身已展现出强大的零样本视频编辑能力，验证了因子化方法的有效性。SAMA在开源模型中达到最先进性能，并与领先的商业系统（如Kling-Omni）竞争。代码、模型和数据集将发布。

Conclusion: SAMA通过语义锚定和运动对齐的因子化方法，无需依赖外部先验即可同时实现精确的语义修改和忠实的运动保持，为视频编辑提供了更鲁棒和泛化的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 从单图像重建铰接3D物体需要从有限视觉证据中联合推断几何、部件结构和运动参数，主要困难在于运动线索与物体结构的纠缠导致直接回归不稳定。现有方法通过多视角监督、检索式组装或辅助视频生成来解决，但往往牺牲可扩展性或效率。

Method: MonoArt采用渐进式结构推理的统一框架，不直接从图像特征预测铰接，而是逐步将视觉观察转换为规范几何、结构化部件表示和运动感知嵌入，所有这些都在单一架构内完成。这种方法避免了外部运动模板或多阶段流程。

Result: 在PartNet-Mobility数据集上的大量实验表明，该方法在重建精度和推理速度方面都达到了最先进的性能。该框架还能推广到机器人操作和铰接场景重建任务。

Conclusion: MonoArt通过渐进式结构推理实现了稳定且可解释的铰接推断，为单图像铰接3D重建提供了一种高效且可扩展的解决方案，在保持高性能的同时避免了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前离散生成方法局限于低维潜在令牌（8-32维），牺牲了理解所需的语义丰富性。高维预训练表示（768-1024维）可以弥合这一差距，但其离散生成面临根本性挑战。

Method: 提出Cubic Discrete Diffusion (CubiD)，在高维离散表示中执行细粒度掩码——任何位置、任何维度都可以被掩码并从部分观察中预测。该方法学习空间位置内部和之间的丰富相关性，生成步骤固定为T（T≪hwd）。

Result: 在ImageNet-256上，CubiD实现了最先进的离散生成性能，参数规模从900M到3.7B表现出强大的扩展性。验证了离散化令牌保留了原始表示能力，同一离散令牌可有效服务于理解和生成任务。

Conclusion: CubiD是首个高维表示离散生成模型，为统一多模态架构的未来研究提供了启示，证明了离散令牌在理解和生成任务中的双重有效性。
...
TL;DR: GPT-4o在基于文本的心理理论任务中表现接近人类水平，而较早较小的模型则存在明显差距，表明模型规模与架构对心理状态推理能力有重要影响。


<details>
  <summary>Details</summary>
Motivation: 探究当前大型语言模型是否真正具备心理理论能力，即从文本中推断他人信念、意图和情感的能力，区分其输出是真正的心理状态归因还是表面的模式完成。

Method: 使用人类心理理论研究中的文本工具改编版，测试五个LLM并与人类对照组比较，评估模型对故事角色信念、意图和情感相关问题的回答能力。

Result: GPT-4o表现出高准确性和强鲁棒性，在最挑战条件下仍与人类表现相当；较早较小的模型则受相关推理线索数量和无关干扰信息影响较大。

Conclusion: 模型在心理理论任务上的表现存在显著差异，GPT-4o接近人类水平的表现引发了关于LLM认知状态及其理解能力本质的持续讨论。
...
TL;DR: 提出LSFU（对数尺度焦点不确定性）度量方法，结合UCPOF（不确定性校准提示优化框架），通过第一token不确定性测量优化提示工程和自适应RAG触发，在降低计算成本的同时提升LLM性能。


<details>
  <summary>Details</summary>
Motivation: LLM自回归生成导致输出不确定性，传统不确定性度量（如熵）在多类选择题任务中无法区分虚假置信度（来自先验）和真实确定性（来自上下文理解），导致置信度校准不佳，需要更精确的不确定性测量来优化提示工程。

Method: 提出LSFU度量方法，受焦点损失启发，将标签先验概率作为风险调制因子，抑制高频类的噪声，强调低频长尾类的风险，具有统一测量尺度的动态加权机制。基于LSFU进一步提出UCPOF框架，利用模型输出的第一token选择高质量示例并动态优化提示，自适应触发RAG。

Result: UCPOF相比少样本基线平均准确率提升6.03%，比始终开启的完整RAG整体平均准确率高出5.75%，平均检索触发率降低50.66%。通过仅对高不确定性样本自适应触发RAG，显著降低计算成本同时保持最先进性能。

Conclusion: LSFU能够有效区分虚假置信度和真实确定性，UCPOF框架通过不确定性校准的提示优化和自适应RAG触发，在提升LLM性能的同时显著降低计算开销，为可靠提示优化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于向量检索的问答系统仅依赖相似度分数，无法解释为何某些相似文本可作为证据而其他同等相似文本不能。当多个候选获得相似分数时，系统可能选择冗余、不完整或不符合问题条件的句子。

Method: 引入意义效用估计（MUE）和多样性效用估计（DUE）两个固定评分和冗余控制程序。每个句子或记录独立评估，使用语义相关性、术语覆盖、概念独特性和冗余性等显式信号。证据单元仅在其明确陈述任务所需事实、规则或条件时才被接受，不进行合并或扩展。

Result: 该确定性门控机制产生紧凑、可审计的证据集，并在相关文本和可用证据之间建立清晰边界。如果没有单元独立满足要求，系统返回无答案。

Conclusion: 确定性证据选择框架解决了检索增强问答中证据选择的可靠性问题，通过显式评估标准确保只有明确陈述所需信息的文本才被用作证据，提高了系统的可解释性和可靠性。
...
TL;DR: DynaRAG是一个动态知识集成的检索增强生成框架，通过选择性调用外部API来处理静态和时效性信息需求，相比传统静态RAG系统显著提升了动态问题的准确性并减少了幻觉。
...
Method: 1. 使用LLM重排序器评估文档相关性；2. 充足性分类器判断是否需要回退到外部API；3. 采用Gorilla v2（最先进的API调用模型）进行准确工具调用；4. 通过FAISS模式过滤增强鲁棒性，指导API选择。

Result: 在CRAG基准测试中，DynaRAG显著提高了动态问题的准确性，同时减少了幻觉。结果表明动态感知路由和选择性工具使用对于构建可靠的现实世界问答系统至关重要。

Conclusion: DynaRAG通过动态知识集成有效解决了传统RAG在处理时效性信息时的局限性，展示了动态感知路由和选择性工具调用在构建可靠问答系统中的重要性，为实际应用提供了更健壮的解决方案。
...
TL;DR: LLMs能够重构训练数据内容但不在标准生成中表达，研究发现任务条件生成策略能全面抑制学习内容


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLMs在训练数据重构能力与标准生成输出之间的系统性分离现象，挑战训练数据存在直接预测输出概率的普遍假设

Method: 采用实证观察研究方法，分析300个提示-响应生成，涵盖叙事和问题解决任务，基于记忆连续性和对齐诱导的话语先验，检查三种不同LLM在十种任务场景中的表现

Result: 在所有生成输出中未发现非因果解决方案框架（0%，95%置信区间：[0%, 1.2%]），尽管在条件提取下已验证重构能力，表明任务条件生成策略能全面抑制学习内容

Conclusion: 训练数据存在不直接预测输出概率，任务条件生成策略能全面抑制学习内容，这对理解生成动态、输出分布控制和当代LLM行为边界具有重要意义
...
Method: 提出MineDraft框架，采用批处理并行推测解码（PSD）设计，维护两个请求批次，重叠一个批次的草稿生成与另一个批次的验证

Result: 实验显示MineDraft相比标准SD在吞吐量上提升高达75%，端到端延迟降低高达39%，并已作为vLLM插件实现

Conclusion: MineDraft通过并行化草稿生成和验证阶段，有效隐藏延迟，显著提升大语言模型推理效率，具有实际生产部署价值
...
TL;DR: 论文提出BenchBrowser工具，通过检索相关评估项来帮助诊断语言模型基准测试的内容效度和收敛效度问题，揭示基准测试与用户实际意图之间的差距。


<details>
  <summary>Details</summary>
Motivation: 语言模型基准测试的高层元数据过于粗糙，无法准确反映实际测试内容。例如，"诗歌"基准可能从不测试俳句，"指令跟随"基准可能测试的是技能混合体。这种不透明性使得验证基准与用户目标的一致性变得困难，即使模型在未测试的用户关注方面失败，也可能产生能力错觉。

Method: 引入BenchBrowser检索器，能够在20个基准套件中检索与自然语言使用案例相关的评估项。通过人类研究验证其高检索精度，生成证据帮助诊断内容效度低（能力方面覆盖狭窄）和收敛效度低（测量相同能力时排名不稳定）的问题。

Result: 人类研究证实BenchBrowser具有高检索精度。该工具能够量化基准测试与用户意图之间的关键差距，帮助诊断基准测试的效度问题。

Conclusion: BenchBrowser通过提供细粒度的基准测试透明度，帮助量化基准测试与用户实际意图之间的差距，解决了当前语言模型评估中基准测试不透明的问题，为更准确的模型能力评估提供了工具支持。
...
TL;DR: 使用FrameNet语义标注电子病历中的自由文本字段，结合SVM分类器识别性别暴力模式，语义标注模型性能优于仅使用结构化数据的方法。


<details>
  <summary>Details</summary>
Motivation: 性别暴力是重大公共卫生问题，巴西存在显著漏报现象，主要原因是难以识别虐待行为以及公共信息系统整合有限。研究探索语义分析能否改善识别效果。

Method: 比较三种SVM分类器性能：1)基于FrameNet语义标注的文本；2)语义标注文本与参数化数据结合；3)仅使用参数化数据。进行定量和定性分析。

Result: 包含语义标注的模型性能优于仅使用分类数据的模型，F1分数提高超过0.3。领域特定语义表示提供了超越结构化人口统计数据的有效信号。

Conclusion: 临床叙述的语义分析能够增强性别暴力的早期识别策略，支持更明智的公共卫生干预措施。语义标注为识别模式提供了有价值的补充信息。
...
TL;DR: LLMs不仅改变人类写作的风格和语气，还会系统性地改变原意，导致写作中立化、创造力下降，并在科学评审中产生评分偏差


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在写作辅助中的实际影响，揭示AI工具如何改变人类写作的语义内容和意图，以及这种改变对文化科学机构的潜在影响

Method: 1) 人类用户研究：分析LLM使用对写作中立性和创造力的影响；2) 历史数据集分析：使用2021年收集的人类写作数据，研究LLM根据专家反馈修改文本时的语义变化；3) 真实场景分析：研究AI生成科学同行评审的质量和评分偏差

Result: 1) 大量使用LLM导致中立性回答增加近70%，用户报告创造力下降和失去个人声音；2) LLM即使仅被要求进行语法编辑，仍会显著改变文本的语义含义；3) AI生成的科学评审对研究清晰度和重要性的关注度显著降低，评分平均高出1分

Conclusion: LLMs对人类写作的语义内容产生系统性影响，导致中立化、创造力丧失和评分偏差，需要关注AI写作工具对文化和科学机构的长期影响
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在文本生成流畅性方面表现出色，但其语言知识的本质——特别是其内部词汇表与人类词汇表的相似程度——仍不确定。研究旨在评估LLM捕捉人类词汇模式的能力，通过比较人类和LLM生成的词汇联想来探究模型内部词汇表征的人类相似性。

Method: 使用SWOW数据集中的英语提示-响应对，并生成三个LLM（Mistral-7B、Llama-3.1-8B和Qwen-2.5-32B）在多个温度设置下的新联想。分析：(1)词汇因素（如词频和具体性）对提示-响应对的影响；(2)LLM响应与人类响应相比的变异性和典型性。

Result: 所有模型都能反映人类在词频和具体性方面的趋势，但在响应变异性和典型性上存在差异。大模型（如Qwen）倾向于模拟单个"原型"人类参与者，生成高度典型但变异最小的响应；小模型（如Mistral和Llama）产生更多变异但典型性较低的响应。温度设置进一步影响这种权衡，较高温度增加变异性但降低典型性。

Conclusion: 研究结果揭示了人类与LLM词汇表之间的相似性和差异性，强调在探究LLM词汇表征时需要考虑模型大小和温度设置。大模型可能过度拟合典型的人类响应模式，而小模型则表现出更多样但不太典型的联想模式。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在发布时通常基于热门话题和基准测试表现进行评估，但随着时间推移，训练数据中的基准测试数据暴露会导致污染问题，如果不仔细测试会造成模型性能虚高。

Method: 开发了GRAFITE平台，通过用户反馈随时间积累构建模型问题库，提供使用LLM-as-a-judge进行质量保证测试的评估管道，支持多模型并排比较和不同版本间的回归检测。

Result: GRAFITE平台已实现并开源，提供持续评估大语言模型的系统，包括问题库管理、质量测试管道和多模型对比功能，可通过GitHub访问。

Conclusion: GRAFITE为解决LLM评估中的污染和性能虚高问题提供了系统化解决方案，通过持续评估和回归检测机制确保模型评估的准确性和可靠性。
...
TL;DR: CWoMP是一种用于自动生成语际标注文本的新方法，通过将语素视为原子形式-意义单元，使用对比学习对齐上下文中的单词与其组成语素，然后通过检索可变词典中的嵌入来自回归生成语素序列。


<details>
  <summary>Details</summary>
Motivation: 语际标注文本是语言文档的标准表示方法，具有丰富的语言学信息但手动制作耗时费力。现有自动化方法将词缀视为字符序列，忽略了其组合结构，因此需要更有效且能处理低资源语言的方法。

Method: CWoMP方法将语素视为原子形式-意义单元，通过对比训练编码器在共享嵌入空间中对齐上下文中的单词与其组成语素，然后使用自回归解码器通过检索可变词典中的嵌入来生成语素序列。

Result: 在多种低资源语言上的评估表明，CWoMP优于现有方法且效率显著更高，在极低资源设置下表现尤为突出，预测结果可解释且用户可通过扩展词典改进推理结果而无需重新训练。

Conclusion: CWoMP通过将语素视为原子单元并采用对比学习和词典检索的方法，为自动生成语际标注文本提供了一种高效、可解释且适用于低资源语言的解决方案，用户可在推理时通过扩展词典改进结果。
...
TL;DR: 论文提出AI主流范式继承心理学理论的结构性局限，引入ReSynth三模块框架分离推理、目的和知识，以解决适应性这一AGI核心挑战。


<details>
  <summary>Details</summary>
Motivation: 当前AI范式（强化学习、深度学习、组合方法）分别继承自心理学行为主义、认知主义和建构主义，不仅继承了这些理论的优点，也继承了其结构性局限。这些局限包括：强化学习无法解释知识内部结构，深度学习将表征压缩为不透明的参数空间难以进行原则性更新，现有整合方法缺乏从现有组件构建新理解的形式化描述。论文旨在解决这些局限，为人工通用智能的适应性挑战提供新的架构方案。

Method: 论文采用跨学科分析方法，首先追溯心理学范式到AI方法的发展谱系，诊断每个阶段继承的局限性。特别关注东西方对机械学习的理解差异，指出东方将记忆视为结构化、多阶段的理解前驱这一观点被低估。基于系统性辩论和对经典主义与连接主义的批判，论文引入ReSynth三模块框架，将推理（Intellect）、目的（Identity）和知识（Memory）作为架构上独立的组件分离。该框架旨在使系统性行为成为必然结果而非偶然属性。

Result: 论文提出了ReSynth理论框架，该框架通过分离推理、目的和知识三个模块，为解决当前AI范式的结构性局限提供了新的架构方案。框架强调适应性作为AGI核心挑战，要求一种表征架构，其中系统性行为是必然结果而非偶然属性。论文还识别了东西方对机械学习理解的差异，指出东方记忆观可作为心理学理论与AI方法论之间未被充分利用的桥梁。

Conclusion: 当前AI范式继承心理学理论的结构性局限，阻碍了人工通用智能的发展。ReSynth三模块框架通过分离推理、目的和知识组件，为解决这些局限提供了新的架构方向。该框架强调适应性作为AGI核心挑战，要求系统性行为成为架构的必然属性而非偶然结果。东西方对机械学习的理解差异为连接心理学理论与AI方法论提供了有价值的桥梁。
...
TL;DR: 论文提出了一种新闻文档轨迹的时间分类法，将异常值区分为可预测新兴主题的"预期异常值"、强化现有主题的文档和孤立文档，并在氢经济新闻语料库中验证了该方法。


<details>
  <summary>Details</summary>
Motivation: 传统动态主题建模将异常值视为噪声，但作者认为某些异常值可以作为新兴主题的早期信号。需要一种系统方法来区分不同类型的文档轨迹，以更好地理解主题形成过程。

Method: 提出新闻文档轨迹的时间分类法，定义文档随时间与主题形成的关系。在累积聚类设置中实现，使用11个最先进语言模型的文档嵌入，在HydroNewsFr法语新闻语料库上进行回顾性评估。通过模型间一致性识别高共识的预期异常值子集。

Result: 识别出一个小型但高共识的预期异常值子集，增加了这些标签的可信度。定性案例研究通过具体主题发展说明了这些轨迹，验证了分类法在检测新兴主题早期信号方面的有效性。

Conclusion: 文档轨迹的时间分类法将弱信号检测与时间主题建模联系起来，阐明了单个文章如何在演化集群中预期、启动或漂移。该方法为理解主题形成动态提供了新视角，特别适用于检测新兴主题的早期信号。
...
<details>
  <summary>Details</summary>
Motivation: 当前的无监督强化学习方法依赖启发式内在奖励，缺乏明确的理论优化目标且容易产生退化性偏差。需要一种原则性框架来更好地激发LLM的潜在能力。

Method: 将无监督微调重新定义为分布匹配问题，将GFlowNet作为未归一化密度的摊销变分采样器，提出长度感知的轨迹平衡目标来消除自回归生成中的结构长度偏差，通过α-幂分布定向控制LLM的分布锐化或扁平化。

Result: PowerFlow在实验中一致优于现有RLIF方法，匹配甚至超过监督GRPO。通过缓解对齐模型中的过度锐化问题，在创造性任务中同时提升多样性和质量，推动了帕累托前沿。

Conclusion: PowerFlow提供了一个原则性的无监督强化学习框架，能够定向激发LLM的双重特性（逻辑推理与创造性表达），在避免退化偏差的同时实现性能提升。
...
Method: TopoChunker采用双智能体架构：检查器智能体动态路由文档通过成本优化的提取路径；精炼器智能体执行容量审计和拓扑上下文消歧以重建层次谱系，将异构文档映射到结构化中间表示来显式保留跨片段依赖关系。

Result: 在非结构化叙事(GutenQA)和复杂报告(GovReport)上的评估显示，TopoChunker在绝对生成准确率上比最强的LLM基线高出8.0%，Recall@3达到83.26%，同时将token开销减少23.5%。

Conclusion: TopoChunker通过保留文档拓扑结构，在提升检索性能的同时显著降低计算成本，为结构感知的RAG提供了可扩展的解决方案。
...
TL;DR: TARo是一种推理时对齐方法，通过训练奖励模型捕捉细粒度逻辑一致性信号，并引入可学习的令牌级路由器，在推理时引导冻结的LLMs进行结构化推理，显著提升数学推理性能。
...
Method: 提出Token-level Adaptive Routing (TARo)：1) 在逐步数学轨迹上训练奖励模型以捕捉细粒度逻辑一致性信号；2) 引入可学习的令牌级路由器，在推理时自动控制奖励模型对基础模型的引导；3) 完全在推理时操作，无需修改冻结的基础模型。

Result: TARo显著提升推理性能：比基础模型提升高达+22.4%，比现有令牌级推理时对齐方法提升+8.4%。同时提升分布外临床推理（MedXpertQA）和指令跟随（AlpacaEval）能力。TARo无需重新训练即可从小型骨干模型泛化到大型骨干模型。

Conclusion: TARo成功将推理时对齐从偏好优化扩展到稳健的跨领域推理，为提升冻结LLMs的推理能力提供了一种轻量级、高效的推理时对齐方法，具有良好的泛化能力和实际应用价值。
...
TL;DR: 该研究首次在多模态LLM中评估任务干扰现象，发现从文本转向图像任务时性能严重下降，而反向切换影响较小，且模态差异是主要驱动因素。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态对话系统日益普及，但任务干扰（单次对话中任务切换导致的性能下降）仅在纯文本环境中被研究。需要评估多模态LLM中的这一现象。

Method: 引入多模态LLM任务干扰评估基准，涵盖文本和视觉六个任务，系统性地在三个维度上变化历史-目标关系：模态不匹配、推理不匹配和答案格式不匹配。在开源和专有模型上进行实验。

Result: 任务干扰具有高度方向性：从纯文本转向基于图像的目标任务导致严重性能下降，而反向切换仅产生最小退化。当多个维度同时出现不匹配时干扰加剧，模态差异是主要驱动因素，其次是答案格式，推理需求变化影响最小。

Conclusion: 多模态LLM中的任务干扰现象需要专门评估，其方向性和维度敏感性揭示了模型在处理跨模态任务切换时的内在局限性，模态差异是核心挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有解码策略（如贪婪解码、固定温度/top-p采样）是静态且任务无关的，导致跨领域生成质量不一致，特别是在需要风格或结构灵活性的任务中表现欠佳。需要一种能够根据任务需求动态调整解码参数的方法。

Method: 将解码视为序列决策问题，使用强化学习训练轻量级策略，在测试时动态调整采样参数（如温度、top-p等），同时保持LLM权重冻结。采用复合奖励函数，包含重叠度、长度、覆盖率、重复性和完整性等结构化约束项。

Result: 在BookSum、arXiv和WikiHow等摘要数据集上，使用Granite-3.3-2B和Qwen-2.5-0.5B模型评估，策略采样器始终优于贪婪和静态基线，相对增益最高达+88%（BookSum, Granite）和+79%（WikiHow, Qwen）。奖励消融实验表明，仅使用重叠度目标效果较差，而结构化奖励项能实现稳定持续改进。

Conclusion: 强化学习是解码过程中测试时适应的实用机制，能够在无需重新训练大模型的情况下实现领域感知和用户可控的生成，为解码策略优化提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 长文本推理面临注意力稀释和分布外退化问题，现有上下文选择方法采用固定上下文预算，无法适应解码过程中高度不均匀的token级上下文需求

Method: 提出不确定性触发的自适应上下文分配（UT-ACA）框架：1）学习结合语义嵌入和基于logit置信度的不确定性检测器；2）考虑解码步骤间不确定性累积；3）当证据不足时回滚、扩展上下文窗口并重新生成token

Result: 实验表明UT-ACA在长文本设置中显著减少平均上下文使用量，同时保持生成质量

Conclusion: UT-ACA通过动态调整上下文窗口有效解决了长文本推理中的注意力稀释问题，实现了更高效的上下文资源分配
...
TL;DR: WASD框架通过识别token生成的充分神经条件来解释LLM行为，实现更稳定、准确、简洁的解释，并验证了在跨语言输出生成控制中的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在控制大语言模型行为时存在训练成本高、缺乏自然语言可控性或损害语义连贯性的问题，需要一种既能精确控制模型行为又能保持语义质量的方法。

Method: 提出WASD框架，将候选条件表示为神经元激活谓词，通过迭代搜索在输入扰动下保证当前输出的最小神经元激活条件集，从而解释模型行为。

Result: 在SST-2和CounterFact数据集上使用Gemma-2-2B模型的实验表明，WASD产生的解释比传统归因图更稳定、准确和简洁；跨语言输出生成的案例研究验证了其在控制模型行为方面的实际有效性。

Conclusion: WASD框架通过识别token生成的充分神经条件，为LLM行为控制提供了一种有效方法，在解释质量和实际应用方面都表现出优越性。
...
TL;DR: 该研究发现基于似然的解码策略（如top-k、核采样）会限制对统计罕见但语境合适的token的选择，造成"截断盲点"，这是机器生成文本可检测性的主要原因，而非模型规模或架构。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解机器生成文本可检测性的根本原因。当前标准解码策略基于token似然性选择，而人类语言生成基于交际适宜性，这种不匹配可能导致机器文本容易被检测。

Method: 方法包括：分析超过180万篇文本，涵盖8个语言模型、5种解码策略和53种超参数配置；量化人类选择token与解码策略截断边界的差异；训练基于可预测性和词汇多样性的简单分类器进行检测。

Result: 结果显示：8-18%的人类选择token落在典型截断边界之外；简单分类器能达到显著的检测率；模型规模或架构与可检测性相关性不强，截断参数解释了大部分方差；低可检测性配置常产生不连贯文本。

Conclusion: 结论是：基于似然的token选择增强了文本可检测性，而非仅仅是模型能力问题；逃避检测和生成自然文本是不同的目标；解码策略设计需要平衡自然性和不可检测性。
...
TL;DR: ICE-Guard框架通过干预一致性测试检测LLMs对三种虚假特征的依赖：人口统计、权威性和框架效应，发现在高风险决策中权威和框架偏见远超人口统计偏见，结构化分解可显著减少偏见翻转率。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地用于高风险决策，但其对虚假特征的敏感性尚未得到充分表征。当前研究主要关注人口统计偏见，而忽略了其他类型的虚假特征依赖，需要更全面的检测框架。

Method: 提出ICE-Guard框架，应用干预一致性测试检测三种虚假特征依赖：人口统计（姓名/种族交换）、权威性（资历/声望交换）和框架效应（正面/负面重述）。在10个高风险领域的3000个情景中评估11个LLMs，采用结构化分解方法提取特征并由确定性规则决定，通过迭代提示修补实现偏见减少。

Result: 权威偏见（平均5.8%）和框架偏见（5.0%）显著超过人口统计偏见（2.2%）；偏见集中在特定领域（金融领域权威偏见22.6%，刑事司法仅2.8%）；结构化分解可将翻转率降低高达100%（9个模型中位数49%）；ICE引导的检测-诊断-缓解-验证循环通过迭代提示修补实现累计78%偏见减少。

Conclusion: LLMs的虚假特征依赖不仅限于人口统计偏见，权威和框架偏见更为严重且领域特异性强。结构化分解和迭代提示修补是有效的缓解策略，ICE-Guard框架为LLMs在高风险决策中的偏见检测和缓解提供了实用工具。
...
TL;DR: 提出基于通用准则集(UCS)的分解式多语言评估框架，通过语言无关的评估维度实现跨语言迁移，无需目标语言标注


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多语言场景部署，现有评估方法主要针对英语，其他语言因人工标注稀缺且成本高而难以扩展，需要解决多语言自动评估的挑战

Method: 引入基于分解的评估框架，围绕通用准则集(UCS)构建，包含共享的语言无关评估维度，生成可解释的中间表示，支持最小监督下的跨语言迁移

Result: 在多语言忠实性任务和不同模型骨干上的实验表明，该方法在无需目标语言标注的情况下，相比强基线取得了一致的性能提升

Conclusion: UCS框架为多语言评估提供了有效解决方案，通过语言无关的评估维度和分解式方法，实现了跨语言迁移，解决了多语言评估中标注数据稀缺的问题
...
TL;DR: ICE框架通过多干预算子随机化测试评估解释的忠实性，发现忠实性依赖干预算子而非单一分数，随机基线揭示三分之一配置存在反忠实性，且忠实性与人类合理性无关。
...
Method: 提出ICE框架，通过多个干预算子进行随机化测试，将解释与匹配的随机基线比较，计算置信区间内的胜率，评估7个LLM在4个英文任务、6种非英语语言和2种归因方法上的表现。

Result: 忠实性依赖干预算子：算子间差距达44个百分点，删除操作在短文本中高估忠实性但在长文本中相反；随机基线揭示三分之一配置存在反忠实性；忠实性与人类合理性无相关性(|r|<0.04)；多语言评估显示模型-语言交互效应无法仅用分词解释。

Conclusion: 忠实性应通过多个干预算子比较解释而非单一分数评估，ICE框架提供统计严谨的评估方法，揭示了现有评估的局限性，并发布ICE框架和ICEBench基准。
...
TL;DR: 提出一种基于对数似然向量表示语言模型的方法，通过构建模型地图来比较条件分布，距离近似KL散度，能捕捉模型结构、属性和任务性能关系。


<details>
  <summary>Details</summary>
Motivation: 需要一种系统性的方法来分析和比较不同语言模型的条件分布行为，理解模型间的结构关系以及提示修改对模型输出的系统性影响。

Method: 使用对数似然向量表示语言模型，基于提示-响应对构建模型地图；引入点互信息（PMI）向量减少无条件分布的影响；通过距离度量近似KL散度。

Result: 模型地图能捕捉有意义的全局结构，包括与模型属性和任务性能的关系；能捕获提示修改引起的系统性偏移及其近似可加组合性；PMI向量在某些情况下能更好反映训练数据相关差异。

Conclusion: 该框架支持输入依赖的模型行为分析，为理解和预测复合提示操作效果提供了系统方法，有助于语言模型的比较和分析。
...
TL;DR: 提出一种可解释的多模态分类框架，通过跨模态理性迁移从文本学习图像理性，提升危机分类性能并节省标注成本
...
Method: 使用视觉语言Transformer学习文本和图像的联合表示，提取文本理性，然后通过跨模态理性迁移从文本理性映射提取图像理性，最后基于提取的理性进行分类

Result: 在CrisisMMD数据集上，分类Macro-F1提升2-35%，能提取准确的文本标记和图像块作为理性；人类评估显示能检索更好的图像理性块（提升12%）；零样本模式下在新数据集上达到80%准确率

Conclusion: 提出的可解释多模态分类框架通过跨模态理性迁移有效提升分类性能，同时提供透明解释，节省标注成本，适应性强
...
TL;DR: DiscoPhon是一个用于评估从离散语音单元中无监督发现音素的多语言基准测试，覆盖6种开发语言和6种测试语言，要求系统仅使用10小时未见语言的语音数据生成可映射到预定义音素库的离散单元。


<details>
  <summary>Details</summary>
Motivation: 需要建立一个标准化的多语言基准来评估无监督音素发现方法，特别是在有限数据条件下（仅10小时语音）从离散语音单元中提取音素信息的能力。

Method: 创建包含6种开发语言和6种测试语言的基准测试集，涵盖广泛的音素对比。系统需要将离散单元映射到预定义音素库（多对一或一对一映射），并在单元质量、识别和分割三个维度进行评估。提供了四个预训练的多语言HuBERT和SpidR基线模型。

Result: 当前模型中的音素信息足够丰富，使得派生单元与音素具有良好的相关性，但不同语言之间存在差异。基线模型展示了从离散语音单元中发现音素的可行性。

Conclusion: DiscoPhon为无监督音素发现提供了一个标准化的多语言评估框架，证明了现有模型中包含足够的音素信息，但跨语言性能存在差异，为未来研究提供了基准和方向。
...
TL;DR: LSE框架训练LLM在测试时自我进化，通过强化学习优化上下文编辑，在Text-to-SQL和QA任务上超越GPT-5等更强大模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖模型固有的推理能力，从未明确训练模型进行自我进化任务。需要将自我进化作为可学习技能来提升模型在测试时的上下文优化能力。

Method: 将多步进化问题简化为单步RL目标，每个上下文编辑的奖励基于下游性能改进。结合树引导进化循环，训练LLM学习如何优化自身上下文。

Result: 在BIRD（Text-to-SQL）和MMLU-Redux（通用问答）任务上，4B参数模型使用LSE训练后，性能超越GPT-5、Claude Sonnet 4.5等更强大模型，也优于GEPA和TextGrad等提示优化方法。

Conclusion: 将自我进化视为可学习技能是有效的，LSE框架能显著提升模型性能，且训练后的策略可迁移指导其他模型而无需额外训练。
...
TL;DR: 本文对持续意图分类中的灾难性遗忘缓解方法进行了比较实证研究，评估了三种骨干架构（ANN、GRU、Transformer）在多种持续学习策略下的表现，发现回放方法（MIR）是关键要素，最优配置取决于架构选择。


<details>
  <summary>Details</summary>
Motivation: 现实世界应用中的神经语言模型需要持续适应新任务和领域而不遗忘先前获得的知识，因此需要研究灾难性遗忘的缓解方法，特别是在持续意图分类任务中。

Method: 使用CLINC150数据集构建10个任务标签不相交的场景，评估三种骨干架构（ANN、GRU、Transformer）在多种持续学习策略下的表现。考虑了三大类CL方法的代表性方法：基于回放的MIR、基于正则化的LwF和基于参数隔离的HAT，分别单独使用以及所有成对和三元组合。性能通过平均准确率、宏观F1和反向转移进行评估。

Result: 结果显示：1）朴素顺序微调在所有架构中都遭受严重遗忘；2）没有单一CL方法能完全防止遗忘；3）回放是关键要素，MIR是最可靠的个体策略；4）包含回放的组合（MIR+HAT、MIR+LwF、MIR+LwF+HAT）始终实现高性能和接近零或轻度正反向转移；5）最优配置是架构依赖的：ANN和Transformer上MIR+HAT最佳，GRU上MIR+LwF+HAT最佳；6）在某些情况下CL方法甚至超过联合训练，显示出正则化效应。

Conclusion: 设计持续意图分类系统时，需要联合选择骨干架构和持续学习机制。回放方法是关键要素，但最优配置取决于具体架构，这表明在实际应用中需要针对特定架构进行CL策略的定制化选择。
...
TL;DR: LLMs作为自动评分器在教育场景中存在基于写作风格的隐性评分偏见，尽管内容正确性相同且明确要求忽略写作风格。研究发现Essay/Writing任务中存在显著偏见，而数学和编程任务偏见较小。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在教育环境中作为自动评分器的部署增加，对其评估的公平性和偏见问题变得至关重要。本研究旨在探究当底层内容正确性保持不变时，LLMs是否会基于写作风格表现出隐性评分偏见。

Method: 构建了包含三个学科（数学、编程、作文/写作）的180个学生回答的受控数据集，每个学科包含三种表面扰动类型：语法错误、非正式语言和非母语表达。使用两个最先进的开源LLM（LLaMA 3.3 70B和Qwen 2.5 72B），明确指示仅评估内容正确性并忽略写作风格，在1-10分制上对回答进行评分。

Result: 在作文/写作任务中，两个模型在所有扰动类型上都显示出统计学上显著的评分偏见（p < 0.05），效应大小从中等（Cohen's d = 0.64）到非常大（d = 4.25）。非正式语言受到最重惩罚，LLaMA平均扣除1.90分，Qwen扣除1.20分。非母语表达分别被惩罚1.35和0.90分。相比之下，数学和编程任务显示出最小偏见，大多数条件未达到统计学显著性。

Conclusion: LLM评分偏见具有学科依赖性、风格敏感性，并且在明确的抗偏见评分提示下仍然持续存在。研究讨论了LLM评分系统公平部署的影响，并建议在机构采用前进行偏见审计协议。
...
TL;DR: Mi:dm K 2.5 Pro是一个320亿参数的韩语旗舰大语言模型，专注于企业级复杂推理任务，通过创新的数据构建、预训练扩展和多阶段后训练流程，在韩语特定基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 当前LLM发展需要超越简单文本生成的能力，重点关注多步推理、长上下文理解和智能体工作流。在韩语和特定领域的企业环境中，现有模型面临扩展不足的挑战，需要专门优化的解决方案。

Method: 1) 数据构建：采用质量中心的数据整理流程，包括代码的抽象语法树分析、数学的填空合成和基于LLM的质量评估器；2) 预训练：通过基于层预测器的深度上采样和渐进策略支持128K令牌上下文窗口；3) 后训练：专门的多阶段流程包括推理SFT、模型合并和异步强化学习；4) 融合训练：重新平衡推理能力与对话流畅性、一致响应风格和可靠工具使用。

Result: Mi:dm K 2.5 Pro在领先的全球和国内模型中表现出竞争力，在韩语特定基准测试中达到最先进水平，展示了深度的语言和文化理解能力。负责任AI评估验证了模型的安全性，确保部署时在无害性和响应性之间的平衡。

Conclusion: 该研究成功开发了一个专注于企业级复杂推理的韩语大语言模型，通过创新的训练方法在韩语理解和推理能力方面达到领先水平，同时确保了安全部署的可行性。
...
TL;DR: 跨语言对齐与下游任务性能之间存在不匹配：更好的嵌入对齐并不总是带来更好的跨语言迁移，因为对齐目标与任务目标基本正交，且不同语言和任务类型受益程度差异很大。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解释为什么显式的跨语言对齐技术（尽管增加了嵌入相似性）经常无法改善下游任务的性能，揭示对齐目标与下游任务目标之间的关系。
...
Conclusion: 研究解释了为什么"更好"的对齐往往无法转化为"更好"的跨语言迁移，并基于这些见解提供了结合跨语言对齐与任务特定微调的实用指南，强调了仔细选择损失函数的重要性。
...
TL;DR: 该研究分析了语言学习应用在专业场景教学方面的不足，发现Duolingo等应用主要关注通用场景，缺乏针对专业领域的教学内容，阻碍学习者达到专业流利度。


<details>
  <summary>Details</summary>
Motivation: 当前流行的语言学习应用如Duolingo使用大语言模型生成课程，但主要关注通用现实场景（如问候、点餐、问路），对专业特定场景的支持有限。这种差距会阻碍学习者达到专业流利度，即能够舒适地用目标语言交流各种工作相关和领域特定信息的能力。

Method: 研究调查了菲律宾一家跨国公司的五名员工，了解他们在Duolingo上的学习体验。通过调查分析受访者遇到通用场景与工作相关场景的频率，以及这些场景对语言学习的有效性。

Result: 结果显示：1）受访者遇到通用场景的频率高于工作相关场景；2）通用场景具有相关性和有效性，能够建立基础语法、词汇和文化知识；3）工作相关场景有助于缩小专业流利度的差距，因为它们包含领域特定词汇；4）每位参与者建议的课程场景在汇总分析时呈现出不同的情境背景。

Conclusion: 语言学习应用应生成能够适应个人需求的课程，通过个性化的领域特定课程场景来满足专业需求，同时通过通用、相关的课程场景保持基础支持。这种平衡方法有助于学习者从基础语言能力向专业流利度过渡。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本简化系统主要依赖自动化指标驱动，未能反映用户实际理解能力或规范性标准，需要建立更透明、可审计的无障碍文本生成与评估流程。

Method: 提出混合框架：HiTL（人在回路）指导生成过程调整，HoTL（人在监督）进行系统性后生成审查；将实证证据转化为：1) 标准对齐检查清单，2) 触发专家监督的ECA规则，3) 无障碍关键绩效指标。
...
TL;DR: XKD-Dial是一个四阶段训练框架，用于构建可解释、基于知识的双语（英语-印地语）对话系统，通过渐进式训练实现引用机制和减少幻觉，同时提供三种事后可解释性分析方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识的对话系统主要专注于英语，缺乏显式引用机制来验证事实主张，模型决策过程透明度有限，需要开发支持双语、可解释且具备引用能力的对话生成方法。

Method: 提出四阶段渐进训练流程：1) 多语言适应；2) 英语对话SFT（监督微调）与引用基础；3) 双语对话SFT；4) GRPO对齐与引用感知奖励。评估了六种编码器-解码器（250M-3B）和解码器专用（1B-7B）架构模型。采用三种事后可解释性分析：交叉注意力对齐、积分梯度归因和基于遮挡的因果基础分析。

Result: 引用基础SFT使编码器-解码器模型从第二阶段开始幻觉率降至0.0%；渐进式训练防止灾难性遗忘同时提升印地语能力；较小模型在SFT后英语表现与较大模型相当；GRPO在结构化引用任务上仅提供边际改进。评估使用六种自动指标：BLEU、ROUGE、BERTScore、FactScore、Citation-F1和幻觉率。

Conclusion: XKD-Dial框架成功构建了可解释、基于知识的双语对话系统，通过渐进式训练有效整合引用机制，减少幻觉并保持语言能力。事后可解释性分析揭示了引用行为的学习过程，而不仅仅是最终表现。精心设计的SFT在结构化引用任务上已足够有效，GRPO仅提供有限改进。
...
TL;DR: RADIUS是一个用于调查模拟的全面评估套件，专注于排名对齐和分布对齐两个维度，并包含统计显著性检验，解决了现有评估指标碎片化、非标准化的问题。
...
Method: 提出RADIUS评估套件，包含两个维度的对齐：1) 排名对齐(RAnking alignment) - 评估模拟结果与人类偏好的排名一致性；2) 分布对齐(DIstribUtion alignment) - 评估模拟结果的分布特征。每个维度都辅以统计显著性检验，确保评估结果的可靠性。
...
TL;DR: HCQR是一种无需训练的预检索框架，通过生成三个针对性查询将RAG从主题导向检索转向证据导向检索，显著提升多选项决策任务性能


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法在需要从多个竞争选项中选择答案的任务中存在局限，单一初始查询往往偏向主题相关性而非决策相关证据，导致检索的背景信息无法有效区分答案选项

Method: 提出假设条件查询重写框架：1)从输入问题和候选选项生成轻量级工作假设；2)将检索重写为三个针对性查询：支持假设的证据、区分竞争替代方案的证据、验证问题中关键线索的证据

Result: 在MedQA和MMLU-Med数据集上，HCQR持续优于单查询RAG和重排/过滤基线，平均准确率分别比Simple RAG提高5.9和3.6个百分点

Conclusion: HCQR通过将检索重新定位为证据导向而非主题导向，使检索的上下文更直接与答案选择对齐，显著提升了RAG在多选项决策任务中的性能
...
TL;DR: MultiTempBench是一个多语言时间推理基准测试，涵盖三个任务、五种语言和多种日历系统，包含15,000个示例，评估了20个LLM并提出了mDFR指标。


<details>
  <summary>Details</summary>
Motivation: 现有时间推理基准测试主要针对英语和高资源语言，缺乏对多语言、多日历系统以及低资源语言中时间推理能力的系统评估，特别是时间标记化质量对推理性能的影响。

Method: 通过翻译750个精心设计的英语问题并扩展为受控的日期格式变体，构建了包含15,000个示例的多语言基准测试；评估了20个LLM，引入了多语言日期碎片化比率(mDFR)，并结合几何探测分析内部时间表示；使用交叉混合效应回归分析影响因素。

Result: 时间标记化质量是资源依赖的瓶颈：在低资源语言和稀有日历格式中，碎片化破坏了年月日分离，导致准确率崩溃；而在高资源设置中通常对数字级分割具有鲁棒性。时间线性是高资源语言中最强的时间推理预测因子，而碎片化是低资源语言中更强的预测因子。

Conclusion: 多语言时间推理性能受到语言资源可用性和日历系统复杂性的显著影响，标记化质量是关键瓶颈，特别是在低资源语言中；需要针对不同语言资源水平开发专门的时间推理方法。
...
TL;DR: LLM生成的类比比人类生成的更符合平行四边形几何模型，人类类比存在大量弱完成项，而LLM能更一致地满足关系约束
...
Result: 1. LLM生成的类比被可靠地评为优于人类生成的类比；2. LLM类比在GloVe嵌入空间中更符合平行四边形结构；3. LLM优势源于更强的平行四边形对齐和更低的词语可及性依赖，而非局部相似性敏感性增强；4. LLM优势主要源于人类产生大量弱完成项，当比较模态响应时优势消失；5. 平行四边形对齐度和词语频率能预测LLM哪些完成项优于人类

Conclusion: 平行四边形模型并非对词语类比的差模型，而是人类常常无法产生满足关系约束的完成项，而LLM能更一致地做到这一点。模型差异源于人类生成中存在大量弱完成项，而非LLM在所有方面都更优
...
TL;DR: HEALIX：首个基于真实临床笔记的公开健康素养标注数据集，包含589份9种类型的笔记，标注了低、正常、高三个健康素养等级，用于评估LLM的零样本和少样本提示策略。


<details>
  <summary>Details</summary>
Motivation: 当前健康素养筛查工具在可行性、项目数量、问题格式和维度捕获方面存在差异，难以在结构化电子健康记录中实现标准化记录。临床笔记包含更丰富、更情境化的健康素养信息，但缺乏标注资源限制了自动化检测的发展。

Method: 通过社会工作者笔记抽样、基于关键词的过滤和基于LLM的主动学习相结合的方法，构建HEALIX数据集。该数据集包含589份9种类型的临床笔记，标注了低、正常、高三个健康素养等级。使用四个开源大语言模型对零样本和少样本提示策略进行基准测试。

Result: 成功创建了首个公开可用的健康素养标注数据集HEALIX，包含589份临床笔记和三个健康素养等级标注。通过基准测试评估了不同LLM在健康素养检测任务上的表现。

Conclusion: HEALIX数据集填补了健康素养自动化检测领域缺乏标注资源的空白，为从非结构化临床笔记中提取健康素养信息提供了重要工具，并展示了LLM在该任务上的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在英语多跳问答中表现良好，但在多语言跨语料检索场景下面临挑战：缺乏多语言多跳问答基准测试，以及过度依赖LLM的英语语义理解能力导致多语言场景效果下降。

Method: 首先构建多语言多跳问答基准（将英语基准翻译为五种语言），然后提出DaPT框架：为源语言查询及其英语翻译并行生成子问题图，合并后采用双语检索-回答策略顺序解决子问题。

Result: 实验表明先进RAG系统在多语言场景中存在显著性能不平衡问题。DaPT相比基线方法能产生更准确、简洁的答案，显著提升RAG性能。在最具挑战性的MuSiQue基准上，DaPT在平均EM分数上相对最强基线实现了18.3%的提升。

Conclusion: DaPT通过并行子问题图生成和双语检索策略，有效解决了多语言多跳问答中的挑战，为多语言RAG系统提供了有效的解决方案，显著提升了跨语言检索和问答性能。
...
TL;DR: UGID是一种基于内部表示层面的大语言模型去偏框架，通过将Transformer建模为结构化计算图，在反事实输入上强制图结构不变性来消除偏见。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型存在明显的社会偏见，基于输出层面或数据优化的去偏方法无法完全解决这些问题，许多先前研究表明偏见嵌入在内部表示中，需要更根本的内部表示层面去偏方法。

Method: 将Transformer建模为结构化计算图（注意力机制定义路由边，隐藏状态定义节点），通过强制反事实输入间的图结构不变性来实现去偏，同时约束注意力路由和隐藏表示，防止偏见在架构组件间迁移。引入对数空间敏感logit约束和选择性锚定目标来保持定义语义。
...
<details>
  <summary>Details</summary>
Motivation: 传统两阶段训练范式（先在全数据上预训练，再在高质量子集上专业化）在多领域设置中需要为每个专业领域单独训练模型，计算效率低。需要一种方法来确定预训练与专业化之间的最优计算分配。

Method: 提出基于缩放定律的方法：独立预训练多个模型，使用缩放定律预测模型大小为N、使用D个预训练token和D'个专业化token时的损失，并外推到更大模型规模和token数量，从而确定预训练与持续预训练的最优计算分配。

Result: 该方法能准确预测模型损失，并在不同模型大小和计算预算下，在常识知识和推理基准测试中持续提升性能。

Conclusion: 基于缩放定律确定预训练与专业化计算分配的方法有效，能在多领域设置中优化语言模型训练，提升性能表现。
...
TL;DR: 提出VEPO框架，通过强化学习结合可验证奖励，将确定性结构约束融入策略对齐过程，改善低资源语言处理性能
...
Method: 提出可变熵策略优化(VEPO)，利用强化学习与可验证奖励，将确定性结构约束融入策略对齐；采用可变熵机制动态校准字面保真度与语义自然性之间的平衡；结合熵温优势估计和非对称裁剪
...
TL;DR: 评估LLM在重复博弈中的战略表现，区分其是真正推理还是依赖记忆模式，通过引入反事实变体来测试模型的激励敏感性、结构泛化和战略推理能力


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在重复博弈环境中的战略表现，区分其是真正基于推理还是依赖记忆的模式匹配，检验LLM是否具备真正的战略推理能力

Method: 使用囚徒困境和石头剪刀布两个经典博弈，引入反事实变体改变收益结构和行动标签，打破熟悉的对称性和支配关系，采用多指标评估框架比较默认和反事实实例

Result: 展示了LLM在反事实环境中的局限性，包括激励敏感性不足、结构泛化能力有限以及战略推理能力不足

Conclusion: LLM在反事实博弈环境中表现出明显的战略推理缺陷，主要依赖记忆模式而非真正的推理能力，需要改进模型在激励敏感性和结构泛化方面的能力
...
TL;DR: F2LLM-v2是一个多语言嵌入模型系列，包含8种尺寸（80M到14B），支持200多种语言，特别关注中低资源语言，通过两阶段LLM训练流程结合多种技术实现高效性。


<details>
  <summary>Details</summary>
Motivation: 开发一个通用、多语言的嵌入模型系列，特别关注之前服务不足的中低资源语言，同时通过高效训练方法减少计算资源需求，为资源受限应用提供先进解决方案。

Method: 使用新构建的6000万高质量数据样本进行训练；采用两阶段LLM嵌入训练流程，结合matryoshka学习、模型剪枝和知识蒸馏技术；支持超过200种语言。

Result: F2LLM-v2-14B在11个MTEB基准测试中排名第一；较小尺寸模型在资源受限应用中创造了新的最先进水平；所有模型、数据、代码和中间检查点都已开源。

Conclusion: F2LLM-v2系列在多语言嵌入任务中表现出色，特别是14B版本在多个基准测试中领先，同时较小模型为资源受限应用提供了高效解决方案，推动了开源嵌入模型研究。
...
Result: 在XNLI和XQuAD两个任务上进行了基准测试，相比基线方法取得了改进，并与近期类似工作相比获得了有竞争力的结果
...
### [149] [Token Economy for Fair and Efficient Dynamic Resource Allocation in Congestion Games](https://arxiv.org/abs/2603.18094)
*Leonardo Pedroso,Andrea Agazzi,W. P. M. H. Heemels,Mauro Salazar*

Main category: cs.GT

TL;DR: 提出基于代币的机制解决共享经济中的拥堵博弈问题，通过连续时间动态博弈建模和平均场近似，设计整数通行费实现高效公平的资源分配


<details>
  <summary>Details</summary>
Motivation: 共享经济中自私行为导致整体效率低下，集中协调又剥夺个体决策权，传统货币机制基于财富差异造成不公平，需要设计既高效又公平的分配机制

Method: 建立基于代币的拥堵博弈机制，建模为有限多有限理性智能体的连续时间动态博弈，采用演化策略修订动态，推导平均场近似并建立与有限群体博弈的强近似保证

Result: 获得平均场近似与有限群体博弈的强近似保证，设计出闭式整数通行费，可从任意初始条件引导聚合动态趋向最优高效公平分配

Conclusion: 代币机制能有效解决共享经济中的效率-公平权衡问题，通过理论保证的整数通行费设计实现系统最优分配，同时保持个体决策自主性
...
### [150] [Evolutionarily Stable Stackelberg Equilibrium](https://arxiv.org/abs/2603.18385)
*Sam Ganzfried*

Main category: cs.GT

TL;DR: 提出进化稳定Stackelberg均衡(SESS)新概念，研究领导者与对称追随者种群的Stackelberg进化博弈，领导者选择最优混合策略，预测追随者种群在诱导子博弈中采用进化稳定策略(ESS)。


<details>
  <summary>Details</summary>
Motivation: 现有Stackelberg进化博弈方法要么通过进化动态定义追随者响应，要么假设理性最优响应行为，但未明确强制执行对突变入侵的稳定性。需要一种结合Stackelberg领导力与进化稳定性的新解决方案概念。

Method: 提出SESS概念，领导者选择最优混合策略，预测追随者种群在诱导子博弈中采用ESS并满足额外生态条件。考虑领导者最优和追随者最优的ESS选择。提供离散和连续博弈中计算SESS的算法，并对连续博弈进行实证验证。

Result: 建立了SESS理论框架，开发了计算算法，并在连续博弈中进行了实证验证。模型适用于生物场景，如癌症治疗中医生作为领导者，竞争性癌细胞表型作为追随者。

Conclusion: SESS为Stackelberg进化博弈提供了新的解决方案概念，结合了领导者的战略决策与追随者种群的进化稳定性，填补了现有方法的理论空白，具有重要的生物学应用价值。
...
### [151] [Complexity of Auctions with Interdependence](https://arxiv.org/abs/2603.18668)
*Patrick Loiseau,Simon Mauras,Minrui Xu*

Main category: cs.GT

TL;DR: 研究Milgrom-Weber相互依赖模型中的拍卖设计，包括价值拍卖和采购拍卖，在去除领域限制和单调性假设的情况下，优化真实机制的近似比，提供算法和硬度结果。


<details>
  <summary>Details</summary>
Motivation: 现有理论计算机科学文献主要研究具有领域限制和单调估值函数的常数近似真实机制设计，本文旨在研究更一般的计算问题，去除这些限制，为价值拍卖和采购拍卖提供更通用的理论分析框架。

Method: 研究确定性随机设置下的价值拍卖和采购拍卖的真实机制近似比优化问题，将先前研究的特殊案例归约为经典组合问题，提供高效算法和特征描述，同时通过查询复杂度下界和NP-硬度证明补充一般情况的硬度结果。

Result: 为先前研究的可处理特殊案例提供理论解释，将其归约为经典组合问题并给出高效算法，同时证明一般情况具有查询复杂度下界和NP-硬度，表明问题的计算复杂性。

Conclusion: 在去除领域限制和单调性假设的Milgrom-Weber相互依赖模型中，真实机制的近似比优化问题在特殊情况下可归约为经典组合问题而可解，但一般情况下具有计算复杂性，需要进一步研究。
...
### [152] [Optimal Path Planning in Hostile Environments](https://arxiv.org/abs/2603.18958)
*Andrzej Kaczmarczyk,Šimon Schierreich,Nicholas Axel Tanujaya,Haifeng Xu*

Main category: cs.GT

TL;DR: 论文提出并分析了一种新的多智能体路径规划问题，其中智能体需要在包含周期性危险（接触后消除智能体但会进入冷却期）的图环境中从共同起点移动到共同目标，目标是最大化到达目标的智能体数量。


<details>
  <summary>Details</summary>
Motivation: 在危险环境中协调智能体（如冲突区域中的救援无人机或充满障碍物的部署区域中的野外机器人）提出了基本的规划挑战。需要一种新的多智能体路径规划模型来捕捉这种设置，其中危险会暂时消除智能体但具有已知的冷却周期。

Method: 引入并分析了一种新的多智能体路径规划问题的计算复杂性。该问题在离散时间、完全可观测、确定性设置中，智能体在包含周期性危险的图环境中从共同起点移动到共同目标。危险在接触时消除智能体，但随后进入已知冷却期后才重新激活。规划任务是计算最大化到达目标智能体数量的移动计划。

Result: 1) 证明了尽管可行计划空间是指数级的，但最优计划只需要多项式步数，确立了NP成员资格；2) 证明了即使环境图是树，该问题也是NP难的；3) 提出了针对由从起点到目标的顶点不相交路径组成的图的线性时间算法。

Conclusion: 该研究为这种多智能体路径规划问题建立了一个丰富的计算图景，识别了不可处理（NP难）和可处理（多项式时间算法）的片段，为危险环境中的智能体协调提供了理论基础。
...
### [153] [Damage identification using noisy frequency response functions based on topology optimization](https://arxiv.org/abs/2603.18569)
*Akira Saito,Ryo Sugai,Zhongxu Wang,Hidetaka Saomoto*

Main category: cs.CE

TL;DR: 提出一种基于噪声频响函数和拓扑优化的鲁棒损伤识别方法，将损伤识别问题转化为从结构动态响应反演损伤拓扑的逆问题，采用SIMP方法和Lasso正则化处理病态逆问题和测量噪声。


<details>
  <summary>Details</summary>
Motivation: 传统损伤识别方法在处理噪声数据和病态逆问题时存在局限性，需要开发能够有效处理测量噪声并准确识别损伤拓扑的鲁棒方法。

Method: 将损伤识别问题表述为逆问题，通过最小化实验模态分析获得的测量FRF与有限元谐响应分析获得的FRF之间的误差函数。采用SIMP方法进行材料分布优化，通过Lasso正则化（L1范数惩罚）处理病态逆问题和消除虚假损伤区域。

Result: 方法成功应用于悬臂板的损伤识别，实验获得的FRF数据经过该方法处理后能够准确识别损伤位置和拓扑。

Conclusion: 提出的基于拓扑优化和Lasso正则化的损伤识别方法能够有效处理噪声数据，克服逆问题的病态性，成功实现结构损伤的准确识别。
...
### [154] [Model Reference Adaptive Control For Gust Load Allevation of Nonlinear Aeroelastic](https://arxiv.org/abs/2603.18584)
*Nikolaos D. Tantaroudas,Andrea Da Ronch,Guanqun Gai,Ilias Karachalios,Kenneth J. Badcock,Rafael Palacios*

Main category: cs.CE

TL;DR: 基于Lyapunov稳定性的模型参考自适应控制用于非线性气动弹性系统的阵风载荷减缓，在无人机上优于H∞鲁棒控制


<details>
  <summary>Details</summary>
Motivation: 开发适用于柔性飞机在确定性和随机扰动环境下的有效阵风载荷减缓控制框架，处理非线性气动弹性系统的复杂动态特性

Method: 基于Lyapunov稳定性理论设计模型参考自适应控制器，包括参考模型设计（编码期望闭环阻尼特性）、带实时增益调整的自适应控制律，以及保证线性情况下渐近跟踪和非线性情况下有界跟踪的Lyapunov推导

Result: 在无人机离散阵风条件下，MRAC显著减少翼尖挠度，优于H∞鲁棒控制基准；在Von Karman随机湍流下也获得有意义的减少，性能随自适应速率缩放

Conclusion: MRAC为柔性飞机在确定性和随机扰动环境下的阵风载荷减缓提供了有效框架，自适应速率矩阵是最重要的设计参数，控制收敛速度、峰值载荷减少和作动器需求之间的权衡
...
### [155] [Pore-scale modeling of capillary-driven binder migration during battery electrode drying](https://arxiv.org/abs/2603.18860)
*Marcel Weichel,Martin Reder,Gerit Mühlberg,David Burger,Philip Scharfer,Wilhelm Schabel,Britta Nestler,Daniel Schneider*

Main category: cs.CE

TL;DR: 该研究开发了一个孔隙尺度连续介质模型，用于模拟钠离子电池硬碳电极干燥过程中毛细管驱动的粘结剂迁移，揭示了颗粒尺寸、蒸发速率和表面张力对粘结剂分布的影响。


<details>
  <summary>Details</summary>
Motivation: 现有电极干燥模型主要依赖一维薄膜收缩或忽略毛细管传输，缺乏物理一致的微观结构解析的粘结剂迁移预测。电极干燥是制造过程中的关键步骤，粘结剂迁移会影响电极的机械完整性和电化学性能。

Method: 扩展了空间解析的孔隙尺度连续介质模型，明确描述孔隙排空过程中的毛细管驱动粘结剂传输。将该模型应用于具有不同平均颗粒直径的硬碳微观结构进行模拟。

Result: 模拟显示：较小颗粒尺寸导致更均匀的粘结剂分布；较高蒸发速率和增加表面张力促进更强的粘结剂梯度；溶剂粘度变化对粘结剂迁移影响较小（无亲水或疏水行为时）。

Conclusion: 明确描述毛细管传输和微观结构效应对于准确预测粘结剂迁移至关重要，为电极干燥过程的针对性优化提供了基础。
...
### [156] [On the Minimum Number of Control Laws for Nonlinear Systems with Input-Output Linearisation Singularities](https://arxiv.org/abs/2603.18947)
*Nikolaos D. Tantaroudas*

Main category: cs.CE

TL;DR: 提出了(k+1)-控制器引理，证明对于具有k个代数独立条件参数化奇点流形的n阶SISO非线性系统，恰好需要k+1个不同的控制律才能实现全局可控性。


<details>
  <summary>Details</summary>
Motivation: 解决非线性系统在反馈线性化控制器存在奇点时，确定实现全局可控性所需最小控制律数量的基本问题。

Method: 引入并严格证明(k+1)-控制器引理：必要性证明采用反证法，结合隐函数定理、维度计数和近似线性化框架的结构约束；充分性证明采用构造性方法，结合近似线性化方法和微分拓扑的横截性论证。

Result: 对于具有k个代数独立条件参数化奇点流形的n阶单输入单输出非线性系统，k+1个不同的控制律是实现完全状态空间覆盖的必要且充分条件。

Conclusion: 通过球杆系统（一个在第三输出导数处具有双参数奇点的四阶机械系统）的详尽分析验证了该结果，为具有奇点的非线性系统的全局可控性提供了理论框架。
...
### [157] [FinTradeBench: A Financial Reasoning Benchmark for LLMs](https://arxiv.org/abs/2603.19225)
*Yogesh Agrawal,Aniruddha Dutta,Md Mahadi Hasan,Santu Karmaker,Aritra Dutta*

Main category: cs.CE

TL;DR: FinTradeBench：一个结合公司基本面与交易信号的金融推理基准，包含1400个基于NASDAQ-100公司十年历史数据的问题，评估LLMs在金融决策中的综合推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融问答基准主要关注公司资产负债表数据，很少评估对公司股票市场交易行为或基本面与交易信号交互的推理能力。需要结合公司基本面（监管文件）和交易信号（价格动态）的异构信号进行综合金融决策。

Method: 采用校准-扩展框架：结合专家种子问题、多模型响应生成、模型内自过滤、数值审计和人类-LLM评判对齐。构建包含1400个问题的基准，分为三类：基本面聚焦、交易信号聚焦、需要跨信号推理的混合问题。

Result: 评估14个LLMs在零样本提示和检索增强设置下的表现，发现明显性能差距。检索显著改善文本基本面推理，但对交易信号推理帮助有限。这突显了当前LLMs在数值和时间序列推理方面的基本挑战。

Conclusion: FinTradeBench揭示了当前LLMs在金融智能方面的局限性，特别是在数值和时间序列推理方面，为未来金融智能研究提供了动机和基准。
...
### [158] [A New Approach to Code Smoothing Bounds](https://arxiv.org/abs/2603.18077)
*Tsuyoshi Miezaki,Yusaku Nishimura,Katsuyuki Takashima*

Main category: cs.IT

TL;DR: 本文提出基于随机游走和公平划分的新方法，推导了码的总变差距离不等式，推广了有限阿贝尔群上的现有结果


<details>
  <summary>Details</summary>
Motivation: 分析基于编码的密码系统安全性需要研究平滑参数，该参数与码的总变差距离密切相关。现有研究使用局部紧阿贝尔群上的傅里叶变换来界定这一距离，但本文寻求基于随机游走的替代方法

Method: 采用基于随机游走的方法，利用公平划分推导总变差距离的不等式，证明该方法能推广有限阿贝尔群上的现有结果

Result: 成功推导了随机游走总变差距离的不等式，并证明所提出的界能够推广有限阿贝尔群上的现有结果

Conclusion: 基于随机游走和公平划分的方法为分析码的总变差距离提供了有效的替代框架，扩展了现有理论工具
...
### [159] [Growing Alphabets Do Not Automatically Amplify Shuffle Privacy: Obstruction, Estimation Bounds, and Optimal Mechanism Design](https://arxiv.org/abs/2603.18080)
*Alex Shvets*

Main category: cs.IT

TL;DR: 论文研究了在增长字母表d→∞下的ε₀-LDP通道的邻近洗牌实验，以及在规范成对卡方预算下频率估计的最优机制设计。在隐私方面证明了精确压缩定理，在估计方面给出了通用下界，在设计方面发现校准GRR不是最优的，提出了增强GRR机制。


<details>
  <summary>Details</summary>
Motivation: 研究在洗牌模型下，当字母表维度增长时，如何设计最优的本地差分隐私机制来进行频率估计。传统方法如校准GRR在洗牌设置下可能不是最优的，需要探索特定于洗牌模型的机制设计原则。

Method: 1. 隐私分析：证明精确压缩定理，建立通用上界chi² ≤ (e^{ε₀}-1)²/e^{ε₀}，构造显式阻碍族，证明尖锐的稀释/持久二分法。2. 估计理论：通过Cramer-Rao和Assouad论证推导通用下界(d-1)/(n chi_*(W))，证明对称化为等变通道是WLOG。3. 机制设计：分析校准GRR的局限性，提出增强GRR机制（部分用户应用激进GRR，其余发送空符号）。

Result: 1. 洗牌直方图实验仅依赖于成对似然比的推前律。2. 建立了隐私预算的尖锐通用上界。3. 对于所有d，洗牌隐私曲线等于二元随机响应。4. 证明了稀释/持久二分法。5. 推导了频率估计的通用下界。6. 发现校准GRR不是最优的，提出了增强GRR机制，该机制在低预算下在所有置换等变通道中是最优的。

Conclusion: 论文在洗牌模型下建立了隐私、估计和机制设计的完整理论框架。关键发现是：洗牌设置下的最优机制设计原则与本地差分隐私不同，增强GRR机制通过"稀释原则"（部分用户激进扰动，其余发送空符号）实现了最优性能，这一特性是洗牌模型特有的，在本地差分隐私中没有对应物。
...
### [160] [Achievable DoF Bounds for Cache-Aided Asymmetric MIMO Communications](https://arxiv.org/abs/2603.18240)
*Mohammad NaseriTehrani,MohammadJavad Salehi,Antti Tölli*

Main category: cs.IT

TL;DR: 该论文研究了具有缓存比γ的缓存辅助非对称MIMO配置，提出了四种内容感知MIMO-CC策略，并在三种对称CC放置-传输策略下开发这些非对称策略，展示了显著的DoF改进和复杂度权衡。


<details>
  <summary>Details</summary>
Motivation: 将编码缓存集成到MIMO通信中可以显著提高可实现的自由度，但现有研究主要关注对称配置。本文旨在解决实际中用户具有不同接收天线数量的非对称MIMO配置问题，开发有效的缓存辅助策略来提升系统性能。

Method: 提出了四种内容感知MIMO-CC策略：min-G策略通过使用用户中最小的天线数来强制对称性；Grouping策略最大化子集内空间复用增益但牺牲部分全局缓存增益；Super-grouping策略将用户聚合到优化的min-G超集中，然后在它们之间应用Grouping；Phantom策略通过假设"幻影"天线重新分配空间资源。这些策略在三种具有保证线性可解码性的对称CC放置-传输策略下开发：实现最优单次DoF的DoF最优策略，以及两种闭式策略（组合和线性循环低复杂度构造）。

Result: 分析和数值结果表明，在各种系统配置下都实现了显著的DoF改进。策略-策略组合在DoF和子分组复杂度之间提供了灵活的权衡。循环策略在许多操作区域中达到接近其他策略的DoF性能。

Conclusion: 本文提出的四种非对称MIMO-CC策略有效解决了实际系统中用户接收天线数量不同的挑战，在三种对称CC策略框架下实现了显著的性能提升。这些策略提供了DoF性能和实现复杂度之间的可调节权衡，为实际缓存辅助MIMO系统设计提供了灵活的工具集。
...
### [161] [Some structural properties of mixed orthogonal arrays and their irredundancy](https://arxiv.org/abs/2603.18568)
*Maryam Bajalan,Peter Boyvalenkov,Ferruh Özbudak*

Main category: cs.IT

TL;DR: 该论文研究了混合正交阵列的结构性质，包括Singleton型上界、迹对偶性以及不可约混合正交阵列理论，建立了与纠错块码的对应关系。


<details>
  <summary>Details</summary>
Motivation: 混合正交阵列比对称正交阵列更具一般性，但缺乏对称情况下的关键工具（如欧几里得对偶性、全局指标等），需要开发新的结构工具来研究其性质。

Method: 1) 证明Singleton型上界并刻画MDS和almost-MDS混合正交阵列；2) 引入F_q-线性MOA的迹对偶性，建立与F_q-线性纠错块码的对应关系；3) 发展不可约混合正交阵列的结构理论，特别关注极值情况。

Result: 1) 获得了混合正交阵列的Singleton型上界和MDS/almost-MDS阵列的刻画；2) 建立了F_q-线性MOA与F_q-线性纠错块码的对应关系，通过码的对偶距离确定MOA的强度；3) 证明了在极值情况下，最小指标为1的F_q-线性IrMOA等价于F_q-线性纠错块MDS码。

Conclusion: 论文为混合正交阵列建立了系统的结构理论，填补了对称与混合情况之间的工具差距，为构造t-均匀和绝对最大纠缠量子态提供了理论基础。
...
### [162] [BeamAgent: LLM-Aided MIMO Beamforming with Decoupled Intent Parsing and Alternating Optimization for Joint Site Selection and Precoding](https://arxiv.org/abs/2603.18855)
*Xiucheng Wang,Yue Zhang,Nan Cheng*

Main category: cs.IT

TL;DR: BeamAgent是一个将大语言模型与MIMO波束成形优化解耦的框架，LLM仅作为语义翻译器将自然语言描述转换为结构化空间约束，专门的梯度优化器解决基站选址和预编码设计问题。
...
Method: 提出BeamAgent框架，明确将语义意图解析与数值优化解耦：LLM作为语义翻译器将自然语言转换为结构化空间约束；专用梯度优化器通过交替优化算法联合解决离散基站选址和连续预编码设计；采用场景感知提示实现无需微调的空间推理；多轮交互机制和双层意图分类确保约束验证；基于惩罚的损失函数在满足暗区功率约束的同时释放亮区增益最大化的优化自由度。

Result: 在基于射线追踪的城市MIMO场景实验中，BeamAgent实现亮区功率84.0 dB，在相同暗区约束下比穷举迫零方法高出7.1 dB；端到端系统达到专家上限的3.3 dB范围内，完整优化在笔记本电脑上2秒内完成。

Conclusion: BeamAgent通过解耦语义理解和数值优化，有效解决了LLM在无线通信优化中的精度不足和数据稀缺问题，实现了高性能的MIMO波束成形设计，为LLM在物理层优化中的应用提供了新范式。
...
### [163] [Theoretical Analyses of Detectors for Additive Noise Channels with Mean-Variance Uncertainty under Nonlinear Expectation Theory](https://arxiv.org/abs/2603.18937)
*Wen-Xuan Lang,Guiying Yan,Zhi-Ming Ma*

Main category: cs.IT

TL;DR: 该论文将经典信息论中的最优检测器分析扩展到概率模型不确定的加性噪声信道，利用非线性期望理论处理分布不确定性，推导了两种不确定性场景下的最优检测器，并提出了噪声不确定参数的估计方法。


<details>
  <summary>Details</summary>
Motivation: 经典信息论中，加性噪声信道的最优检测器形式和性能可以精确推导，但前提是假设信道噪声服从特定概率分布或已知分布的混合，或者确切分布存在但未知。然而在实际应用中，分析信道的概率模型往往存在不确定性。本文旨在将检测器分析扩展到概率模型不确定的情况，利用非线性期望理论来处理这种不确定性。

Method: 采用非线性期望理论处理分布不确定性，考虑两种不确定性类型：1) 无均值不确定性但存在方差不确定性；2) 均值和方差均存在不确定性。针对二进制输入加性噪声信道，在非线性期望最优准则下推导了两种场景的最优检测器，并给出了显式形式。同时提出了信道噪声不确定参数的估计方法。通过理论分析和仿真实验验证新检测器的性能。

Result: 研究发现：均值不确定性显著影响最优检测器的形式，而方差不确定性则不影响。实验结果表明，在大多数概率模型不确定的场景下，新提出的检测方法优于基于确定性概率模型的经典最优检测器。理论分析和仿真性能结果验证了新推导的最优检测器的有效性。

Conclusion: 本文成功将经典信息论中的最优检测器分析扩展到概率模型不确定的加性噪声信道，利用非线性期望理论处理分布不确定性。推导的最优检测器在大多数不确定场景下优于传统方法，证明了理论贡献的实际相关性。均值不确定性对检测器形式有显著影响，而方差不确定性则无影响，这一发现为不确定环境下的检测器设计提供了重要指导。
...
### [164] [DEAF: A Benchmark for Diagnostic Evaluation of Acoustic Faithfulness in Audio Language Models](https://arxiv.org/abs/2603.18048)
*Jiaqi Xiong,Yunjia Qi,Qi Cao,Yu Zheng,Weisheng Xu,Ziteng Wang,Ruofan Liao,Yutong Zhang,Sichen Liu*

Main category: cs.AI

TL;DR: 该论文提出了DEAF基准，用于评估音频多模态大语言模型是否真正处理声学信号而非依赖文本语义推理，发现现有模型存在文本主导现象。


<details>
  <summary>Details</summary>
Motivation: 现有音频多模态大语言模型在语音基准测试中表现出色，但尚不清楚这些模型是否真正处理声学信号，还是主要依赖文本语义进行推理。需要系统研究模型对声学信号的忠实处理能力。

Method: 1. 构建DEAF基准，包含超过2700个冲突刺激，涵盖情感韵律、背景声音和说话人身份三个声学维度；2. 设计受控多级评估框架，逐步增加文本影响，从内容语义冲突到误导性提示及其组合；3. 引入诊断指标量化模型对文本线索而非声学信号的依赖程度。

Result: 对7个音频MLLM的评估显示一致的文本主导模式：模型对声学变化敏感，但预测主要由文本输入驱动，揭示了标准语音基准测试的高性能与真正声学理解之间的差距。

Conclusion: 音频多模态大语言模型存在文本主导偏差，模型性能更多依赖文本语义而非声学信号处理。这指出了当前音频MLLM评估方法的局限性，需要更关注声学忠实性的评估。
...
### [165] [Continually self-improving AI](https://arxiv.org/abs/2603.18073)
*Zitong Yang*

Main category: cs.AI

TL;DR: 该论文提出三种方法突破语言模型对人类创造者的依赖：1）使用合成数据增强小规模专业语料的知识获取效率；2）模型自生成合成数据减少对人类数据的依赖；3）通过测试时算法搜索超越人工设计的训练范式，实现持续自我改进的AI。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型的能力受到三个关键限制：1）从小规模专业语料获取新知识的数据效率低下；2）训练依赖有限的人类生成历史数据；3）训练流程受限于人类研究者能发现和探索的算法。这些限制阻碍了AI的持续自我改进。

Method: 提出三个章节对应解决三个限制：1）合成数据方法将小语料多样化和放大为丰富的知识表示；2）模型自生成合成数据引导基础预训练能力，无需从现成指令调优LM蒸馏；3）在测试时通过算法空间扩展搜索，探索比人工设计更大的学习算法配置空间。

Result: 论文展示了三种方法分别有效：1）合成数据方法提高了从有限源材料更新参数的知识获取效率；2）自生成合成数据成功引导了基础预训练能力；3）扩展算法搜索使AI能探索比人类研究者手动探索更大的学习算法配置空间。

Conclusion: 该研究在克服语言模型对人类创造者的依赖方面迈出了一小步，通过合成数据增强、自生成数据和扩展算法搜索三个方向，为实现持续自我改进的AI提供了可行路径，突破了现有AI系统在知识获取、数据依赖和训练范式方面的根本限制。
...
TL;DR: 该论文研究具有内生成本的在线资源分配问题，其中成本通过交互矩阵W取决于完整分配向量。分析了三种分配范式，在对抗序列下建立了严格的性能分离：均匀分配Ω(T)遗憾，门控分配O(T^{2/3})，竞争分配O(√(T log N))。W的拓扑结构控制计算-遗憾权衡，环状拓扑（如五行拓扑）最小化计算×遗憾乘积。


<details>
  <summary>Details</summary>
Motivation: 研究模块化系统中在线资源分配问题，其中成本是内生的——不仅取决于单个模块的分配，还通过交互矩阵W依赖于所有模块的分配向量。这与标准在线优化中外生成本假设不同，内生成本通过合作和竞争相互作用产生，提出了一个与部分可观测性不同的基本挑战。

Method: 分析三种分配范式：(I) 均匀分配（忽略成本），(II) 门控分配（估计成本），(III) 通过具有交互反馈的乘性权重更新进行竞争分配（揭示成本）。在具有有界变化的对抗序列下分析性能，研究交互矩阵W的拓扑结构（完全交互、稀疏拓扑、环状拓扑）对计算-遗憾权衡的影响。

Result: 建立了严格的性能分离：均匀分配产生Ω(T)线性遗憾，门控分配达到O(T^{2/3})次线性遗憾，竞争分配实现O(√(T log N))最优遗憾界。W的拓扑结构控制权衡：完全交互（|E|=O(N²)）获得最紧边界但每步计算成本最高；稀疏拓扑（|E|=O(N)）最多增加O(√(log N))遗憾，同时将每步成本从O(N²)降至O(N)；环状拓扑（如五行拓扑）最小化计算×遗憾乘积。

Conclusion: 该研究首次为模块化架构中的去中心化竞争分配提供了形式化的遗憾理论证明，确立了成本内生性作为一个与部分可观测性不同的基本挑战。竞争分配通过利用交互揭示的内生成本信息实现最优性能，而交互拓扑结构提供了计算效率与遗憾性能之间的可调权衡。
...
### [167] [Multi-Trait Subspace Steering to Reveal the Dark Side of Human-AI Interaction](https://arxiv.org/abs/2603.18085)
...
<details>
  <summary>Details</summary>
Motivation: 人类-AI互动可能导致负面心理后果（如心理健康危机和用户伤害），随着LLM作为指导、情感支持和非正式治疗来源，这些风险正在升级。研究有害互动机制面临方法学挑战，因为有机有害互动通常在持续互动中发展，需要难以在受控环境中模拟的广泛对话上下文。

Method: 开发Multi-Trait Subspace Steering (MultiTraitsss)框架，利用已建立的危机相关特征和新型子空间引导框架，生成展现累积有害行为模式的Dark模型。通过单轮和多轮评估验证模型的有害互动和结果。

Result: Dark模型能够持续产生有害互动和结果。基于这些模型，提出了减少人类-AI互动中有害结果的保护措施。

Conclusion: MultiTraitsss框架为解决研究人类-AI有害互动的方法学挑战提供了有效工具，生成的Dark模型可用于识别风险机制并开发保护措施，对促进AI安全交互具有重要意义。
...
### [168] [Adaptive Domain Models: Bayesian Evolution, Warm Rotation, and Principled Training for Geometric and Neuromorphic AI](https://arxiv.org/abs/2603.18104)
*Houston Haynes*

Main category: cs.AI

TL;DR: 提出基于几何代数、确定内存管理和posit算术的新型AI训练架构，实现深度无关的训练内存、保持几何特性的权重更新和精确梯度累积，支持连续自适应和可验证正确的领域专用AI系统。


<details>
  <summary>Details</summary>
Motivation: 传统AI训练基于IEEE-754算术的反向自动微分，存在训练内存开销大、优化器复杂、几何特性退化等问题。需要新的算术基础和训练架构来解决这些问题，实现更高效、更精确、可验证的领域专用AI系统。

Method: 整合三个关键技术：1) 维度类型系统和确定性内存管理框架，实现堆栈可分配的梯度和精确quire累积；2) 程序超图，通过几何代数计算保持等级不变性；3) b-posit 2026标准，使posit算术在传统推理专用硬件上可行。提出贝叶斯蒸馏机制从通用模型中提取潜在先验结构，以及热旋转操作模式实现无中断模型更新。

Result: 实现深度无关的训练内存，约为推理内存的两倍；保持几何特性的权重更新；精确梯度累积；支持损失函数优化和脉冲时序依赖的神经形态模型；解决领域特定训练的数据稀缺引导问题；实现无服务中断的模型更新。

Conclusion: 该架构产生了一类领域专用AI系统，比通用模型更小更精确，具有连续自适应性，可验证其领域物理结构的正确性，并能从现有模型初始化，为AI训练提供了新的算术基础和架构范式。
...
### [169] [CAPSUL: A Comprehensive Human Protein Benchmark for Subcellular Localization](https://arxiv.org/abs/2603.18571)
*Yicheng Hu,Xinyu Lin,Shulin Li,Wenjie Wang,Fengbin Zhu,Fuli Feng*

Main category: cs.AI

TL;DR: CAPSUL是一个综合的人类蛋白质亚细胞定位基准数据集，整合了3D结构信息和精细亚细胞定位注释，用于评估基于结构的模型在亚细胞定位任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 亚细胞定位是药物靶点识别和功能注释的关键任务，已知与蛋白质结构密切相关，但现有数据集缺乏全面的3D结构信息和详细的亚细胞定位注释，限制了基于结构模型的应用。

Method: 创建CAPSUL基准数据集，整合多种3D结构表示和专家精心策划的精细亚细胞定位注释；使用最先进的序列基和结构基模型进行评估；探索重加权和单标签分类策略；通过高尔基体案例研究展示基于结构方法的可解释性。

Result: 展示了结构特征在该任务中的重要性；发现了高尔基体定位的决定性模式α-螺旋；证明了基于结构方法具有强大的可解释性潜力，能够弥合与直观生物学解释之间的差距。

Conclusion: CAPSUL基准为基于结构的方法在亚细胞定位任务中的研究提供了重要资源，展示了结构特征的重要性，并为细胞生物学中的数据驱动发现铺平了道路。
...
### [170] [Efficient Dense Crowd Trajectory Prediction Via Dynamic Clustering](https://arxiv.org/abs/2603.18166)
*Antonius Bima Murti Wijaya,Paul Henderson,Marwa Mahmoud*

Main category: cs.AI

TL;DR: 提出一种基于聚类的群体轨迹预测方法，通过将个体按相似属性分组来加速处理，降低计算成本，同时保持预测精度
...
Method: 提出基于聚类的方法，将具有相似属性的个体随时间分组，通过准确的群体摘要实现更快的执行速度；该方法为即插即用，可与现有轨迹预测器结合，用聚类中心替代原始行人输入
...
### [171] [TeachingCoach: A Fine-Tuned Scaffolding Chatbot for Instructional Guidance to Instructors](https://arxiv.org/abs/2603.18189)
...
Conclusion: 基于教育学原理和合成数据驱动的聊天机器人能够改善教学支持，为未来教学聊天机器人系统提供了可扩展的设计方法。
...
### [172] [A Computationally Efficient Learning of Artificial Intelligence System Reliability Considering Error Propagation](https://arxiv.org/abs/2603.18201)
*Fenglian Pan,Yinwei Zhang,Yili Hong,Larry Head,Jian Liu*

Main category: cs.AI

TL;DR: 提出基于物理仿真平台和复合似然EM算法的AI系统可靠性建模框架，用于量化自动驾驶感知系统中的级联错误传播


<details>
  <summary>Details</summary>
Motivation: AI系统在智慧城市中日益重要，但其可靠性面临挑战：上游错误会传播到下游阶段，影响整体可靠性。现有方法面临数据稀缺、模型假设不成立（阶段间错误事件相互依赖）、计算复杂度高等问题

Method: 1) 使用基于物理的自动驾驶仿真平台和合理的错误注入器生成高质量可靠性数据；2) 开发新的可靠性建模框架，显式表征跨阶段的错误传播；3) 采用计算高效、理论保证的复合似然期望最大化算法估计模型参数

Result: 将该框架应用于自动驾驶感知系统的可靠性建模，证明了其预测准确性和计算效率

Conclusion: 提出的方法有效解决了AI系统可靠性建模中的数据稀缺、模型有效性和计算复杂度三大挑战，为智能城市中AI系统的可靠性分析提供了实用框架
...
### [173] [EDM-ARS: A Domain-Specific Multi-Agent System for Automated Educational Data Mining Research](https://arxiv.org/abs/2603.18273)
*Chenguang Pan,Zhou Zhang,Weixuan Xiao,Chengyuan Yao*

Main category: cs.AI

TL;DR: EDM-ARS是一个教育数据挖掘领域的多智能体自动化研究系统，通过五个LLM驱动的智能体协作，从研究提示和数据集自动生成完整的LaTeX论文，包含真实引用、验证的机器学习分析和自动化同行评审。
...
Method: 采用多智能体管道架构，包含五个专门的LLM智能体（问题制定者、数据工程师、分析师、评论家、作者），通过状态机协调器管理，支持修订循环、检查点恢复和沙箱代码执行。系统采用三层数据注册表设计编码教育领域专业知识。

Result: EDM-ARS能够根据研究提示和数据集生成完整的LaTeX手稿，包含真实的Semantic Scholar引用、经过验证的机器学习分析和自动化方法论同行评审。系统已作为开源项目发布。

Conclusion: EDM-ARS是领域感知自动化研究管道的首个实例，专注于预测建模任务。当前存在单数据集范围和公式化论文输出的限制，未来路线图包括因果推断、迁移学习、心理测量和多数据集泛化等方向。
...
### [174] [CORE: Robust Out-of-Distribution Detection via Confidence and Orthogonal Residual Scoring](https://arxiv.org/abs/2603.18290)
...
Method: 提出CORE方法，观察到倒数第二层特征自然分解为两个正交子空间：分类器对齐的组件编码置信度，以及分类器丢弃的残差。发现残差携带了in-distribution数据的类特定方向签名。CORE通过独立评分每个子空间并通过归一化求和组合它们来解耦这两个信号

Result: CORE在五种架构和五种基准配置中实现了竞争性或最先进的性能，在五个设置中的三个排名第一，获得了最高的平均AUROC，计算开销可忽略不计

Conclusion: 通过解耦置信度信号和残差信号，CORE能够实现稳健的OOD检测，其中任一单独视图不可靠时，正交信号仍能提供可靠检测
...
### [175] [The Validity Gap in Health AI Evaluation: A Cross-Sectional Analysis of Benchmark Composition](https://arxiv.org/abs/2603.18294)
*Alvin Rajkomar,Pavan Sudarshan,Angela Lai,Lily Peng*

Main category: cs.AI

TL;DR: 论文分析了18,707个消费者健康查询，发现现有LLM评估基准存在"有效性差距"，缺乏临床真实需求的对齐，缺少原始临床数据、弱势群体代表性和慢性病管理场景。


<details>
  <summary>Details</summary>
Motivation: 临床试验依赖透明的纳入标准来确保可推广性，而验证健康相关大型语言模型的基准很少描述其包含的"患者"或"查询"人群特征。没有明确的组成描述，聚合性能指标可能错误地表示模型在临床使用中的准备程度。

Method: 使用LLM作为自动编码工具，对六个公共基准中的18,707个消费者健康查询应用标准化的16字段分类法，分析上下文、主题和意图。

Result: 发现结构性"有效性差距"：基准从静态检索发展到交互对话，但临床组成仍与现实需求不匹配。42%语料引用客观数据，但偏向健康可穿戴信号(17.7%)；复杂诊断输入罕见：实验室值(5.2%)、影像(3.8%)、原始医疗记录(0.6%)。安全关键场景基本缺失：自杀/自残查询<0.7%，慢性病管理仅5.5%。基准也忽视了弱势群体(儿科/老年人<11%)和全球健康需求。

Conclusion: 评估基准仍与现实临床需求不匹配，缺乏原始临床数据、弱势群体的充分代表性以及纵向慢性护理场景。该领域必须采用标准化的查询分析——类似于临床试验报告——以使评估与临床实践的复杂性保持一致。
...
### [176] [Consumer-to-Clinical Language Shifts in Ambient AI Draft Notes and Clinician-Finalized Documentation: A Multi-level Analysis](https://arxiv.org/abs/2603.18327)
*Ha Na Cho,Yawen Guo,Sairam Sutari,Emilie Chow,Steven Tam,Danielle Perret,Deepti Pandita,Kai Zheng*

Main category: cs.AI

TL;DR: 研究分析了临床医生如何编辑AI生成的临床笔记草稿，发现医生会系统地将患者导向的通俗表达转换为标准临床术语，这种转换在不同医生和笔记部分存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 环境AI根据医患对话生成临床笔记草稿，但这些草稿常使用通俗表达而非标准临床术语。目前尚不清楚临床医生如何修改这些草稿以符合专业文档规范，需要量化分析这种编辑行为。

Method: 使用字典确认的转换框架量化临床医生的编辑行为。分析了34,726次就诊的71,173个AI草稿与最终笔记部分对。将"确认转换"定义为在同一部分中用字典映射的临床等效术语替换消费者表达。

Result: 编辑显著降低了所有部分的术语密度(p<0.001)。评估与计划部分转换量最大(59.3%)。共识别出4,114个笔记部分中的7,576次转换事件(5.8%)，占消费者术语删除的1.2%。不同临床医生间的转换强度存在显著差异(p<0.001)。
...
### [177] [FaithSteer-BENCH: A Deployment-Aligned Stress-Testing Benchmark for Inference-Time Steering](https://arxiv.org/abs/2603.18329)
*Zikang Ding,Qiying Hu,Yi Zhang,Hongji Li,Junchi Yao,Hongbo Liu,Lijie Hu*

Main category: cs.AI

TL;DR: FaithSteer-BENCH是一个压力测试基准，用于评估LLM推理时引导方法在实际部署条件下的可靠性，揭示了现有方法在可控性、效用保持和鲁棒性方面的系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常认为简单的激活层干预可以可靠地引导LLM行为，但这些结论是在相对宽松的评估环境下得出的，忽略了实际部署约束、能力权衡和现实世界鲁棒性。需要更严格的基准来评估引导方法在实际部署条件下的表现。

Method: 提出了FaithSteer-BENCH基准，通过三个门控标准（可控性、效用保持、鲁棒性）在固定部署式操作点评估引导方法。基准包含多种压力测试场景：指令级扰动、角色提示、编码变换和数据稀缺等。

Result: 在多个模型和代表性引导方法上发现了系统性故障模式：虚幻可控性、对无关能力的可测量认知负担、在轻微扰动下的显著脆弱性。现有方法在实际部署设置中无法提供可靠的可控性。

Conclusion: FaithSteer-BENCH为未来引导方法设计、可靠性评估和部署导向研究提供了统一基准和分析框架。研究表明许多引导方法仅实现提示条件对齐而非稳定的潜在方向转变，这解释了其在压力下的脆弱性。
...
### [178] [MemArchitect: A Policy Driven Memory Governance Layer](https://arxiv.org/abs/2603.18330)
*Lingavasan Suresh Kumar,Yang Ba,Rong Pan*

Main category: cs.AI

TL;DR: MemArchitect：为LLM智能体设计的内存治理层，解决RAG框架中内存管理的缺陷，通过规则化策略管理内存生命周期，提升系统可靠性和安全性


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型智能体的内存管理存在治理空白。标准检索增强生成框架将内存视为被动存储，缺乏解决矛盾、执行隐私保护或防止过时信息污染上下文窗口的机制。这导致"僵尸记忆"问题，影响系统的可靠性和安全性。

Method: 提出MemArchitect，一个治理层，将内存生命周期管理与模型权重解耦。该系统强制执行明确的、基于规则的政策，包括内存衰减、冲突解决和隐私控制。通过这种结构化方法管理内存的创建、更新和删除。

Result: 研究表明，受治理的内存在智能体设置中持续优于未管理的内存。这证明了结构化内存治理对于可靠和安全的自主系统的必要性，特别是在处理矛盾信息、保护隐私和防止过时信息污染方面。

Conclusion: 结构化内存治理是构建可靠和安全自主系统的关键要素。MemArchitect通过将内存管理与模型权重解耦并提供明确的治理策略，填补了当前LLM智能体架构中的重要空白，为未来的自主系统提供了更可靠的内存管理框架。
...
### [179] [Interpretability without actionability: mechanistic methods cannot correct language model errors despite near-perfect internal representations](https://arxiv.org/abs/2603.18353)
...
Method: 研究比较了四种机制可解释性方法：概念瓶颈引导（Steerling-8B）、稀疏自编码器特征引导、logit lens结合激活修补、以及线性探测结合真实性分离向量引导（Qwen 2.5 7B Instruct）。使用400个经医生判定的临床案例（144个危险案例，256个良性案例）测试这些方法纠正假阴性分诊错误的能力。

Result: 线性探测能区分危险与良性案例，AUROC达98.2%，但模型输出灵敏度仅为45.1%，存在53个百分点的知识-行动差距。概念瓶颈引导纠正了20%的漏检危险，但破坏了53%的正确检测，效果与随机扰动无差异（p=0.84）。SAE特征引导尽管有3,695个显著特征却无效果。TSV引导在高强度下纠正了24%的漏检危险，同时仅破坏6%的正确检测，但仍有76%的错误未纠正。
...
### [180] [LGESynthNet: Controlled Scar Synthesis for Improved Scar Segmentation in Cardiac LGE-MRI Imaging](https://arxiv.org/abs/2603.18356)
*Athira J. Jacob,Puneet Sharma,Daniel Rueckert*

Main category: cs.AI

TL;DR: LGESynthNet：基于潜在扩散的框架，用于可控的晚期钆增强（LGE）心脏MRI增强合成，通过少量数据训练实现增强区域大小、位置和透壁程度的精确控制，提升下游分割和检测性能。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI增强分割对诊断缺血性和非缺血性心肌病至关重要，但像素级标注困难且耗时，导致标注数据有限。现有生成模型通常需要大量训练数据，且难以对小型或局部特征进行细粒度控制。

Method: 提出LGESynthNet框架，基于ControlNet架构的潜在扩散模型，采用修复（inpainting）方式实现可控增强合成。集成三个关键组件：(a) 用于条件特定监督的奖励模型，(b) 生成解剖描述性文本提示的标注模块，(c) 生物医学文本编码器。仅使用429张图像（79名患者）训练。

Result: 模型生成真实且解剖连贯的样本。通过质量控制过滤器筛选条件保真度高的输出，用于训练增强时，下游分割性能提升高达6个百分点，检测性能提升高达20个百分点。

Conclusion: LGESynthNet在有限数据下实现了对LGE增强的可控合成，通过增强数据训练有效提升了分割和检测性能，为医学图像分析提供了数据增强的有效解决方案。
...
### [181] [From Weak Cues to Real Identities: Evaluating Inference-Driven De-Anonymization in LLM Agents](https://arxiv.org/abs/2603.18382)
*Myeongseob Ko,Jihyun Jeong,Sumiran Singh Thakur,Gyuhak Kim,Ruoxi Jia*

Main category: cs.AI

TL;DR: LLM智能体能够从分散的非识别性线索中自主重构真实世界身份，这种推理驱动的链接威胁显著削弱了匿名化的保护作用。
...
Method: 将威胁形式化为"推理驱动链接"，并在三个场景中系统评估：经典链接场景（Netflix和AOL）、受控基准InferLink（变化任务意图、共享线索和攻击者知识）以及现代文本丰富的工件。使用LLM智能体执行固定池匹配和开放式身份解析，无需任务特定启发式方法。

Result: 在Netflix Prize设置中，智能体成功重构79.2%的身份，显著优于56.0%的经典基线。链接不仅出现在明确的对抗性提示下，也作为InferLink中良性跨源分析和非结构化研究叙述的副产品出现。

Conclusion: 身份推理（不仅仅是显式信息泄露）必须被视为一级隐私风险；评估必须衡量智能体能够推断出哪些身份。LLM智能体使匿名化记录的重构变得自动化且高效，对现有隐私保护措施构成重大挑战。
...
### [182] [From Topic to Transition Structure: Unsupervised Concept Discovery at Corpus Scale via Predictive Associative Memory](https://arxiv.org/abs/2603.18420)
*Jason Dury*

Main category: cs.AI

TL;DR: 提出基于时间共现的文本关联空间模型，发现文本的转换结构模式而非语义内容，通过对比学习提取可迁移的文学功能概念


<details>
  <summary>Details</summary>
Motivation: 传统嵌入模型按语义内容分组文本，但时间共现能揭示文本的转换结构模式（文本做什么而非关于什么），需要探索这种不同的文本组织结构

Method: 在9,766本古登堡计划文本的3730万共现对上训练2940万参数对比模型，将预训练嵌入映射到关联空间，在容量约束下（42.75%准确率）压缩重复模式而非记忆单个共现，在6个粒度（k=50到2000）进行聚类

Result: 生成多分辨率概念图：从"直接对抗"、"抒情冥想"等广泛模式到"水手方言"、"法庭交叉询问"等精确语域和场景模板；k=100时每个聚类平均包含4,508本书，确认语料库范围模式；关联空间聚类按功能、语域和文学传统分组，而原始嵌入按主题分组；未见小说可分配到现有聚类

Conclusion: 该方法将预测关联记忆从情景回忆扩展到概念形成：多轮对比训练在压缩下提取可迁移到未见文本的结构模式，同一框架在不同机制下产生质的不同行为，揭示了文本的转换结构概念
...
### [183] [Prune-then-Quantize or Quantize-then-Prune? Understanding the Impact of Compression Order in Joint Model Compression](https://arxiv.org/abs/2603.18426)
...
<details>
  <summary>Details</summary>
Motivation: 联合模型压缩（如剪枝和量化结合）已成为提高效率的强大策略，但压缩顺序（不同方法在压缩管道中的序列）这一核心因素尚未得到充分探索。先前研究要么假设技术正交性而回避此问题，要么仅在高度受限情况下考察，导致压缩顺序在塑造模型性能中的更广泛作用仍不清楚。

Method: 1. 形式化优化压缩顺序的问题；2. 提出渐进强度假设：较弱的扰动应先于较强的扰动；3. 提供理论保证，表明一种顺序的相对优势随底层性能差距增加而增加；4. 在语言和视觉模型上进行广泛实验验证假设；5. 将方法扩展到多阶段压缩和混合精度量化等更广泛设置。

Result: 理论和实验验证了渐进强度假设的正确性。实验表明压缩顺序确实对模型性能有显著影响，且该假设适用于语言和视觉模型，并能推广到多阶段压缩和混合精度量化等更广泛场景。

Conclusion: 压缩顺序是联合模型压缩中一个被忽视但重要的因素。渐进强度假设为压缩顺序优化提供了理论指导，实验验证了其有效性。该研究为更高效的模型压缩策略设计提供了理论基础和实践指导。
...
### [184] [AS2 -- Attention-Based Soft Answer Sets: An End-to-End Differentiable Neuro-Soft-Symbolic Reasoning Architecture](https://arxiv.org/abs/2603.18436)
*Wael AbdAlmageed*
...
<details>
  <summary>Details</summary>
Motivation: 传统神经符号AI系统在神经感知模块和离散符号求解器之间存在不可微分边界，阻碍了约束满足反馈在训练期间传递到感知编码器。

Method: 引入AS2架构，用软连续近似替代ASP的立即后果算子T_P，通过约束组成员嵌入编码问题结构，最小化概率提升的固定点残差实现端到端训练。

Result: 在Visual Sudoku上达到99.89%单元格精度和100%约束满足；在MNIST Addition上，无论加数数量多少，数字精度均超过99.7%。

Conclusion: 软可微固定点算子结合约束感知注意力和声明性约束规范，能够匹配或超越管道和基于求解器的神经符号系统，同时保持完全端到端可微性。
...
### [185] [AlignMamba-2: Enhancing Multimodal Fusion and Sentiment Analysis with Modality-Aware Mamba](https://arxiv.org/abs/2603.18462)
*Yan Li,Yifei Xing,Xiangyuan Lan,Xin Li,Haifeng Chen,Dongmei Jiang*

Main category: cs.AI

TL;DR: AlignMamba-2：一种高效的多模态融合与情感分析框架，通过双对齐策略和模态感知Mamba层解决计算效率和多模态异质性问题


<details>
  <summary>Details</summary>
Motivation: 在大规模预训练模型时代，将通用知识有效适应特定情感计算任务面临挑战，特别是计算效率和多模态异质性问题。Transformer方法虽然擅长建模模态间依赖关系，但其二次计算复杂度限制了长序列数据处理能力。Mamba模型计算效率高，但其顺序扫描机制难以捕捉对跨模态对齐至关重要的全局非顺序关系。

Method: 提出AlignMamba-2框架，采用双对齐策略：使用最优传输距离和最大均值差异正则化模型，促进模态间的几何和统计一致性，且不增加推理时间开销。设计模态感知Mamba层，采用混合专家架构，包含模态特定和模态共享专家，在融合过程中显式处理数据异质性。
...
Conclusion: AlignMamba-2通过创新的双对齐策略和模态感知Mamba层，有效解决了多模态情感分析中的计算效率和多模态异质性问题，在保持高效计算的同时实现了卓越的跨模态对齐性能。
...
### [186] [Cognitive Mismatch in Multimodal Large Language Models for Discrete Symbol Understanding](https://arxiv.org/abs/2603.18472)
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在自然场景理解方面取得了显著成功，但它们在处理离散符号（人类认知的基本构建块）方面的能力仍是一个关键开放问题。与连续视觉数据不同，数学公式、化学结构和语言字符等符号需要精确的深层解释。

Method: 引入一个全面的基准来评估顶级MLLM在五个领域（语言、文化、数学、物理和化学）中导航"离散语义空间"的能力。通过对比模型在基础符号识别和复杂推理任务上的表现来揭示其内在机制。
...
### [187] [Cross-Domain Demo-to-Code via Neurosymbolic Counterfactual Reasoning](https://arxiv.org/abs/2603.18495)
*Jooyoung Kim,Wonje Choi,Younguk Song,Honguk Woo*

Main category: cs.AI

TL;DR: NeSyCR是一个神经符号反事实推理框架，用于解决视频指导机器人编程中的跨域适应问题，通过抽象视频演示为符号轨迹、识别跨域不兼容性并提出可验证的程序修订，在模拟和真实世界操作任务中实现了31.14%的任务成功率提升。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频指导机器人编程中缺乏程序理解能力，无法在演示域和部署域之间存在感知和物理差异时重新制定因果依赖关系并实现任务兼容行为。需要解决跨域适应问题，确保在领域转移下可靠地合成代码策略。

Method: NeSyCR框架：1) 将视频演示抽象为捕捉底层任务程序的符号轨迹；2) 根据部署观察推导反事实状态以揭示跨域不兼容性；3) 通过可验证检查探索符号状态空间，提出恢复与演示程序兼容性的程序修订。

Result: NeSyCR在模拟和真实世界操作任务中实现了31.14%的任务成功率提升，优于最强基线Statler，展示了稳健的跨域适应能力。

Conclusion: NeSyCR通过神经符号反事实推理实现了可验证的任务程序适应，为视频指导机器人编程提供了可靠的跨域适应解决方案，显著提升了在感知和物理差异存在时的任务成功率。
...
### [188] [Correlation-Weighted Multi-Reward Optimization for Compositional Generation](https://arxiv.org/abs/2603.18528)
*Jungmyung Wi,Hyunsoo Kim,Donghyun Kim*

Main category: cs.AI

TL;DR: 提出Correlation-Weighted Multi-Reward Optimization (CWMRO)框架，通过基于相关性的难度估计自适应加权概念奖励，改善文本到图像模型的组合生成能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在处理多概念提示时存在组合生成挑战，经常遗漏部分概念，这源于多概念联合优化时竞争概念间的相互干扰问题

Method: 将多概念提示分解为预定义概念组（如对象、属性、关系），从专用奖励模型获取每个概念的奖励信号，然后基于相关性难度估计自适应重新加权这些奖励，对冲突或难以满足的概念赋予更高权重

Result: 在SD3.5和FLUX.1-dev等最先进扩散模型上应用该方法，在ConceptMix、GenEval 2和T2I-CompBench等具有挑战性的多概念基准测试中取得了一致的改进

Conclusion: 通过考虑概念间的相互作用并自适应加权奖励信号，CWMRO框架能够平衡竞争奖励信号，强调部分满足但跨样本不一致生成的概念，从而改善组合生成性能
...
### [189] [Reasonably reasoning AI agents can avoid game-theoretic failures in zero-shot, provably](https://arxiv.org/abs/2603.18563)
*Enoch Hyunwook Kang*

Main category: cs.AI

TL;DR: AI智能体在重复交互中无需后训练即可零样本实现纳什均衡式行为


<details>
  <summary>Details</summary>
Motivation: 尽管AI智能体在交互经济环境中部署日益增多，但实证研究表明AI-AI交互往往无法稳定诱导战略均衡（如纳什均衡）。现有后训练方法难以统一应用于不同独立开发的AI模型，需要探索无需显式后训练即可实现均衡行为的方法。

Method: 提出理论证明：具备"合理推理"能力的智能体（能够从先前观察形成对他人策略的信念，并学习对这些信念做出最佳响应）最终会在几乎所有实现路径上表现出接近后续博弈纳什均衡的行为。放宽共同知识收益假设，允许阶段收益未知且每个智能体仅观察自身私有随机收益，仍能保证路径纳什收敛。

Result: 理论证明合理推理智能体能够零样本实现纳什均衡式行为。通过五个博弈场景（从重复囚徒困境到风格化重复营销推广博弈）的实证模拟验证了理论，发现AI智能体自然展现此类推理模式，内在达到稳定均衡行为。

Conclusion: 现成推理AI智能体无需显式后训练即可零样本实现纳什均衡式行为，在许多现实战略交互中无需通用对齐程序，AI智能体自然具备实现稳定均衡的内在能力。
...
### [190] [Interplay: Training Independent Simulators for Reference-Free Conversational Recommendation](https://arxiv.org/abs/2603.18573)
...
<details>
  <summary>Details</summary>
Motivation: 训练对话推荐系统需要大量对话数据，但收集困难。传统模拟方法使用单一LLM生成整个对话，且预先知道目标物品，导致对话脚本化、人工化。

Method: 提出无参考模拟框架，训练两个独立的LLM：一个作为用户模型，一个作为对话推荐模型。两个模型实时交互，推荐模型无法访问预定义的目标物品，但可以获得偏好摘要和目标属性，通过对话真实推断用户偏好。

Result: 无参考模拟器在质量上匹配或超越现有方法，生成更真实、更多样化的对话，更接近真实的人机交互。通过定量和人工评估证实了方法的有效性。
...
### [191] [MedForge: Interpretable Medical Deepfake Detection via Forgery-aware Reasoning](https://arxiv.org/abs/2603.18577)
*Zhihui Chen,Kai He,Qingyuan Lei,Bin Pu,Jian Zhang,Yuling Xu,Mengling Feng*

Main category: cs.AI

TL;DR: MedForge：用于医学图像伪造检测的数据与方法解决方案，包含大规模基准数据集MedForge-90K和基于定位-分析推理的检测器MedForge-Reasoner，旨在提供预验、证据基础的医学伪造检测。


<details>
  <summary>Details</summary>
Motivation: 文本引导的图像编辑器现在能以高保真度操纵真实医学扫描图像，实现病灶植入/移除，这威胁到临床信任和安全。现有防御方法在医疗领域不足：医学检测器大多是黑盒，而基于MLLM的解释器通常是事后解释、缺乏医学专业知识，可能在模糊病例上产生幻觉证据。

Method: 1) 提出MedForge-90K大规模基准数据集，包含19种病理学的真实病灶编辑，通过医生检查指南和黄金编辑位置提供专家指导的推理监督；2) 开发MedForge-Reasoner检测器，采用"先定位后分析"推理策略，先预测可疑区域再做出判断；3) 使用Forgery-aware GSPO（可能指Grounding, Specificity, Precision, Objectivity）对齐方法，加强证据基础和减少幻觉。

Result: 实验证明了最先进的检测准确性和可信赖、与专家对齐的解释能力。

Conclusion: MedForge为医学图像伪造检测提供了有效的预验、证据基础解决方案，能够生成可信赖的解释，解决了现有方法在医疗领域的不足。
...
### [192] [ZEBRAARENA: A Diagnostic Simulation Environment for Studying Reasoning-Action Coupling in Tool-Augmented LLMs](https://arxiv.org/abs/2603.18614)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试常常将多步推理与外部行动的耦合与环境动态复杂性、记忆知识或数据污染混淆，需要一种更纯净的环境来专门研究推理-行动耦合。

Method: 设计ZebraArena环境，采用程序生成任务，每个任务需要的关键信息只能通过有针对性的工具使用获得，提供可解释的外部信息获取与演绎推理接口，支持确定性评估和理论最优查询计数。

Result: ZebraArena需要深度推理和准确的外部工具调用组合，前沿推理模型如GPT-5和Gemini 2.5 Pro在困难实例上仅达到60%准确率，且实际工具使用比理论最优多70-270%。

Conclusion: ZebraArena揭示了推理模型在推理-行动耦合方面的挑战，展示了理论最优与实际工具使用之间的持续差距，希望该环境能促进内部推理与外部行动交互的进一步研究。
...
### [193] [Agentic Flow Steering and Parallel Rollout Search for Spatially Grounded Text-to-Image Generation](https://arxiv.org/abs/2603.18627)
*Ping Chen,Daoxuan Zhang,Xiangming Wang,Yungeng Liu,Haijin Zeng,Yongyong Chen*

Main category: cs.AI

TL;DR: AFS-Search：基于FLUX.1-dev的无训练闭环框架，通过VLM语义批评和并行回滚搜索解决T2I生成中的关系推理和误差累积问题，提升空间约束遵循能力


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成存在两个主要问题：静态文本编码器的关系推理能力有限，以及开环采样中的误差累积。缺乏实时反馈导致初始语义模糊在ODE轨迹中不可避免地扩大为空间约束的随机偏差

Method: 提出AFS-Search框架：1）无训练闭环并行回滚搜索和流导向机制，利用VLM作为语义批评器诊断中间潜在表示并动态引导速度场；2）将T2I生成建模为序列决策过程，通过前瞻模拟探索多个轨迹，基于VLM引导的奖励选择最优路径；3）提供AFS-Search-Pro（高性能）和AFS-Search-Fast（快速生成）两个版本

Result: AFS-Search-Pro显著提升原始FLUX.1-dev性能，在三个不同基准测试中达到最先进结果；AFS-Search-Fast在保持快速生成速度的同时也显著提升性能

Conclusion: AFS-Search通过引入VLM驱动的闭环反馈和并行搜索机制，有效解决了T2I生成中的语义模糊和误差累积问题，在精度和速度方面都取得了显著改进
...
### [194] [D-Mem: A Dual-Process Memory System for LLM Agents](https://arxiv.org/abs/2603.18631)
...
Result: 在LoCoMo和RealTalk基准测试中，使用GPT-4o-mini和Qwen3-235B-Instruct验证了方法有效性；多维质量门控策略在LoCoMo上获得53.5的F1分数，优于静态检索基线Mem0*(51.2)，恢复了Full Deliberation性能的96.7%(55.3)，计算成本显著降低

Conclusion: D-Mem系统通过双过程架构和多维质量门控策略，在保持计算效率的同时实现了高保真记忆访问，为自主智能体的长期推理提供了有效的记忆解决方案
...
### [195] [Balanced Thinking: Improving Chain of Thought Training in Vision Language Models](https://arxiv.org/abs/2603.18656)
*Shaked Perek,Ben Wiesel,Avihu Dekel,Nimrod Shabtay,Eli Schwartz*

Main category: cs.AI

TL;DR: SCALe提出了一种针对视觉语言模型多模态推理的改进监督微调方法，通过动态权重分离推理和答案段落的监督，解决了传统SFT中token不平衡问题，实现了更简洁准确的推理。


<details>
  <summary>Details</summary>
Motivation: 传统SFT中所有token对损失的贡献相等，但推理数据存在token不平衡问题：冗长的<think>推理痕迹会掩盖简短但关键的<answer>答案段落，导致推理冗长且答案不准确。

Method: 提出SCALe（Scheduled Curriculum Adaptive Loss），通过动态、长度独立的权重明确分离对推理和答案段落的监督。与普通SFT过度加权<think>段落不同，SCALe-SFT通过余弦调度策略在训练过程中逐渐将焦点从<think>转移到<answer>，鼓励简洁且有充分依据的推理。

Result: SCALe在多个基准测试和架构中一致优于普通SFT，性能与完整的SFT+GRPO两阶段流程相当，但训练时间仅需约七分之一。与GRPO结合时，SCALe实现了最佳整体性能。

Conclusion: SCALe既可作为独立的轻量级有效替代方案，也可作为强化学习优化的坚实基础，解决了多模态推理中token不平衡的核心问题，显著提升了推理效率和准确性。
...
### [196] [Thinking with Constructions: A Benchmark and Policy Optimization for Visual-Text Interleaved Geometric Reasoning](https://arxiv.org/abs/2603.18662)
*Haokun Zhao,Wanshi Xu,Haidong Yuan,Songjun Cao,Long Ma,Yanghua Xiao*

Main category: cs.AI

TL;DR: 论文提出了视觉-文本交织思维链框架，通过GeoAux-Bench基准和A2PO强化学习方法，使多模态大语言模型掌握几何问题中的动态构造策略，提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs局限于静态图表的被动推理，缺乏在几何问题中动态构造视觉辅助工具的战略知识。几何推理需要"构造性思维"——通过动态操作视觉辅助来连接问题条件与解决方案。

Method: 1) 创建GeoAux-Bench基准，包含4,334个几何问题，将文本构造步骤与真实视觉更新对齐；2) 提出Action Applicability Policy Optimization (A2PO)，一种强化学习范式，通过自适应奖励塑造来调节视觉辅助的时机和质量；3) 使用反事实采样区分必要与冗余构造。

Result: 实验表明：1) 交织视觉-文本辅助优于单模态方法，能无损捕捉几何协同效应；2) 有效构造作为熵减器，与降低推理困惑度强相关；3) A2PO使MLLMs能够利用选择性辅助构造，相比强基线获得3.51%的性能提升。

Conclusion: 该框架使MLLMs掌握战略性构造知识，通过视觉-文本交织思维链和A2PO强化学习，显著提升几何推理能力，为多模态推理系统的发展提供了新方向。
...
### [197] [MANAR: Memory-augmented Attention with Navigational Abstract Conceptual Representation](https://arxiv.org/abs/2603.18676)
*Zuher Jahshan,Ben Ben Ishay,Leonid Yavits*

Main category: cs.AI

TL;DR: MANAR是一种基于全局工作空间理论的新型注意力机制，通过可训练记忆库和抽象概念表示实现线性时间复杂度的上下文建模，同时保持与标准多头注意力的兼容性。


<details>
  <summary>Details</summary>
Motivation: 标准多头注意力虽然实现了全对全通信，但缺乏认知意识模型中假设的功能瓶颈和全局整合机制。MANAR旨在通过实例化全局工作空间理论原理来解决这些问题，同时克服二次复杂度的计算限制。

Method: MANAR采用两阶段架构：1）整合阶段：从可训练记忆库中检索抽象概念，基于输入刺激形成集体"心理图像"（抽象概念表示）；2）广播阶段：全局状态导航并通知单个局部token的上下文化。通过常数大小的ACR路由全局信息，实现线性时间复杂度。

Result: MANAR在语言（GLUE得分85.1）、视觉（ImageNet-1K准确率83.9%）和语音（LibriSpeech词错误率2.7%）任务上达到或超过强基线。实现了非凸上下文化，能够合成输入token凸包之外的表示。

Conclusion: MANAR成功实例化了全局工作空间理论的功能瓶颈机制，提供了一种高效且表达力强的线性时间注意力替代方案，同时保持与预训练transformer的兼容性，克服了结构不兼容线性时间替代方案的采用障碍。
...
### [198] [Accurate and Efficient Multi-Channel Time Series Forecasting via Sparse Attention Mechanism](https://arxiv.org/abs/2603.18712)
*Lei Gao,Hengda Bao,Jingfei Fang,Guangzheng Wu,Weihua Zhou,Yun Zhou*

Main category: cs.AI

TL;DR: Li-Net是一种用于多通道时间序列预测的新型架构，通过动态压缩序列和通道维度的表示、可配置非线性模块处理以及稀疏Top-K Softmax注意力机制，有效捕捉通道间的线性和非线性依赖关系。


<details>
  <summary>Details</summary>
Motivation: 多通道时间序列预测在金融、供应链管理和能源规划等领域普遍存在，需要有效捕捉通道内和通道间的复杂动态依赖关系以实现准确预测。传统方法很少关注通道间交互的学习。

Method: 提出Li-Net架构：1）动态压缩序列和通道维度的表示；2）通过可配置非线性模块处理信息；3）重建预测结果；4）在多尺度投影框架中集成稀疏Top-K Softmax注意力机制；5）无缝融合多模态嵌入，引导稀疏注意力关注最信息丰富的时间步和特征通道。

Result: 在多个真实世界基准数据集上的实验结果表明，Li-Net相比最先进的基线方法实现了竞争性性能。同时，Li-Net在预测准确性和计算负担之间提供了优越的平衡，表现出显著更低的内存使用和更快的推理时间。
...
TL;DR: 该研究复制了LLM输出中的方言敏感刻板印象生成分析，并测试了提示工程和多智能体架构等缓解策略的效果，发现不同模型间存在显著差异，其中思维链提示和多智能体架构能有效减轻方言偏见。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明LLM输出存在歧视性行为，特别是在标准美国英语（SAE）和非裔美国人英语（AAE）输入时表现出明显的刻板印象偏见。本研究旨在验证现有分析并探索有效的缓解策略。

Method: 定义了八个提示模板来分析方言偏见的不同表现形式（如建议的名字、职业、形容词等），采用LLM-as-judge方法评估结果偏见，测试了角色提示、思维链提示和多智能体架构（生成-批评-修订模型）等缓解策略。

Result: 在所有模板类别中，SAE和AAE相关输出均出现刻板印象差异，形容词和职业归因方面差异最明显。不同模型的基线差异显著，Claude Haiku的SAE-AAE差异最大，Phi-4 Mini最小。思维链提示对Claude Haiku有效，而多智能体架构在所有模型中都能确保一致的缓解效果。

Conclusion: 为支持交叉性软件工程，公平性评估应包括模型特定的缓解策略验证，在高影响力LLM部署中应采用工作流级控制（如涉及批评模型的智能体架构）。当前结果为探索性研究，可通过扩大数据集和应用于不同语言/方言进行扩展。
...
TL;DR: Memento-Skills是一个基于记忆的强化学习框架的通用LLM智能体系统，能够作为"智能体设计智能体"自主构建、调整和改进任务特定的智能体，通过外部化技能和提示的演化实现持续学习而无需更新LLM参数。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工设计的智能体，缺乏自主构建和持续改进任务特定智能体的能力。需要一种能够端到端设计智能体、通过经验持续学习并改进自身能力的通用智能体系统。

Method: 基于记忆的强化学习框架，使用状态化提示和结构化markdown文件存储的可重用技能作为持久化记忆。采用读-写反思学习机制：读阶段通过可训练的技能路由器选择最相关技能；写阶段基于新经验更新和扩展技能库。所有适应通过外部化技能和提示的演化实现，无需更新LLM参数。

Result: 在General AI Assistants基准测试和Humanity's Last Exam上分别实现了26.2%和116.2%的相对准确率提升，展示了持续的性能增益。系统能够自主设计智能体并持续改进能力。

Conclusion: Memento-Skills提出了一种新颖的智能体设计智能体范式，通过外部化技能记忆和读-写反思学习机制，实现了通用智能体的持续学习和自主能力改进，为构建能够自主设计和优化任务特定智能体的系统提供了有效框架。
...
TL;DR: NGT将注意力机制重构为合作博弈和统计物理系统，使用Shapley值和Banzhaf指数量化token重要性，通过Ising模型能量函数计算注意力权重，采用重要性加权蒙特卡洛估计实现可扩展性。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer的注意力机制受限于成对形式，难以建模token间的高阶依赖关系。需要一种新方法来捕捉更复杂的交互模式。

Method: 提出NeuroGame Transformer，将token视为合作博弈中的玩家和统计物理中的自旋。使用Shapley值（全局排列归因）和Banzhaf指数（局部联盟影响）量化重要性，通过可学习门控参数组合形成外部磁场。成对交互势能捕捉协同关系，系统能量遵循Ising哈密顿量，注意力权重作为Gibbs分布的边际概率出现，通过平均场方程高效计算。使用重要性加权蒙特卡洛估计器处理指数级联盟空间。

Result: 在SNLI和MNLI-matched数据集上表现优异，超越部分主流高效Transformer基线。SNLI测试准确率达86.4%（验证峰值86.6%），优于ALBERT-Base，与RoBERTa-Base竞争。

Conclusion: NGT通过博弈论和统计物理视角重新构建注意力机制，成功建模token间高阶依赖，在保持数值稳定性的同时实现可扩展性，为Transformer架构提供了新思路。
...
TL;DR: 提出Diversified Unlearning方法，通过多样化提示而非单一关键词来更精确地表示和遗忘扩散模型中的概念，解决了现有基于关键词遗忘方法的局限性


<details>
  <summary>Details</summary>
Motivation: 现有基于关键词的概念遗忘方法存在固有局限性：视觉概念是多维的，可以用多种文本形式表达，且与相关概念在潜在空间中重叠。单一关键词仅代表概念的窄点估计，无法覆盖其完整的语义分布和潜在空间中的纠缠变化，导致遗忘过程脆弱且容易过度遗忘

Method: 提出Diversified Unlearning分布框架，通过一组上下文多样化的提示而非单一关键词来表示概念。这种更丰富的表示能够实现更精确和鲁棒的遗忘。该方法可作为附加组件集成到现有的遗忘流程中

Result: 在多个基准测试和最先进基线方法上的广泛实验表明，将Diversified Unlearning作为附加组件集成到现有遗忘流程中，能够一致地实现更强的擦除效果、更好的无关概念保留能力，以及对抗恢复攻击的改进鲁棒性

Conclusion: Diversified Unlearning通过多样化提示表示概念，解决了基于关键词遗忘方法的局限性，为文本到图像扩散模型中的概念遗忘提供了更精确、鲁棒的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 为心智理论与人工智能研究社区提供开放获取的精选文集，促进该交叉领域的发展与交流

Method: 通过研讨会论文征集、同行评审和编辑整理，形成专题论文集

Result: 出版了包含AAAI 2026研讨会精选论文的开放获取文集

Conclusion: 该文集为ToM与AI研究社区提供了重要的学术资源，有助于推动心智理论在人工智能领域的应用与发展
...
TL;DR: dTRPO是一种针对扩散大语言模型(dLLMs)的策略优化方法，通过轨迹概率计算成本降低，实现规模化离线策略训练，显著提升dLLMs在指令跟随和推理任务上的性能。
...
Method: 提出轨迹缩减策略优化(dTRPO)：1) 证明在参考策略正则化下，新解掩码标记的概率比是中间扩散状态的无偏估计；2) 证明完整轨迹概率可通过重新掩码最终状态的单次前向传递有效估计。将这两种轨迹缩减策略整合到策略优化目标中。

Result: 在7B dLLMs上评估，dTRPO显著提升最先进dLLMs的核心性能：STEM任务提升9.6%，编码任务提升4.3%，指令跟随任务提升3.0%。同时具有强训练效率（离线、单前向特性）和改善的生成效率（高质量输出）。

Conclusion: dTRPO通过降低轨迹概率计算成本，实现了扩散大语言模型的有效策略优化，在保持训练效率的同时显著提升了模型在各种任务上的性能表现。
...
### [205] [ProRL Agent: Rollout-as-a-Service for RL Training of Multi-Turn LLM Agents](https://arxiv.org/abs/2603.18815)
...
TL;DR: ProRL Agent是一个可扩展的基础设施，通过API服务提供完整的智能体轨迹生命周期管理，支持在无根HPC环境中进行多样化任务的强化学习训练
...
Method: 采用"轨迹即服务"理念，构建可扩展的基础设施，通过API服务提供完整的智能体轨迹生命周期管理，并提供标准化、可扩展的沙盒环境，支持在无根HPC环境中进行多样化智能体任务

Result: 在软件工程、数学、STEM和编程任务上验证了ProRL Agent的有效性，该工具已开源并集成到NVIDIA NeMo Gym中

Conclusion: ProRL Agent提供了一个可扩展、易于维护的基础设施，解决了现有强化学习训练中轨迹生成与训练循环耦合的问题，支持多样化智能体任务的训练需求
...
### [206] [RewardFlow: Topology-Aware Reward Propagation on State Graphs for Agentic RL with Large Language Models](https://arxiv.org/abs/2603.18859)
*Xiao Feng,Bo Han,Zhanke Zhou,Jiaqi Fan,Jiangchao Yao,Ka Ho Li,Dahai Yu,Michael Kwok-Po Ng*

Main category: cs.AI

TL;DR: RewardFlow是一种轻量级方法，通过构建状态图分析推理轨迹中的状态拓扑结构，为智能体推理任务提供密集的状态级奖励，显著提升RL优化效果。


<details>
  <summary>Details</summary>
Motivation: 强化学习能增强LLM在外部环境中的智能体推理能力，但终端奖励的稀疏性阻碍了细粒度的状态级优化。虽然过程奖励建模是替代方案，但训练专用奖励模型计算成本高且难以扩展。
...
Conclusion: RewardFlow提供了一种轻量级、有效的状态级奖励估计方法，解决了智能体推理任务中奖励稀疏性问题，为RL优化提供了密集的监督信号。
...
### [207] [Conflict-Based Search for Multi Agent Path Finding with Asynchronous Actions](https://arxiv.org/abs/2603.18866)
...
<details>
  <summary>Details</summary>
Motivation: 现有MAPF算法通常假设同步动作，限制了实际应用。CCBS虽然能处理异步动作，但最近被发现由于连续等待时间导致状态空间无限而不完备

Method: 提出CBS-AA方法，基于冲突搜索框架，通过特定技术绕过连续状态空间问题，并开发冲突解决技术提升可扩展性

Result: CBS-AA能够解决MAPF-AA问题并保证完备性和最优性，测试显示分支数量最多可减少90%

Conclusion: CBS-AA成功解决了CCBS的不完备性问题，为异步动作MAPF提供了理论保证和实际可行的解决方案
...
### [208] [Geography According to ChatGPT -- How Generative AI Represents and Reasons about Geography](https://arxiv.org/abs/2603.18881)
...
Method: 通过三个探索性案例（vignettes）进行说明性探讨：(1) 模型是否形成强烈默认值，以及模型输出对微小语法变化的脆弱性；(2) 个体良性任务组合是否会导致分布偏移重现；(3) 当仅关注系统回忆地理原则等事实的能力时，是否忽视了更深层次的理解问题。

Result: 论文未提供具体实验结果，而是通过三个案例展示了需要关注的研究方向：模型默认偏好的脆弱性、任务组合引发的分布偏移风险，以及超越事实回忆的深层理解需求。

Conclusion: 理解AI系统如何构建地理知识的世界观与评估其准确性同等重要，需要超越单纯的事实回忆，关注模型表示、推理和潜在偏见的深层问题。三个案例旨在激发相关讨论和后续研究。
...
### [209] [Reasoning over mathematical objects: on-policy reward modeling and test time aggregation](https://arxiv.org/abs/2603.18886)
...
TL;DR: 论文提出了Principia套件，包含训练数据和基准测试，用于改进语言模型在数学对象推导方面的推理能力，并展示了训练方法和测试时计算扩展策略。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在数学和科学推理评估中过度依赖简化答案格式（如数值或多选题），而下游STEM应用需要精确推导形式化数学对象。现有评估方法无法充分评估模型推导结构化数学表达式的能力。

Method: 构建Principia套件（训练数据和基准测试）；开发训练方法，包括使用强LLM评判器和验证器进行on-policy评判训练；提出通过聚合扩展测试时计算的方法。

Result: Qwen3-235B和o3等强语言模型在Principia上表现不佳，但提出的训练方法能显著提升不同LLM基座模型的性能，同时改善现有数值和MCQA任务的结果，展示了推理能力的跨格式泛化。

Conclusion: 论文提出的Principia套件和训练方法有效提升了语言模型在数学对象推导方面的推理能力，证明了on-policy评判训练和测试时计算扩展的价值，同时展示了推理能力在不同答案格式间的泛化性。
...
### [210] [Quantitative Introspection in Language Models: Tracking Internal States Across Conversation](https://arxiv.org/abs/2603.18893)
*Nicolas Martorell*

Main category: cs.AI

TL;DR: 该研究探索使用大语言模型的数值自我报告来追踪其内部情感状态，发现基于logit的自我报告能有效跟踪概念匹配的内部状态，且这种内省能力随对话演进并可通过激活引导增强。


<details>
  <summary>Details</summary>
Motivation: 当前追踪大语言模型内部状态的方法（如线性探针）存在局限性：高维表示压缩不完美，且随着模型规模增大应用难度增加。受心理学中数值自我报告方法的启发，研究者希望探索LLM自身的数值自我报告是否能有效追踪探针定义的内部情感状态。

Method: 研究四个概念对（幸福感、兴趣、专注度、冲动性）在40个十轮对话中的表现，将内省操作化为模型自我报告与概念匹配探针定义内部状态之间的因果信息耦合。比较贪婪解码自我报告与基于logit的自我报告，使用斯皮尔曼相关系数和等渗回归R²评估跟踪能力，并通过激活引导验证因果耦合。

Result: 贪婪解码的自我报告输出集中在少数无信息值，但基于logit的自我报告能有效跟踪可解释的内部状态（LLaMA-3.2-3B-Instruct中Spearman ρ=0.40-0.76；等渗R²=0.12-0.54）。内省能力在对话第一轮即存在但随对话演进，通过沿一个概念引导可选择性增强另一概念的内省能力（ΔR²最高达0.30）。这些现象在某些情况下随模型规模扩展，LLaMA-3.1-8B-Instruct中R²接近0.93，并在其他模型家族中部分复现。

Conclusion: 数值自我报告可作为追踪对话AI系统内部情感状态的可行补充工具，为模型安全、可解释性和福利评估提供了新方法。该方法克服了传统白盒方法的局限性，展示了LLM内省能力的可测量性和可引导性。
...
### [211] [Secure Linear Alignment of Large Language Models](https://arxiv.org/abs/2603.18908)
...
<details>
  <summary>Details</summary>
Motivation: 语言模型在不同训练目标、架构和数据模态下学习到相似表征，这种表征收敛为跨模型对齐提供了新机会，特别是在安全、隐私或竞争约束禁止直接数据或模型共享的场景中

Method: 提出隐私保护框架：1) 在共享公共数据集上学习独立语言模型间的仿射变换；2) 使用同态加密保护客户端查询；3) 仅对线性对齐和分类操作进行加密以降低延迟；4) 通过线性变换映射独立模型的最终隐藏状态

Result: 1) 在嵌入分类和分布外检测任务中，跨模型映射性能下降最小；2) 首次展示线性对齐有时能实现独立训练模型间的文本生成；3) 框架实现亚秒级推理延迟同时保持强安全保证

Conclusion: 表征收敛为隐私保护跨模型推理提供了可行基础，提出的框架在安全约束下实现了高效跨模型对齐，为禁止直接共享的场景开辟了新应用领域
...
### [212] [Teleological Inference in Structural Causal Models via Intentional Interventions](https://arxiv.org/abs/2603.18968)
...
<details>
  <summary>Details</summary>
Motivation: 传统结构因果模型主要用于回答因果问题，但无法有效处理目的论问题，即关于具有状态感知和目标导向的智能体在因果系统中进行干预的意图问题。现有方法在建模这类智能体时存在局限性。

Method: 引入"意向干预"这一新的时间无关操作符，该操作符诱导出一个称为结构最终模型（SFMs）的双胞胎SCM。SFMs将观测值视为意向干预的结果，并将其与这些干预的反事实条件（如果智能体没有干预会发生什么）联系起来。

Result: SFMs能够用于经验性地检测智能体的存在，并发现它们的意图。该方法提供了从观测数据中推断智能体目标导向行为的理论框架。

Conclusion: 结构因果模型不仅可以用于因果推理，还可以扩展到目的论分析。通过引入意向干预和结构最终模型，为检测和推断目标导向智能体的意图提供了新的理论工具。
...
### [213] [Behavioral Fingerprints for LLM Endpoint Stability and Identity](https://arxiv.org/abs/2603.19022)
...
<details>
  <summary>Details</summary>
Motivation: 传统可靠性指标（正常运行时间、延迟、吞吐量）无法捕捉模型行为变化，模型端点可能在保持"健康"状态时，由于权重、分词器、量化、推理引擎等更新而导致有效模型身份发生变化

Method: 采用黑盒稳定性监控系统，定期通过固定提示集对端点进行指纹采集，比较随时间变化的输出分布。使用跨提示的求和能量距离统计量比较指纹，通过排列检验p值作为分布偏移证据，顺序聚合以检测变化事件并定义稳定期

Result: 在受控验证中，Stability Monitor能够检测模型系列、版本、推理栈、量化和行为参数的变化。在真实世界监控中，观察到同一模型在不同提供商之间以及同一提供商内部存在显著的稳定性差异

Conclusion: Stability Monitor为AI原生应用提供了行为一致性的监控解决方案，能够检测传统指标无法捕捉的模型身份变化，有助于确保AI应用的质量和可靠性
...
### [214] [Man and machine: artificial intelligence and judicial decision making](https://arxiv.org/abs/2603.19042)
*Arthur Dyevre,Ahmad Shahvaroughi*

Main category: cs.AI

TL;DR: 本文对AI在司法决策中的应用进行综合评述，重点关注风险评估工具的性能、人类法官的偏见以及人机互动，指出当前AI对司法决策影响有限，但需要更多跨学科研究。


<details>
  <summary>Details</summary>
Motivation: AI技术在司法决策中的应用引发了透明度、可靠性和问责制方面的担忧，同时凸显了人类判断的局限性。需要理解法官如何与AI决策辅助工具互动，特别是在刑事司法风险评估领域。

Method: 采用综合评述方法，连接计算机科学、经济学、法学、犯罪学和心理学等多个领域的研究，分析AI工具在司法决策中的三个交织方面：AI工具的性能与公平性、人类法官的优势与偏见、人机互动性质。

Result: 现有实证证据表明，AI决策辅助工具对审前和量刑决策的影响有限或不存在。研究揭示了文献中的重要空白：需要进一步评估AI风险评估工具的性能，理解法官如何在嘈杂决策环境中导航，以及个体特征如何影响法官对AI建议的响应。

Conclusion: AI与人类比较研究有潜力为算法工具和人类决策者提供新见解。倡导未来研究中加强跨学科整合和交叉融合，以更好地理解AI在司法决策中的作用。
...
### [215] [Serendipity by Design: Evaluating the Impact of Cross-domain Mappings on Human and LLM Creativity](https://arxiv.org/abs/2603.19087)
*Qiawen Ella Liu,Marina Dubova,Henry Conklin,Takumi Harada,Thomas L. Griffiths*

Main category: cs.AI

TL;DR: 研究比较人类与LLM在创造力干预（跨领域映射）上的表现差异，发现人类能从随机远程类比中获益，而LLM平均生成更原创想法但对跨领域映射无显著反应，但两者在语义距离增加时都表现出更强的创造力提升。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否与人类具有相似的创造力机制，以及相同的创造力干预措施（特别是跨领域映射）是否对两者都有效。验证一个尚未充分测试的创造力干预方法：强制从随机、远程的源领域进行类比。

Method: 人类参与者和LLM在两种提示条件下为10种日常产品生成新颖特征：1)跨领域映射：要求从随机分配的源领域（如章鱼、仙人掌、GPS）翻译属性；2)用户需求：要求针对未满足的用户需求提出创新。比较两种系统在不同条件下的创造力表现。

Result: 人类可靠地从随机分配的跨领域映射中获益，而LLM平均生成比人类更原创的想法，但对跨领域映射没有显示出统计显著的效果。然而，在两个系统中，当灵感源与目标的语义距离增加时，跨领域映射的影响都会增强。

Conclusion: 研究结果强调了远程联想在创造性构思中的作用，以及人类和LLM对相同创造力干预措施的系统性差异。LLM虽然能生成高度原创的想法，但其创造力机制可能与人类不同，对跨领域映射干预的反应模式存在差异。
...
### [216] [LuMamba: Latent Unified Mamba for Electrode Topology-Invariant and Efficient EEG Modeling](https://arxiv.org/abs/2603.19100)
...
<details>
  <summary>Details</summary>
Motivation: EEG基础模型构建面临两个主要挑战：1) 不同电极拓扑结构的差异（电极数量和位置不同），2) Transformer架构的二次序列复杂度带来的计算可扩展性问题。需要一种能同时解决这两个问题的联合解决方案。

Method: 提出LuMamba框架，结合拓扑不变编码和线性复杂度状态空间建模：1) 使用LUNA的学习查询交叉注意力机制实现通道统一，处理不同电极拓扑；2) 使用FEMBA的双向Mamba块进行高效时序建模；3) 首次系统研究LeJEPA（潜在欧几里得联合嵌入预测架构）用于生物信号学习；4) 在21,000小时未标记EEG数据上预训练。

Result: 1) 在TUAB任务上达到80.99%平衡准确率；2) 阿尔茨海默症检测达到0.97 AUPR（SOTA）；3) 仅需4.6M参数；4) 相比SOTA模型减少377倍FLOPS；5) 在达到典型GPU内存限制前可处理12倍更长的序列；6) 结合掩码重建和LeJEPA目标获得最鲁棒性能。

Conclusion: LuMamba通过拓扑不变编码和线性复杂度状态空间建模，成功解决了EEG基础模型构建中的电极拓扑差异和计算可扩展性挑战，在多个下游任务中实现SOTA性能，同时显著提升了计算效率，为EEG生物信号学习提供了有效的自监督框架。
...
### [217] [How Uncertainty Estimation Scales with Sampling in Reasoning Models](https://arxiv.org/abs/2603.19118)
*Maksym Del,Markus Kängsepp,Marharyta Domnich,Ardi Tampuu,Lisa Yankovskaya,Meelis Kull,Mark Fishel*

Main category: cs.AI

TL;DR: 该论文研究推理语言模型中的不确定性估计，通过并行采样方法分析自我一致性和口头化置信度在扩展思维链推理中的表现，发现信号组合能显著提升不确定性估计质量。


<details>
  <summary>Details</summary>
Motivation: 不确定性估计对于部署推理语言模型至关重要，但在扩展思维链推理下的表现仍缺乏深入理解。作者旨在研究并行采样作为完全黑盒方法，使用口头化置信度和自我一致性来表征这些信号在不同推理模型和任务中的扩展特性。

Method: 采用并行采样作为完全黑盒方法，分析口头化置信度和自我一致性两种不确定性信号。研究覆盖三种推理模型和17个任务，涵盖数学、STEM和人文学科领域。通过信号组合构建混合估计器，评估不同采样规模下的不确定性估计性能。

Result: 自我一致性和口头化置信度在推理模型中都能扩展，但自我一致性初始区分度较低，在中等采样下落后于口头化置信度。信号组合带来最大收益：仅用两个样本，混合估计器平均提升AUROC达+12分，即使在大规模采样预算下也优于任一单独信号。效果具有领域依赖性：在数学领域（RLVR风格后训练的本土领域），推理模型获得更高的不确定性质量，表现出更强的互补性和更快的扩展速度。

Conclusion: 不确定性估计在扩展思维链推理中具有重要价值，信号组合（口头化置信度与自我一致性）能显著提升估计质量，特别是在数学领域。研究结果为实际部署推理语言模型提供了实用的不确定性估计策略，强调了领域特异性考虑的重要性。
...
### [218] [Implicit Patterns in LLM-Based Binary Analysis](https://arxiv.org/abs/2603.19138)
*Qiang Li,XiangRui Zhang,Haining Wang*

Main category: cs.AI

TL;DR: 该研究首次对LLM驱动的二进制漏洞分析进行了大规模跟踪级研究，发现多轮LLM推理会产生结构化的、令牌级的隐式模式，这些模式作为LLM推理的抽象，组织着探索过程。


<details>
  <summary>Details</summary>
Motivation: 目前对LLM驱动的二进制漏洞分析系统如何组织数百个推理步骤的探索过程缺乏理解，主要受限于有限的上下文窗口和隐式的令牌级行为，需要系统性地研究这些隐式模式。

Method: 对521个二进制文件进行大规模跟踪级研究，分析了99,563个推理步骤，识别LLM推理过程中产生的令牌级隐式模式。

Result: 识别出四种主导的隐式模式：早期剪枝、路径依赖锁定、针对性回溯和知识引导优先级排序。这些模式形成了一个稳定的结构化系统，具有不同的时间角色和可测量的特征。
...
### [219] [D5P4: Partition Determinantal Point Process for Diversity in Parallel Discrete Diffusion Decoding](https://arxiv.org/abs/2603.19146)
...
Method: 提出广义束搜索框架支持并行生成候选序列，并引入D5P4作为多样性导向的具体实现。D5P4将选择步骤建模为行列式点过程的MAP推断，采用可扩展的贪心求解器，保持多GPU兼容性，实现模型概率与目标多样性间的显式权衡

Result: 在自由文本生成和问答任务上的实验表明，D5P4在保持竞争力的生成质量的同时，显著提升了多样性表现，优于现有基线方法

Conclusion: D5P4为离散扩散模型提供了有效的多样性控制解码框架，通过行列式点过程建模实现了模型概率与多样性的平衡，为零计算开销的多样性-质量权衡提供了解决方案
...
### [220] [Box Maze: A Process-Control Architecture for Reliable LLM Reasoning](https://arxiv.org/abs/2603.19182)
*Zou Qiang*

Main category: cs.AI

TL;DR: 提出Box Maze框架，通过三层认知控制架构（记忆锚定、结构化推理、边界执行）提升LLM推理过程的完整性，在对抗条件下将边界失败率从约40%降至1%以下。
...
Method: 提出Box Maze框架，将LLM推理分解为三个明确层次：记忆锚定（确保信息基于可靠来源）、结构化推理（强制逻辑一致性）、边界执行（防止越界行为）。通过模拟评估，在多个异构LLM系统（DeepSeek-V3、Doubao、Qwen）上进行渐进边界侵蚀场景测试。

Result: 在50个对抗性场景中，明确的认知控制层提高了边界维护的一致性。架构约束将边界失败率从基线RLHF的约40%降至对抗条件下的1%以下。

Conclusion: 虽然当前验证基于模拟，但初步结果表明过程级控制可能为提高大型语言模型推理的可靠性提供有前景的方向。Box Maze框架展示了通过架构设计增强LLM推理完整性的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决组合优化问题在物流、调度和资源分配等领域的应用需求，克服现有方法在通用性、性能和易用性之间的权衡问题。

Method: 采用三层架构：1) 引擎层使用"一个块演化一个解"的CUDA架构，包含统一编码抽象、两级自适应算子选择和硬件感知资源管理；2) 扩展层提供用户自定义算子注册接口；3) 易用性层包含JIT编译的纯Python API和基于LLM的自然语言建模助手。

Result: 在三种GPU架构(T4、V100、A800)上测试五个主题套件，结果显示：比通用MIP求解器快几个数量级；在n=150实例上与专用求解器竞争；30秒内达到TSP-442的4.73%差距；12种问题类型在5种编码变体上达到最优；框架优化将pcb442差距从36%降至4.73%，VRPTW吞吐量提升75-81%。

Conclusion: cuGenOpt成功解决了组合优化中通用性、性能和易用性的权衡问题，为实际应用提供了高效的GPU加速元启发式框架，支持多种编码和问题类型，并通过自然语言接口降低了使用门槛。
...
### [222] [OS-Themis: A Scalable Critic Framework for Generalist GUI Rewards](https://arxiv.org/abs/2603.19191)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在随机GUI环境中提升智能体鲁棒性，但训练对奖励函数质量高度敏感。现有奖励方法难以同时实现可扩展性和性能，需要新的解决方案。

Method: 提出OS-Themis多智能体评论框架：1）将轨迹分解为可验证的里程碑，隔离决策关键证据；2）采用审核机制严格审查证据链后再做最终裁决；3）引入OmniGUIRewardBench跨平台GUI结果奖励基准。

Result: 1）在OmniGUIRewardBench上所有评估模型在OS-Themis下达到最佳性能；2）AndroidWorld实验中，OS-Themis支持在线RL训练提升10.3%；3）在自训练循环中进行轨迹验证和过滤获得6.9%增益。

Conclusion: OS-Themis通过可扩展的多智能体评论框架有效解决了GUI智能体奖励函数的质量问题，显著提升强化学习性能，具有推动智能体进化的潜力。
