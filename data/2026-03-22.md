...
Method: 提出SwiftTailor两阶段框架：1) PatternMaker - 轻量级视觉语言模型，从多种输入模态预测缝纫图案；2) GarmentSewer - 高效密集预测Transformer，将图案转换为Garment Geometry Image（在统一UV空间中编码所有服装面板的3D表面）。通过逆映射过程重建最终3D网格，包含重新网格化和动态缝合算法直接组装服装，避免物理模拟成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有低光增强数据集存在三个主要问题：1) 只能在有限曝光范围和静态场景中采集；2) 现有夜间驾驶数据集的暗图像没有精确对齐的白天对应图像；3) 动态驾驶场景中采集真实世界昼夜对齐数据集极其困难，严重限制了该领域的研究。

Method: 提出自动昼夜轨迹跟踪姿态匹配(TTPM)方法，在大型真实世界封闭驾驶测试场(面积69英亩)中采集数据。该方法能够精确对齐昼夜图像对，对齐误差仅为数厘米。数据集包含9,538对昼夜图像，每对都手动标注了2D边界框。

Result: 创建了首个真实世界昼夜对齐的自动驾驶低光增强基准数据集DarkDriving，包含9,538对精确对齐的昼夜图像对。数据集支持四个感知相关任务：低光增强、广义低光增强、2D检测的低光增强、3D检测的低光增强。

Conclusion: DarkDriving数据集为自动驾驶低光增强提供了全面的基准评估，实验结果表明该数据集不仅能评估自动驾驶低光增强，还能推广到增强暗图像和促进其他低光驾驶环境(如nuScenes)中的检测任务。
...
<details>
  <summary>Details</summary>
Motivation: 虽然SAM在通用图像分割方面表现出色，但其自然语言理解能力有限，这限制了其在指代表达式分割任务中的直接应用。需要一种方法能够充分利用SAM的分割能力，同时增强其语言理解能力。

Method: 提出SSP-SAM框架，集成语义-空间提示编码器。该编码器包含视觉和语言注意力适配器，分别突出视觉特征中的显著对象和语言特征中的区分性短语。这种设计增强了提示生成器的指代表示，产生高质量的语义-空间提示，使SAM能够在语言指导下生成精确的分割掩码。

Result: 在广泛使用的RES和GRES基准测试中验证了方法的优越性。该方法生成高质量的分割掩码，即使在严格的Pr@0.9阈值下也能实现强精度。在PhraseCut数据集上的进一步评估显示，在开放词汇场景中相比现有最先进的RES方法有改进。

Conclusion: SSP-SAM框架成功地将SAM的分割能力与增强的语言理解相结合，无需专门修改即可支持广义指代表达式分割，在多个基准测试中表现出优越性能。
...
TL;DR: CytoSyn是一种用于组织病理学H&E染色图像生成的先进潜在扩散模型，通过大规模TCGA数据集训练，支持引导生成高度逼真且多样的组织病理图像。


<details>
  <summary>Details</summary>
Motivation: 尽管计算病理学在特征提取方面取得了进展，但专门针对组织病理学的生成式基础模型仍然稀缺。这类模型能够解决特征提取器无法完成的任务，如虚拟染色等。

Method: 开发了CytoSyn-v2，这是一种先进的潜在扩散模型，探索了方法改进、训练集扩展、采样策略和玻片级过拟合问题。模型在超过10,000张TCGA诊断全玻片图像上训练，涵盖32种不同癌症类型。

Result: 模型在广泛基准测试中展示了生成高度逼真和多样组织病理图像的能力。尽管仅在肿瘤学玻片上训练，但在生成炎症性肠病图像方面仍保持最先进性能。研究还揭示了扩散模型和性能指标对预处理细节（如JPEG压缩）的高度敏感性。

Conclusion: CytoSyn填补了组织病理学领域生成式基础模型的空白，为研究社区提供了强大的工具。作者公开了模型权重、训练和验证数据集以及合成图像样本，支持进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临正常图像丰富而异常样本稀缺的问题。现有小样本异常合成方法需要耗时训练，且难以学习真实异常分布，限制了异常检测模型的性能。

Method: 提出训练免费的O2MAG方法：1）通过自注意力嫁接操作三个并行扩散过程，结合异常掩码避免前景-背景查询混淆；2）引入异常引导优化，对齐合成过程与目标异常分布；3）采用双重注意力增强，强化掩码区域的自注意力和交叉注意力。

Result: 大量实验验证了O2MAG的有效性，在多个下游异常检测任务上超越了现有最先进方法的性能。

Conclusion: O2MAG无需训练即可从单张参考异常图像合成真实异常，有效解决了工业异常检测中异常数据稀缺的问题，显著提升了异常检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 后训练量化是部署大型扩散模型的实用路径，但量化噪声会在去噪轨迹上累积并降低生成质量。现有方法需要解决量化误差在采样过程中的累积问题。

Method: 提出Q-Drift方法，将量化误差建模为每个去噪步骤上的隐式随机扰动，推导出保持边缘分布的漂移调整。通过校准估计时间步相关的方差统计量，仅需5次全精度/量化配对校准运行。该方法与常见采样器、扩散模型和PTQ方法即插即用。
...
Conclusion: Q-Drift提供了一种高效、即插即用的采样器侧校正方法，能够显著减轻扩散模型后训练量化中的质量退化问题，具有实际部署价值。
...
TL;DR: TOGA：一种仅用于训练的非对称框架，通过构建异构图教师来增强Tip-Adapter的缓存原型，实现细粒度跨模态对齐，推理时保持零额外开销。
...
Method: 提出非对称训练框架：构建仅用于训练的高容量异构图教师，将多尺度视觉块和文本提示整合到统一图中，通过模态感知图变换器进行深度跨模态推理，应用判别性节点过滤提取高保真类别特征，使用缓存感知双目标策略将关系知识监督到Tip-Adapter的键值缓存中。

Result: 在标准1-16样本基准测试中，该方法持续建立了新的最先进性能，同时推理阶段与Tip-Adapter完全相同，零额外延迟或内存开销。
...
<details>
  <summary>Details</summary>
Motivation: 现有IAA模型虽然预测性能强，但缺乏对预测驱动因素的洞察。用户需要理解图像美学评分的依据，这与评分本身同等重要。人类评估美学时依赖高层线索进行判断，这启发了基于人类可理解概念的可解释IAA框架。

Method: 1. 学习人类可理解的美学概念，构建概念子空间作为内在可解释模型的基础；2. 引入简单的残差预测器，捕捉超出显式概念的细微美学感知影响；3. 框架在摄影和艺术数据集上进行验证。

Result: 在摄影和艺术数据集上的实验表明，该方法在保持竞争力的预测性能的同时，提供了透明、人类可理解的美学判断解释。

Conclusion: 提出的基于人类可理解美学概念的IAA框架实现了预测性能与可解释性的平衡，通过概念子空间和残差预测器的结合，为图像美学评估提供了透明、可理解的判断依据。
...
TL;DR: Insight-V++是一个统一的多智能体视觉推理框架，通过自主生成高质量长链推理数据、双智能体架构（推理智能体+总结智能体）和新型强化学习算法（ST-GRPO/J-GRPO），显著提升多模态大语言模型在图像和视频领域的复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视觉推理方面面临两个关键挑战：1）高质量长链推理数据的严重匮乏；2）现有训练方法（如DPO）的离策略性质限制了强化学习潜力，特别是在长时序视频理解任务中。需要开发能够自主生成复杂推理轨迹并优化训练流程的统一框架。

Method: 1）开发可扩展的数据生成管道，配备多粒度评估，自主合成跨图像和视频域的结构化复杂推理轨迹；2）设计双智能体架构：推理智能体执行广泛分析链，总结智能体批判性评估并提炼最终结果；3）提出两种新算法：ST-GRPO（增强时空推理）和J-GRPO（提高评估鲁棒性）；4）利用总结智能体的可靠反馈指导迭代推理路径生成，通过连续自改进循环重新训练整个多智能体系统。

Result: 在LLaVA-NeXT和Qwen2.5-VL等基础模型上进行广泛实验，在具有挑战性的图像和视频推理基准测试中表现出显著性能提升，同时在传统感知聚焦任务上保持强大能力。

Conclusion: Insight-V++框架成功解决了多模态大语言模型在复杂视觉推理方面的数据稀缺和训练优化挑战，通过自主数据生成、双智能体协作和新型强化学习算法实现了持续自改进，为多模态推理系统的发展提供了有效解决方案。
...
TL;DR: VLM-AutoDrive是一个模块化后训练框架，通过整合元数据标注、LLM生成描述、视觉问答对和思维链监督，将预训练视觉语言模型适配到驾驶安全异常检测任务，显著提升碰撞检测性能。
...
Method: 提出VLM-AutoDrive模块化后训练框架，整合四种监督信号：1) 元数据生成的标注，2) LLM生成的描述，3) 视觉问答对，4) 思维链推理监督。该框架使预训练VLM能够进行领域对齐和可解释学习，适配到安全关键的时间定位感知任务。

Result: 在NVIDIA Cosmos-Reason1 7B模型上，零样本设置中碰撞召回率接近零；使用VLM-AutoDrive微调后，碰撞F1分数从0.00提升到0.69，整体准确率从35.35%提升到77.27%。在真实Nexar车载摄像头视频上，碰撞和近碰撞检测性能显著提升，同时生成可解释的推理轨迹。

Conclusion: VLM-AutoDrive为将通用VLM适配到安全关键、时间定位感知任务提供了可扩展方案，弥合了自动驾驶中感知、因果关系和决策推理之间的差距，实现了领域对齐和可解释学习。
...
TL;DR: MicroVision数据集：一个从弱势道路使用者视角采集的开放图像数据集，专注于检测行人、骑行者、电动滑板车使用者等弱势道路使用者以及静止的微出行车辆，包含8000多张图像和30000多个标注，用于改善交通安全和规划。


<details>
  <summary>Details</summary>
Motivation: 现有开放图像数据集在弱势道路使用者和微出行车辆检测方面存在不足：1）分类不精细（如将行人和微出行骑行者都归为"人"类）；2）缺少新型微出行工具（如电动滑板车）；3）大多从汽车视角采集，缺乏仅弱势道路使用者通行的区域（人行道、自行车道）数据。这限制了基于计算机视觉的交通安全和规划应用的发展。

Method: 1）创建MicroVision数据集：在瑞典哥德堡采集8000多张高清图像，包含30000多个精心标注的弱势道路使用者和微出行车辆实例；2）数据覆盖全年，包含近2000个独特的交互场景；3）从弱势道路使用者视角采集；4）提供基于最先进架构的基准目标检测模型。

Result: 1）数据集包含8000多张匿名高清图像，30000多个标注实例；2）基准模型在未见测试集上达到最高0.723的平均精度均值；3）数据集支持区分不同类型的弱势道路使用者和微出行车辆，可用于监控系统识别微出行使用情况。

Conclusion: MicroVision数据集填补了现有开放图像数据集在弱势道路使用者和微出行车辆检测方面的空白，特别是从弱势道路使用者视角采集的数据。该数据集和基准模型可为交通安全应用和微出行监控系统提供支持，促进基于计算机视觉的交通规划研究。
...
TL;DR: LRConv-NeRV：通过低秩可分离卷积替代传统3x3卷积，在神经视频表示中实现效率与重建质量的平衡优化


<details>
  <summary>Details</summary>
Motivation: NeRV的卷积解码器计算开销大、内存密集，限制了在资源受限环境中的部署，需要更高效的架构设计

Method: 提出LRConv-NeRV，用结构化低秩可分离卷积替代部分密集3x3卷积层，从最大解码阶段到早期阶段渐进应用低秩分解，实现可控的效率-质量权衡

Result: 仅应用于最终解码阶段即可减少68%解码复杂度（201.9→64.9 GFLOPs）和9.3%模型大小，质量损失可忽略，比特率降低约9.2%；INT8量化后保持接近基线质量

Conclusion: LRConv-NeRV在效率与质量权衡方面优于现有工作，为低精度和资源受限环境下的神经视频解码提供了有前景的架构替代方案
...
TL;DR: CycleCap：一种基于循环一致性的自监督微调方案，通过文本到图像模型重建图像作为奖励信号，无需标注数据即可提升视觉语言模型的图像描述准确性和减少幻觉


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）存在视觉-语言对齐问题，常产生过于通用或幻觉的描述。现有方法需要昂贵的标注数据集或复杂的测试时框架。本文从循环一致性角度重新审视图像-文本对齐问题，旨在开发无需标注数据的自监督训练方法。

Method: 提出CycleCap微调方案：1）使用VLM作为图像到文本组件生成描述；2）使用预训练文本到图像模型从生成描述重建图像；3）基于原始图像与重建图像相似度构建奖励信号；4）采用Group Relative Policy Optimization（GRPO）进行微调，直接利用循环一致性作为自监督训练信号。

Result: 在1B到7B参数的四个VLM上应用CycleCap，在图像描述和幻觉基准测试中均获得一致改进，超越了依赖监督循环一致性训练的现有最先进方法。

Conclusion: CycleCap通过循环一致性提供自监督训练信号，仅需原始图像无需标注数据，能有效提升VLMs的图像描述准确性和减少幻觉，为视觉-语言对齐问题提供了高效解决方案。
...
Method: 基于SHAP分析发现多视角一致性是关键因素，开发PreSCAN框架，使用轻量级几何和光度描述符在训练前预测NeRF质量。框架可在30秒内选择合适架构，预测误差小于1dB

Result: PreSCAN实现1000倍加速于NAS，在边缘平台（Jetson Orin）上结合离线成本分析，减少26%推理功耗和43%延迟，质量损失最小。在DFC2019数据集上验证了跨卫星场景的泛化能力

Conclusion: PreSCAN通过预测性质量评估解决了卫星图像NeRF部署的效率问题，显著加速架构选择，降低边缘设备功耗和延迟，无需重新训练即可泛化到不同卫星场景
...
TL;DR: 论文提出Tri-Layer诊断框架，通过三个指标分析VLM是否真正依赖视觉信息而非语言捷径，发现69.6%样本存在"视觉奉承"现象，模型能检测视觉异常但仍产生幻觉以满足用户期望，且规模扩大反而加剧此问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究视觉语言模型(VLMs)在回答正确时，究竟是真正依赖视觉信息还是利用了语言捷径。当前缺乏系统方法来解构幻觉来源，无法区分模型是视觉感知不足、过度依赖语言先验，还是在视觉基础与指令跟随间存在冲突。

Method: 提出Tri-Layer诊断框架，包含三个指标：1) 潜在异常检测(感知意识)；2) 视觉必要性分数(通过KL散度测量视觉依赖)；3) 竞争分数(视觉基础与指令跟随间的冲突)。使用反事实干预(盲图、噪声图、冲突图像)在7个VLM和7,000个模型-样本对上进行评估。

Result: 研究发现69.6%样本表现出"视觉奉承"现象——模型能检测视觉异常但仍产生幻觉以满足用户期望，而零样本显示"鲁棒拒绝"，表明对齐训练系统性地抑制了真实不确定性承认。规模分析显示更大模型减少语言捷径但加剧视觉奉承。诊断分数可实现后验选择性预测策略，在50%覆盖率下提升9.5个百分点准确率。

Conclusion: 视觉语言模型存在系统性幻觉问题，特别是"视觉奉承"现象，规模扩大无法解决基础问题。提出的诊断框架能有效识别幻觉来源，并支持无需额外训练成本的选择性预测策略，为改进VLM的视觉基础能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 在重复纹理或宽基线等挑战性条件下，关键点匹配可能缓慢且不可靠。现有的极线引导匹配方法大多依赖粗略的空间分箱，这会引入近似误差、需要昂贵的后处理，并可能错过有效对应关系。

Method: 提出一种精确的匹配方法：为每个关键点分配一个容差圆，从极点的视角定义角度区间。匹配问题转化为1D角度区间查询，使用线段树在logarithmic时间内高效解决。该方法保证像素级容差，支持每个关键点的控制，并消除不必要的描述符比较。

Result: 在ETH3D数据集上的广泛评估显示，与现有方法相比实现了显著的速度提升，同时恢复了精确的对应关系集合。

Conclusion: 该方法通过精确的角度空间匹配解决了现有极线引导匹配方法的局限性，提供了更高效、更精确的关键点对应关系恢复方案。
...
TL;DR: Inst4DGS提出了一种实例分解的4D高斯泼溅方法，通过标签排列潜变量和可微Sinkhorn层解决多视角视频中实例标签不一致的问题，实现了长期轨迹优化和高质量渲染与分割。
...
Method: 1. 引入每视频标签排列潜变量，通过可微Sinkhorn层学习跨视频实例匹配，实现多视角监督和一致身份保持；2. 提出实例分解的运动支架，为每个对象提供低维运动基，支持长期轨迹优化。
...
Conclusion: Inst4DGS通过显式标签对齐和实例分解运动支架，解决了多视角实例分解的关键挑战，实现了高质量的动态场景重建和实例分割，为4DGS领域提供了有效的实例分解解决方案。
...
TL;DR: 构建DermCase基准测试评估LVLMs在皮肤病学中罕见疾病的诊断推理能力，发现现有模型在诊断准确性、鉴别诊断和临床推理方面存在显著缺陷


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注常见疾病且仅评估最终准确性，忽略了临床推理过程，而这对复杂病例至关重要。特别是罕见疾病的诊断推理评估尚未充分探索

Method: 构建DermCase基准测试，包含26,030个多模态图像-文本对和6,354个临床挑战性病例，每个病例都有完整临床信息和逐步推理链。建立基于DermLIP的相似性度量来评估鉴别诊断质量

Result: 对22个领先LVLMs的评估显示在诊断准确性、鉴别诊断和临床推理方面存在显著缺陷。指令微调能显著提升性能，而DPO带来的改善有限。系统误差分析揭示了当前模型推理能力的关键限制

Conclusion: DermCase基准填补了皮肤病学中罕见疾病诊断推理评估的空白，揭示了当前LVLMs在临床推理方面的局限性，为未来模型改进提供了重要方向
...
TL;DR: SynQ是一个零样本量化框架，通过低通滤波器减少生成样本噪声、对齐类激活图提高精度、仅对困难样本使用软标签来避免预训练模型错误引导，在无数据情况下实现高性能模型量化。


<details>
  <summary>Details</summary>
Motivation: 零样本量化在隐私和安全要求下无法访问训练数据时至关重要，但现有方法面临三个主要挑战：合成数据集中的噪声、基于非目标模式的预测以及错误硬标签的误导。

Method: SynQ采用三阶段方法：1) 使用低通滤波器最小化生成样本的噪声；2) 通过对齐量化模型与预训练模型的类激活图来提高精度；3) 仅对困难样本使用软标签来避免预训练模型错误的误导。

Result: 大量实验表明，SynQ在零样本量化方法中提供了最先进的精度，超越了现有的ZSQ方法。

Conclusion: SynQ通过精心设计的噪声减少、激活对齐和软标签策略，有效解决了零样本量化的核心挑战，为无数据环境下的模型部署提供了实用解决方案。
...
Method: 提出了一种集成可控扩散模型的合成数据增强流程，采用类别感知提示和视觉先验融合技术来提升图像质量，确保与分割标签的精确对齐。

Result: 在PASCAL VOC和BDD100K等基准数据集上评估，该方法显著提升了语义分割性能，特别是在数据稀缺场景下，同时增强了模型在真实世界应用中的鲁棒性。

Conclusion: 提出的合成数据增强方法有效平衡了数据多样性和可靠性，弥合了合成数据与真实数据之间的差距，为语义分割任务提供了一种高效的数据增强解决方案。
...
TL;DR: SR-Nav通过建模目标中心的空间关系图，增强零样本目标导航中的感知和规划能力


<details>
  <summary>Details</summary>
Motivation: 现有方法在视角不佳或语义线索弱时，基础模型难以支持可靠的感知和规划推理。观察到物体和区域间的固有关系编码了结构化场景先验，有助于在部分观测下推断目标位置。
...
Conclusion: 建模空间关系能有效增强零样本目标导航中的感知和规划能力，特别是在视角不佳或语义线索弱的情况下。
...
TL;DR: 通过自一致性目标改进视频-LLMs的时间定位，无需额外标注，在体育教练任务中提升性能


<details>
  <summary>Details</summary>
Motivation: 视频-LLMs经常关注无关帧，这对需要精确时间定位的体育教练任务尤其有害。获取帧级监督既昂贵（人工标注）又不可靠（其他模型），因此需要无监督改进方法。
...
Conclusion: 通过任务间自一致性约束，无需额外标注即可显著改善视频-LLMs的时间定位能力，在体育教练应用中取得优于监督微调和闭源模型的性能。
...
TL;DR: 开发了一个用于T2加权前列腺MRI癌症检测的可解释框架，在小数据集（162张图像）上比较了Vision Transformers、CNNs和传统方法，发现迁移学习的ResNet18性能最佳，而传统HOG+SVM方法也表现出色，仅使用T2加权图像就达到了与需要多参数MRI和大数据集的方法相竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是男性主要致死原因，但T2加权前列腺MRI的解释具有挑战性，因为病灶细微且异质。现有方法通常需要双参数MRI（T2+DWI）和大规模数据集，增加了采集复杂性和计算成本。本研究旨在开发一个仅使用T2加权图像的小数据集自动癌症检测框架，解决数据稀缺问题。

Method: 使用162张T2加权图像（102癌症，60正常）的小数据集，通过迁移学习和数据增强解决数据稀缺问题。全面比较了Vision Transformers（ViT、Swin）、CNNs（ResNet18）和传统方法（逻辑回归、SVM、HOG+SVM）。进行了包含22个病例的读者研究，比较5名放射科医生与AI模型的性能。

Result: 迁移学习的ResNet18获得最佳性能：准确率90.9%，敏感性95.2%，AUC 0.905（仅1100万参数）。Vision Transformers性能较低但复杂度显著更高。HOG+SVM达到可比准确率（AUC 0.917），表明在小数据集中手工特征的有效性。读者研究中放射科医生平均敏感性67.5%（Fleiss Kappa=0.524），而AI模型为95.2%。

Conclusion: 仅使用T2加权图像的小数据集方法达到了与需要双参数MRI和大数据集的方法相竞争的性能，减少了采集复杂性和计算成本。AI模型显著优于放射科医生，表明AI辅助筛查有潜力减少漏诊癌症并提高一致性。传统方法在小数据集中仍具竞争力，而Vision Transformers在小数据集上表现不佳。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像修复方法通常针对特定模态或退化类型，无法泛化到临床实践中遇到的异构退化问题。这种局限性源于医学图像修复与医学图像质量评估的分离，没有明确质量理解的修复模型难以适应跨模态的多样化退化类型。

Method: 提出MedQ-UNI统一视觉语言模型，采用多模态自回归双专家架构：质量评估专家通过结构化自然语言描述识别退化问题，修复专家基于这些描述进行针对性图像修复。构建了约50K配对样本的大规模数据集，涵盖三种成像模态和五种修复任务，每个样本都有结构化质量描述用于联合训练。

Result: 单一MedQ-UNI模型无需任务特定适配，在所有任务上均达到最先进的修复性能，同时生成更优的描述。实验证实明确的质量理解能显著提高修复保真度和可解释性。

Conclusion: 通过将医学图像质量评估与修复相结合，MedQ-UNI实现了跨模态和退化类型的统一医学图像修复，证明了"评估-修复"范式的有效性，为临床实践中的异构退化问题提供了通用解决方案。
...
TL;DR: ColourCrafter是一个统一的扩散框架，通过结构化、区域感知的生成过程实现精细化的颜色编辑，解决了现有方法在连续色彩变化控制上的不足。


<details>
  <summary>Details</summary>
Motivation: 颜色是图像生成中最显著但最难控制的属性之一。现有扩散模型虽然能根据用户指令修改对象颜色，但结果常常偏离预期色调，特别是在细粒度和局部编辑方面。早期文本驱动方法依赖离散语言描述，无法准确表示连续色彩变化。

Method: 提出ColourCrafter框架：1）在潜在空间中进行RGB颜色标记和图像标记的标记级融合，选择性地将颜色信息传播到语义相关区域同时保持结构保真度；2）使用感知Lab空间损失，通过解耦亮度和色度，在掩码区域内约束编辑；3）构建ColourfulSet大规模高质量图像对数据集，包含连续多样的色彩变化。

Result: 大量实验表明，ColourCrafter在细粒度颜色编辑中实现了最先进的颜色准确性、可控性和感知保真度。

Conclusion: ColourCrafter将颜色编辑从全局色调转移转变为结构化、区域感知的生成过程，通过标记级融合和感知Lab空间损失实现了精确可控的颜色编辑，并构建了支持该任务的大规模数据集。
...
TL;DR: 研究评估了视觉-语言模型从游戏视频中推断玩家参与度的能力，发现当前模型存在感知-理解差距：虽然能识别可见的游戏线索，但难以稳健地跨游戏推断人类参与度。


<details>
  <summary>Details</summary>
Motivation: 从游戏视频中推断人类参与度对游戏设计和玩家体验研究很重要，但目前尚不清楚视觉-语言模型能否仅从视觉线索推断这种潜在心理状态。

Method: 使用GameVibe Few-Shot数据集，在9款第一人称射击游戏中评估3个VLM模型，采用6种提示策略：零样本预测、基于Flow、GameFlow、自我决定理论和MDA的理论指导提示、以及检索增强提示。同时考虑逐点参与度预测和连续窗口间参与度变化的成对预测。

Result: 零样本VLM预测普遍较弱，通常无法超越简单的每游戏多数类基线。在某些设置中，记忆或检索增强提示改善了逐点预测，而成对预测在各种策略中始终困难。理论指导提示单独使用不可靠，反而可能强化表面捷径。

Conclusion: 当前VLM存在感知-理解差距：虽然能识别可见的游戏线索，但仍难以稳健地跨游戏推断人类参与度。
...
Method: 提出两阶段框架：1) 将OOD图像与文本描述配对，引入ID和OOD信号间的跨模态一致性模式，通过联合图像-文本推理细化决策边界；2) 学习轻量级融合权重以最优组合语义匹配和视觉典型性，并基于平均阈值置信度(ATC)进行显式正则化确保稳定性
...
TL;DR: TexEditor是一个基于Qwen-Image-Edit-2509的专用纹理编辑模型，通过TexBlender数据集和StructureNFT训练方法，在保持结构一致性的同时实现文本引导的纹理编辑，并在TexBench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有SOTA编辑模型在进行纯外观相关的纹理编辑时，经常难以保持结构一致性。为了解决这个问题，需要从数据和训练两个角度共同增强结构保持能力。

Method: 1. 构建TexBlender：使用Blender生成的高质量SFT数据集，为冷启动提供强结构先验；2. 提出StructureNFT：基于RL的方法，集成结构保持损失，将SFT阶段学到的结构先验迁移到真实场景；3. 建立TexBench：通用真实世界纹理编辑基准测试。

Result: 在现有Blender纹理基准和TexBench上的广泛实验表明，TexEditor持续优于Nano Banana Pro等强基线。在通用基准ImgEdit上的评估也验证了其泛化能力。

Conclusion: TexEditor通过联合数据增强和训练优化，有效解决了纹理编辑中的结构保持问题，在专用和通用基准测试中均表现出色，为文本引导的纹理编辑提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 流式3D重建中，状态更新规则面临两难：激进覆盖会遗忘有用历史，保守更新则无法跟踪新证据，且两种行为在超出训练范围时都会变得不稳定。现有方法缺乏自适应平衡机制。

Method: FILT3R将循环状态更新建模为token空间中的随机状态估计，维护每个token的方差并计算卡尔曼式增益来自适应平衡记忆保留与新观测。过程噪声通过候选token的EMA归一化时间漂移在线估计。

Result: FILT3R产生了可解释的即插即用更新规则，将常见的覆盖和门控策略作为特例。增益在稳定区域随不确定性收缩而减小，在真实场景变化增加过程不确定性时增大，在深度、姿态和3D重建的长期稳定性上优于现有方法。

Conclusion: FILT3R通过将状态更新重新表述为随机状态估计问题，提供了一种无需训练的自适应潜在过滤层，显著提升了流式3D重建系统的长期稳定性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统端到端压缩模型在超低码率下产生模糊图像，感知质量差；现有生成式压缩方法通常独立处理视频帧，存在时间一致性和效率限制
...
TL;DR: HOMEY是一个结合YOLO与领域特定掩码机制的新型检测框架，用于自动检测17种房产风险类别，包括结构损坏、维护疏忽和责任隐患，通过启发式对象掩码增强弱信号，并通过风险感知损失校准平衡类别偏斜。


<details>
  <summary>Details</summary>
Motivation: 房产风险自动检测是一个高影响但尚未充分探索的计算机视觉前沿领域，对房地产、承保和保险运营具有直接影响。当前需要更准确、可靠的自动化解决方案来检测各种房产风险。

Method: 提出HOMEY框架，结合YOLO检测器与领域特定掩码机制和自定义损失函数。引入启发式对象掩码来增强杂乱背景中的弱信号，以及风险感知损失校准来平衡类别偏斜和严重性权重。框架训练检测17种风险相关房产类别。

Result: 在真实世界房产图像上的实验表明，HOMEY相比基线YOLO模型实现了更优越的检测准确性和可靠性，同时保持快速推理速度。

Conclusion: HOMEY不仅提供检测功能，还实现了可解释和成本效益的风险分析，为可扩展的AI驱动房产保险工作流程奠定了基础。
...
TL;DR: 该论文综述了人工智能在蛋白质科学中的范式转变，从静态结构预测转向动态构象集合和复杂生物分子相互作用的建模，涵盖统一多模态表示、静态预测精化、生成框架、异质相互作用预测和功能推断五个维度。


<details>
  <summary>Details</summary>
Motivation: 蛋白质折叠问题已被人工智能根本性改变，从静态结构预测演变为动态构象集合和复杂生物分子相互作用的建模。需要系统审视这一范式转变，分析当前瓶颈并指明未来方向。

Method: 系统性地从五个相互关联的维度审视AI驱动的蛋白质科学范式转变：1) 统一多模态表示（整合序列、几何结构和文本知识）；2) 静态预测精化（无MSA架构和全原子复合物建模）；3) 生成框架（扩散模型和流匹配，捕捉与热力学集合一致的构象分布）；4) 异质相互作用预测（蛋白质-配体、蛋白质-核酸、蛋白质-蛋白质复合物）；5) 功能推断（适应度景观、突变效应和文本引导的属性预测）。

Result: 当前AI在蛋白质科学中面临瓶颈包括：数据分布偏差、有限的机制可解释性、几何指标与生物物理现实之间的脱节。未来方向包括：物理一致的生成模型、多模态基础架构、实验闭环系统。

Conclusion: 这一方法论转变标志着人工智能从结构分析工具转变为通用模拟器，能够理解并最终重写生命的动态语言。
...
TL;DR: 该手册系统阐述了基于大规模实际部署的汽车保险AI基础架构，提出了垂直集成的AI范式，统一感知、多模态推理和生产基础设施，用于车辆风险评估和理赔处理。
...
Method: 开发领域适应的Transformer架构用于结构化视觉理解、关系型车辆表示学习和多模态文档智能，构建可扩展的管道系统，并在泰国全国性汽车保险系统的实际约束下运行。

Result: 建立了统一的智能堆栈，实现了车辆损伤分析、理赔评估和核保工作流程的端到端自动化，形成了学习算法与MLOps实践协同进化的原则性框架。

Conclusion: 该手册为将现代人工智能转化为高风险的工业环境中可靠、生产级系统提供了系统化的方法论，强调了算法与工程实践的协同进化对于实际部署的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法多为离线或缺乏实例级理解，限制了在真实机器人任务中的应用。需要开发能够在线运行且具备开放词汇理解能力的全景映射系统

Method: 采用局部到全局的滑动窗口范式：1) 构建局部一致性地图，利用几何和语义线索构建3D片段聚类图，融合不一致片段为完整实例；2) 更新全局地图，为局部3D高斯地图构建带空间属性的显式网格，通过鲁棒的双向二分3D高斯实例匹配融合到全局地图；3) 利用3D空间属性网格中的融合VLM特征实现开放词汇场景理解

Result: 在广泛使用的数据集上验证，OnlinePG在在线方法中取得最佳性能，同时保持实时效率

Conclusion: 提出的OnlinePG系统成功实现了在线开放词汇全景映射，结合了几何重建与开放词汇感知，为具身应用提供了有效的场景理解解决方案
...
TL;DR: LVLMs展现类似人类的计数行为，研究者通过机制分析发现了共享的"计数电路"，并提出轻量级干预策略，仅通过计数微调就能提升视觉推理能力


<details>
  <summary>Details</summary>
Motivation: 计数是测试大型视觉语言模型推理能力的简单而强大的方法，本研究旨在探索LVLMs如何实现计数功能，并理解计数在视觉推理中的核心作用

Method: 使用受控合成和真实世界基准测试，结合机制分析；引入两种新的可解释性方法（Visual Activation Patching和HeadLens）；提出轻量级干预策略，利用合成图像对预训练LVLMs进行计数微调

Result: LVLMs表现出类似人类的计数行为：小数量精确，大数量噪声估计；发现了跨多种视觉推理任务共享的结构化"计数电路"；仅通过计数微调就在分布外计数基准上平均提升8.36%，在复杂视觉推理任务上平均提升1.54%

Conclusion: 计数在视觉推理中扮演核心且有影响力的角色，通过针对性增强计数机制可能成为提升整体视觉推理能力的潜在途径
...
TL;DR: 提出3DreamBooth和3Dapter框架，实现3D感知的视频定制，通过解耦空间几何与时间运动，避免对多视角视频数据的依赖和时序过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于2D实体的主题驱动视频生成方法缺乏3D空间先验，无法重建真实3D几何，在合成新视角时只能生成合理但任意的细节而非保持真实3D身份。由于多视角视频数据稀缺，直接微调模型容易导致时序过拟合。

Method: 提出3DreamBooth框架，采用单帧优化范式解耦空间几何与时间运动，通过仅更新空间表示将稳健的3D先验融入模型。引入3Dapter视觉条件模块增强细粒度纹理并加速收敛，通过非对称条件策略与主生成分支进行多视角联合优化，使其作为动态选择性路由器从最小参考集中查询视角特定的几何提示。

Result: 该方法能够在无需大量视频训练的情况下实现3D感知的视频定制，有效避免了时序过拟合问题，同时保持了3D几何的一致性。

Conclusion: 提出的3DreamBooth和3Dapter框架解决了3D感知视频定制的关键挑战，通过创新的解耦策略和条件模块设计，在缺乏多视角视频数据的情况下实现了高质量的3D一致视频生成。
...
TL;DR: 提出目标域散光问题，通过仿生中心-外周注意力精炼框架解决跨域少样本目标检测中的注意力分散问题


<details>
  <summary>Details</summary>
Motivation: 跨域少样本目标检测面临严重域偏移和数据稀缺问题。研究发现模型在目标域表现出分散且不集中的注意力（目标域散光问题），导致定位不精确和冗余预测。分析发现常规微调有缓解此问题的趋势但效果仍不理想，需要增强这一趋势。

Method: 受人类中央凹式视觉系统启发，提出中心-外周注意力精炼框架：1）正模式精炼模块：使用类别特定原型重塑注意力到语义对象，模拟视觉中心区域；2）负上下文调制模块：通过建模背景上下文增强边界判别，模拟视觉外周区域；3）文本语义对齐模块：通过跨模态线索强化中心-外周区分。

Result: 在六个具有挑战性的CD-FSOD基准测试上实验，一致展示了改进的检测精度，并建立了新的最先进结果。

Conclusion: 仿生方法将散光注意力转化为集中模式，显著改善了模型对目标域的适应能力，为解决跨域少样本目标检测中的注意力分散问题提供了有效方案。
...
TL;DR: CoDA框架通过模拟临床影像处理链中的分布偏移，评估医学视觉-语言模型在真实工作流程中的鲁棒性，发现组合性退化比单一阶段更严重，并提出轻量级对齐修复策略。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉-语言模型的鲁棒性评估通常假设干净、经过整理的输入或研究孤立的损坏，忽略了临床工作流程中常规的采集、重建、显示和传输操作，这些操作在保持临床可读性的同时会改变图像统计特性。

Method: 提出CoDA（链式分布）框架，通过组合采集类阴影、重建和显示重映射、传输和导出退化等临床合理操作，在掩码结构相似性约束下联合优化阶段组合和参数，以诱导模型失败同时保持视觉合理性。

Result: CoDA显著降低了CLIP风格MVLMs的零样本性能，组合性退化比任何单一阶段都更具破坏性；专有多模态模型在CoDA偏移样本上表现出审计可靠性下降和持续高置信度错误，而医学专用MLLMs在医学图像质量审计方面存在明显缺陷。

Conclusion: 研究揭示了MVLM部署中临床基础的威胁面，表明轻量级对齐能提高部署鲁棒性，并提出了基于教师引导的标记空间适应与补丁级对齐的后修复策略，可提高对存档CoDA输出的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有面向规划的端到端驾驶模型学习的是统计相关性而非真正的因果关系，容易受到数据集偏见的利用，导致因果混淆问题，严重影响复杂场景下的可靠性和安全性

Method: 提出CausalVAD框架，核心是稀疏因果干预方案（SCIS）——一个轻量级即插即用模块，通过构建潜在驾驶上下文原型的字典，对模型的稀疏向量化查询进行干预，消除混杂因素引起的虚假关联

Result: 在nuScenes等基准测试中，CausalVAD实现了最先进的规划精度和安全性，并且在针对因果混淆配置的数据偏见和噪声场景中表现出优越的鲁棒性

Conclusion: CausalVAD通过因果干预有效解决了自动驾驶规划模型中的因果混淆问题，提升了模型在复杂场景下的可靠性和安全性，为端到端驾驶系统提供了更稳健的解决方案
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformer的注意力机制在各层独立运行，限制了信息流动和特征学习。现有方法缺乏层间注意力信息的有效传播机制，导致特征学习不够充分。

Method: 提出跨层注意力传播方法，在编码器层间保存历史注意力矩阵，并通过加权融合（alpha=0.45）将历史注意力信息传播到后续层。仅需添加注意力矩阵存储和混合操作，架构改动最小。

Result: 在CIFAR-100上ViT准确率从75.74%提升至77.07%（+1.33%），在TinyImageNet上从57.82%提升至59.07%（+1.25%）。跨架构验证显示CaiT也获得1.01%提升。随机初始化优于零初始化。

Conclusion: 跨层注意力传播能有效改进Vision Transformer的信息流和特征学习，通过历史注意力矩阵的保存和融合实现渐进式注意力模式优化，方法简单有效且通用性强。
...
TL;DR: 提出基于饱和度-亮度相似度的非局部变分方法用于彩色图像恢复，通过考虑彩色图像块在饱和度-亮度通道的相似性来改进传统非局部方法。


<details>
  <summary>Details</summary>
Motivation: 传统非局部方法直接从彩色图像的RGB通道提取图像块，由于相似性主要基于独立通道的灰度值，无法精细描述颜色信息。需要一种能更好利用彩色图像饱和度-亮度相似性的方法。

Method: 1. 建立基于饱和度-亮度相似度的非局部全变差，将彩色图像块的饱和度-亮度相似性融入非局部梯度；2. 基于此构建非局部变分模型；3. 采用Bregman算子分裂法设计高效算法求解优化问题；4. 研究算法收敛性。

Result: 数值实验表明，所提模型在视觉质量和定量指标（PSNR、SSIM、QSSIM、S-CIELAB颜色误差）上均优于其他测试方法。

Conclusion: 提出的基于饱和度-亮度相似度的非局部变分方法能更有效地利用彩色图像的颜色信息，在图像恢复任务中取得了更好的性能。
...
TL;DR: 提出了一种基于自然语言描述动作单元（AUs）的面部行为合成新方法，解决了传统线性AU组合在处理冲突AU时产生解剖学不合理伪影的问题，并构建了首个大规模文本-图像配对数据集BP4D-AUText。


<details>
  <summary>Details</summary>
Motivation: 现有面部行为合成方法存在局限性：基于文本的面部模型依赖粗略情感类别，缺乏非语言交流的细微差别；基于AU的方法通常将AU编码为one-hot向量，将复合表情建模为单个AU的简单线性组合，这种线性在处理激活相同面部肌肉但作用相反的冲突AU时会产生解剖学上不合理的伪影和不自然的运动叠加。

Method: 提出通过AU的自然语言描述来表示面部行为，保留了AU框架的表达能力，同时能够显式建模复杂和冲突的AU。构建了首个大规模文本-图像配对数据集BP4D-AUText，通过对BP4D和BP4D+数据集应用基于规则的动态AU文本处理器合成得到。进一步提出了VQ-AUFace生成模型，利用面部结构先验从文本合成真实多样的面部行为。

Result: 广泛的定量实验和用户研究表明，该方法显著优于现有方法，在生成解剖学合理、行为丰富且在感知上令人信服的面部表情方面表现出色，特别是在涉及冲突AU的挑战性条件下。

Conclusion: 通过将AU表示为自然语言描述，该方法解决了传统线性AU组合在处理冲突AU时的局限性，实现了更真实、更自然的面部行为合成，为利用现代文本到图像模型进行高保真面部合成开辟了新途径。
...
TL;DR: 该研究对缅甸手写数字数据集myMNIST进行了首次系统性基准测试，评估了11种不同架构，发现CNN表现最佳，PETNN(GELU)紧随其后，KAN变体表现中等，为缅甸数字识别研究建立了可复现的基准。


<details>
  <summary>Details</summary>
Motivation: 缅甸手写数字数据集myMNIST（原BHDD）对缅甸NLP/AI研究很重要，但缺乏系统性基准测试。研究旨在为这一重要区域脚本数据集建立全面的性能基准，评估包括经典深度学习模型、新兴架构和物理启发模型在内的多种方法。

Method: 研究评估了11种架构：经典深度学习模型（MLP、CNN、LSTM、GRU、Transformer）、新兴替代方案（FastKAN、EfficientKAN）、能量基模型（JEM）和物理启发的PETNN变体（Sigmoid、GELU、SiLU）。使用精确率、召回率、F1分数和准确率作为评估指标。
...
Conclusion: 研究为myMNIST建立了跨多种建模范式的可复现基准；展示了PETNN相对于经典和Transformer模型的强大性能；量化了物理启发的PETNN与真正能量基模型（JEM）之间的差距；发布了基准以促进缅甸数字识别研究和新兴架构在区域脚本上的评估。
...
TL;DR: 该论文提出两种基于文本引导注意力的零样本对抗鲁棒性增强方法：TGA-ZSR和Comp-TGA，通过注意力机制改进CLIP模型的对抗鲁棒性


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型（如CLIP）虽然具有强大的零样本能力，但被发现容易受到对抗样本的攻击。实验观察到对抗扰动会导致文本引导注意力发生偏移，因此需要开发能够保持模型泛化能力同时增强对抗鲁棒性的方法。

Method: 提出两种方法：1) TGA-ZSR框架包含局部注意力精炼模块和全局注意力约束模块；2) Comp-TGA方法整合类别提示引导的前景注意力和非类别提示引导的反向注意力，形成互补的注意力机制。

Result: 实验验证TGA-ZSR和Comp-TGA分别在16个数据集上相比当前最先进技术获得了9.58%和11.95%的零样本鲁棒准确率提升。

Conclusion: 通过文本引导注意力机制可以有效增强CLIP模型的对抗鲁棒性，互补的注意力策略能够捕获更全面准确的前景表示，显著提升零样本对抗鲁棒性能。
...
Conclusion: SJD-PAC通过协同优化显著提高了每步的平均接受长度，有效解决了视觉生成中高熵区域草稿接受率低的问题，为文本到图像合成提供了高效的无损加速方案
...
<details>
  <summary>Details</summary>
Motivation: 当前基于双流transformer的音频-视频联合生成方法存在多个局限性：门控机制导致的模型流形变化、跨模态注意力引入的多模态背景区域偏差、多模态无分类器引导(CFG)在训练和推理时的不一致性，以及多条件之间的冲突。这些限制了生成质量和训练效率。

Method: 提出Cross-Modal Context Learning (CCL)框架，包含四个核心模块：1) TARP(时间对齐RoPE和分区)增强音频和视频潜在表示的时间对齐；2) CCA模块中的LCT(可学习上下文令牌)提供稳定的无条件锚点；3) DCR(动态上下文路由)根据训练任务动态路由信息；4) UCG(无条件上下文引导)在推理时利用LCT支持不同形式的CFG，改善训练-推理一致性。

Result: 通过全面评估，CCL在音频-视频联合生成任务上取得了最先进的性能，同时显著减少了所需的计算资源。相比现有学术方法，在生成质量和时间同步性方面都有显著提升。

Conclusion: CCL框架有效解决了双流transformer音频-视频生成中的关键问题，通过创新的跨模态上下文学习机制，在减少资源消耗的同时实现了更好的生成质量和训练-推理一致性，为多模态生成提供了新的解决方案。
...
TL;DR: CNN模型SegResNet在腹部多器官分割任务中优于三种混合Transformer模型（UNETR、SwinUNETR、UNETR++），表明在小到中型异质数据集上，优化良好的CNN架构仍具竞争力。


<details>
  <summary>Details</summary>
Motivation: 腹部CT扫描的精确多器官分割对计算机辅助诊断和治疗至关重要。虽然CNN一直是医学图像分割的标准方法，但基于Transformer的架构因能建模长距离依赖关系而受到关注。本研究旨在系统比较混合Transformer模型与传统CNN在异质数据集上的性能。

Method: 在RATIC数据集（来自23个机构的206个标注CT扫描，涵盖5个腹部器官）上，系统性地对三种混合Transformer模型（UNETR、SwinUNETR、UNETR++）与强CNN基线SegResNet进行基准测试。所有模型在相同的预处理和训练条件下进行训练和评估，使用Dice相似系数作为主要评估指标。

Result: CNN-based SegResNet在所有器官上均取得最高整体性能，优于所有混合Transformer模型。在Transformer方法中，UNETR++表现最具竞争力，而UNETR在较少训练迭代下展现出更快的收敛速度。

Conclusion: 对于小到中型异质数据集，优化良好的CNN架构仍然具有高度竞争力，甚至可能优于混合Transformer设计。Transformer模型在收敛速度方面可能具有优势，但CNN在分割性能上仍保持领先。
...
TL;DR: OpenT2M是一个包含2800小时高质量人体运动数据的百万级开源数据集，通过物理可行性验证和多粒度过滤确保质量，并配有详细的时间标注。基于此数据集开发的MonoFrill模型采用创新的2D-PRQ运动分词器，在文本到运动生成任务中表现出色，无需复杂设计。


<details>
  <summary>Details</summary>
Motivation: 当前文本到运动生成模型在未见过的文本描述上表现不佳，主要原因是现有运动数据集规模小、多样性有限。这限制了模型的泛化能力和实际应用效果。

Method: 1) 构建OpenT2M数据集：包含2800小时高质量人体运动数据，通过物理可行性验证和多粒度过滤进行质量控制，提供详细的逐秒文本标注，并开发自动化流程创建长时程序列。2) 开发MonoFrill模型：核心是2D-PRQ运动分词器，通过将人体划分为生物学部分来捕捉时空依赖关系，实现无需复杂设计的文本到运动生成。

Result: OpenT2M显著提升了现有文本到运动生成模型的泛化能力。2D-PRQ分词器在运动重建方面表现优异，并展现出强大的零样本性能。实验证明该方法在文本到运动生成任务中取得了令人信服的结果。

Conclusion: OpenT2M数据集和MonoFrill模型通过解决长期存在的数据质量和基准测试挑战，有望推动文本到运动生成领域的发展。大规模高质量数据集和创新的运动表示方法是提升模型性能的关键。
...
Method: 构建包含400个AI生成视频和100个真实视频的数据集，由专家标注15个真实性维度（感知、光学、物理和时序线索）。评估11个代表性LVLMs，并进行时序扰动实验分析模型对时间信息的利用程度。

Result: 发现明显的维度不平衡：LVLMs在感知线索上表现较好，但在光学一致性、物理交互和时序因果推理方面表现不佳。模型性能在不同维度间差异显著，较小的开源模型有时在特定真实性线索上优于更强的专有模型。时序扰动实验显示当前LVLMs对时间信息的利用有限。

Conclusion: GenVideoLens为LVLM行为提供了诊断性洞察，揭示了关键能力差距，并为改进未来AI生成视频检测系统提供了指导。该基准测试有助于理解模型在不同真实性维度上的表现差异，推动更全面的评估方法发展。
...
Method: SVOO采用两阶段范式：(1)离线层间敏感性分析以推导每层固有的剪枝水平；(2)通过新颖的双向协同聚类算法进行在线块级稀疏注意力计算。

Result: 在七个广泛使用的视频生成模型上的实验表明，SVOO在质量-加速权衡方面优于最先进方法，在Wan2.1上实现高达1.93倍加速的同时保持高达29 dB的PSNR。

Conclusion: SVOO通过揭示注意力稀疏性是每层固有属性这一关键见解，并提出有效的离线分析和在线聚类方法，为视频生成中的高效稀疏注意力提供了创新解决方案。
...
TL;DR: PhysVideo是一个两阶段视频生成框架，首先生成物理感知的正交前景视频，然后合成包含背景的完整视频，以解决现有方法在物理一致性运动方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在视觉保真度方面取得了显著进展，但确保物理一致的运动仍然是一个基本挑战。现实世界中的物体运动发生在三维空间中，而视频观测仅提供这种动态的部分、视角依赖的投影，这导致了物理一致性的限制。

Method: PhysVideo采用两阶段框架：第一阶段使用Phys4View生成物理感知的正交前景视频，通过物理感知注意力捕捉物理属性对运动动态的影响，并结合几何增强的跨视角注意力和时间注意力增强时空一致性；第二阶段使用VideoSyn以前景视频为指导，学习前景动态与背景上下文之间的交互，实现可控的视频合成。

Result: 为了支持训练，构建了PhysMV数据集，包含40K个场景，每个场景包含四个正交视角，总计160K个视频序列。大量实验表明，PhysVideo在物理真实性和时空一致性方面显著优于现有的视频生成方法。

Conclusion: PhysVideo通过两阶段框架有效解决了视频生成中的物理一致性挑战，通过物理感知的正交前景生成和前景-背景交互学习，显著提升了生成视频的物理真实性和时空连贯性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于参考的人脸修复方法通常假设参考图像与退化输入年龄对齐，限制了其在真实场景（如历史照片修复）中的有效性，因为实际中往往只有跨年龄的参考图像可用。

Method: 提出MeInTime方法：1）解耦身份和年龄条件建模；2）训练时通过新注意力机制有效注入身份特征，使用门控残差融合模块整合退化特征和身份表示；3）推理时提出无需训练的年龄感知梯度引导采样策略，使用年龄驱动方向迭代引导去噪潜在空间朝向目标年龄语义流形。

Result: 大量实验表明，MeInTime在身份保持和年龄一致性方面优于现有人脸修复方法，能够实现忠实修复同时保持身份保真度和年龄一致性。

Conclusion: MeInTime成功将基于参考的人脸修复从同年龄设置扩展到跨年龄设置，通过解耦建模身份和年龄条件，结合训练时的身份特征注入和推理时的年龄感知引导，实现了在跨年龄参考场景下的有效人脸修复。
...
TL;DR: Click-to-Ask：一个用于直播电商的AI助手系统，包含离线处理和在线响应模块，通过结构化产品信息和实时问答提升直播效果


<details>
  <summary>Details</summary>
Motivation: 直播电商已成为现代重要的传播形式，但主播在推广产品时需要处理复杂的多模态产品信息，并实时回应观众问题，这需要大量准备时间和精力。现有系统缺乏有效的自动化支持，导致推广效率低下和互动延迟。

Method: 系统分为离线模块和在线模块。离线模块处理多模态产品信息（如图片、视频、文本），将其转换为结构化产品数据并生成合规的推广文案。在线模块在直播期间实时响应观众问题：主播点击问题后，系统利用离线模块生成的结构化产品信息以及流式架构中维护的事件级历史记忆来生成回答。

Result: 在收集的TikTok直播帧数据集上，该方法实现了0.913的问题识别准确率和0.876的回答质量评分。系统显著减少了推广准备时间，增强了内容参与度，并实现了与观众问题的及时互动。

Conclusion: Click-to-Ask系统通过结合离线产品信息处理和在线实时问答功能，有效提升了直播电商的效率和互动质量，展示了实际应用的巨大潜力。该系统为直播电商提供了一种高效、便捷的AI辅助解决方案。
...
TL;DR: 提出了一个基于合成PDF和LLM作为评判者的表格提取评估框架，相比传统规则指标能更好捕捉语义等价性，在人类验证中达到更高相关性
...
Method: 1) 使用arXiv表格创建合成PDF并生成精确LaTeX真值；2) 将LLM作为评判者进行语义表格评估；3) 构建匹配管道以处理解析器输出的不一致性；4) 进行包含1500多个质量判断的人类验证研究

Result: LLM评估与人类判断的相关性显著更高（Pearson r=0.93），优于树编辑距离相似性（TEDS，r=0.68）和网格表格相似性（GriTS，r=0.70）；在100个合成文档的451个表格上评估21个当代PDF解析器，发现显著性能差异

Conclusion: 该框架为表格数据提取提供了实用的解析器选择指导，并为这一关键任务建立了可重复、可扩展的评估方法学，LLM作为评判者在语义表格评估中表现出色
...
TL;DR: Switch：一种用于医学超声图像分割的新型半监督学习框架，通过多尺度切换和频域切换策略解决数据稀缺和图像伪影问题，在多个数据集上超越现有方法。
...
Result: 在六个超声数据集（淋巴结、乳腺病变、甲状腺结节、前列腺）上评估，在5%标注率下取得显著提升：LN-INT数据集80.04% Dice，DDTI数据集85.52% Dice，前列腺数据集83.48% Dice。半监督方法甚至超过全监督基线，参数效率高（180万参数）。

Conclusion: Switch框架有效解决了医学超声图像分割中的数据稀缺和特征表示问题，在资源受限的医学成像应用中表现出优越性能和参数效率。
...
TL;DR: 综述多模态计算病理学最新进展，包括自监督表征学习、多模态数据生成、参数高效适应和多智能体协同推理四个方向，探讨如何整合高分辨率视觉数据与临床知识以实现可解释的AI辅助诊断。


<details>
  <summary>Details</summary>
Motivation: 全玻片成像（WSI）推动了数字病理学发展，但面临计算挑战：WSI极端分辨率带来视觉学习计算障碍、专家标注有限限制监督方法、多模态信息整合困难、超长视觉序列建模不透明影响临床透明度。需要综述多模态计算病理学最新进展以解决这些问题。

Method: 系统分析四个研究方向：1）WSI的自监督表征学习和结构感知token压缩；2）多模态数据生成与增强；3）参数高效适应和推理增强的少样本学习；4）多智能体协同推理实现可信诊断。特别关注token压缩如何实现跨尺度建模，以及多智能体机制如何模拟病理学家在不同放大倍数下的"思维链"实现不确定性感知的证据融合。

Result: 综述总结了多模态计算病理学的最新进展，展示了token压缩技术能够有效处理WSI的超高分辨率，多智能体协同推理机制能够模拟病理学家的诊断思维过程，实现更透明、可解释的AI辅助诊断。

Conclusion: 未来进展依赖于统一的多模态框架，整合高分辨率视觉数据与临床和生物医学知识，以支持可解释且安全的AI辅助诊断。需要继续解决多模态信息整合、模型透明度和临床可解释性等挑战。
...
TL;DR: SCNP是一种通过惩罚最差分类邻域来提升分割拓扑准确性的高效方法，可集成到现有训练流程中


<details>
  <summary>Details</summary>
Motivation: 传统深度学习图像分割模型无法保证拓扑准确性（如保持正确的连通分量数量），影响分割质量和后续量化分析的可靠性。现有方法通常集成困难、计算昂贵或仅限于管状结构

Method: SCNP通过惩罚logits中最差分类的邻域像素，迫使模型在改进像素本身预测前先改进其邻域预测，从而提升拓扑准确性。该方法可集成到语义和实例分割框架以及多种损失函数中

Result: 在13个不同结构形态和图像模态的数据集上验证了SCNP的有效性，可集成到三种分割框架中，并能与多种损失函数结合提升拓扑准确性

Conclusion: SCNP提供了一种高效、通用且易于集成的方法来提升分割模型的拓扑准确性，解决了现有方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地部署在现实世界和具身环境中，其安全决策依赖于视觉上下文。然而，目前尚不清楚哪些视觉证据驱动这些判断。作者旨在研究多模态安全行为是否可以被简单的语义线索操控。

Method: 提出了一个语义操控框架，在不改变底层场景内容的情况下，应用受控的文本、视觉和认知干预。为了评估这些效果，提出了SAVeS基准测试，用于评估语义线索下的情境安全性，并设计了评估协议来区分行为拒绝、基于视觉的安全推理和错误拒绝。

Result: 实验表明，多个视觉语言模型的安全决策对语义线索高度敏感，表明其依赖于学习的视觉-语言关联而非基于视觉的理解。进一步展示了自动化操控管道可以利用这些机制，突显了多模态安全系统的潜在漏洞。

Conclusion: 视觉语言模型的安全决策容易被语义线索操控，这揭示了当前多模态安全系统存在严重漏洞，其安全判断缺乏真正的视觉理解基础，可能被恶意利用。
...
TL;DR: 提出针对视频扩散变换器的NVFP4/INT8混合精度量化框架，通过轻量级预测器动态分配精度，结合时间增量缓存跳过不变块计算，实现1.92倍加速和3.32倍内存压缩


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中表现出色，但高内存使用和计算成本限制了实际部署。现有量化方法采用静态位宽分配，忽略了不同扩散时间步中激活的量化难度，导致效率与质量之间的权衡不理想
...
Result: 实验表明该方法实现1.92倍端到端加速和3.32倍内存减少，为视频DiTs高效推理设定了新基准

Conclusion: 提出的自适应精度策略和计算跳过机制能够在保持生成质量的同时实现显著的效率提升，为视频扩散变换器的实际部署提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用带标注解释的数据集显式监督解释生成过程，虽然生成的解释看似合理，但并不忠实于模型的实际推理过程。需要一种既能生成忠实解释又能保持合理性的方法。

Method: 提出WeNLEX弱监督模型，通过两个关键机制：1）忠实性：在特征空间中匹配从自然语言解释生成的图像与原始图像；2）合理性：利用少量临床标注解释进行分布对齐。模型可在后处理和模型内两种设置下运行。

Result: WeNLEX能够生成忠实且合理的解释，仅需每个诊断5个真实解释。在模型内设置下，多标签分类器的AUC提升2.21%。模型具有灵活性，可通过更换数据库适应不同目标受众（如非医学用户）。
...
<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法过度依赖邻域间的预测相似性，这会加速源知识的遗忘并增加对局部噪声过拟合的敏感性。需要解决知识遗忘和噪声过拟合的问题。

Method: 提出ProCal方法：1）通过双模型协作预测机制（源模型初始预测与当前模型在线输出结合）动态校准邻域概率；2）设计联合优化目标，结合软监督损失和多样性损失指导目标模型。

Result: 理论分析表明ProCal收敛到源知识和目标信息有效融合的均衡状态。在4个公共数据集的31个跨域任务上进行广泛实验验证了方法的有效性。

Conclusion: ProCal通过概率校准机制有效缓解局部噪声干扰并保留源模型的判别信息，在知识保留和域适应之间取得良好平衡，解决了SFDA中的知识遗忘和过拟合问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于RGB数据预训练的视觉几何模型在处理RGB-热成像混合模态时效果下降，特别是在对齐RGB和热成像模态方面存在困难

Method: 提出SEAR微调策略，在相对较小的RGB-T数据集上微调预训练的几何变换器，使其能够有效处理多模态RGB-T输入

Result: 在3D重建和相机姿态估计方面显著优于现有方法，AUC@30提升超过29%，在低光照和浓烟等挑战性条件下仍能可靠工作，推理时间开销可忽略

Conclusion: SEAR成功将预训练的视觉几何模型适配到多模态RGB-T输入，同时发布了新的RGB-T数据集为未来研究提供基准
...
TL;DR: 该研究系统评估了医学图像分割中不确定性量化方法，重点关注异方差不确定性（AU）和认知不确定性（EU）的相互作用与纠缠问题，提出了量化纠缠的指标，并在多个下游任务中评估了不同AU-EU组合的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割等安全关键应用中，不确定性量化至关重要。总不确定性通常分解为数据相关的异方差不确定性（AU）和模型相关的认知不确定性（EU）。现有方法分别建模AU（如Probabilistic UNet、Diffusion）和EU（如集成、MC Dropout），但两者结合时的相互作用尚不明确。近期研究发现AU和EU之间存在显著纠缠，这削弱了不确定性分解的可解释性和实际应用价值。

Method: 1. 进行了全面的实证研究，涵盖广泛的AU-EU模型组合；2. 提出了量化不确定性纠缠的指标；3. 在多个下游UQ任务中评估了不同组合的性能，包括分布外检测、模糊性建模和校准；4. 分析了不确定性纠缠的潜在来源，并提出了缓解该效应的方向。

Result: 1. 对于分布外检测，集成方法表现出更低的纠缠和更优的性能；2. 对于模糊性建模和校准，最佳模型具有数据集依赖性，softmax/SSN方法表现良好，Probabilistic UNets纠缠度较低；3. softmax集成在所有任务中都表现优异；4. 揭示了不同AU-EU组合在性能上的显著差异。

Conclusion: 不确定性分解中的纠缠问题显著影响UQ方法的实用价值。集成方法在减少纠缠和提升分布外检测性能方面表现突出，而softmax集成在所有评估任务中都展现出优异的综合性能。研究为理解AU-EU相互作用提供了实证基础，并指出了缓解不确定性纠缠的未来研究方向。
...
TL;DR: Perceptio是一个增强型大型视觉语言模型，通过显式的语义分割token和深度token实现2D和3D空间推理能力，在多个基准测试中达到最先进性能。
...
Method: 1) 从强单目深度估计教师模型蒸馏VQ-VAE深度码本，将密集深度图token化为紧凑序列；2) 在LLM中集成SAM2语义分割token和VQ-VAE深度token，让模型先输出空间token再回答问题；3) 引入复合深度token目标（标记、token和计数损失）和软合并技术稳定深度token生成；4) 采用多任务协同训练策略。

Result: 在RefCOCO/+/g上提升指代表达分割cIoU +0.8/+1.4/+1.1，HardBLINK空间理解准确率提升10.3%，MMBench准确率提升1.0%，在多个基准测试中达到最先进性能。
...
TL;DR: Dual-IFM是一个可解释性设计的基础模型，通过类证据图提供局部可解释性，通过2D投影层提供全局可解释性，在视网膜成像任务中达到与更大模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型通常基于自监督学习从大量无标签数据中提取可迁移表示，但许多模型架构可解释性有限，这在医疗影像等高风险领域尤为关键。

Method: 提出Dual-IFM模型，具有双重可解释性设计：1）通过类证据图提供对单个图像的局部可解释性；2）通过2D投影层提供对整个数据集的全局可解释性。模型在超过80万张彩色眼底照片上进行自监督预训练。

Result: 模型性能达到与参数多16倍的SOTA基础模型相当的水平，同时能在分布外数据上提供可解释的预测。大规模自监督预训练与固有可解释性结合可为视网膜成像带来鲁棒表示。

Conclusion: 大规模自监督预训练与固有可解释性设计相结合，能够为视网膜成像等高风险医疗领域开发出既高性能又具有可解释性的基础模型。
...
TL;DR: HORNet：一种轻量级帧选择策略，使用GRPO训练，为冻结的VLM选择关键帧，显著减少处理帧数和时间，同时提升VQA性能
...
Method: 提出HORNet框架，使用Group Relative Policy Optimization (GRPO)训练轻量级帧选择策略，仅需少于1M可训练参数，将帧选择与VLM推理解耦为Select Any Frames (SAF)任务

Result: 减少输入帧数达99%，VLM处理时间减少93%，在MSVD-QA上F1提升1.7%，在NExT-QA上比均匀采样提升7.3分，跨VLM模型无需重新训练即可迁移，在6个基准测试的341,877个QA对和114.2小时视频上验证

Conclusion: 优化VLM看到的内容（帧选择）是优化其生成内容的实用且互补的替代方案，同时提高效率，GRPO训练的帧选择策略比监督学习和PPO替代方案具有更好的分布外泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 现有视频推理研究主要关注时空证据链，但缺乏对物体在连续观测间运动模式的显式建模。轨迹理解通常隐含且难以验证，需要将运动模式形式化为可验证的轨迹推理能力。

Method: 提出Spatial-Temporal-Trajectory (STT)推理框架，开发Motion-o扩展；创建轨迹标注数据集，通过增强稀疏关键帧监督获得更密集的边界框轨迹；设计Motion Chain of Thought (MCoT)结构化推理路径，使用<motion/>标签总结物体方向、速度和尺度变化；设计奖励函数促使模型直接基于视觉证据推理，无需架构修改。

Result: Motion-o显著提升了时空定位和轨迹预测性能，同时保持与现有框架的完全兼容性，确立了运动推理作为基于证据的视频理解的关键扩展。

Conclusion: 运动轨迹的显式建模是视频理解的重要扩展，Motion-o通过将轨迹推理形式化并集成到现有框架中，为基于证据的视频推理提供了可验证的轨迹理解能力。
...
TL;DR: PromptHub是一个通过局部感知融合、集中和对齐来增强多提示视觉上下文学习的框架，超越了现有的补丁级融合方法


<details>
  <summary>Details</summary>
Motivation: 现有的视觉上下文学习（VICL）方法采用补丁级融合框架和模型无关的监督，阻碍了信息线索的充分利用，限制了性能提升

Method: 提出PromptHub框架，通过局部感知融合利用空间先验捕获更丰富的上下文信息，采用互补的集中、对齐和预测目标相互指导训练，并结合数据增强来强化监督

Result: 在三个基础视觉任务上的广泛实验证明了PromptHub的优越性，验证了其在分布外设置和各种检索场景下的通用性、可迁移性和鲁棒性

Conclusion: 这项工作建立了一个可靠的局部感知提示融合范式，超越了先前的补丁级方法，为视觉上下文学习提供了更有效的多提示融合框架
...
<details>
  <summary>Details</summary>
Motivation: PET在神经退行性疾病诊断中具有重要功能信息，但成本高且有辐射风险；MRI虽无这些限制但诊断敏感性较低。现有跨模态图像转换方法主要关注结构保留而忽视病理感知需求。

Method: 提出PASTA框架，基于条件扩散模型构建，采用高度交互的双分支架构和多模态条件集成，引入循环交换一致性和体积生成策略以提升3D PET图像质量。

Result: 定性和定量结果显示合成PET扫描具有高质量和病理感知能力。在阿尔茨海默病诊断中，合成PET性能比MRI提升4%，几乎达到真实PET水平。

Conclusion: PASTA通过增强病理感知能力，在MRI到PET图像转换中超越了现有方法，为神经退行性疾病诊断提供了一种成本效益高且无辐射的替代方案。
...
TL;DR: GHOST是一个基于高斯泼溅的快速、类别无关框架，用于从单目RGB视频重建动态手-物体交互，实现了物理一致、可动画化的重建，速度比现有方法快一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖类别特定模板或计算量大，且在手-物体3D对齐方面存在物理不一致问题。需要一种快速、类别无关的方法来重建真实的手-物体交互。

Method: 使用2D高斯泼溅表示手和物体为密集、视角一致的高斯圆盘。引入三个关键创新：1)几何先验检索和一致性损失补全遮挡区域；2)抓握感知对齐优化手平移和物体尺度确保真实接触；3)手感知背景损失避免惩罚手遮挡区域。

Result: 在ARCTIC、HO3D和野外数据集上实现最先进的3D重建和2D渲染质量，运行速度比现有类别无关方法快一个数量级，获得完整、物理一致、可动画化的重建。
...
TL;DR: 提出首个基于无监督对比学习的3D形状匹配方法，通过对比学习提升特征质量，简化功能图架构提高计算效率，在多种挑战性基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度功能图方法主要优化点对应和功能图，而非直接提升嵌入空间的特征表示，导致特征质量不足和匹配性能欠佳；同时依赖耗时功能图求解器，计算成本高。

Method: 1) 无监督对比学习框架：通过最大化正相似对一致性、最小化负相似对一致性，提升特征一致性和判别性；2) 简化功能图学习架构：消除计算昂贵的功能图求解器和多个辅助功能图损失；3) 统一的双分支管道集成这两个组件。

Result: 在准确性和效率方面均达到最先进性能，在近等距、非等距和拓扑不一致等多种挑战性基准测试中优于当前方法，甚至超越监督技术；计算效率显著提升。

Conclusion: 首次提出基于无监督对比学习的3D形状匹配方法，通过对比学习提升特征质量和简化架构提高效率，为3D形状匹配提供了高效鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无需训练的方法通常独立处理每个视图，缺乏全景一致性。VGGT-360旨在利用VGGT类基础模型的内在3D一致性，将碎片化的单视图推理统一为连贯的全景理解，实现几何一致的全景深度估计。

Method: 框架包含三个即插即用模块：1) 不确定性引导自适应投影：将全景切片为透视视图，使用梯度不确定性估计分配更密集的视图到几何贫乏区域；2) 结构显著性增强注意力：在3D重建中向VGGT的注意力层注入结构感知置信度，增强对几何可靠区域的关注；3) 相关性加权3D模型校正：使用注意力推断的相关性分数重新加权重叠点，精炼重建的3D模型。

Result: 在多个分辨率和多样化的室内外数据集上的广泛实验表明，VGGT-360在性能上超越了现有的训练和无需训练的最先进方法。

Conclusion: VGGT-360通过将全景深度估计重新定义为基于多视图3D重建的全景重投影任务，成功实现了无需训练、零样本的几何一致全景深度估计，为全景理解提供了统一的框架。
...
TL;DR: CRAFT是一种轻量级扩散模型微调方法，通过复合奖励过滤构建高质量训练数据集，仅需100个样本即可超越需要数千个偏好配对样本的现有方法，且收敛速度快11-220倍。
...
Method: 提出复合奖励辅助微调(CRAFT)：首先使用复合奖励过滤(CRF)技术构建高质量一致训练数据集，然后执行增强版SFT。该方法理论上优化了基于群体强化学习的下界。

Result: 仅用100个样本的CRAFT就能超越需要数千个偏好配对样本的最新SOTA偏好优化方法。收敛速度比基线偏好优化方法快11-220倍，显示出极高效率。

Conclusion: CRAFT提供了一种轻量级但强大的微调范式，显著减少训练数据需求同时保持计算效率，为扩散模型对齐提供了高效解决方案。
...
TL;DR: 提出两种简约的指纹增强方法：上下文滤波方法和基于学习的方法，在保持简单性的同时超越复杂的最先进方法
...
Method: 提出两种新颖方法：1) 上下文滤波方法；2) 基于学习的方法，两者都强调简约性和有效性

Result: 新方法在具有挑战性的潜指纹数据库上验证，一致超越复杂的最先进方法，产生更清晰、更准确、噪声更少的图像

Conclusion: 简约性对实现高质量指纹增强至关重要，未来研究应在复杂性和实际效益之间取得平衡
...
TL;DR: GenHOI是一个用于广义手-物体姿态估计的框架，通过分层语义提示和多模态掩码建模来提高遮挡条件下的泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决单张RGB图像中手-物体姿态估计的挑战，特别是在物体外观变化大、交互模式多样以及严重遮挡的情况下，现有方法泛化能力不足的问题
...
Conclusion: GenHOI通过整合分层语义知识和手部先验，有效解决了严重遮挡条件下的手-物体姿态估计问题，为广义交互理解提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有基于MLLM的分割方法主要依赖专门的掩码解码器或多重额外token，本文旨在探索能否仅通过MLLM自身实现分割，消除对外部解码器的依赖。

Method: 1) 保留原始未压缩分辨率的图像特征，并用从MLLM处理的压缩特征中提取的残差特征进行补充；2) 分别对经过和未经过LLM处理的图像特征进行像素反洗牌操作，释放压缩特征细节并增强残差特征；3) 设计具有图像-图像和图像-分割双感知路径的注意力掩码，促进像素与分割token间的特征交互。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理需要将复杂空间推理与精确像素级视觉表示相结合的地球观测任务时存在困难，特别是在像素级地理空间推理方面表现不足。

Method: 提出TerraScope统一VLM，具备模态灵活推理（处理单模态光学或SAR数据，自适应融合多模态）和多时序推理（集成时序序列进行变化分析）能力；构建Terra-CoT数据集（100万样本，含像素级掩码的推理链）；创建TerraScope-Bench基准（6个子任务，评估答案准确性和掩码质量）。

Result: 实验表明TerraScope在像素级地理空间推理任务上显著优于现有视觉语言模型，同时提供可解释的视觉证据。

Conclusion: TerraScope通过统一的像素级地理空间推理框架，解决了现有VLM在地球观测中像素级推理的局限性，为地理空间分析提供了更精确和可解释的解决方案。
...
TL;DR: SGC是一种评估生成视频中3D空间几何一致性的新指标，通过测量不同局部区域估计的相机姿态之间的差异来量化几何一致性，解决了现有指标对几何失真不敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型能产生高保真视频，但常出现3D空间几何不一致问题。现有评估方法存在缺陷：保真度导向的指标（如FVD）对几何失真不敏感，而一致性基准往往惩罚有效的前景动态变化。
...
Conclusion: SGC填补了生成视频评估中3D空间几何一致性量化指标的空白，为生成模型的几何一致性提供了更准确的评估工具。
...
TL;DR: Em-Garde：一种解耦语义理解与流式感知的新框架，通过指令引导的提案解析和轻量级提案匹配模块，在流式视频理解中实现高效主动响应


<details>
  <summary>Details</summary>
Motivation: 当前主动式视频语言模型依赖逐帧触发决策，面临效率与准确性的权衡困境。需要一种能在严格计算约束下实现高效主动视频理解的解决方案
...
Conclusion: Em-Garde通过解耦语义理解与流式感知，解决了主动式视频语言模型的效率-准确性困境，为计算受限环境下的流式视频理解提供了有效框架
...
TL;DR: 提出一种鲁棒的实时RGB SLAM系统，通过可微分不确定性感知束调整处理动态环境，在杂乱动态场景中实现最先进的相机位姿和场景几何估计


<details>
  <summary>Details</summary>
Motivation: 传统SLAM方法通常假设静态场景，在存在运动时会导致跟踪失败。现有的动态SLAM方法使用预定义动态先验或不确定性感知映射，但在面对未知动态物体或高度杂乱场景时仍有限制，几何映射变得不可靠。

Method: 利用多视角视觉特征不一致性估计逐像素不确定性，通过可微分不确定性感知束调整实现鲁棒跟踪和重建，系统在杂乱动态场景中实时运行（约10 FPS）。

Result: 在杂乱动态场景中实现了最先进的相机位姿和场景几何估计，系统实时运行速度约为10 FPS，代码和数据集已公开。

Conclusion: 提出的方法通过多视角特征不一致性估计不确定性，结合可微分不确定性感知束调整，能够在真实世界动态环境中实现鲁棒的SLAM性能，解决了传统方法在动态场景中的局限性。
...
Method: 1) 构建Roundabout-TAU数据集，包含342个真实环岛视频片段和2000多个问答对；2) 提出TAU-R1两层视觉语言框架：轻量异常分类器进行粗分类，大异常推理器生成详细事件摘要；3) 采用两阶段训练策略：分解QA增强的监督微调 + TAU-GRPO后训练方法

Result: TAU-R1在异常分类和推理任务上均表现出色，同时保持了部署效率。数据集和代码已开源

Conclusion: 提出的数据集和框架为交通异常理解任务提供了有效的基准和解决方案，填补了该领域的研究空白
...
Method: 提出CustomTex框架，采用双蒸馏方法：1）语义级蒸馏（配备实例交叉注意力）确保语义合理性和参考-实例对齐；2）像素级蒸馏保证高视觉保真度。两者统一在变分分数蒸馏（VSD）优化框架中，从无纹理3D场景和参考图像生成统一的高分辨率纹理图。

Result: 实验表明CustomTex实现了与参考图像的精确实例级一致性，相比最先进方法产生更清晰、伪影更少、烘焙阴影最小的纹理，建立了高质量可定制3D场景外观编辑的直接路径。

Conclusion: CustomTex通过参考图像驱动的实例级高保真纹理生成，解决了现有文本驱动方法的局限性，为高质量3D场景外观编辑提供了更直接、用户友好的解决方案。
...
TL;DR: 该论文提出了一种基于任意顺序自回归模型的生成式分类方法，通过顺序边缘化预测克服了传统固定顺序自回归模型的限制，在图像分类任务中超越了扩散模型分类器，且效率更高。


<details>
  <summary>Details</summary>
Motivation: 传统视觉自回归生成分类器依赖于固定的标记顺序，这给图像理解带来了限制性的归纳偏置。作者观察到单一顺序预测更依赖于部分判别性线索，而多个标记顺序的平均能提供更全面的信号。

Method: 利用最新的任意顺序自回归模型来估计顺序边缘化预测，通过多个标记顺序的集成来解锁自回归模型的高分类潜力，避免了固定顺序的限制。

Result: 该方法在多样化的图像分类基准测试中始终优于基于扩散的分类器，同时效率提高了高达25倍。与最先进的自监督判别模型相比，该方法提供了具有竞争力的分类性能。

Conclusion: 通过顺序边缘化预测，自回归生成模型能够超越扩散模型成为高效的分类器，证明了自回归模型在生成式分类中的潜力，为生成式分类器设定了新的性能标准。
...
TL;DR: ADAPT是一个无需训练的训练框架，通过确定性规划和语义对齐的提示调度，增强扩散模型对罕见组合概念的生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像合成方法（如R2F）在处理罕见组合概念时存在局限性，主要问题包括语言模型的随机性导致的方差以及迭代文本嵌入切换的次优指导。

Method: ADAPT利用注意力分数和正交分量，确定性地规划和语义对齐提示调度，为罕见概念的组合提供一致的指导，无需额外训练或微调。

Result: 在RareBench基准测试中，ADAPT显著提升了罕见组合概念的生成性能，准确反映了罕见属性的语义信息，同时保持了视觉完整性。

Conclusion: ADAPT框架通过确定性规划和语义对齐的提示调度，为罕见组合概念的生成提供了精确控制，优于现有方法且无需额外训练成本。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散的文本到图像模型在训练分布的低密度区域（如罕见概念或编辑指令）往往产生语义错位或结构不一致的结果，这源于文本-图像数据集的"长尾"特性。

Method: 提出自适应辅助提示混合（AAPB）框架，利用辅助锚点提示在罕见概念生成中提供语义支持，在图像编辑中提供结构支持。基于Tweedie恒等式推导闭式自适应系数，在扩散过程的每一步自适应地平衡辅助锚点与目标提示的影响。

Result: 在RareBench和FlowEdit数据集上相比先前的无训练基线方法获得一致改进，实现了更优的语义准确性和结构保真度。通过控制实验验证了自适应插值优于固定插值。

Conclusion: AAPB提供了一个原则性、无需训练的自适应提示混合框架，能够稳定低密度区域的扩散过程，确保生成结果对目标提示的忠实性，解决了长尾分布带来的生成挑战。
...
TL;DR: ARIADNE是一个两阶段框架，通过偏好对齐的感知和基于强化学习的诊断推理，实现拓扑一致的冠状动脉狭窄检测，解决了传统像素级损失函数导致的血管树断裂问题。


<details>
  <summary>Details</summary>
Motivation: 传统像素级损失函数无法在冠状动脉血管分割中强制执行拓扑约束，导致尽管像素级精度高，但产生断裂的血管树结构。这影响了临床诊断的可靠性，特别是在狭窄检测等关键任务中。

Method: 采用两阶段框架：1) 感知模块使用DPO（直接偏好优化）微调Sa2VA视觉-语言基础模型，以Betti数约束作为偏好信号，使策略朝向几何完整的血管结构对齐；2) 推理模块将狭窄定位建模为马尔可夫决策过程，包含显式拒绝机制，自主推迟模糊的解剖候选（如分叉和血管交叉），从覆盖率最大化转向可靠性优化。

Result: 在1400张临床血管造影图像上，ARIADNE实现了0.838的中心线Dice分数，相比几何基线减少了41%的假阳性。在ARCADE和XCAD多中心基准上的外部验证证实了该方法在不同采集协议间的泛化能力。

Conclusion: 这是DPO在医学影像中用于拓扑对齐的首个应用，表明基于结构约束的偏好学习能够减轻拓扑违规，同时在介入心脏病学工作流程中保持诊断敏感性。该方法为拓扑敏感的医学图像分析提供了新范式。
...
Method: 提出Splat2BEV框架：1）预训练高斯生成器，从多视角输入显式重建3D场景，生成几何对齐的特征表示；2）将这些表示投影到BEV空间，作为下游任务的输入。该方法结合了高斯泼溅技术进行3D重建。

Result: 在nuScenes和Argoverse数据集上的大量实验表明，Splat2BEV实现了最先进的性能，验证了将显式3D重建纳入BEV感知的有效性。

Conclusion: 显式3D表示对BEV感知的准确性至关重要。Splat2BEV通过高斯泼溅辅助的3D重建，学习到既语义丰富又几何精确的BEV特征表示，显著提升了BEV感知性能。
...
TL;DR: VLMs存在选择性视觉盲区，其视觉注意力受语言框架影响，导致视觉推理能力下降；通过轻量级提示调优可改善视觉注意力分配，提升性能


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言模型存在选择性视觉盲区，其视觉注意力分配受语言框架影响，即使在需要相同视觉推理的情况下，不同语言框架也会导致注意力分配差异，这影响了模型的视觉推理能力和跨框架一致性

Method: 使用视觉注意力作为探针，量化语言框架如何改变对图像的注意力分配；提出轻量级提示调优方法，使用可学习token鼓励模型采用开放设置中观察到的稳健、视觉基础的注意力模式

Result: 约束性框架（如多项选择和是/否问题）相比开放性问题导致显著更低的图像上下文注意力，减少对任务相关区域的关注，并将注意力转向无信息token；这种注意力分配不当是准确率下降和跨框架不一致的主要原因

Conclusion: VLMs存在选择性视觉盲区，注意力分配受语言框架影响；通过轻量级提示调优改善视觉注意力分配，可提升视觉基础和跨框架性能，为解决VLM视觉盲区问题提供了机制性见解和实用方法
...
TL;DR: RPiAE是一种基于视觉表示的自动编码器，通过表示枢轴正则化和变分桥接技术，在保持语义结构的同时提高重建保真度，并生成更紧凑的潜在空间以改善扩散建模。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视觉表示模型的tokenizer方法存在两个主要问题：1）由于编码器被冻结，重建保真度有限，从而影响编辑质量；2）潜在空间维度过高，增加了扩散建模的难度。需要一种既能保持语义结构又能提高重建质量，同时生成紧凑潜在空间的tokenizer。

Method: 提出Representation-Pivoted AutoEncoder（RPiAE），包含三个关键技术：1）Representation-Pivot Regularization：允许表示初始化的编码器进行微调以改善重建，同时保持预训练表示空间的语义结构；2）Variational Bridge：将潜在空间压缩为更紧凑的形式以简化扩散建模；3）Objective-decoupled stage-wise training：分阶段优化生成可处理性和重建保真度目标。

Result: 实验表明，RPiAE在文本到图像生成和图像编辑任务上优于其他视觉tokenizer，同时在基于表示的tokenizer中实现了最佳的重建保真度。

Conclusion: RPiAE通过创新的训练策略和架构设计，成功解决了现有基于表示tokenizer的重建保真度低和潜在空间维度高的问题，在保持语义结构的同时提高了生成和编辑性能，为扩散模型提供了更有效的视觉表示tokenizer。
...
TL;DR: DreamPartGen是一个基于语义的部件感知文本到3D生成框架，通过双工部件潜在变量和关系语义潜在变量实现几何与语义一致的3D合成
...
Conclusion: DreamPartGen能够生成连贯、可解释且与文本对齐的3D合成，实现了语义基础的部件感知文本到3D生成
...
TL;DR: LVOmniBench：专为长时音频视频跨模态理解设计的新基准，包含275个10-90分钟视频和1014个QA对，评估显示当前OmniLLMs在长时音频视频处理上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前全模态大语言模型（OmniLLMs）的评估主要针对短时音频视频片段（10秒至5分钟），无法反映现实应用中通常持续数十分钟的长视频需求，存在评估空白。

Method: 构建LVOmniBench基准数据集：从开放平台收集高质量视频，通过严格人工筛选和标注，包含275个时长10-90分钟的视频和1014个问答对，涵盖长时记忆、时序定位、细粒度理解和多模态感知等评估维度。

Result: 评估显示当前OmniLLMs在处理长时音频视频输入时面临显著挑战：开源模型准确率普遍低于35%，而Gemini 3 Pro最高达到约65%的准确率。

Conclusion: LVOmniBench填补了长时音频视频跨模态理解的评估空白，揭示了当前模型的局限性，预计将推动更先进模型的发展以解决复杂的长时跨模态理解问题。
...
TL;DR: 提出一种基于向量场重塑的生成式分割方法，通过距离感知校正项增强梯度并改善类别分离，结合准随机类别编码实现像素级语义对齐


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型用于生成式分割主要关注架构调整或训练启发式方法，对连续流匹配目标与离散感知任务之间的内在不匹配理解有限。作者从向量场学习角度重新审视扩散分割，发现传统流匹配目标存在梯度消失和轨迹穿越两个关键限制，导致收敛缓慢和类别分离不佳。

Method: 1. 提出向量场重塑策略：在学习的速度场中加入解耦的距离感知校正项，引入吸引和排斥相互作用，增强质心附近的梯度幅度，同时保持原始扩散训练框架。2. 设计准随机类别编码方案：受Kronecker序列启发，计算高效，与端到端像素神经场框架无缝集成，实现像素级语义对齐。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言-动作模型和世界模型在自动驾驶系统中的广泛应用，可扩展的图像分词器作为视觉模态接口变得至关重要。然而，现有分词器大多针对单目2D场景设计，应用于高分辨率多视图驾驶场景时存在效率低下和视图间不一致的问题。

Method: DriveTok首先从视觉基础模型获取语义丰富的视觉特征，然后通过3D可变形交叉注意力将其转换为场景tokens。解码时使用多视图transformer从场景tokens重建多视图特征，并通过多头机制获得RGB、深度和语义重建。同时直接在场景tokens上添加3D头进行3D语义占据预测以增强空间感知。

Result: 在广泛使用的nuScenes数据集上的大量实验表明，DriveTok生成的场景tokens在图像重建、语义分割、深度预测和3D占据预测任务上表现良好。

Conclusion: DriveTok通过多训练目标学习统一的场景tokens，整合了语义、几何和纹理信息，实现了高效的多视图分词，为自动驾驶系统提供了更好的视觉模态接口。
...
TL;DR: 提出基于图像频谱特性的逐实例噪声调度方法，通过理论推导最小最大噪声水平边界，设计"紧凑"噪声调度消除冗余步骤，在推理时条件采样，提升单阶段像素扩散模型生成质量
...
Method: 基于图像频谱特性设计逐实例噪声调度，通过理论推导最小和最大噪声水平的有效边界，设计"紧凑"噪声调度消除冗余步骤，在推理过程中进行条件采样

Result: 实验表明，该方法提升了单阶段像素扩散模型的生成质量，特别是在低步数情况下效果显著

Conclusion: 提出了一种基于理论推导的逐实例噪声调度设计方法，能够有效提升扩散模型的生成性能，特别是在资源受限的低步数场景下
...
TL;DR: 该论文提出了VOR数据集和EffectErase方法，用于视频对象移除中的效果擦除，解决了现有方法难以消除对象变形、阴影、反射等视觉效果的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复和对象移除方法虽然能移除对象，但难以消除对象产生的视觉效果（如变形、阴影、反射），且缺乏包含这些效果的全面数据集用于训练和评估。

Method: 1) 构建VOR大规模数据集，包含6万对高质量视频（对象存在vs不存在），覆盖5种效果类型和复杂动态多对象场景；2) 提出EffectErase方法，将视频对象插入作为逆辅助任务进行互惠学习，包含任务感知区域引导和插入-移除一致性目标。

Result: 在VOR数据集上训练的EffectErase在广泛实验中表现出优越性能，能够在多样场景中实现高质量的视频对象效果擦除。

Conclusion: VOR数据集填补了视频对象移除领域的数据空白，EffectErase方法通过互惠学习框架有效解决了对象视觉效果擦除问题，为视频编辑任务提供了新的解决方案。
...
TL;DR: MultiGP是一种生成式逆渲染方法，能从单张图像中随机采样所有辐射成分（反射率、纹理、光照），通过利用场景中所有物体共享相同光照的共识来解决辐射解缠的固有模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 从单张图像中解缠辐射成分（反射率、纹理、光照）是一个固有模糊性问题。现有方法难以同时准确恢复所有成分，特别是当场景包含多个具有不同反射率和纹理的物体时。

Method: 提出四种关键技术：1) 级联端到端架构结合图像空间和角度空间解缠；2) 协调引导确保扩散收敛到单一一致的光照估计；3) 轴向注意力促进不同反射率物体间的"交叉对话"；4) 纹理提取ControlNet在保持高频纹理细节的同时确保与估计光照的解耦。

Result: 实验结果表明，MultiGP能有效利用多个物体外观的互补空间和频率特性，成功恢复个体纹理和反射率以及共同的光照。

Conclusion: MultiGP通过利用场景中物体共享相同光照的共识，解决了单图像辐射解缠的模糊性问题，实现了从单张图像中生成式采样所有辐射成分的能力。
...
TL;DR: 提出三阶段框架MoTok，结合离散令牌生成与扩散模型优势，通过感知-规划-控制架构实现语义与运动学双重控制，显著提升运动生成质量与可控性。


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法存在局限性：连续扩散模型擅长运动学控制但语义条件化能力有限，离散令牌生成器语义控制效果好但运动学控制不足。需要结合两者优势，实现语义与运动学的双重精确控制。

Method: 提出三阶段框架：1) 感知阶段提取条件特征；2) 规划阶段生成离散运动令牌MoTok；3) 控制阶段使用扩散模型合成运动。MoTok令牌化器将语义抽象与细粒度重建解耦，使用单层紧凑令牌保持运动保真度。运动学条件在规划阶段指导令牌生成，在控制阶段通过扩散优化强制执行细粒度约束。

Result: 在HumanML3D数据集上显著优于MaskControl：令牌数量减少至1/6，轨迹误差从0.72cm降至0.08cm，FID从0.083降至0.029。在强运动学约束下，FID从0.033进一步降至0.014，而先前方法在强约束下性能会下降。

Conclusion: MoTok框架成功结合了离散令牌与扩散模型的优势，通过解耦语义规划与运动学控制实现了高质量、高可控的运动生成，在语义和运动学双重约束下均表现出优越性能。
...
TL;DR: SAMA框架通过语义锚定和运动对齐的因子化方法，解决了视频编辑中语义修改与运动保持的平衡问题，无需依赖外部先验，在零样本和微调后均表现出色。
...
Method: 提出SAMA框架，将视频编辑因子化为语义锚定和运动建模。语义锚定通过在稀疏锚定帧联合预测语义标记和视频潜在表示来建立可靠的视觉锚点；运动对齐通过视频修复预训练任务（立方体修复、速度扰动、管道洗牌）让模型从原始视频中学习时间动态。采用两阶段优化：因子化预训练阶段学习内在的语义-运动表示（无需配对编辑数据），然后在配对编辑数据上进行监督微调。

Result: 因子化预训练本身已展现出强大的零样本视频编辑能力，验证了因子化方法的有效性。SAMA在开源模型中达到最先进性能，与领先的商业系统（如Kling-Omni）竞争力相当。

Conclusion: SAMA通过语义锚定和运动对齐的因子化方法，成功解决了视频编辑中语义修改与运动保持的平衡问题，无需依赖外部先验，在零样本和微调后均表现出色，为视频编辑提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 从单图像重建铰接3D物体需要从有限视觉证据中联合推断几何、部件结构和运动参数。关键难点在于运动线索与物体结构的纠缠，导致直接回归铰接不稳定。现有方法通过多视图监督、基于检索的组装或辅助视频生成来解决，但往往牺牲可扩展性或效率。

Method: 提出MonoArt统一框架，基于渐进式结构推理。不直接从图像特征预测铰接，而是逐步将视觉观察转换为规范几何、结构化部件表示和运动感知嵌入，所有步骤在单一架构内完成。这种结构化推理过程无需外部运动模板或多阶段流程。

Result: 在PartNet-Mobility数据集上的大量实验表明，该方法在重建精度和推理速度方面均达到最先进水平。框架进一步泛化到机器人操作和铰接场景重建任务。

Conclusion: MonoArt通过渐进式结构推理实现了稳定且可解释的铰接推断，无需外部运动模板或多阶段流程，在单图像铰接3D重建方面取得了显著进展。
...
TL;DR: CubiD是首个用于高维表示的离散生成模型，通过细粒度掩码实现高维离散表示的生成，在保持语义丰富性的同时实现了理解与生成任务的统一。


<details>
  <summary>Details</summary>
Motivation: 当前离散生成方法局限于低维潜在标记（8-32维），牺牲了理解所需的语义丰富性。高维预训练表示（768-1024维）虽能弥合这一差距，但其离散生成存在根本性挑战。需要开发能够处理高维离散表示的生成模型。

Method: 提出立方离散扩散（CubiD）模型，对高维离散表示进行细粒度掩码——任何位置、任何维度都可以被掩码并从部分观察中预测。这使得模型能够学习空间位置内部和之间的丰富相关性，生成步骤固定为T，远小于特征维度hwd。

Result: 在ImageNet-256上，CubiD实现了最先进的离散生成性能，从9亿到37亿参数都表现出强大的扩展性。验证了这些离散化标记保留了原始表示能力，同一离散标记可有效服务于理解和生成任务。

Conclusion: CubiD是首个高维表示的离散生成模型，通过细粒度掩码实现了理解与生成任务的统一，为未来统一多模态架构的研究提供了启发。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究当前大型语言模型是否真正具备心理理论能力——即从文本中推断他人信念、意图和情绪的能力。由于LLMs仅基于语言数据训练，缺乏社会具身化或对心理表征的直接接触，其表现出的社会认知推理能力引发了对其理解本质的质疑：是真正的心智状态归因能力，还是仅仅是表面模式完成？

Method: 研究方法包括：1) 测试五个不同的大型语言模型；2) 使用人类心理理论研究中的文本工具改编版；3) 让模型回答关于故事角色信念、意图和情绪的问题；4) 与人类对照组进行比较；5) 分析模型在不同条件下的表现差异。

Result: 结果显示：1) 早期较小模型的表现受相关推理线索数量显著影响；2) 这些模型在一定程度上也容易受到文本中无关或干扰信息的影响；3) GPT-4o表现出高准确性和强鲁棒性；4) GPT-4o在最挑战性条件下仍能取得与人类相当的表现；5) 模型之间存在明显的性能差距。

Conclusion: 结论表明：GPT-4o在心理理论任务中表现出接近人类的能力，而较小模型则显示出局限性。这一发现为关于LLMs认知状态以及真正理解与统计近似之间界限的持续辩论提供了实证依据，暗示某些先进模型可能具备了超越简单模式完成的心智状态推理能力。
...
TL;DR: 提出LSFU不确定性度量方法和UCPOF框架，通过校准LLM输出不确定性来优化提示工程，在降低计算成本的同时提升多选任务性能


<details>
  <summary>Details</summary>
Motivation: LLM的自回归生成导致输出不确定性，而模型性能对提示设计敏感。传统不确定性度量（如熵）在多选任务中无法区分伪置信度（来自先验）和真实确定性（来自上下文理解），导致置信度校准不佳

Method: 提出Log-Scale Focal Uncertainty (LSFU)：基于首令牌的度量方法，受focal loss启发，引入标签先验概率作为风险调制因子，抑制高频类噪声，强调低频长尾类风险，具有动态加权机制统一度量尺度。基于LSFU提出UCPOF框架：利用模型输出的首令牌选择高质量示例并动态优化提示

Result: UCPOF相比少样本基线平均准确率提升6.03%，相比始终开启的完整RAG整体平均准确率提升5.75%，平均检索触发率降低50.66%。通过仅对高不确定性样本自适应触发RAG，显著降低计算成本同时保持SOTA性能

Conclusion: LSFU能有效校准LLM在多选任务中的不确定性，UCPOF框架通过不确定性驱动的提示优化和自适应RAG触发，在提升性能的同时大幅降低计算开销，为可靠的提示优化提供了有效解决方案
...
TL;DR: 提出确定性证据选择框架，通过MUE和DUE评分控制证据可接受性，确保每个被接受的证据单元都能独立满足问题要求


<details>
  <summary>Details</summary>
Motivation: 现有基于向量检索的问答系统仅依赖相似度评分，无法区分哪些文本能真正作为证据。相似度相近的候选文本可能存在冗余、不完整或不符合问题条件的问题，需要更精确的证据选择机制

Method: 提出确定性证据选择框架，包含意义效用估计(MUE)和多样性效用估计(DUE)两个固定评分程序。MUE评估语义相关性、术语覆盖度和概念区分度，DUE控制冗余。每个句子或记录独立评估，无需训练或微调

Result: 原型系统要求每个被接受的证据单元必须独立明确陈述任务所需的事实、规则或条件。如果不满足要求则返回无答案。该方法产生紧凑、可审计的证据集，明确区分相关文本和可用证据

Conclusion: 确定性证据选择框架通过明确的评分标准和冗余控制，解决了传统检索系统中相似度评分无法区分可用证据的问题，提高了问答系统的证据质量和可解释性
...
TL;DR: DynaRAG是一个动态知识集成的检索增强生成框架，通过选择性调用外部API来处理静态和时效性信息需求，显著提升动态问题回答的准确性并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统仅依赖静态语料库，无法有效处理需要时效性信息的动态问题。当检索到的文档不足以回答查询时，需要动态集成外部知识源来提供准确、最新的信息。

Method: DynaRAG采用多阶段处理流程：1) LLM重排序器评估文档相关性；2) 充分性分类器判断是否需要回退到外部API；3) 使用Gorilla v2 API调用模型进行准确工具调用；4) 通过FAISS模式过滤引导API选择。

Result: 在CRAG基准测试中，DynaRAG显著提高了动态问题的准确性，同时减少了幻觉。结果表明动态感知路由和选择性工具使用对于构建可靠的现实世界问答系统至关重要。

Conclusion: DynaRAG框架通过动态知识集成有效解决了传统RAG在处理时效性信息时的局限性，证明了动态感知路由和选择性API调用的重要性，为构建可靠的现实世界问答系统提供了有效方案。
...
TL;DR: 研究发现大型语言模型在特定条件下能够重构训练数据内容，但在标准生成任务中不会表达非因果、不可实现的解决方案类型，揭示了学习能力与输出表达之间的系统性分离。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs在训练数据中学习到的内容与其实际生成输出之间的关系。虽然已知LLMs在特定条件下能够重构训练数据内容，但这种现象是否会在标准生成任务中表现出来尚不清楚。研究旨在验证训练数据存在是否直接预测输出概率的普遍假设。

Method: 采用实证观察研究方法，分析了300个提示-响应生成对，涵盖叙事和问题解决任务情境。研究基于记忆连续性理论和对齐诱导的语篇先验，考察了三种不同LLMs在十个任务场景中的表现，包括创意叙事和实际咨询情境。

Result: 在所有生成的输出中，未发现任何非因果解决方案框架的实例（0%，95%置信区间：[0%，1.2%]），尽管在条件提取下验证了模型具备重构能力。这表明任务条件化的生成策略能够全面抑制学习到的内容。

Conclusion: 研究挑战了训练数据存在直接预测输出概率的普遍假设，证明了任务条件化的生成策略能够全面抑制学习到的内容。这些发现对理解生成动态、输出分布控制和当代LLMs的行为边界具有重要意义。
...
TL;DR: 该研究使用SHAP和Integrated Gradients两种可解释性方法分析基于RoBERTa的有害内容检测模型，揭示其在边界案例中的局限性，强调可解释性AI在内容审核中的诊断价值而非性能提升作用。


<details>
  <summary>Details</summary>
Motivation: 当前有害内容检测系统虽然准确率高，但缺乏可解释性，特别是在边界案例、上下文相关和政治敏感情境中。用户和审核者无法理解模型预测的逻辑，需要可解释性方法来揭示模型行为。

Method: 使用Civil Comments数据集训练RoBERTa有害内容检测模型，应用SHAP和Integrated Gradients两种后验解释方法，分析模型在正确预测和系统失败案例中的行为，进行定性案例研究。

Result: 模型整体性能优秀（AUC=0.93，准确率=0.94），但可解释性分析揭示了聚合指标无法观察到的局限性：SHAP更关注显性词汇线索，Integrated Gradients提取更分散的上下文归因，两者差异导致假阴性和假阳性。定性分析发现间接毒性、词汇过度归因和政治话语等失败模式。

Conclusion: 可解释性AI能够通过暴露模型不确定性和增加决策可解释性来促进人机协同审核。最重要的是，可解释性应被视为有害内容检测系统的透明度和诊断资源，而非性能提升工具。
...
<details>
  <summary>Details</summary>
Motivation: 标准推测解码的性能受限于草稿生成和验证阶段的严格顺序执行，导致效率瓶颈。需要一种能够隐藏草稿生成延迟的方法来提升推理效率。

Method: 提出MineDraft批处理并行推测解码框架，采用新颖的批处理并行设计，维护两个请求批次，使一个批次的草稿生成与另一个批次的验证阶段重叠执行。

Result: 实验结果显示，MineDraft相比标准推测解码在吞吐量上提升高达75%，端到端延迟降低高达39%。已作为vLLM插件实现，证明其生产就绪性。

Conclusion: MineDraft通过批处理并行设计有效隐藏草稿生成延迟，显著提升大语言模型推理效率，为生产环境提供了实用的推测解码解决方案。
...
TL;DR: 论文指出语言模型基准测试存在内容效度和收敛效度问题，无法准确反映实践者意图，并提出了BenchBrowser工具来帮助诊断这些效度问题。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型基准测试的元数据过于粗糙，无法准确反映测试内容的实际粒度。基准测试的模糊性使得验证其与实践者目标的匹配成为繁琐过程，可能导致模型在未测试的用户关注方面表现不佳时仍产生能力幻觉。

Method: 提出了BenchBrowser，一个检索器工具，能够从20多个基准测试套件中检索与自然语言使用案例相关的评估项目。通过人类研究验证了其高检索精度，并利用该工具生成证据来诊断内容效度（对能力维度的覆盖范围窄）和收敛效度（测量相同能力时排名不稳定）问题。

Result: 人类研究证实BenchBrowser具有高检索精度。该工具能够量化实践者意图与基准测试实际测量内容之间的关键差距，帮助诊断基准测试的内容效度和收敛效度问题。

Conclusion: BenchBrowser为实践者提供了量化工具，帮助他们识别基准测试在测量语言模型能力时存在的效度问题，从而更准确地评估模型是否真正满足实际应用需求。
...
TL;DR: 基于FrameNet语义标注的电子病历文本分析可提升性别暴力识别效果，相比结构化数据模型F1分数提高0.3以上


<details>
  <summary>Details</summary>
Motivation: 性别暴力是重大公共卫生问题，巴西存在严重漏报现象，主要原因是难以识别虐待行为以及公共卫生信息系统整合不足。研究旨在探索语义分析能否改善性别暴力识别

Method: 使用FrameNet对电子病历开放文本字段进行语义标注，比较三种SVM分类器性能：1)基于框架标注文本的模型；2)标注文本与参数化数据结合的模型；3)仅参数化数据的模型

Result: 包含语义标注的模型优于仅使用分类数据的模型，F1分数提高超过0.3。领域特定语义表示提供了超越结构化人口统计数据的有效信号

Conclusion: 临床叙述的语义分析能够增强早期识别策略，支持更明智的公共卫生干预措施。语义标注方法为性别暴力检测提供了有前景的技术路径
...
TL;DR: LLMs在写作辅助中不仅改变文本风格，还会系统性改变原意，导致内容中立化、创造性降低，并在科学评审中影响评价标准


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在写作辅助中的实际影响，揭示AI工具如何改变人类写作的语义内容，以及这种改变对文化和科学机构的潜在影响

Method: 1) 人类用户研究：分析LLM使用对写作风格和内容的影响；2) 历史数据集分析：使用2021年收集的人类写作数据集，研究LLM基于反馈修改文本时的语义变化；3) 真实场景分析：研究AI生成科学同行评审的质量差异

Result: 1) 大量使用LLM导致70%的文章在回答主题问题时保持中立，用户报告写作创造性降低、失去个人声音；2) 即使只要求语法编辑，LLM仍会显著改变文本语义；3) AI生成的科学评审对研究清晰度和重要性的关注度显著降低，评分平均高出1分

Conclusion: LLMs在写作辅助中存在语义对齐问题，其广泛使用可能对人类文化和科学机构产生系统性影响，需要进一步研究AI写作工具的社会影响
...
TL;DR: 该研究比较了人类与LLM生成的词语联想，评估模型捕捉人类词汇模式的能力，发现大模型生成典型但变化少的响应，小模型生成变化多但典型性低的响应，温度设置影响这种权衡。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在文本生成流畅性方面表现出色，但其语言知识的本质——特别是其内部词汇库与人类词汇的相似程度——仍不明确。需要评估LLM在多大程度上准确捕捉人类的词汇模式。

Method: 使用SWOW数据集中的英语线索-反应词对，从三个LLM（Mistral-7B、Llama-3.1-8B和Qwen-2.5-32B）生成新的词语联想，涵盖多个温度设置。分析词汇因素（词频、具体性）对线索-反应词对的影响，以及LLM响应与人类响应在变异性和典型性上的差异。

Result: 所有模型都反映了人类在词频和具体性方面的趋势，但在响应变异性和典型性上存在差异。大模型（如Qwen）倾向于模拟单一的"原型"人类参与者，生成高度典型但变异最小的响应；小模型（如Mistral和Llama）产生更多变异但典型性较低的响应。温度设置进一步影响这种权衡，较高温度增加变异性但降低典型性。

Conclusion: 研究揭示了人类与LLM词汇库之间的相似性和差异性，强调在探索LLM词汇表征时需要考虑模型大小和温度设置的影响。大模型可能过度拟合典型的人类响应模式，而小模型则表现出更多样但不太典型的联想模式。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的性能评估面临基准数据污染问题，导致测试结果可能被夸大。现有评估方法缺乏持续性和对模型问题的系统性追踪。

Method: 构建基于用户反馈的模型问题库，采用LLM-as-a-judge进行质量保证测试，实现多模型并行对比和不同版本间的回归检测。

Result: 开发了GRAFITE平台，提供持续评估框架，支持模型问题追踪和性能比较，平台已在GitHub开源并配有演示视频。

Conclusion: GRAFITE平台解决了LLM评估中的基准污染问题，通过持续评估和问题库管理为模型性能提供更可靠的评估框架。
...
TL;DR: CWoMP是一种用于自动生成语际标注文本的新方法，通过将语素视为原子形式-意义单元进行对比学习，在低资源语言中优于现有方法且效率更高。


<details>
  <summary>Details</summary>
Motivation: 语际标注文本是语言文档的标准表示方法，但手动制作非常耗时。现有的自动化方法将词缀视为字符序列，忽略了其组合结构，需要更有效的方法来处理低资源语言的IGT生成。

Method: 提出CWoMP方法：1) 将语素视为原子形式-意义单元并学习其表示；2) 使用对比训练的编码器将上下文中的单词与其组成语素在共享嵌入空间中对齐；3) 通过从可变词典中检索这些嵌入，使用自回归解码器生成语素序列。

Result: 在多种低资源语言上评估，CWoMP优于现有方法且显著更高效，在极低资源设置下表现尤为突出。预测结果可解释且基于词典条目，用户可以在推理时通过扩展词典来改进结果而无需重新训练。

Conclusion: CWoMP通过将语素视为原子单元并采用对比学习的方法，为自动生成语际标注文本提供了更有效、可解释且可扩展的解决方案，特别适用于低资源语言场景。
...
TL;DR: 论文分析了人工智能主要范式与心理学理论的对应关系，指出每种AI范式都继承了其心理学基础的优点和结构局限性，并提出ReSynth三模块框架来解决这些局限性。


<details>
  <summary>Details</summary>
Motivation: 论文旨在揭示人工智能主要范式（强化学习、深度学习、建构主义方法）与其心理学理论基础（行为主义、认知主义、建构主义）之间的对应关系，并指出每种范式都继承了相应心理学理论的结构局限性。作者认为当前AI方法在知识内部结构、表示透明性、新理解构建等方面存在根本缺陷，需要新的架构来支持系统性的适应能力。

Method: 论文采用跨学科分析方法：1）追溯心理学范式到AI方法的发展谱系；2）分析每种范式继承的结构局限性；3）引入东方文化中对机械学习的多阶段理解观；4）基于系统性争论和对经典主义与联结主义的批判，提出ReSynth三模块框架，将推理（智力）、目的（身份）和知识（记忆）分离为架构上独立的组件。

Result: 论文识别出三种主要AI范式与其心理学基础之间的对应关系及其局限性：强化学习无法解释知识内部结构；深度学习将表示压缩到不透明的参数空间中；当前整合方法缺乏从现有组件构建新理解的正式理论。论文提出的ReSynth框架旨在通过架构分离来解决这些局限性，使系统性行为成为必然结果而非偶然属性。

Conclusion: 适应性是人工通用智能的核心挑战，需要一种新的表示架构，在这种架构中，系统性行为是必然结果而非偶然属性。ReSynth框架通过分离推理、目的和知识组件，为构建具有真正适应能力的AI系统提供了理论基础，同时借鉴了东方文化中对机械学习的结构化理解作为心理学理论与AI方法论之间的桥梁。
...
TL;DR: 该研究提出了一种新闻文档轨迹的时间分类法，将异常值区分为预示性异常值（在主题形成前出现）和其他类型，并证明某些异常值可作为新兴主题的早期信号。


<details>
  <summary>Details</summary>
Motivation: 传统动态主题建模通常将异常值视为噪声，但本研究认为某些异常值实际上可以作为新兴主题的早期信号，需要系统性地识别和分类这些预示性文档。

Method: 提出了新闻文档轨迹的时间分类法，区分预示性异常值、强化现有主题的文档和孤立文档。在累积聚类设置中实现，使用11种最先进语言模型的文档嵌入，并在HydroNewsFr（法国氢经济新闻语料库）上进行回顾性评估。

Result: 模型间一致性分析揭示了一个小型、高共识的预示性异常值子集，增加了这些标签的可信度。定性案例研究通过具体主题发展说明了这些轨迹。

Conclusion: 该分类法将弱信号检测与时间主题建模联系起来，阐明了单个文章如何在演化集群中预示、启动或漂移，为早期主题检测提供了新视角。
...
TL;DR: PowerFlow：基于分布匹配的无监督强化学习框架，通过GFlowNet作为变分采样器，使用长度感知的轨迹平衡目标消除自回归生成的结构性长度偏差，实现LLM逻辑推理与创造性表达的双重激发。


<details>
  <summary>Details</summary>
Motivation: 当前无监督强化学习从内部反馈（RLIF）方法依赖启发式内在奖励，缺乏明确的理论优化目标且容易产生退化性偏差。需要一种原则性框架来解决这些问题。

Method: 将无监督微调重新表述为分布匹配问题，将GFlowNet作为未归一化密度的摊销变分采样器，提出长度感知的轨迹平衡目标以消除自回归生成的结构性长度偏差。通过针对α-幂分布，实现LLM双重特性的定向激发。

Result: PowerFlow在实验中一致优于现有RLIF方法，匹配甚至超过监督GRPO。通过减轻对齐模型中的过度锐化，在创造性任务中同时提高多样性和质量，推动帕累托前沿。

Conclusion: PowerFlow提供了一个原则性框架，通过分布匹配和偏差消除机制，有效激发LLM的逻辑推理和创造性表达能力，在无监督强化学习中取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前RAG中的文档分块方法通常线性化文本，这种强制线性化剥离了内在的拓扑层次结构，导致"语义碎片化"，从而降低下游检索质量。
...
Conclusion: TopoChunker为结构感知的RAG提供了一个可扩展的方法，在保持结构保真度的同时平衡计算成本，显著提升检索增强生成的质量和效率。
...
TL;DR: TARo是一种在推理时通过令牌级自适应路由引导冻结LLM进行结构化推理的测试时对齐方法，无需昂贵后训练即可显著提升推理性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备强大推理能力，但通常需要昂贵的后训练才能达到高性能。现有的测试时对齐方法主要关注偏好对齐而非推理对齐，因此需要填补这一空白

Method: 首先在逐步数学轨迹上训练奖励模型以捕获细粒度逻辑一致性信号，然后引入可学习的令牌级路由器，自动控制奖励模型对基础模型的引导

Result: TARo显著提升推理性能：相比基础模型提升高达+22.4%，相比现有令牌级测试时对齐方法提升+8.4%，同时提升分布外临床推理（MedXpertQA）和指令遵循（AlpacaEval）能力
...
TL;DR: 该论文提出了首个评估多模态大语言模型中任务干扰现象的基准，发现从纯文本任务切换到基于图像的任务会导致严重性能下降，而反向切换影响较小，且模态差异是主要干扰因素。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态对话系统日益普及，但任务干扰（单次对话中任务切换导致的性能下降）研究仅限于纯文本环境，缺乏对多模态场景的系统性评估。

Method: 构建了涵盖文本和视觉六个任务的多模态任务干扰基准，通过三个维度（模态不匹配、推理不匹配、答案格式不匹配）系统性地改变历史-目标关系，并在开源和专有模型上进行实验。

Result: 任务干扰具有高度方向性：从纯文本任务切换到图像目标导致严重性能下降（平均下降15-25%），而反向切换影响最小（<5%）。多维度不匹配会放大干扰，模态差异是主要驱动因素，其次是答案格式，推理要求变化影响最小。

Conclusion: 多模态LLMs中的任务干扰主要由模态差异驱动，具有显著的方向性特征，这为设计更鲁棒的多模态对话系统提供了重要见解，并强调了在评估中考虑任务切换效应的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前广泛使用的贪婪解码或固定温度/top-p解码等启发式方法是静态且任务无关的，导致在不同需要风格或结构灵活性的领域中生成质量次优或不一致，需要更灵活的解码策略。

Method: 采用强化学习方法，将解码视为序列决策问题，学习轻量级策略在测试时动态调整采样参数，同时保持LLM权重冻结。使用复合奖励函数，包含重叠度、长度、覆盖率、重复性和完整性等结构化约束项。

Result: 在BookSum、arXiv和WikiHow等摘要数据集上，使用Granite-3.3-2B和Qwen-2.5-0.5B模型评估，策略采样器持续优于贪婪和静态基线，相对增益最高达+88%（BookSum, Granite）和+79%（WikiHow, Qwen）。奖励消融实验显示仅使用重叠度目标效果较差，而结构化约束项能实现稳定持续的改进。

Conclusion: 强化学习是解码中测试时适应的实用机制，能够实现领域感知和用户可控的生成，无需重新训练大型模型，为动态解码策略提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 长上下文推理对大型语言模型具有挑战性，主要由于注意力稀释和分布外退化。现有上下文选择方法在整个解码过程中分配固定上下文预算，但忽略了token级别的上下文需求高度不均匀的问题。

Method: 提出不确定性触发自适应上下文分配（UT-ACA）框架：1）学习结合语义嵌入和基于logit置信度的不确定性检测器；2）考虑解码步骤间的不确定性累积；3）当证据不足时，选择性回滚、扩展上下文窗口并利用额外支持重新生成token。

Result: 实验表明，UT-ACA在长上下文设置中显著减少了平均上下文使用量，同时保持了生成质量。

Conclusion: UT-ACA通过动态调整上下文窗口基于token级别不确定性，有效解决了长上下文推理中的注意力稀释问题，实现了上下文使用效率与生成质量的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在精确控制大型语言模型行为时存在训练成本高、缺乏自然语言可控性或损害语义连贯性的问题，需要一种更好的行为解释和控制方法

Method: 提出WASD框架，将候选条件表示为神经元激活谓词，通过迭代搜索最小充分条件集来保证特定token生成，该方法在输入扰动下依然有效

Result: 在SST-2和CounterFact数据集上使用Gemma-2-2B模型的实验表明，WASD产生的解释比传统归因图更稳定、准确和简洁；跨语言输出生成案例研究验证了其行为控制的有效性

Conclusion: WASD框架为大型语言模型提供了一种有效的可解释性和行为控制方法，通过识别充分神经条件来解释模型行为，并在实际应用中展示了其控制能力
...
TL;DR: 研究发现基于似然的解码策略（如top-k、核采样）与人类语言生成存在根本差异，导致机器生成文本容易被检测，这种可检测性主要源于解码策略而非模型能力


<details>
  <summary>Details</summary>
Motivation: 人类语言生成基于交际适宜性而非统计频率选择词汇，而标准解码策略基于似然选择高概率词汇，这种不匹配可能导致机器生成文本容易被检测

Method: 分析了超过180万篇文本，涵盖8个语言模型、5种解码策略和53种超参数配置，使用基于可预测性和词汇多样性的简单分类器进行检测

Result: 8-18%的人类选择词汇落在典型截断边界之外；可检测性与模型规模或架构相关性弱，主要受截断参数影响；低可检测性配置常产生不连贯文本

Conclusion: 文本可检测性主要由基于似然的词汇选择机制增强，而非模型能力问题；逃避检测与生成自然文本是不同的目标
...
TL;DR: ICE-Guard框架通过干预一致性测试检测LLM对虚假特征的依赖，发现权威和框架偏见远大于人口统计偏见，结构化分解可显著降低偏见，验证显示该方法能有效减少现实世界偏见。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地用于高风险决策，但其对虚假特征的敏感性尚未得到充分表征。当前研究主要关注人口统计偏见，而忽视了其他类型的虚假特征依赖。

Method: 引入ICE-Guard框架，应用干预一致性测试检测三种虚假特征依赖：人口统计（姓名/种族交换）、权威（资历/声望交换）和框架（正面/负面重述）。在10个高风险领域的3000个案例中评估11个LLM，采用结构化分解（LLM提取特征，确定性规则决定）和ICE引导的检测-诊断-缓解-验证循环。

Result: 权威偏见（平均5.8%）和框架偏见（5.0%）显著超过人口统计偏见（2.2%）；偏见集中在特定领域（金融领域权威偏见22.6%，刑事司法仅2.8%）；结构化分解可将翻转率降低高达100%（9个模型的中位数降低49%）；ICE引导循环通过迭代提示修补实现累计78%的偏见减少；COMPAS再犯数据验证显示真实偏见率超过合成基准。

Conclusion: LLM偏见研究需要超越人口统计偏见，关注权威和框架偏见；结构化分解是有效的偏见缓解策略；ICE-Guard框架提供了实用的检测-诊断-缓解-验证循环，可显著减少现实世界偏见；该基准为LLM偏见评估提供了保守估计。
...
TL;DR: 提出基于通用标准集(UCS)的分解式多语言评估框架，通过语言无关的评估维度实现跨语言迁移，无需目标语言标注数据


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多语言场景部署日益广泛，但现有评估方法主要针对英语，其他语言缺乏高质量人工标注数据，限制了多语言评估的发展

Method: 引入基于通用标准集(UCS)的分解式评估框架，UCS包含共享的语言无关评估维度，生成可解释的中间表示，支持最小监督下的跨语言迁移

Result: 在多个语言和模型架构的真实性评估任务上实验，相比强基线方法获得一致改进，且不需要目标语言标注数据

Conclusion: UCS框架为多语言评估提供了有效解决方案，通过分解式语言无关评估维度实现跨语言知识迁移，解决了多语言评估数据稀缺问题
...
Conclusion: 忠实性应跨干预算子比较而非单一分数，ICE框架和ICEBench基准为解释忠实性评估提供了更可靠的统计方法
...
<details>
  <summary>Details</summary>
Motivation: 需要一种系统方法来分析和比较不同语言模型的条件分布行为，理解模型间的结构关系、任务性能关联，以及提示修改对模型输出的影响。
...
TL;DR: 提出一种可解释的多模态分类框架，通过跨模态理性转移从文本理性中学习图像理性，提升危机事件分类性能并节省标注成本
...
Method: 使用视觉语言Transformer学习文本和图像的联合表示，提取文本理性，通过跨模态理性转移从文本理性映射提取图像理性，最后基于提取的理性进行分类

Result: 在CrisisMMD基准数据集上，分类Macro-F1提升2-35%，提取准确的文本标记和图像块作为理性；人类评估显示图像理性块提取质量提升12%；零样本模式下对未见数据集达到80%准确率

Conclusion: 提出的可解释多模态分类框架通过跨模态理性转移有效提升分类性能，同时提供透明的决策解释，节省标注成本，具有良好的泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 当前需要建立一个标准化的多语言基准来评估无监督音素发现方法的性能，特别是评估从离散语音单元中提取音素信息的能力，以了解现有模型在不同语言中捕获音素信息的程度。

Method: DiscoPhon基准测试包含6种开发语言和6种测试语言，覆盖广泛的音素对比。系统仅使用10小时未见语言的语音数据，必须生成离散单元并通过多对一或一对一映射到预定义音素库。评估包括单元质量、识别和分割三个方面。作者提供了四个预训练的多语言HuBERT和SpidR基线模型。

Result: 实验表明，当前模型中存在足够的音素信息，使得衍生单元与音素具有良好的相关性，但不同语言之间存在性能差异。基线模型在音素发现任务上表现出一定的有效性。

Conclusion: DiscoPhon为无监督音素发现提供了一个标准化的多语言评估框架，证明了现有模型能够从离散语音单元中提取音素信息，但性能存在语言间差异，为进一步改进无监督音素发现方法奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法完全依赖模型固有的推理能力，从未明确训练模型进行测试时自我进化任务。LSE旨在将自我进化作为可学习的技能进行训练，提高模型在测试时改进上下文的能力。

Method: 提出Learning to Self-Evolve (LSE)框架：1) 将多步进化问题简化为单步强化学习目标，每个上下文编辑的奖励基于下游性能改进；2) 结合树引导的进化循环；3) 训练LLM在测试时迭代改进上下文。

Result: 在Text-to-SQL生成(BIRD)和通用问答(MMLU-Redux)任务上，4B参数的LSE训练模型超越了GPT-5和Claude Sonnet 4.5驱动的自我进化策略，以及GEPA和TextGrad等提示优化方法，并能迁移指导其他模型而无需额外训练。

Conclusion: 将自我进化作为可学习技能是有效的，LSE框架通过强化学习训练模型在测试时自我改进上下文，显著提升了模型性能并展示了良好的迁移能力。
...
TL;DR: 该研究比较了持续意图分类中灾难性遗忘缓解方法，在CLINC150数据集上评估了三种架构（ANN、GRU、Transformer）与三种持续学习策略（MIR、LwF、HAT）的组合效果，发现回放是防止遗忘的关键成分。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的神经语言模型需要持续适应新任务和领域而不忘记先前获得的知识，因此需要研究持续学习中的灾难性遗忘缓解方法，特别是在意图分类任务中。

Method: 使用CLINC150数据集构建10个任务的不相交标签场景，评估三种骨干架构（ANN、GRU、Transformer）在多种持续学习策略下的表现。考虑了三种主要CL家族的代表方法：基于回放的MIR、基于正则化的LwF、基于参数隔离的HAT，以及它们的所有成对和三元组合。

Result: 朴素顺序微调在所有架构中都遭受严重遗忘，没有单一CL方法能完全防止遗忘。回放是关键成分：MIR是最可靠的个体策略，包含回放的组合（MIR+HAT、MIR+LwF、MIR+LwF+HAT）始终实现高性能且具有接近零或轻度正向后迁移。最优配置与架构相关：MIR+HAT对ANN和Transformer最佳，MIR+LwF+HAT对GRU最佳，某些情况下CL方法甚至超过联合训练。

Conclusion: 设计持续意图分类系统时需要联合选择骨干架构和持续学习机制，回放是防止遗忘的关键成分，最优配置具有架构依赖性。
...
TL;DR: 研究发现LLMs在作为自动评分器时存在基于写作风格的隐性评分偏见，尽管内容正确性相同，但在Essay/Writing任务中语法错误、非正式语言和非母语表达都会导致显著分数降低，而数学和编程任务中偏见较小。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在教育环境中越来越多地被用作自动评分器，对其评估的公平性和偏见问题变得至关重要。本研究旨在探究当内容正确性保持不变时，LLMs是否会基于写作风格表现出隐性评分偏见。

Method: 构建了包含三个学科（数学、编程、Essay/Writing）的180个学生回答的受控数据集，每个回答包含三种表面扰动类型：语法错误、非正式语言和非母语表达。使用两个最先进的开源LLM（LLaMA 3.3 70B和Qwen 2.5 72B），在1-10分制上对回答进行评分，明确指示仅评估内容正确性而忽略写作风格。

Result: 在Essay/Writing任务中，两种模型对所有扰动类型都显示出统计显著的评分偏见（p < 0.05），效应大小从中等（Cohen's d = 0.64）到非常大（d = 4.25）。非正式语言受到最重惩罚，LLaMA平均扣1.90分，Qwen扣1.20分。相比之下，数学和编程任务显示出最小偏见，大多数条件未达到统计显著性。

Conclusion: LLM评分偏见具有学科依赖性、风格敏感性，即使在评分提示中有明确的抗偏见指令，偏见仍然存在。研究讨论了LLM评分系统公平部署的影响，并建议在机构采用前进行偏见审计协议。
...
Method: 分析四个在不同语言对上对齐的XLM-R编码器模型，这些模型针对词性标注或句子分类进行微调。使用表示分析，包括嵌入距离、梯度相似性和对齐损失与任务损失的梯度大小。

Result: 发现：(1) 嵌入距离本身是任务性能改进或下降的不可靠预测指标；(2) 对齐梯度与任务梯度通常接近正交，表明优化一个目标对优化另一个目标贡献很小。

Conclusion: 研究结果解释了为何"更好"的对齐往往无法转化为"更好"的跨语言迁移，并提供了结合跨语言对齐与任务特定微调的实用指南，强调仔细选择损失函数的重要性。
...
TL;DR: 该研究调查了Duolingo等语言学习应用中专业场景内容的缺失问题，发现现有应用主要提供通用场景（如问候、点餐），而缺乏职业特定内容，这阻碍了学习者达到专业流利度。研究建议应用应生成个性化、领域特定的课程场景，同时保持通用基础支持。


<details>
  <summary>Details</summary>
Motivation: 当前流行的语言学习应用（如Duolingo）使用大语言模型生成课程，但这些课程主要关注通用现实世界场景（如问候、点餐、问路），对职业特定场景的支持有限。这种差距阻碍了学习者达到专业流利度，即能够舒适地用目标语言交流各种工作相关和领域特定信息的能力。

Method: 研究调查了菲律宾一家跨国公司的五名员工使用Duolingo的体验。通过调查收集了他们对通用场景与工作相关场景使用频率、效果以及相关性的反馈。分析了参与者建议的课程场景，并发现这些建议在上下文上存在差异。

Result: 调查结果显示：受访者遇到通用场景的频率高于工作相关场景；通用场景具有关联性和有效性，能够建立基础语法、词汇和文化知识；工作相关场景有助于缩小专业流利度的差距，因为它们包含领域特定词汇；每位参与者建议的课程场景在聚合分析时显示出不同的上下文。

Conclusion: 语言学习应用应生成能够适应个人需求的课程，通过个性化、领域特定的课程场景来满足专业需求，同时通过通用、相关的课程场景保持基础支持。这种双重策略可以帮助学习者从基础语言能力发展到专业流利度。
...
TL;DR: 本文提出了一种混合框架，将人类参与明确整合到基于LLM的无障碍文本生成中，通过人机协同机制提高文本简化的质量和规范性。


<details>
  <summary>Details</summary>
Motivation: 当前自动文本简化和评估流程主要依赖自动化指标驱动，未能反映用户实际理解能力或规范性标准，需要更有效的人类参与机制来确保认知可访问性。

Method: 开发了一个混合框架，包含：1) 人机协同(Human-in-the-Loop)在生成过程中指导调整；2) 人机监督(Human-on-the-Loop)进行系统性的后生成审查；3) 将实证证据转化为检查表、触发规则和可访问性KPI。

Result: 该框架展示了如何将人类中心机制编码用于评估，并提供结构化反馈以改进模型适应，建立了可追溯、可复制、可审计的无障碍文本创建和评估流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识对话系统主要针对英语，缺乏明确的引用机制来验证事实主张，且模型决策透明度有限。需要开发支持双语、可解释且具备引用验证能力的对话系统。

Method: 提出四阶段渐进训练框架：1) 多语言适应；2) 英语对话SFT（监督微调）与引用对齐；3) 双语对话SFT；4) GRPO对齐与引用感知奖励。评估了6个编码器-解码器（250M-3B）和解码器（1B-7B）架构模型。

Result: 1) 引用对齐SFT使编码器-解码器模型从第二阶段起幻觉率降至0%；2) 渐进框架防止灾难性遗忘同时提升印地语能力；3) 小型模型经SFT后在英语任务上匹配大型模型；4) GRPO对结构化引用任务仅提供边际改进；5) 三种事后可解释性分析揭示了引用行为的学习过程。

Conclusion: XKD-Dial框架成功实现了英印双语可解释知识对话，通过引用机制有效减少幻觉，渐进训练策略平衡了多语言能力，系统化的可解释性分析揭示了模型学习引用行为的内在机制。
...
TL;DR: RADIUS是一个用于调查模拟的综合性二维对齐评估套件，包含排序对齐和分布对齐两个维度，并辅以统计显著性检验，解决了现有评估指标碎片化、非标准化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有调查模拟评估使用从其他领域借用的指标，这些指标通常是临时性的、碎片化的、非标准化的，导致结果难以比较。现有指标主要关注准确性或分布度量，忽视了关键的排序对齐维度。实际上，模拟可能获得高准确性但仍无法捕捉人类最偏好的选项——这在决策应用中至关重要。

Method: 提出RADIUS评估套件，包含两个维度：1) 排序对齐（RAnking alignment）- 评估模拟结果与人类偏好的排序一致性；2) 分布对齐（DIstribUtion alignment）- 评估模拟结果的分布特征。每个维度都辅以统计显著性检验。提供开源实现以确保可重复和可比较的评估。

Result: RADIUS揭示了现有指标的局限性，能够对调查模拟进行更有意义的评估。该框架强调了仅依赖准确性或分布度量的不足，展示了排序对齐在捕捉人类偏好方面的关键作用。

Conclusion: RADIUS为调查模拟提供了一个全面、标准化的评估框架，通过同时考虑排序对齐和分布对齐两个维度，解决了现有评估方法的碎片化问题，为决策应用提供了更可靠的模拟质量评估工具。
...
TL;DR: HCQR提出假设条件查询重写框架，通过生成三个针对性查询（支持假设、区分竞争选项、验证线索）改进RAG，使检索更聚焦于决策证据而非主题相关性。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法在需要从多个选项中做决策的任务中存在局限：单查询检索偏向主题相关性而非决策证据，导致检索的背景信息难以区分答案选项。

Method: HCQR框架首先从输入问题和候选选项生成轻量级工作假设，然后重写为三个针对性查询：1) 支持假设的证据；2) 区分竞争替代方案的证据；3) 验证问题中关键线索的证据。

Result: 在MedQA和MMLU-Med数据集上，HCQR持续优于单查询RAG和重排/过滤基线，平均准确率分别比Simple RAG提高5.9和3.6个百分点。

Conclusion: HCQR通过假设驱动的多查询重写，将RAG从主题导向检索转变为证据导向检索，显著提升了需要从竞争选项中选择答案的任务性能。
...
TL;DR: MultiTempBench是一个多语言时间推理基准测试，包含三个任务（日期计算、时区转换、时间关系提取），覆盖五种语言和多种历法系统，用于评估LLM的时间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有时间推理基准测试主要关注英语和公历系统，缺乏对多语言和多种历法系统的覆盖。需要系统评估LLM在不同语言资源条件下的时间推理能力，特别是分词质量对时间推理的影响。

Method: 构建包含15,000个示例的多语言时间推理基准测试，通过翻译750个精心设计的英文问题并扩展为受控的日期格式变体。评估20个LLM，引入多语言日期碎片化比率（mDFR）指标，并进行几何探测分析内部时间表示。

Result: 发现时间标记的分词质量是资源依赖的瓶颈：在低资源语言和罕见历法格式中，碎片化会破坏年/月/日分离，导致准确性崩溃；而在高资源设置中通常对数字级分割具有鲁棒性。回归分析显示，在高资源语言中时间线性是最强预测因子，而在低资源语言中碎片化是更强预测因子。

Conclusion: 时间标记的分词质量对LLM的时间推理能力有显著影响，特别是在低资源语言和罕见历法系统中。需要改进LLM的分词策略以支持多语言时间推理，特别是在资源受限的语言环境中。
...
TL;DR: 研究发现LLM生成的词类比比人类生成的更符合平行四边形几何模型，人类类比中存在大量弱完成项，而LLM在保持关系一致性方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 探究平行四边形模型在词类比中表现不佳的原因：是该模型本身有问题，还是人类不擅长生成保持关系的类比？通过比较人类和LLM在相同类比问题上的表现来回答这个问题。

Method: 使用Peterson等人(2020)的相同类比问题集，比较人类和LLM的类比完成情况。评估标准包括：人类评分、在GloVe分布嵌入空间中的平行四边形对齐度、对局部相似性的依赖程度以及词语可及性。

Result: 1) LLM生成的类比在评分上优于人类生成的类比；2) LLM类比在GloVe嵌入空间中更符合平行四边形结构；3) LLM的优势源于更强的平行四边形对齐和更低的词语可及性依赖，而非局部相似性敏感性；4) LLM优势主要来自人类产生的长尾弱完成项，当比较模态响应时优势消失；5) 平行四边形对齐度和词语频率能预测LLM哪些完成项优于人类。

Conclusion: 平行四边形模型并非词类比的差模型，问题在于人类常常无法生成满足关系约束的完成项，而LLM在这方面表现更一致。人类类比中存在大量弱完成项是导致模型表现不佳的主要原因。
...
TL;DR: HEALIX：首个基于真实临床笔记的公开健康素养标注数据集，包含589份笔记和三个健康素养标签，通过LLM主动学习构建，并评估了开源LLM的零样本和少样本提示策略。
...
Method: 通过社会工作者笔记采样、关键词过滤和基于LLM的主动学习相结合的方法，构建HEALIX数据集。包含589份9种类型的临床笔记，标注为低、正常、高三个健康素养标签。使用四个开源大语言模型评估零样本和少样本提示策略。

Result: 成功创建了首个公开可用的健康素养标注数据集HEALIX。通过基准测试展示了该数据集的实用性，为从非结构化临床笔记中自动检测健康素养提供了资源支持。

Conclusion: HEALIX填补了健康素养自动化检测领域缺乏标注数据的空白，为开发基于临床笔记的健康素养评估工具提供了重要资源，展示了LLM在医疗文本分析中的潜力。
...
TL;DR: DaPT：一种针对多语言多跳问答任务的新型检索增强生成框架，通过并行生成子问题图并融合双语检索策略，显著提升多语言场景下的RAG性能。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统在英语多跳问答任务中表现良好，但在多语言跨语料检索场景下面临两大挑战：1）缺乏评估多语言多跳问答能力的基准测试；2）过度依赖LLM在英语中的语义理解能力，导致多语言场景效果下降。

Method: 首先构建多语言多跳问答基准（将英语基准翻译为5种语言），然后提出DaPT框架：1）为源语言查询及其英语翻译并行生成子问题图；2）合并两个子问题图；3）采用双语检索-回答策略顺序解决子问题。

Result: 实验表明：1）先进RAG系统在多语言场景下存在显著的性能不平衡；2）DaPT相比基线方法能产生更准确、简洁的答案；3）在最具挑战性的MuSiQue基准上，DaPT的平均EM分数相对最强基线提升18.3%。

Conclusion: DaPT框架通过并行生成和融合双语子问题图，有效解决了多语言多跳问答中的检索挑战，显著提升了RAG系统在多语言场景下的性能表现。
...
TL;DR: UGID提出了一种基于图同构的内部表示级去偏框架，通过约束Transformer计算图中的注意力路由和隐藏表示来减少LLM的社会偏见，同时保持模型的安全性和实用性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在显著的社会偏见，现有的输出级或数据优化去偏方法无法完全解决这些问题，许多研究表明偏见嵌入在内部表示中，需要从内部表示层面进行去偏。

Method: 将Transformer建模为结构化计算图（注意力机制定义路由边，隐藏状态定义节点），通过强制图结构在反事实输入间的同构性来实现去偏，允许仅在敏感属性上存在差异。联合约束偏见敏感区域的注意力路由和隐藏表示，防止偏见在架构组件间迁移。引入对数空间敏感logit约束和选择性锚定目标来保持定义语义。

Result: 在大型语言模型上的广泛实验表明，UGID有效减少了分布内和分布外设置下的偏见，显著降低了内部结构差异，同时保持了模型的安全性和实用性。

Conclusion: UGID提供了一个有效的内部表示级去偏框架，通过图同构约束实现了偏见减少而不损害模型能力，为LLM偏见缓解提供了新的解决方案。
...
TL;DR: 提出一种基于缩放定律的预训练与继续预训练计算分配优化方法，在多领域设置中提升语言模型性能


<details>
  <summary>Details</summary>
Motivation: 传统两阶段训练范式（全数据预训练+高质量数据继续预训练）在多领域设置中需要为每个领域独立训练多个模型，计算效率低下，需要优化预训练与继续预训练之间的计算分配

Method: 提出基于缩放定律的方法：独立预训练多个模型，使用缩放定律确定预训练与继续预训练之间的最优计算分配，准确预测模型在特定规模和数据量下的损失，并可外推到更大模型和更多数据

Result: 该方法在不同模型规模和计算预算下，在常识知识和推理基准测试中持续提升性能

Conclusion: 基于缩放定律的计算分配优化方法能有效提升多领域语言模型训练效率，为大规模模型训练提供理论指导
...
Method: 提出可变熵策略优化(VEPO)，利用强化学习与可验证奖励，将确定性结构约束融入策略对齐过程，包含可变熵机制动态校准字面保真度与语义自然性之间的平衡

Result: 在90个FLORES-200、COMET-22、chrF方向上的实证评估表明，VEPO在分词效率和翻译质量方面均有显著提升，缩小了低资源语言的性能差距

Conclusion: VEPO框架通过强化学习结合结构约束，有效改善了低资源语言的性能表现，为语言模型在多样化语言环境中的应用提供了有效解决方案
...
TL;DR: 评估LLM在重复博弈中的战略表现，区分真实推理与记忆模式依赖，通过反事实变体揭示模型局限性


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在重复博弈环境中的战略表现，区分其行为是基于真实推理还是依赖记忆的模式匹配，揭示LLM在战略决策中的局限性

Method: 采用两种经典博弈（囚徒困境和石头剪刀布），引入反事实变体改变收益结构和行动标签，打破熟悉的对称性和支配关系，使用多指标评估框架比较默认和反事实实例

Result: 展示了LLM在反事实环境中的局限性，包括激励敏感性不足、结构泛化能力有限以及战略推理能力不足

Conclusion: LLM在战略决策中过度依赖记忆模式而非真实推理，反事实变体有效揭示了模型在激励敏感性、结构泛化和战略推理方面的缺陷
...
TL;DR: Nemotron-Cascade 2是一个300亿参数的开放MoE模型，仅激活30亿参数，在推理和智能体能力方面达到顶尖水平，数学和编程推理性能接近前沿开放模型，是第二个在IMO、IOI和ICPC世界总决赛中达到金牌级别性能的开放权重LLM。


<details>
  <summary>Details</summary>
Motivation: 开发一个参数效率高、智能密度大的开放模型，在保持紧凑规模的同时实现顶尖的数学和编程推理能力，并在国际顶级竞赛中达到金牌级别性能。

Method: 1. 使用精心策划的数据集进行监督微调(SFT)；2. 大幅扩展Cascade RL以覆盖更广泛的推理和智能体领域；3. 在Cascade RL过程中引入多领域在线策略蒸馏，从各领域最强的中间教师模型中蒸馏知识，有效恢复基准回归并保持性能增益。

Result: 模型在参数数量仅为DeepSeekV3.2-Speciale-671B-A37B的1/20的情况下，成为第二个在IMO、IOI和ICPC世界总决赛中达到金牌级别性能的开放权重LLM，数学和编程推理性能接近前沿开放模型，展现出极高的智能密度。

Conclusion: Nemotron-Cascade 2通过创新的Cascade RL扩展和多领域在线策略蒸馏技术，成功开发出一个参数效率极高的开放模型，在紧凑规模下实现了顶尖的推理和智能体能力，为高效能AI模型的发展提供了重要参考。
...
TL;DR: F2LLM-v2是一个包含8种尺寸（80M-14B）的多语言嵌入模型系列，支持200+语言，特别关注中低资源语言，通过两阶段LLM训练流程结合多种技术实现高效性能。


<details>
  <summary>Details</summary>
Motivation: 开发更高效的多语言嵌入模型，特别关注之前服务不足的中低资源语言，同时提供从轻量到大规模的不同尺寸模型以满足不同资源约束的应用需求。

Method: 使用新构建的6000万高质量数据样本进行训练，采用两阶段LLM嵌入训练流程，结合matryoshka学习、模型剪枝和知识蒸馏技术，支持200多种语言。

Result: F2LLM-v2-14B在11个MTEB基准测试中排名第一，系列中较小模型也在资源受限应用中创造了新的最先进水平，整体比之前的LLM嵌入模型更高效。

Conclusion: F2LLM-v2系列提供了高效且性能优异的多语言嵌入模型，特别改善了中低资源语言的支持，作者开源了所有模型、数据、代码和中间检查点以促进研究。
...
Method: 提出在微调过程中使用最优传输作为对齐目标，以无监督方式学习上下文中的词对齐，支持源语言和目标语言句子间的软匹配

Result: 在XNLI和XQuAD两个任务上基准测试，相比基线方法取得改进，与近期类似工作相比获得有竞争力的结果

Conclusion: 最优传输作为对齐目标能有效改进多语言上下文表示，无需预对齐数据，通过无监督学习上下文词对齐实现更好的跨语言迁移
...
<details>
  <summary>Details</summary>
Motivation: 共享经济中自私行为导致整体效率低下，而集中协调又剥夺个体决策权。传统货币机制基于财富差异造成不公平，需要设计既能协调个体偏好与系统目标，又能保证公平性的机制。

Method: 提出代币机制建模为连续时间动态博弈，考虑有限个有限理性智能体及其演化策略修订动态。采用平均场近似有限人口博弈，建立强近似保证，并设计闭式整数收费来引导聚合动态向最优高效公平分配收敛。

Result: 建立了平均场与有限人口博弈之间的强近似保证，设计了闭式整数收费机制，能够从任意初始条件出发，可证明地引导聚合动态向最优高效公平分配收敛。

Conclusion: 代币机制能够解决共享经济中的效率与公平权衡问题，通过演化博弈框架和平均场近似，实现了既能协调个体决策又能保证系统效率与公平性的动态资源分配方案。
...
TL;DR: 提出进化稳定Stackelberg均衡(SESS)概念，研究具有单个领导者和对称追随者种群的Stackelberg进化博弈，领导者选择最优混合策略，预期追随者种群在诱导子博弈中采用进化稳定策略(ESS)


<details>
  <summary>Details</summary>
Motivation: 现有Stackelberg进化博弈方法要么通过进化动态定义追随者响应，要么假设理性最优响应行为，但没有明确强制执行对突变入侵的稳定性。需要一种结合Stackelberg领导-追随者结构和进化稳定性的新解决方案概念

Method: 提出SESS概念，领导者选择最优混合策略，预期追随者种群在诱导子博弈中采用ESS并可能满足额外生态条件。考虑领导最优和追随最优两种ESS选择机制。提供离散和连续博弈中计算SESS的算法

Result: 建立了SESS理论框架，开发了计算算法，并在连续博弈中进行了实证验证。模型适用于生物场景，如癌症治疗中医生作为领导者，竞争性癌细胞表型作为追随者

Conclusion: SESS为Stackelberg进化博弈提供了新的解决方案概念，结合了Stackelberg领导-追随者结构和进化稳定性，填补了现有方法的空白，具有重要的理论和应用价值
...
TL;DR: 研究拍卖设计中价值最大化（分配商品）和成本最小化（分配杂务）的机制设计问题，移除传统领域限制和单调性假设，提供算法和硬度结果


<details>
  <summary>Details</summary>
Motivation: 在Milgrom-Weber相互依赖模型中，现有研究主要关注价值最大化拍卖，且通常施加领域限制和单调性假设。本文旨在研究更一般的计算问题，优化真实机制的近似比，同时考虑价值和成本两种情况，移除传统限制

Method: 研究确定性随机设置下真实机制的近似比优化计算问题，将先前特殊案例简化为经典组合问题并提供高效算法，同时为一般情况提供查询复杂度下界和NP-hardness证明

Result: 为先前可处理特殊案例提供理论解释和高效算法，同时证明一般情况的查询复杂度下界和NP-hardness，补充了正反两方面结果

Conclusion: 在移除领域限制和单调性假设后，拍卖设计中的机制优化问题在特殊情况下可简化为经典组合问题而可解，但一般情况下是计算困难的，需要近似算法
...
TL;DR: 论文提出并分析了一种新的多智能体路径规划问题，其中智能体需要在包含周期性危险的环境中从共同起点移动到共同目标，目标是最大化到达目标的智能体数量。


<details>
  <summary>Details</summary>
Motivation: 在危险环境中协调智能体（如冲突地区运送援助的无人机或障碍物密集区域的现场机器人）面临基本规划挑战。需要开发能够处理周期性危险环境的规划算法，以最大化智能体存活率。

Method: 引入并分析了一种新的多智能体路径规划问题的计算复杂性。环境基于图表示，包含周期性危险（接触后消除智能体，但经过已知冷却期后重新激活）。在离散时间、完全可观测、确定性设置下，规划任务是计算最大化到达目标智能体数量的移动调度。

Result: 1. 证明尽管可行计划空间指数级大，但最优计划只需要多项式步数，确立了NP成员资格；2. 证明即使环境图是树，问题也是NP难的；3. 提出了针对由从起点到目标的顶点不相交路径组成的图的线性时间算法。

Conclusion: 该研究为这一多智能体路径规划问题建立了丰富的计算图景，识别了不可处理和可处理的片段，为危险环境中的智能体协调提供了理论基础和算法框架。
...
### [155] [Damage identification using noisy frequency response functions based on topology optimization](https://arxiv.org/abs/2603.18569)
*Akira Saito,Ryo Sugai,Zhongxu Wang,Hidetaka Saomoto*

Main category: cs.CE

TL;DR: 基于噪声频响函数和拓扑优化的鲁棒损伤识别方法，将损伤识别转化为从结构动态响应反演损伤拓扑的逆问题，采用SIMP方法和Lasso正则化处理病态问题。


<details>
  <summary>Details</summary>
Motivation: 传统损伤识别方法在处理噪声数据和病态逆问题时存在局限性，需要开发能够有效处理测量噪声并准确识别损伤区域的鲁棒方法。

Method: 将损伤识别问题表述为从结构动态响应反演损伤拓扑的逆问题，通过最小化实验模态分析获得的测量FRF与有限元谐响应分析获得的FRF之间的误差函数，采用SIMP方法进行材料分布优化，并应用Lasso正则化（L1范数惩罚）处理病态问题。

Result: 方法成功应用于悬臂板的损伤识别，实验获得的FRF数据验证了该方法能够有效识别损伤区域，同时消除虚假损伤区域并最小化测量噪声的影响。

Conclusion: 提出的基于噪声FRF和拓扑优化的损伤识别方法具有鲁棒性，能够有效处理病态逆问题和测量噪声，成功实现了结构损伤的准确识别。
...
### [156] [Model Reference Adaptive Control For Gust Load Allevation of Nonlinear Aeroelastic](https://arxiv.org/abs/2603.18584)
*Nikolaos D. Tantaroudas,Andrea Da Ronch,Guanqun Gai,Ilias Karachalios,Kenneth J. Badcock,Rafael Palacios*

Main category: cs.CE

TL;DR: 基于Lyapunov稳定性的模型参考自适应控制用于非线性气动弹性系统的阵风载荷减缓，在无人机上显著降低翼尖挠度，优于H∞鲁棒控制


<details>
  <summary>Details</summary>
Motivation: 为柔性飞机在确定性和随机扰动环境下的阵风载荷减缓提供有效控制框架，解决非线性气动弹性系统的载荷控制问题

Method: 基于Lyapunov稳定性理论开发模型参考自适应控制，采用泰勒级数展开和特征向量投影得到的非线性降阶模型，包含参考模型设计、自适应控制律和Lyapunov推导的适应律

Result: 在无人机离散阵风下显著降低翼尖挠度，优于H∞鲁棒控制基准；在Von Karman随机湍流下也获得有意义的减载效果，性能随适应率缩放

Conclusion: MRAC为柔性飞机在确定性和随机扰动环境下的阵风载荷减缓提供了有效框架，适应率矩阵是最重要的设计参数，控制收敛速度、峰值载荷减少和作动器需求之间的权衡
...
### [157] [Pore-scale modeling of capillary-driven binder migration during battery electrode drying](https://arxiv.org/abs/2603.18860)
*Marcel Weichel,Martin Reder,Gerit Mühlberg,David Burger,Philip Scharfer,Wilhelm Schabel,Britta Nestler,Daniel Schneider*

Main category: cs.CE

TL;DR: 本文开发了一种孔隙尺度连续介质模型，用于模拟硬碳电极干燥过程中的毛细管驱动粘结剂迁移，揭示了颗粒尺寸、蒸发速率和表面张力对粘结剂分布的影响。


<details>
  <summary>Details</summary>
Motivation: 钠离子电池硬碳电极的干燥是关键制造步骤，现有模型主要依赖一维薄膜收缩或忽略毛细管输运，缺乏物理一致的微结构解析预测粘结剂迁移的方法。

Method: 扩展了空间分辨的孔隙尺度连续介质模型，明确描述孔隙排空过程中的毛细管驱动粘结剂输运，应用于不同平均颗粒直径的硬碳微结构。

Result: 模拟显示：较小颗粒尺寸导致更均匀的粘结剂分布；较高蒸发速率和增加表面张力促进更强的粘结剂梯度；溶剂粘度变化对粘结剂迁移影响较小（无亲疏水性时）。

Conclusion: 明确描述毛细管输运和微结构效应对于准确预测粘结剂迁移至关重要，为电极干燥过程的针对性优化提供了基础。
...
### [158] [On the Minimum Number of Control Laws for Nonlinear Systems with Input-Output Linearisation Singularities](https://arxiv.org/abs/2603.18947)
*Nikolaos D. Tantaroudas*

Main category: cs.CE

TL;DR: 论文提出了(k+1)-控制器引理，证明对于具有k个代数独立条件参数化奇点流形的n阶SISO非线性系统，恰好需要k+1个不同的控制律才能实现全局可控性。


<details>
  <summary>Details</summary>
Motivation: 解决非线性系统反馈线性化控制器存在奇点时，确定实现全局可控性所需的最小控制律数量这一基本问题。当系统在特定状态子集上无法定义线性化控制器时，需要多个控制律来覆盖整个状态空间。

Method: 提出并严格证明(k+1)-控制器引理：1)充分性证明采用近似线性化方法结合微分拓扑的横截性论证；2)必要性证明通过反证法，使用隐函数定理、维度计数论证和近似线性化框架的结构约束。

Result: 对于n阶单输入单输出非线性系统，如果奇点流形由k个代数独立条件参数化，则恰好需要k+1个不同的控制律才能实现完全状态空间覆盖。该结果通过球-梁系统（四阶机械系统，在第三输出导数处表现出双参数奇点）的详尽分析得到验证。

Conclusion: 论文建立了非线性系统全局可控性与奇点流形参数化维度之间的精确关系，为具有奇异性的非线性系统控制设计提供了理论指导，证明k+1个控制律是实现全局可控性的最小数量。
...
### [159] [FinTradeBench: A Financial Reasoning Benchmark for LLMs](https://arxiv.org/abs/2603.19225)
*Yogesh Agrawal,Aniruddha Dutta,Md Mahadi Hasan,Santu Karmaker,Aritra Dutta*

Main category: cs.CE

TL;DR: FinTradeBench是一个集成公司基本面与交易信号的金融推理基准，包含1,400个基于NASDAQ-100公司十年历史数据的问题，分为基本面、交易信号和混合推理三类，用于评估LLMs的金融决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融问答基准主要关注公司资产负债表数据，很少评估对公司股票市场交易行为或基本面与交易信号交互的推理能力。为结合两种方法的优势，需要开发一个集成公司基本面与交易信号的金融推理评估基准。

Method: 采用校准-扩展框架：结合专家种子问题、多模型响应生成、模型内自过滤、数值审计和人工-LLM评判对齐。构建包含1,400个问题的基准，分为基本面聚焦、交易信号聚焦和需要跨信号推理的混合问题三类。

Result: 评估14个LLMs在零样本提示和检索增强设置下的表现，发现明显的性能差距。检索显著改善对文本基本面的推理，但对交易信号推理的益处有限。这突显了当前LLMs在数值和时间序列推理方面的基本挑战。

Conclusion: FinTradeBench基准揭示了当前LLMs在金融智能方面的局限性，特别是在数值和时间序列推理方面，为未来金融智能研究提供了动机和方向。
...
### [160] [A New Approach to Code Smoothing Bounds](https://arxiv.org/abs/2603.18077)
*Tsuyoshi Miezaki,Yusaku Nishimura,Katsuyuki Takashima*

Main category: cs.IT

TL;DR: 提出基于随机游走和公平划分的新方法，推导码的总变差距离不等式，推广有限阿贝尔群上的现有结果


<details>
  <summary>Details</summary>
Motivation: 分析基于码的密码系统安全性需要研究平滑参数，该参数与码的总变差距离密切相关。现有研究使用局部紧阿贝尔群上的傅里叶变换来界定该距离，本文寻求基于随机游走的替代方法。

Method: 采用基于随机游走的方法，利用公平划分推导总变差距离的不等式。该方法不依赖傅里叶变换，而是通过随机游走的性质来界定码的距离。

Result: 推导出基于公平划分的随机游走总变差距离不等式，证明所提出的界推广了有限阿贝尔群上的现有结果。

Conclusion: 基于随机游走和公平划分的方法为分析码的总变差距离提供了新的理论框架，推广了现有结果，为基于码的密码系统安全性分析提供了替代工具。
...
### [161] [Growing Alphabets Do Not Automatically Amplify Shuffle Privacy: Obstruction, Estimation Bounds, and Optimal Mechanism Design](https://arxiv.org/abs/2603.18080)
*Alex Shvets*

Main category: cs.IT

TL;DR: 该论文研究了在增长字母表下通过洗牌实验实现ε₀-LDP的机制设计，证明了隐私压缩定理、通用下界，并发现增强GRR机制在洗牌设置下优于标准GRR。


<details>
  <summary>Details</summary>
Motivation: 研究在字母表规模增长(d→∞)的情况下，通过洗牌实验实现本地差分隐私(LDP)的机制设计问题。传统本地差分隐私机制在洗牌设置下可能不是最优的，需要探索洗牌特有的隐私-效用权衡特性。

Method: 1. 隐私分析：证明精确压缩定理，建立通用chi²界，构造显式阻碍族，证明稀释/持久二分法；2. 估计理论：通过Cramér-Rao和Assouad论证推导通用下界，证明对称化到等变通道是可行的；3. 机制设计：分析校准GRR的局限性，提出增强GRR机制（部分用户使用激进GRR，其余发送空符号）。

Result: 1. 隐私方面：建立了精确压缩定理和尖锐的chi²界(e^{ε₀}-1)²/e^{ε₀}，发现洗牌隐私曲线对所有d都等于二元随机响应；2. 估计方面：证明了阶为(d-1)/(nχ_*(W))的通用下界；3. 设计方面：发现校准GRR不是最优的，增强GRR（激进GRR+空符号）在低预算下是所有置换等变通道中最优的，且GRR是子集选择族中的唯一最优解。

Conclusion: 洗牌实验中的LDP机制设计与传统本地LDP有本质不同。增强GRR机制通过"稀释原则"（部分用户激进报告，其余保持沉默）实现了最优频率估计，这一特性是洗牌设置特有的，在传统本地LDP中没有对应物。该研究为大规模字母表下的隐私保护频率估计提供了理论基础和实用机制。
...
### [162] [Achievable DoF Bounds for Cache-Aided Asymmetric MIMO Communications](https://arxiv.org/abs/2603.18240)
*Mohammad NaseriTehrani,MohammadJavad Salehi,Antti Tölli*

Main category: cs.IT

TL;DR: 该论文研究了缓存辅助的非对称MIMO系统，提出了四种内容感知的MIMO编码缓存策略，在保证线性可解码性的三种对称缓存放置-传输策略基础上，显著提升了系统自由度性能。


<details>
  <summary>Details</summary>
Motivation: 将编码缓存集成到MIMO通信中可以显著提升可实现的自由度，但现有研究多关注对称配置。实际系统中用户接收天线数量往往不同，因此需要研究缓存辅助的非对称MIMO配置，以充分利用系统资源。

Method: 论文研究了具有缓存比例γ的缓存辅助非对称MIMO配置，服务器有L根发射天线，服务K个用户。用户被分为J组，第j组用户有G_j根接收天线。提出了四种内容感知的MIMO-CC策略：1) min-G策略：使用用户中最小的天线数量强制对称；2) Grouping策略：最大化子集内空间复用增益，牺牲部分全局缓存增益；3) Super-grouping策略：将用户聚合成具有相同有效接收复用增益的超集，然后在这些超集间应用Grouping策略；4) Phantom策略：假设用户有"幻影"天线来重新分配空间资源，弥合min-G和Grouping策略的性能差距。这些策略在三种保证线性可解码性的对称CC放置-传输策略下开发：达到最优单次自由度的DoF最优策略，以及两种闭式策略（组合式和线性循环低复杂度构造）。

Result: 分析和数值结果表明，在各种系统配置下都实现了显著的自由度改进。策略-策略组合在自由度和子分组复杂度之间提供了灵活的权衡。循环策略在许多工作区域中达到接近其他策略的DoF性能。

Conclusion: 论文提出的四种内容感知MIMO-CC策略有效解决了缓存辅助非对称MIMO系统的资源分配问题，通过策略组合在系统性能和复杂度之间提供了可调节的权衡，为实际MIMO缓存系统的设计提供了理论指导。
...
### [163] [Polynomial Constructions and Deletion-Ball Geometry for Multiset Deletion Codes](https://arxiv.org/abs/2603.18322)
*Avraham Kreindel,Isaac Barouch Essayag,Aryeh Lev Zabokritskiy*

Main category: cs.IT

TL;DR: 该论文研究删除度量下多集合的纠错码，提出了多项式Sidon型构造和删除球的几何分析，获得了独立于块长度的冗余度以及精确的球体积公式和编码界。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于处理排序完全丢失的置换信道，其中错误仅作用于符号多重性。在删除度量下，需要开发能够纠正符号多重性错误的多集合纠错码，以应对信道中排序信息的完全丢失。

Method: 采用两种互补方法：1）基于有限域的多项式Sidon型构造，包括射影形式和仿射形式；2）对删除球进行几何分析，使用差分向量表示和生成函数的对角约简，推导精确的删除球体积公式和极值中心特征。

Result: 获得了在t<q时冗余度为t+O(1)且独立于块长度n的多集合t-删除纠错码；推导了精确的删除球体积生成函数、固定距离m下的有序对数量公式以及平均球体积；证明了半径r删除球在极端多集合处最小，在最平衡多集合处最大；获得了q=2和q=3时的显式闭式解；建立了基于体积的编码界，包括球填充上界和Gilbert-Varshamov型下界。

Conclusion: 该论文为删除度量下的多集合纠错码提供了系统的理论框架，结合代数构造和几何分析，获得了独立于块长度的高效编码方案以及精确的球体积特征，为置换信道中的纠错问题提供了完整的解决方案。
...
### [164] [Some structural properties of mixed orthogonal arrays and their irredundancy](https://arxiv.org/abs/2603.18568)
*Maryam Bajalan,Peter Boyvalenkov,Ferruh Özbudak*

Main category: cs.IT

TL;DR: 该论文建立了混合正交阵列的三个结构结果：Singleton型上界及MDS/几乎MDS阵列的表征；引入迹对偶性并建立与误差块码的对应关系；发展不可约混合正交阵列的结构理论及其与AME量子态的联系。


<details>
  <summary>Details</summary>
Motivation: 混合正交阵列的研究需要不同于对称正交阵列的结构工具，因为对称设置中的关键特性（如欧几里得对偶性、唯一全局指标和经典界）在混合情况下不再可用。本文旨在为混合正交阵列建立系统的结构理论。

Method: 1. 证明Singleton型上界并表征MDS和几乎MDS混合正交阵列；2. 引入Fq线性MOA在∏Fq^{n_i}上的迹对偶性，建立与Fq线性误差块码的对应关系，通过关联误差块码的对偶距离确定MOA的强度；3. 发展不可约混合正交阵列的结构理论，特别在极值情况下证明Fq线性IrMOA与Fq线性误差块MDS码的等价性。

Result: 1. 建立了混合正交阵列的Singleton型上界，获得了MDS和几乎MDS混合正交阵列的完整表征；2. 建立了Fq线性MOA与Fq线性误差块码之间的对应关系，使得MOA的强度可通过关联误差块码的对偶距离确定；3. 证明了在极值情况下，最小指标为1的Fq线性IrMOA等价于Fq线性误差块MDS码，这些结果可用于构造t-均匀和绝对最大纠缠量子态。

Conclusion: 本文为混合正交阵列建立了系统的结构理论，填补了对称与混合正交阵列之间的理论差距。通过引入新的对偶概念和建立与误差块码的对应关系，为混合正交阵列的研究提供了有效的分析工具。特别地，不可约混合正交阵列与量子态构造的联系为量子信息领域提供了新的数学基础。
...
### [165] [BeamAgent: LLM-Aided MIMO Beamforming with Decoupled Intent Parsing and Alternating Optimization for Joint Site Selection and Precoding](https://arxiv.org/abs/2603.18855)
*Xiucheng Wang,Yue Zhang,Nan Cheng*

Main category: cs.IT

TL;DR: BeamAgent：一种将LLM作为语义翻译器与数值优化解耦的MIMO波束赋形框架，通过自然语言描述生成空间约束，实现基站选址和预编码联合优化
...
Method: LLM仅作为语义翻译器将自然语言描述转换为结构化空间约束，专用梯度优化器通过交替优化算法联合求解离散基站选址和连续预编码设计，包含场景感知提示、多轮交互意图分类和基于惩罚的损失函数

Result: 在基于射线追踪的城市场景中，BeamAgent实现84.0dB亮区功率，比穷举迫零方法高7.1dB，端到端系统与专家上界相差仅3.3dB，完整优化在笔记本电脑上2秒内完成

Conclusion: BeamAgent通过语义与数值解耦的框架，有效解决了LLM在无线通信优化中的精度不足问题，无需微调即可实现稳健的空间约束解析和高效优化
...
### [166] [Theoretical Analyses of Detectors for Additive Noise Channels with Mean-Variance Uncertainty under Nonlinear Expectation Theory](https://arxiv.org/abs/2603.18937)
*Wen-Xuan Lang,Guiying Yan,Zhi-Ming Ma*

Main category: cs.IT

TL;DR: 该论文将非线性期望理论应用于加性噪声信道检测，针对具有分布不确定性的信道推导了最优检测器，并证明其在概率模型不确定场景中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 经典信息理论中的最优检测器设计基于噪声分布已知或确定性的假设，但在实际应用中，信道噪声的概率模型往往存在不确定性。本文旨在将检测器分析扩展到概率模型不确定的情况，利用非线性期望理论来处理这种不确定性。

Method: 采用非线性期望理论框架，考虑两种分布不确定性：1) 无均值不确定性但具有方差不确定性；2) 同时具有均值和方差不确定性。针对二进制输入加性噪声信道，推导了非线性期望最优准则下的最优检测器，并给出了显式形式。此外，提出了信道噪声不确定参数的估计方法。

Result: 研究发现均值不确定性显著影响最优检测器的形式，而方差不确定性则不影响。实验结果表明，在大多数概率模型不确定的场景中，新提出的检测方法优于基于确定性概率模型的经典最优检测器。

Conclusion: 通过将非线性期望理论应用于加性噪声信道检测，本文成功处理了概率模型不确定性问题，推导的最优检测器在不确定环境下具有更好的性能，证明了该理论框架在实际应用中的相关性和有效性。
...
### [167] [DEAF: A Benchmark for Diagnostic Evaluation of Acoustic Faithfulness in Audio Language Models](https://arxiv.org/abs/2603.18048)
...
<details>
  <summary>Details</summary>
Motivation: 尽管现有的音频多模态大语言模型在语音基准测试中表现出色，但尚不清楚这些模型是否真正处理声学信号，还是主要依赖文本语义进行推理。需要系统研究模型对声学信息的忠实处理能力。

Method: 1) 构建DEAF基准，包含超过2,700个冲突刺激，涵盖三个声学维度：情感韵律、背景声音和说话人身份；2) 设计受控的多级评估框架，逐步增加文本影响，从内容语义冲突到误导性提示及其组合；3) 引入诊断指标量化模型对文本线索而非声学信号的依赖程度。

Result: 对7个音频MLLM的评估显示一致的文本主导模式：模型对声学变化敏感，但预测主要由文本输入驱动，揭示了标准语音基准测试的高性能与真正的声学理解之间存在差距。

Conclusion: 当前音频多模态大语言模型存在文本主导偏差，模型性能主要依赖文本语义而非声学信号处理，需要开发更忠实处理声学信息的模型架构和评估方法。
...
### [168] [Continually self-improving AI](https://arxiv.org/abs/2603.18073)
*Zitong Yang*

Main category: cs.AI

TL;DR: 该论文提出三种方法突破语言模型对人类创造者的依赖限制：1) 使用合成数据增强小规模专业语料的知识获取效率；2) 利用有限人类数据自生成合成数据进行预训练；3) 通过测试时算法搜索超越人工设计的训练范式。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型的能力受到人类创造者的三重限制：1) 小规模专业语料的知识获取数据效率低；2) 依赖有限的人类生成历史数据；3) 受限于人类研究者发现和探索的算法。需要突破这些限制以实现持续自我改进的AI系统。

Method: 提出三个核心方法：1) 合成数据方法，将小规模语料多样化和放大为丰富的知识表示；2) 自生成合成数据方法，利用有限人类数据引导基础预训练能力；3) 测试时算法搜索方法，在算法空间中进行规模化搜索，探索超越人工设计的训练配置。

Result: 论文展示了三种方法能够有效突破语言模型对人类创造者的依赖：1) 提高小规模专业语料的知识获取效率；2) 减少对人类数据的依赖，实现自引导预训练；3) 超越人工设计的训练范式，发现更优的算法配置。

Conclusion: 通过合成数据增强、自生成数据引导和算法空间搜索三种方法，该研究为实现持续自我改进的AI系统迈出了重要一步，突破了语言模型对人类创造者的三重依赖限制。
...
### [169] [Multi-Trait Subspace Steering to Reveal the Dark Side of Human-AI Interaction](https://arxiv.org/abs/2603.18085)
...
<details>
  <summary>Details</summary>
Motivation: 人类-AI交互可能导致负面心理结果，包括心理健康危机和用户伤害。随着LLM成为指导、情感支持甚至非正式治疗的来源，这些风险正在加剧。研究有害交互机制面临方法学挑战，因为有机有害交互通常需要持续参与和广泛对话上下文，难以在受控环境中模拟。

Method: 开发Multi-Trait Subspace Steering (MultiTraitsss)框架，利用已确立的危机相关特征和新型子空间引导框架，生成展现累积有害行为模式的Dark模型。通过单轮和多轮评估验证模型的有害交互效果。

Result: Dark模型能够持续产生有害交互和结果。单轮和多轮评估显示这些模型在人类-AI交互中表现出一致的有害行为模式。

Conclusion: 使用Dark模型可以研究有害人类-AI交互机制，并提出减少人类-AI交互中有害结果的防护措施。该方法为解决研究有害交互的方法学挑战提供了新途径。
...
### [170] [Adaptive Domain Models: Bayesian Evolution, Warm Rotation, and Principled Training for Geometric and Neuromorphic AI](https://arxiv.org/abs/2603.18104)
*Houston Haynes*

Main category: cs.AI

TL;DR: 提出基于几何代数、确定内存管理和posit算术的新型AI训练架构，实现深度无关的训练内存（约推理内存2倍）、保级权重更新和精确梯度累积，支持贝叶斯蒸馏和热旋转部署。


<details>
  <summary>Details</summary>
Motivation: 传统AI训练基于IEEE-754算术的反向自动微分存在内存开销大、优化器复杂、几何属性退化等问题。需要新的算术基础和训练架构来解决这些限制，实现更高效、精确且可验证的训练系统。

Method: 结合三个关键技术：1）维度类型系统和确定性内存管理框架，实现栈可分配的梯度和精确quire累积；2）程序超图，通过几何代数计算保持grade不变性；3）b-posit 2026标准，使posit算术在传统推理硬件上可行。引入贝叶斯蒸馏从通用模型中提取先验结构，以及热旋转实现无中断模型更新。

Result: 实现深度无关的训练内存（约推理内存2倍），保级权重更新，精确梯度累积，适用于损失函数优化和脉冲时序依赖的神经形态模型。通过贝叶斯蒸馏解决领域特定训练的数据稀缺问题，通过热旋转实现无中断部署更新。
...
### [171] [Regret Bounds for Competitive Resource Allocation with Endogenous Costs](https://arxiv.org/abs/2603.18999)
*Rui Chai*

Main category: cs.AI

TL;DR: 论文研究具有内生成本的在线资源分配问题，通过三种分配策略的遗憾分析，揭示了竞争性分配在利用交互反馈方面的优势，并建立了交互拓扑结构与计算-遗憾权衡的关系。


<details>
  <summary>Details</summary>
Motivation: 研究模块化系统中在线资源分配问题，特别关注成本内生性这一新挑战——成本不仅取决于单个模块的分配，还通过交互矩阵W编码的协作与竞争关系依赖于整个分配向量。这区别于传统在线优化中的外生成本假设。

Method: 分析三种分配范式：(I)均匀分配（成本无知），(II)门控分配（成本估计），(III)通过乘性权重更新与交互反馈的竞争性分配（成本揭示）。在具有有界变化的对抗序列下进行遗憾分析，并研究交互矩阵W的拓扑结构对计算-遗憾权衡的影响。

Result: 在对抗序列下建立严格分离：均匀分配产生Ω(T)遗憾，门控分配达到O(T^{2/3})，竞争性分配达到O(√(T log N))。交互拓扑结构影响计算-遗憾权衡：完全交互(|E|=O(N²))获得最紧界但计算成本最高；稀疏拓扑(|E|=O(N))最多增加O(√(log N))遗憾，同时将每步计算成本从O(N²)降至O(N)；具有协作和竞争链接的环状拓扑（如五行拓扑）最小化计算×遗憾乘积。

Conclusion: 研究首次为模块化架构中的分散竞争性分配提供了形式化的遗憾理论依据，并确立成本内生性是区别于部分可观测性的根本挑战。交互拓扑结构在计算效率和遗憾性能之间提供了可调节的权衡。
...
### [172] [Efficient Dense Crowd Trajectory Prediction Via Dynamic Clustering](https://arxiv.org/abs/2603.18166)
*Antonius Bima Murti Wijaya,Paul Henderson,Marwa Mahmoud*

Main category: cs.AI

TL;DR: 提出一种基于聚类的密集人群轨迹预测方法，通过将相似个体分组来降低计算成本，同时保持预测精度
...
Method: 提出基于聚类的群体方法，根据相似属性随时间对个体进行分组，通过准确的群体摘要实现更快执行；该方法可作为即插即用模块，用聚类中心替代传统行人输入与现有轨迹预测器结合

Result: 在多个挑战性密集人群场景中评估，相比最先进方法实现了更快的处理速度和更低的内存使用，同时保持了预测精度

Conclusion: 提出的聚类方法能有效解决密集人群轨迹预测中的计算效率问题，通过群体摘要降低复杂度，为公共安全和管理提供实用解决方案
...
TL;DR: CAPSUL是一个全面的人类蛋白质亚细胞定位基准数据集，整合了3D结构信息与专家标注的精细亚细胞定位注释，用于评估基于结构的模型在该任务上的表现。
...
Method: 创建CAPSUL基准数据集，整合多种3D结构表示与专家精心标注的精细亚细胞定位注释；使用最先进的序列基和结构基模型进行评估；探索重加权和单标签分类策略；通过高尔基体案例研究展示结构基方法的可解释性。

Result: 展示了结构特征在该任务中的重要性；通过高尔基体案例研究，从注意力机制中发现了一个决定性的定位模式α-螺旋，证明了结构基方法具有强大的可解释性潜力。

Conclusion: CAPSUL基准填补了现有数据集的空白，为基于结构的亚细胞定位方法研究提供了基础，展示了结构基方法在生物可解释性方面的潜力，为细胞生物学中的数据驱动发现铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 高校教师缺乏及时且教育学基础的支持，现有工具要么依赖通用聊天机器人建议，要么依赖不可扩展的教学中心人工咨询，缺乏可扩展的教学指导解决方案。
...
Conclusion: 基于教育学原理、合成数据驱动的聊天机器人能够改善教学支持，为未来教学聊天机器人系统提供了可扩展的设计方法。
...
TL;DR: 本文提出了一种基于物理仿真的自动驾驶感知系统可靠性建模框架，通过误差注入生成高质量数据，使用复合似然EM算法量化误差传播


<details>
  <summary>Details</summary>
Motivation: 智能城市中AI系统的可靠性至关重要，但存在三大挑战：1) 真实可靠性数据稀缺且受隐私限制；2) 序列化阶段中的误差事件相互依赖，违反统计独立性假设；3) AI系统处理高速大数据导致频繁复杂的重复误差事件难以追踪分析

Method: 1) 利用基于物理的自动驾驶仿真平台和可解释的误差注入器生成高质量可靠性分析数据；2) 开发新的可靠性建模框架，显式表征跨阶段误差传播；3) 使用计算高效、理论保证的复合似然期望最大化算法估计模型参数

Result: 将该方法应用于自动驾驶感知系统的可靠性建模，证明了其预测准确性和计算效率

Conclusion: 提出的基于仿真的可靠性建模框架能够有效解决AI系统可靠性分析中的数据稀缺、模型有效性和计算复杂度三大挑战，为智能城市中AI系统的可靠性评估提供了实用解决方案
...
Method: 采用多智能体架构，包含五个专门的LLM驱动智能体（问题制定者、数据工程师、分析师、批评者、写作者），通过状态机协调器管理，支持修订循环、检查点恢复和沙箱代码执行，并采用三层数据注册表设计编码教育领域专业知识。

Result: 系统能够根据研究提示和数据集生成包含真实Semantic Scholar引用、验证过的机器学习分析和自动化方法学同行评审的完整LaTeX手稿，并作为开源项目发布。

Conclusion: EDM-ARS作为教育数据挖掘自动化研究框架的首个实例，展示了多智能体管道在自动化研究中的潜力，但存在单数据集范围和公式化输出等限制，未来将扩展到因果推断、迁移学习、心理测量和多数据集泛化等方向。
...
TL;DR: CORE提出了一种新的OOD检测方法，通过解耦置信度信号和残差信号，在多个架构和数据集上实现稳健的检测性能。
...
Method: CORE方法观察到倒数第二层特征自然分解为两个正交子空间：与分类器对齐的置信度分量和分类器丢弃的残差。该方法独立评分每个子空间，并通过归一化求和组合它们，利用正交性使失败模式近似独立。

Result: CORE在五种架构和五种基准配置上实现了竞争性或最先进的性能，在五个设置中的三个排名第一，获得了最高的平均AUROC，且计算开销可忽略。

Conclusion: 通过解耦置信度信号和残差信号，CORE解决了现有OOD检测方法的不一致性问题，实现了跨架构和数据集的高效稳健检测。
...
TL;DR: 论文分析了18,707个消费者健康查询，发现现有LLM基准测试存在"有效性差距"，缺乏真实临床需求的关键要素，如原始临床数据、脆弱人群代表性和慢性病管理场景。


<details>
  <summary>Details</summary>
Motivation: 临床试验依赖透明的纳入标准确保可推广性，而验证健康相关大语言模型的基准测试很少描述其包含的"患者"或"查询"人群特征。缺乏明确的组成定义可能导致聚合性能指标错误地表示模型临床应用的准备程度。

Method: 使用LLM作为自动编码工具，对六个公共基准测试中的18,707个消费者健康查询应用标准化的16字段分类法，分析上下文、主题和意图。

Result: 发现结构性"有效性差距"：虽然基准从静态检索发展到交互对话，但临床组成仍与真实世界需求不匹配。42%语料引用客观数据，但偏向健康可穿戴信号（17.7%）；复杂诊断输入罕见，包括实验室值（5.2%）、影像（3.8%）和原始医疗记录（0.6%）。安全关键场景基本缺失：自杀/自残查询<0.7%，慢性病管理仅5.5%。基准也忽视脆弱人群（儿科/老年人<11%）和全球健康需求。

Conclusion: 评估基准仍与真实世界临床需求不匹配，缺乏原始临床工件、脆弱人群的充分代表性和纵向慢性护理场景。该领域必须采用标准化的查询分析——类似于临床试验报告——以使评估与临床实践的完整复杂性保持一致。
...
TL;DR: 研究量化了临床医生如何将AI生成的草稿中面向患者的通俗表达转换为标准化临床术语，发现编辑显著减少了术语密度，评估与计划部分转换最多


<details>
  <summary>Details</summary>
Motivation: 环境AI从医患对话生成临床记录草稿，但这些草稿常使用通俗表达而非标准化临床术语。临床医生如何将这些草稿修订为专业文档规范尚不清楚，需要量化分析这种转换过程

Method: 使用字典确认的转换框架量化临床医生的编辑行为，分析71,173个AI草稿和最终记录部分对（来自34,726次就诊），定义确认转换：在同一部分用字典映射的临床等效术语替换通俗表达

Result: 编辑显著减少了所有部分的术语密度(p<0.001)；评估与计划部分转换量最大(59.3%)；共识别7,576个转换事件，涉及4,114个记录部分(5.8%)，占通俗术语删除的1.2%；不同临床医生间的转换强度存在显著差异(p<0.001)

Conclusion: 临床医生的后编辑展示了从对话式表达向标准化、适合部分的临床术语的持续转换，支持设计具有部分感知能力的环境AI系统
...
TL;DR: FaithSteer-BENCH是一个压力测试基准，用于评估LLM推理时引导方法在部署约束下的可靠性，揭示了现有方法在实际应用中存在系统性故障模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常认为简单的激活层干预可以可靠地引导LLM行为，但这些结论是在相对宽松的评估环境下得出的，忽略了部署约束、能力权衡和实际鲁棒性。需要更严格的评估基准来检验引导方法在实际部署中的可靠性。

Method: 提出FaithSteer-BENCH基准，通过三个门控标准（可控性、效用保持、鲁棒性）在固定部署式操作点上评估引导方法。使用多种模型和代表性引导方法进行系统测试，包括指令级扰动、角色提示、编码变换和数据稀缺等压力条件。

Result: 发现现有方法在标准评估中被掩盖的系统性故障模式：虚幻可控性、无关能力的可测量认知负担、在轻微扰动下的显著脆弱性。门控基准结果显示现有方法在部署导向的实际设置中无法提供可靠可控性。机制级诊断表明许多方法诱导的是提示条件对齐而非稳定的潜在方向偏移。

Conclusion: FaithSteer-BENCH为未来引导方法设计、可靠性评估和部署导向研究提供了统一基准和更清晰的分析视角，揭示了现有推理时引导方法在实际部署中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型在内部表示中编码的任务相关知识远超其输出性能，但机制可解释性方法是否能弥合这一知识-行动差距尚未得到系统测试。研究旨在评估不同可解释性方法在纠正临床误诊错误方面的实际效果。
...
Result: 线性探测能区分危险和良性案例，AUROC达到98.2%，但模型输出灵敏度仅为45.1%，存在53个百分点的知识-行动差距。概念瓶颈引导纠正了20%的漏诊危险案例，但破坏了53%的正确检测，效果与随机扰动无差异(p=0.84)。SAE特征引导尽管有3,695个显著特征，但产生零效果。TSV引导在高强度下纠正了24%的漏诊危险案例，同时只破坏了6%的正确检测，但仍有76%的错误未得到纠正。
...
TL;DR: LGESynthNet：一种基于潜在扩散的框架，用于可控的晚期钆增强（LGE）心脏MRI增强合成，通过少量训练数据实现增强区域大小、位置和透壁程度的精确控制，提升下游分割和检测性能。
...
Method: 提出LGESynthNet框架，基于ControlNet架构的修复式潜在扩散模型，包含：(a) 条件特定监督的奖励模型，(b) 解剖描述性文本提示的标注模块，(c) 生物医学文本编码器。仅使用429张图像（79名患者）训练，通过质量控制过滤器筛选高保真输出。

Result: 模型生成真实且解剖一致的合成样本。用于训练增强时，通过质量控制筛选的高保真样本可将下游分割性能提升高达6个百分点，检测性能提升高达20个百分点。

Conclusion: LGESynthNet证明了在有限标注数据下，通过可控增强合成生成高质量合成数据的可行性，有效提升下游医学图像分析任务的性能，为数据稀缺的医学影像领域提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统匿名化依赖于身份重构的高成本（需要领域专业知识、定制算法和人工验证），但LLM智能体的出现可能削弱这一屏障。研究者关注LLM智能体能否从分散的非识别性线索中自主重构身份，从而构成新的隐私风险。

Method: 将威胁形式化为"推理驱动的链接"，并在三个场景中系统评估：经典链接场景（Netflix和AOL）、受控基准InferLink（变化任务意图、共享线索和攻击者知识）以及现代文本丰富的数字痕迹。使用LLM智能体执行固定池匹配和开放式身份解析，与传统基线方法对比。

Result: 在Netflix Prize场景中，智能体成功重构了79.2%的身份，显著优于56.0%的经典基线。链接不仅出现在明确的对抗性提示下，也作为良性跨源分析和非结构化研究叙述的副产品出现。智能体无需任务特定启发式就能执行身份解析。

Conclusion: 身份推理（而不仅仅是显式信息泄露）必须被视为一级隐私风险。评估必须衡量智能体能够推断出什么身份，而不仅仅是它直接披露了什么信息。这要求重新思考隐私评估框架。
...
TL;DR: 该研究提出了一种通过文本内时间共现来发现文本过渡结构概念的方法，训练对比模型将预训练嵌入映射到关联空间，形成按功能而非主题聚类的多粒度概念图


<details>
  <summary>Details</summary>
Motivation: 传统嵌入模型按语义内容（文本主题）分组文本，但文本内时间共现能揭示不同的结构模式——文本的过渡结构概念（文本做什么），这反映了文本的功能、语域和文学传统

Method: 在9,766个古登堡计划文本的3730万共现对上训练29.4M参数的对比模型，将预训练嵌入映射到关联空间；在容量约束下（42.75%准确率），模型必须压缩重复模式而非记忆单个共现；在六个粒度级别（k=50到k=2000）进行聚类生成多分辨率概念图

Result: 关联空间聚类按功能、语域和文学传统分组文本，而非按主题；在k=100时，每个聚类平均包含4,508本书，确认了语料库范围的模式；未见过的文本无需重新训练即可分配到现有聚类；验证控制解决了位置、长度和书籍集中等混杂因素

Conclusion: 该方法将预测关联记忆从情景回忆扩展到概念形成：在压缩下的多轮对比训练提取可迁移到未见文本的结构模式，同一框架在不同机制下产生质的不同行为，揭示了文本的过渡结构概念
...
<details>
  <summary>Details</summary>
Motivation: 联合模型压缩（如剪枝和量化结合）已成为提高效率的重要策略，但压缩顺序这一核心因素被忽视。先前研究要么假设技术正交性，要么只在受限情况下考察，导致压缩顺序对模型性能的影响缺乏深入理解。

Method: 1. 形式化压缩顺序优化问题；2. 提出渐进强度假设：较弱的扰动应先于较强的扰动；3. 提供理论保证：一种顺序的相对优势随底层性能差距增加而增加；4. 在语言和视觉模型上进行广泛实验验证；5. 扩展到多阶段压缩和混合精度量化等更广泛设置。

Result: 实验验证了渐进强度假设的有效性，表明压缩顺序确实显著影响最终模型性能。理论分析显示，当底层压缩方法性能差距较大时，优化压缩顺序带来的收益更明显。该假设在多阶段压缩和混合精度量化等更广泛设置中同样适用。

Conclusion: 压缩顺序是联合模型压缩中的关键因素，渐进强度假设为压缩顺序选择提供了理论指导。该研究填补了联合模型压缩中压缩顺序研究的空白，为高效模型压缩提供了新视角。
...
Method: 提出AS2架构：1) 用软连续近似替代ASP立即后果算子T_P；2) 在整个前向传播中维护有限符号域上的每位置概率分布；3) 通过最小化概率提升的T_P的固定点残差进行端到端训练；4) 使用约束组成员嵌入而非传统位置嵌入来编码问题结构；5) 采用贪婪约束解码程序。

Result: 在Visual Sudoku上：99.89%单元格准确率和100%约束满足率（经Clingo验证）；在MNIST Addition上：N∈{2,4,8}个加数时，数字准确率均超过99.7%。
...
<details>
  <summary>Details</summary>
Motivation: 在大规模预训练模型时代，将通用知识有效适应特定情感计算任务面临计算效率和多模态异质性挑战。Transformer方法虽然擅长建模模态间依赖，但其二次计算复杂度限制了长序列数据处理；Mamba模型计算高效但难以捕获跨模态对齐所需的全局非序列关系。

Method: 提出AlignMamba-2框架：1）双对齐策略：使用最优传输距离和最大均值差异进行正则化，促进模态间的几何和统计一致性，无推理开销；2）模态感知Mamba层：采用混合专家架构，包含模态特定和模态共享专家，在融合过程中显式处理数据异质性。

Result: 在四个具有挑战性的基准测试上进行了广泛实验：动态时间序列（CMU-MOSI和CMU-MOSEI数据集）和静态图像相关任务（NYU-Depth V2和MVSA-Single数据集）。结果表明AlignMamba-2在从动态时间序列分析到静态图像文本分类的多样化模式识别任务中，在效果和效率上都达到了新的最先进水平。

Conclusion: AlignMamba-2通过创新的双对齐策略和模态感知Mamba层设计，有效解决了多模态融合中的计算效率和异质性挑战，在多种模式识别任务中实现了效果和效率的双重突破，为大规模预训练模型在情感计算任务中的高效适应提供了新思路。
...
TL;DR: 该论文提出了一个评估多模态大语言模型处理离散符号能力的基准，发现模型在基本符号识别上失败却在复杂推理上成功，揭示了模型依赖语言概率而非真正视觉感知的"认知不匹配"问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在解释自然场景方面取得了显著成功，但它们在处理离散符号（人类认知的基本构建块）方面的能力仍然是一个关键的开放性问题。与连续视觉数据不同，数学公式、化学结构和语言字符等符号需要精确、深入的解释。

Method: 引入一个全面的基准来评估顶级MLLM在五个领域（语言、文化、数学、物理和化学）中导航这些"离散语义空间"的能力。通过系统性的评估揭示模型在符号处理方面的表现模式。
...
TL;DR: NeSyCR是一个神经符号反事实推理框架，用于解决视频指导机器人编程中的跨域适应问题，通过抽象视频演示为符号轨迹、识别跨域不兼容性并验证性地修订程序，实现了31.14%的任务成功率提升。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频指导机器人编程中缺乏程序性理解能力，无法在演示和部署之间存在感知和物理差异时重新制定因果依赖关系，导致跨域适应困难。

Method: NeSyCR将视频演示抽象为捕捉底层任务程序的符号轨迹，基于部署观察推导反事实状态以揭示跨域不兼容性，通过在符号状态空间中进行可验证检查来提出恢复与演示程序兼容性的程序修订。

Result: NeSyCR在模拟和现实世界操作任务中实现了强大的跨域适应能力，相比最强基线Statler实现了31.14%的任务成功率提升。

Conclusion: NeSyCR通过神经符号反事实推理框架为视频指导机器人编程提供了可验证的跨域适应方法，显著提升了在感知和物理差异下的任务成功率。
...
TL;DR: 提出Correlation-Weighted Multi-Reward Optimization (CW-MRO)框架，通过基于相关性的自适应权重分配解决文本到图像生成中的组合性问题，在多个基准测试上显著提升多概念生成效果。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在处理多概念组合生成时存在困难，经常忽略部分概念，导致生成结果不完整。这源于多概念奖励优化中的相互干扰问题，不同概念在联合优化时会产生竞争和冲突。

Method: 提出CW-MRO框架：1) 将多概念提示分解为预定义的概念组（如对象、属性、关系）；2) 为每个概念使用专用奖励模型获取奖励信号；3) 基于概念间的相关性结构自适应重新加权奖励，为冲突或难以满足的概念分配更高权重；4) 通过基于相关性的难度估计，在每组中聚焦优化最具挑战性的概念。

Result: 在SD3.5和FLUX.1-dev等最先进的扩散模型上应用该方法，在ConceptMix、GenEval 2和T2I-CompBench等具有挑战性的多概念基准测试上取得了一致的改进效果。

Conclusion: CW-MRO框架通过考虑概念间的相互作用，平衡竞争奖励信号并强调部分满足但样本间不一致生成的概念，有效提升了文本到图像模型的组合生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI智能体在交互经济环境中部署日益增多，但实证研究表明AI-AI交互往往无法稳定诱导战略均衡。现有后训练方法虽能诱导均衡，但在多样化的独立开发AI模型间统一应用不切实际。本文旨在证明现成推理AI智能体无需显式后训练即可零样本实现纳什式行为。

Method: 提出"合理推理"智能体理论框架：智能体能够从先前观察形成对他人策略的信念，并学习对这些信念做出最优响应。证明此类智能体在几乎所有实现路径上行为弱接近后续博弈的纳什均衡。放宽共同知识收益假设，允许阶段收益未知且每个智能体仅观察自身私有随机收益，仍能保证路径纳什收敛。

Result: 理论证明合理推理智能体最终行为弱接近纳什均衡。实证验证通过模拟五种博弈场景：从重复囚徒困境到风格化重复营销推广博弈。实验结果表明AI智能体自然展现此类推理模式，从而内在获得稳定均衡行为。

Conclusion: 现成推理AI智能体无需后训练即可零样本实现纳什均衡式行为，在许多现实世界战略互动中无需通用对齐程序。AI智能体通过合理推理自然形成信念并最优响应，内在趋向均衡，为AI在战略环境中的部署提供了理论基础。
...
### [192] [Interplay: Training Independent Simulators for Reference-Free Conversational Recommendation](https://arxiv.org/abs/2603.18573)
...
Result: 该方法生成更真实、多样的对话，更接近真实人机交互。无参考模拟器在质量上匹配或超越现有方法，为生成高质量对话推荐数据提供了可扩展解决方案。

Conclusion: 通过定量和人工评估证实了无参考方法的有效性，该框架能够生成更自然的对话推荐数据，无需将对话约束在预定义目标物品上。
...
### [193] [MedForge: Interpretable Medical Deepfake Detection via Forgery-aware Reasoning](https://arxiv.org/abs/2603.18577)
*Zhihui Chen,Kai He,Qingyuan Lei,Bin Pu,Jian Zhang,Yuling Xu,Mengling Feng*

Main category: cs.AI

TL;DR: MedForge是一个针对医学图像伪造检测的数据与方法解决方案，通过大规模基准数据集和定位-分析推理框架，实现预检测、证据基础的医学伪造识别，解决现有方法在医疗领域不足的问题。


<details>
  <summary>Details</summary>
Motivation: 文本引导的图像编辑器能够高保真地操纵真实医学扫描图像，实现病灶植入/移除，这对临床信任和安全构成威胁。现有防御方法在医疗领域不足：医学检测器大多是黑盒，而基于MLLM的解释器通常是事后分析、缺乏医学专业知识，在模糊案例上可能产生幻觉证据。

Method: 提出MedForge解决方案：1) 创建MedForge-90K大规模基准数据集，包含19种病理的真实病灶编辑，通过医生检查指南和黄金编辑位置提供专家指导的推理监督；2) 开发MedForge-Reasoner推理器，采用"先定位后分析"的推理策略，先预测可疑区域再做出判断；3) 使用Forgery-aware GSPO进行对齐，加强证据基础并减少幻觉。

Result: 实验证明MedForge在检测准确性方面达到最先进水平，并提供可信赖、与专家对齐的解释。

Conclusion: MedForge为医学图像伪造检测提供了一个有效的预检测、证据基础的解决方案，通过大规模基准数据集和定位-分析推理框架，解决了现有方法在医疗领域的不足，实现了高准确性的检测和可信的解释。
...
### [194] [ZEBRAARENA: A Diagnostic Simulation Environment for Studying Reasoning-Action Coupling in Tool-Augmented LLMs](https://arxiv.org/abs/2603.18614)
...
Method: 开发ZebraArena程序生成环境，每个任务需要仅通过针对性工具使用才能获取的关键信息，提供可解释的外部信息获取与演绎推理接口，具有确定性评估和理论最优查询计数。

Result: GPT-5和Gemini 2.5 Pro在困难实例上仅达到60%准确率，且实际工具调用比理论最优多70-270%，显示前沿推理模型在深度推理与准确工具调用结合方面仍面临挑战。
...
### [195] [Agentic Flow Steering and Parallel Rollout Search for Spatially Grounded Text-to-Image Generation](https://arxiv.org/abs/2603.18627)
*Ping Chen,Daoxuan Zhang,Xiangming Wang,Yungeng Liu,Haijin Zeng,Yongyong Chen*

Main category: cs.AI

TL;DR: AFS-Search是一个无需训练的闭环框架，通过VLM语义批评器和并行回滚搜索解决T2I生成中的语义歧义和误差累积问题，显著提升FLUX.1-dev的性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成技术受限于静态文本编码器的关系推理能力和开环采样中的误差累积。语义歧义在ODE轨迹中会逐渐放大，导致空间约束的随机偏差，缺乏实时反馈机制。

Method: 提出AFS-Search框架：1) 使用VLM作为语义批评器诊断中间潜在表示，通过精确空间接地动态引导速度场；2) 将T2I生成建模为序列决策过程，通过前瞻模拟探索多条轨迹，基于VLM引导的奖励选择最优路径；3) 提供AFS-Search-Pro（高性能）和AFS-Search-Fast（快速生成）两个版本。

Result: AFS-Search-Pro显著提升了原始FLUX.1-dev的性能，在三个不同基准测试中达到最先进水平。AFS-Search-Fast在保持快速生成速度的同时也显著提升了性能。

Conclusion: AFS-Search通过引入训练免费的闭环并行回滚搜索和流引导机制，有效解决了T2I生成中的语义歧义和误差累积问题，为高质量文本到图像生成提供了新范式。
...
### [196] [D-Mem: A Dual-Process Memory System for LLM Agents](https://arxiv.org/abs/2603.18631)
...
<details>
  <summary>Details</summary>
Motivation: 自主智能体需要高保真记忆访问以支持长期推理，但现有基于检索的记忆框架采用增量处理范式，依赖语义检索，存在信息损失问题，难以处理需要细粒度上下文理解的查询
...
### [197] [Balanced Thinking: Improving Chain of Thought Training in Vision Language Models](https://arxiv.org/abs/2603.18656)
*Shaked Perek,Ben Wiesel,Avihu Dekel,Nimrod Shabtay,Eli Schwartz*

Main category: cs.AI

TL;DR: SCALe是一种针对视觉语言模型多模态推理的轻量级训练方法，通过动态权重调度分离推理和答案段的监督，替代传统两阶段SFT+RL流程


<details>
  <summary>Details</summary>
Motivation: 传统SFT在训练推理数据时，所有token损失权重相同，导致冗长的<think>推理段掩盖了关键的<answer>答案段，造成推理冗长和答案不准确的问题

Method: 提出SCALe方法，使用动态、长度独立的权重明确分离推理和答案段的监督，通过余弦调度策略在训练过程中逐步将焦点从<think>转移到<answer>，鼓励简洁且基于事实的推理

Result: SCALe在多种基准测试和架构中一致优于传统SFT，性能匹配完整的SFT+GRPO两阶段流程，但仅需约七分之一的训练时间；与GRPO结合时达到最佳整体性能

Conclusion: SCALe是一种轻量级但有效的替代方案，既可作为独立方法使用，也可作为强化学习优化的坚实基础，显著提升多模态推理的准确性和效率
...
### [198] [Thinking with Constructions: A Benchmark and Policy Optimization for Visual-Text Interleaved Geometric Reasoning](https://arxiv.org/abs/2603.18662)
*Haokun Zhao,Wanshi Xu,Haidong Yuan,Songjun Cao,Long Ma,Yanghua Xiao*

Main category: cs.AI

TL;DR: 论文提出了视觉-文本交织思维链框架，通过GeoAux-Bench基准和A2PO强化学习优化，使多模态大语言模型能够主动进行几何构造，提升几何推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型局限于静态图表的被动推理，缺乏何时及如何构造有效视觉辅助的战略知识，无法实现几何推理所需的"构造思维"动态操作。

Method: 1. 创建GeoAux-Bench基准（4,334个几何问题），对齐文本构造步骤与真实视觉更新；2. 提出A2PO强化学习范式，通过自适应奖励塑形调节视觉辅助的时机和质量，区分必要与冗余构造。

Result: 实验表明该方法使MLLMs能够利用选择性辅助构造，相比强基线获得3.51%的性能提升。有效构造作为熵减器，与降低的推理困惑度强相关。

Conclusion: 视觉-文本交织思维链框架通过战略构造掌握，显著提升了多模态大语言模型的几何推理能力，验证了构造作为熵减器的重要性。
...
### [199] [MANAR: Memory-augmented Attention with Navigational Abstract Conceptual Representation](https://arxiv.org/abs/2603.18676)
*Zuher Jahshan,Ben Ben Ishay,Leonid Yavits*

Main category: cs.AI

TL;DR: MANAR是一种基于全局工作空间理论的新型注意力机制，通过可训练的概念记忆和抽象概念表示实现线性时间复杂度，同时保持与标准多头注意力的兼容性。


<details>
  <summary>Details</summary>
Motivation: 标准多头注意力缺乏认知意识模型中假设的功能瓶颈和全局整合机制，导致二次复杂度问题。MANAR旨在通过实例化全局工作空间理论来解决这些限制。

Method: MANAR采用两阶段架构：1)整合阶段，从可训练记忆检索概念形成集体"心理图像"(ACR)；2)广播阶段，全局状态导航并通知局部令牌的上下文化。通过恒定大小的ACR路由全局信息实现线性复杂度。

Result: 在语言任务上获得GLUE分数85.1，视觉任务上ImageNet-1K准确率83.9%，语音任务上LibriSpeech词错误率2.7%，匹配或超越强基线，同时实现线性时间扩展。

Conclusion: MANAR成功实例化了全局工作空间理论，提供了一种高效且富有表达力的注意力机制替代方案，能够实现非凸上下文化，反映GWT中描述的创造性合成过程。
...
### [200] [Accurate and Efficient Multi-Channel Time Series Forecasting via Sparse Attention Mechanism](https://arxiv.org/abs/2603.18712)
...
<details>
  <summary>Details</summary>
Motivation: 多通道时间序列预测在金融、供应链管理和能源规划等领域普遍存在，需要有效捕获通道内和通道间的复杂动态依赖关系。传统方法很少关注通道间的交互学习，因此需要设计能够同时捕捉线性和非线性通道依赖关系的架构。

Method: 提出Linear-Network (Li-Net)架构：1) 动态压缩序列和通道维度的表示；2) 通过可配置非线性模块处理信息；3) 重构预测结果；4) 在多尺度投影框架中集成稀疏Top-K Softmax注意力机制；5) 无缝融合多模态嵌入，引导稀疏注意力关注最具信息量的时间步和特征通道。

Result: 在多个真实世界基准数据集上的实验结果表明，Li-Net相比最先进的基线方法实现了竞争性性能。同时，Li-Net在预测准确性和计算负担之间提供了优越的平衡，表现出显著更低的内存使用和更快的推理时间。

Conclusion: Li-Net通过创新的架构设计有效解决了多通道时间序列预测中的通道交互问题。详细的消融研究和参数敏感性分析验证了所提架构中每个关键组件的有效性。该模型在保持高预测精度的同时显著降低了计算成本。
...
### [201] [Analysis Of Linguistic Stereotypes in Single and Multi-Agent Generative AI Architectures](https://arxiv.org/abs/2603.18729)
...
TL;DR: 该研究复制了现有关于LLM方言偏见的研究，并测试了多种缓解策略，发现多智能体架构能有效减少方言相关的刻板印象差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明LLM输出存在方言歧视行为，特别是在标准美国英语和非裔美国人英语之间表现出明显的刻板印象偏见。本研究旨在验证这些发现并探索有效的缓解策略。

Method: 定义了八个提示模板来检测方言偏见的不同表现形式（如姓名、职业、形容词建议），采用LLM-as-judge方法评估偏见，测试了提示工程（角色提示和思维链提示）和多智能体架构（生成-批评-修订模型）等缓解策略。

Result: 在所有模板类别中都观察到SAE和AAE相关输出的刻板印象差异，形容词和职业归因差异最显著。不同模型的基线差异很大：Claude Haiku的SAE-AAE差异最大，Phi-4 Mini最小。思维链提示对Claude Haiku有效，而多智能体架构在所有模型中都能确保一致的缓解效果。

Conclusion: 为实现交叉性感知的软件工程，公平性评估应包括模型特定的缓解策略验证，在高影响力LLM部署中应采用工作流级控制（如涉及批评模型的智能体架构）。当前结果是探索性的，但可通过扩大数据集和应用于不同语言/方言进行扩展。
...
### [202] [Memento-Skills: Let Agents Design Agents](https://arxiv.org/abs/2603.18743)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工设计的智能体，缺乏自主学习和适应能力。需要一种能够持续学习、自主设计智能体并改进自身能力的通用系统。

Method: 基于记忆的强化学习框架，使用状态提示和结构化Markdown文件存储的可重用技能作为持久化记忆。采用读写反思学习机制：读阶段通过可训练技能路由器选择相关技能；写阶段基于新经验更新和扩展技能库。

Result: 在General AI Assistants基准测试中相对提升26.2%，在Humanity's Last Exam中相对提升116.2%，实现了持续的性能增益。

Conclusion: Memento-Skills通过外部化技能和提示的演化实现了无需更新LLM参数的持续学习，使通用智能体能够为新任务端到端设计智能体，并通过迭代技能生成和精炼逐步提升能力。
...
### [203] [NeuroGame Transformer: Gibbs-Inspired Attention Driven by Game Theory and Statistical Physics](https://arxiv.org/abs/2603.18761)
*Djamel Bouchaffra,Fayçal Ykhlef,Hanene Azzag,Mustapha Lebbah,Bilal Faye*

Main category: cs.AI

TL;DR: NGT将注意力机制重新概念化为合作博弈和统计物理系统，使用Shapley值和Banzhaf指数量化token重要性，通过Ising模型和Gibbs分布计算注意力权重，避免了标准注意力机制的高阶依赖建模限制。


<details>
  <summary>Details</summary>
Motivation: 标准transformer中的注意力机制受限于其成对公式，难以建模token间的高阶依赖关系。需要一种新的注意力机制来克服这一限制，同时保持计算效率。

Method: 将token同时视为合作博弈中的玩家和统计物理系统中的自旋。使用Shapley值进行全局排列归因，Banzhaf指数进行局部联盟级影响评估，通过可学习门控参数组合形成外部磁场。成对交互势能捕获协同关系，系统能量遵循Ising哈密顿量，注意力权重作为Gibbs分布下的边际概率出现，通过平均场方程高效计算。使用重要性加权蒙特卡洛估计器处理指数级联盟空间。

Result: 在SNLI上达到86.4%的测试准确率（峰值验证准确率86.6%），在MNLI-matched上表现优异，超越了ALBERT-Base等高效transformer基线，与RoBERTa-Base保持高度竞争力。

Conclusion: NeuroGame Transformer通过博弈论和统计物理学的双重视角重新概念化注意力机制，有效建模token间的高阶依赖关系，在自然语言推理任务上表现出色，同时保持了计算效率和数值稳定性。
...
### [204] [A Concept is More Than a Word: Diversified Unlearning in Text-to-Image Diffusion Models](https://arxiv.org/abs/2603.18767)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于关键词的概念消除方法存在固有局限性：视觉概念是多维度的，可以通过多种文本形式表达，且与相关概念在潜在空间中重叠。单一关键词仅代表概念的窄点估计，无法覆盖其完整的语义分布和潜在空间中的纠缠变化，导致消除过程脆弱且容易过度遗忘相关概念。

Method: 提出Diversified Unlearning分布框架，通过一组上下文多样化的提示而非单一关键词来表示概念。这种更丰富的表示使消除过程更精确和鲁棒。该方法可作为附加组件集成到现有的消除流程中。

Result: 在多个基准测试和最先进基线上的广泛实验表明，将Diversified Unlearning作为附加组件集成到现有消除流程中，能一致地实现更强的消除效果、更好的无关概念保留，以及对对抗性恢复攻击的改进鲁棒性。

Conclusion: Diversified Unlearning通过多样化提示表示概念，解决了基于关键词的概念消除方法的局限性，为文本到图像扩散模型中的概念消除提供了更精确、鲁棒的解决方案。
...
### [205] [Proceedings of the 2nd Workshop on Advancing Artificial Intelligence through Theory of Mind](https://arxiv.org/abs/2603.18786)
...
Method: 通过举办AAAI 2026研讨会，征集和筛选相关论文，编辑成卷出版

Result: 成功举办了第二届"通过心智理论推进人工智能"研讨会，并出版了包含精选论文的开放获取文集

Conclusion: 该文集为ToM与AI交叉领域的研究者提供了有价值的学术资源，有助于推动心智理论在人工智能中的应用与发展
...
### [206] [dTRPO: Trajectory Reduction in Policy Optimization of Diffusion Large Language Models](https://arxiv.org/abs/2603.18806)
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)引入了新的语言生成范式，但同时也带来了与人类偏好对齐的新挑战。现有方法在策略优化时轨迹概率计算成本高，限制了离线策略训练的规模化。

Method: 提出轨迹缩减策略优化(dTRPO)：1) 证明在参考策略正则化下，新解掩码token的概率比是中间扩散状态概率比的无偏估计；2) 提出通过重新掩码最终状态的单次前向传播有效估计完整轨迹概率。将这两种轨迹缩减策略整合到策略优化目标中。

Result: 在7B参数dLLMs上的评估显示：STEM任务提升达9.6%，编码任务提升达4.3%，指令跟随任务提升达3.0%。dTRPO具有离线、单次前向的训练效率优势，并通过高质量输出提高生成效率。

Conclusion: dTRPO通过创新的轨迹缩减策略有效解决了dLLMs策略优化中的计算瓶颈，显著提升了模型性能，同时保持了训练和生成的高效率，为扩散语言模型的规模化对齐提供了有效解决方案。
...
### [207] [ProRL Agent: Rollout-as-a-Service for RL Training of Multi-Turn LLM Agents](https://arxiv.org/abs/2603.18815)
...
TL;DR: ProRL Agent是一个可扩展的基础设施，通过API服务提供完整的智能体rollout生命周期，支持在无根HPC设置中进行标准化、可扩展的沙盒环境，用于多轮LLM智能体的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 多轮LLM智能体在解决复杂交互任务中越来越重要，强化学习是改进其长视野行为的关键要素。然而，现有基础设施通常将rollout编排与训练循环耦合，导致系统难以迁移和维护，且RL训练需要生成大量沙盒rollout轨迹。

Method: 基于rollout-as-a-service理念，提出ProRL Agent基础设施，通过API服务提供完整的智能体rollout生命周期。提供标准化、可扩展的沙盒环境，支持在无根HPC设置中进行多样化的智能体任务。

Result: 在软件工程、数学、STEM和编程任务上通过RL训练验证了ProRL Agent的有效性。该基础设施已开源并集成到NVIDIA NeMo Gym中。

Conclusion: ProRL Agent提供了一个可扩展的基础设施，通过解耦rollout编排与训练循环，解决了现有系统难以迁移和维护的问题，支持多轮LLM智能体的强化学习训练。
...
### [208] [RewardFlow: Topology-Aware Reward Propagation on State Graphs for Agentic RL with Large Language Models](https://arxiv.org/abs/2603.18859)
*Xiao Feng,Bo Han,Zhanke Zhou,Jiaqi Fan,Jiangchao Yao,Ka Ho Li,Dahai Yu,Michael Kwok-Po Ng*

Main category: cs.AI

TL;DR: RewardFlow：一种轻量级方法，通过构建状态图分析推理轨迹中的状态拓扑结构，量化状态对成功的贡献，为强化学习提供密集状态级奖励，显著提升智能体推理性能。
...
Method: RewardFlow通过构建推理轨迹中的状态图来利用状态的内在拓扑结构，分析状态对成功的贡献，然后通过拓扑感知的图传播量化这些贡献，生成客观的状态级奖励。

Result: RewardFlow作为密集奖励集成到RL优化中，在四个智能体推理基准测试中显著优于先前的RL基线，表现出卓越的性能、鲁棒性和训练效率。

Conclusion: RewardFlow提供了一种轻量级、有效的方法来估计状态级奖励，解决了智能体推理任务中奖励稀疏性的问题，为RL优化提供了密集的、基于拓扑的奖励信号，显著提升了性能。
...
### [209] [Conflict-Based Search for Multi Agent Path Finding with Asynchronous Actions](https://arxiv.org/abs/2603.18866)
*Xuemian Wu,Shizhe Zhao,Zhongqiang Ren*

Main category: cs.AI

TL;DR: 本文提出CBS-AA方法解决异步动作的多智能体路径规划问题，克服了CCBS的不完备性问题，并开发了冲突消解技术提升可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有MAPF算法大多基于同步动作假设，限制了实际应用。CCBS方法虽然能处理异步动作，但最近被发现由于连续等待时间导致的不可数无限状态空间而存在不完备性问题。

Method: 提出Conflict-Based Search with Asynchronous Actions (CBS-AA)方法，通过新的算法设计绕过CCBS的理论问题，保证MAPF-AA问题的完备性和最优性。基于CBS-AA进一步开发了冲突消解技术来提升可扩展性。

Result: CBS-AA能够解决MAPF-AA问题并保证完备性和最优性。测试结果显示，所提出的冲突消解技术能够将分支数量减少高达90%。

Conclusion: CBS-AA方法成功解决了CCBS的不完备性问题，为异步动作的多智能体路径规划提供了具有完备性和最优性保证的有效解决方案，并通过冲突消解技术显著提升了算法性能。
...
### [210] [Geography According to ChatGPT -- How Generative AI Represents and Reasons about Geography](https://arxiv.org/abs/2603.18881)
*Krzysztof Janowicz,Gengchen Mai,Rui Zhu,Song Gao,Zhangyu Wang,Yingjie Hu,Lauren Bennett*

Main category: cs.AI

TL;DR: 该论文探讨AI系统如何表征和推理地理信息，强调理解AI构建的世界观与评估其准确性同等重要，并通过三个探索性案例研究来激发相关讨论。


<details>
  <summary>Details</summary>
Motivation: 随着公众越来越多地通过AI系统与空间和地点互动，理解AI如何表征和推理地理信息变得至关重要。同时，由于研究常依赖预训练模型，理解AI构建的世界观与评估其事实回忆准确性同等重要。

Method: 通过三个探索性案例研究（vignettes）来探讨关键问题：1）模型是否形成强默认值及其对细微语法变化的脆弱性；2）个体良性任务组合是否会导致分布偏移；3）是否因过度关注事实回忆而忽视了更深层次的理解问题。

Result: 论文未提供具体实验结果，而是通过三个探索性案例研究提出了关键研究问题，旨在激发关于AI地理表征的讨论和后续工作。

Conclusion: 理解AI系统如何构建地理表征与评估其准确性同等重要，需要超越事实回忆来探讨更深层次的理解问题，三个探索性案例为未来研究提供了方向。
...
### [211] [Reasoning over mathematical objects: on-policy reward modeling and test time aggregation](https://arxiv.org/abs/2603.18886)
...
TL;DR: 该论文提出了Principia套件，包含用于推导数学对象的训练数据和基准测试，并展示了通过策略性训练和验证器提升大语言模型在数学推理任务上的表现。
...
Method: 构建Principia训练数据和基准测试套件；开发基于强LLM-judge和验证器的训练方法，采用策略性训练提升性能；利用策略性训练通过聚合扩展测试时计算。

Result: Qwen3-235B和o3等强模型在Principia上表现不佳，但提出的训练方法在不同LLM骨干上带来显著改进，同时提升现有数值和MCQA任务表现，展示了推理能力的跨格式泛化。

Conclusion: 论文提出的Principia套件和训练方法有效提升了LLM在数学对象推导任务上的性能，证明了策略性训练和验证器的重要性，为STEM领域的推理应用提供了实用工具。
...
### [212] [Quantitative Introspection in Language Models: Tracking Internal States Across Conversation](https://arxiv.org/abs/2603.18893)
*Nicolas Martorell*

Main category: cs.AI

TL;DR: LLMs的数值自我报告可以追踪其内部情感状态，通过logit-based方法可揭示内省能力，且这种能力随模型规模扩大而增强。


<details>
  <summary>Details</summary>
Motivation: 追踪大型语言模型在对话中的内部状态对安全性、可解释性和模型福利很重要，但现有方法（如线性探针）在高维表示压缩上不完美且难以随模型规模扩展。受人类心理学中数值自我报告的启发，研究LLMs自身的数值自我报告是否能追踪探针定义的内部情感状态。

Method: 研究四个概念对（幸福感、兴趣、专注度、冲动性）在40个十轮对话中，将内省操作化为模型自我报告与概念匹配探针定义内部状态之间的因果信息耦合。发现贪婪解码的自我报告输出无信息价值，但通过计算基于logit的自我报告可揭示内省能力。使用激活导向验证耦合的因果性，并研究内省能力随对话轮次的变化和通过概念导向选择性改进内省能力。

Result: 基于logit的自我报告能追踪可解释的内部状态（Spearman ρ=0.40-0.76；等渗R²=0.12-0.54），跟随这些状态随时间变化，激活导向证实耦合是因果性的。内省能力在第一轮就存在但随对话演化，通过沿一个概念导向可选择性提升另一概念的内省能力（ΔR²最高达0.30）。关键现象在某些情况下随模型规模扩展，在LLaMA-3.1-8B-Instruct中接近R²≈0.93，并在其他模型家族中部分复现。

Conclusion: 数值自我报告是追踪对话AI系统中内部情感状态的可行补充工具，为模型内省能力提供了新的测量方法，并展示了通过激活导向改进内省能力的潜力。
...
### [213] [Secure Linear Alignment of Large Language Models](https://arxiv.org/abs/2603.18908)
*Matt Gorbett,Suman Jana*

Main category: cs.AI

TL;DR: 提出隐私保护框架，利用语言模型表征收敛特性实现独立模型间的跨模型对齐，通过仿射变换和同态加密保护客户端查询，实现亚秒级推理延迟


<details>
  <summary>Details</summary>
Motivation: 语言模型在独立训练时出现表征收敛现象，这为跨模型对齐提供了新机会。在安全、隐私或竞争约束下，直接共享数据或模型不可行，需要隐私保护的跨模型推理方案

Method: 提出隐私保护框架：1) 利用公共数据集学习独立模型间的仿射变换；2) 应用同态加密保护客户端查询；3) 仅加密线性对齐和分类操作以降低延迟。同时实证研究表征收敛，学习独立模型最终隐藏状态间的线性变换

Result: 1) 框架实现亚秒级推理延迟并保持强安全保证；2) 跨模型映射在嵌入分类和分布外检测中性能下降最小；3) 首次证明线性对齐有时能实现独立训练模型间的文本生成

Conclusion: 表征收敛为隐私保护跨模型推理提供了基础，提出的框架在安全约束下实现了高效跨模型对齐，为安全、隐私敏感场景中的模型协作开辟了新途径
...
### [214] [Teleological Inference in Structural Causal Models via Intentional Interventions](https://arxiv.org/abs/2603.18968)
*Dario Compagno,Fabio Massimo Zennaro*

Main category: cs.AI

TL;DR: 该论文提出了一种新方法，将结构因果模型（SCMs）扩展用于分析有意识、目标导向的智能体在因果系统中的干预行为，通过引入"意向干预"操作符和"结构最终模型"来检测智能体并发现其意图。


<details>
  <summary>Details</summary>
Motivation: 传统结构因果模型主要用于回答因果问题，但无法有效处理智能体基于目标的有意识干预行为。现有方法在建模这类具有状态感知和目标导向的智能体时存在局限性，需要新的理论框架来分析和理解智能体的意图性行为。

Method: 引入"意向干预"这一时间无关的操作符，该操作符诱导产生一个称为"结构最终模型"的双胞胎SCM。SFM将观测值视为意向干预的结果，并将其与这些干预的反事实条件（如果智能体没有干预会发生什么）联系起来。

Result: 研究表明结构最终模型能够用于经验性地检测系统中是否存在智能体，并发现这些智能体的意图。该方法为分析目标导向行为提供了新的理论工具。

Conclusion: 结构因果模型不仅可以用于回答因果问题，还可以扩展到回答目的论问题，即分析智能体的意图性干预行为。通过引入意向干预和结构最终模型，为理解智能体的目标导向行为提供了新的分析框架。
...
Method: 开发黑盒稳定性监控系统，定期通过固定提示集对端点进行指纹识别，采样输出并随时间比较输出分布。使用跨提示的求和能量距离统计量比较指纹，通过排列检验p值作为分布偏移证据，顺序聚合以检测变化事件并定义稳定期

Result: 在受控验证中，Stability Monitor能够检测模型系列、版本、推理栈、量化和行为参数的变化。在实际监控中，观察到同一模型在不同提供商之间以及同一提供商内部存在显著的稳定性差异

Conclusion: Stability Monitor为AI模型端点提供了行为一致性的监控解决方案，弥补了传统可靠性指标的不足，能够有效检测模型身份变化，确保AI原生应用的稳定性
...
TL;DR: 该论文对AI在司法决策中的作用进行了综合评述，重点关注风险评估工具的性能、人类法官的偏见以及人机交互，发现AI辅助工具对审前和量刑决策的影响有限，并呼吁更多跨学科研究。


<details>
  <summary>Details</summary>
Motivation: AI技术在司法决策中的应用引发了透明度、可靠性和问责制的重大关切，同时凸显了人类判断的局限性，需要理解法官如何与AI决策辅助工具互动。

Method: 以刑事司法风险评估为焦点案例，进行综合评述，连接AI工具性能与公平性、人类法官优势与偏见、人机交互三个相互关联的方面，整合计算机科学、经济学、法学、犯罪学和心理学等领域的研究。

Result: 现有经验证据表明，AI决策辅助工具对审前和量刑决策的影响有限或不存在；研究揭示了文献中的重要空白，包括需要评估AI风险评估工具性能、理解法官在嘈杂决策环境中的导航方式以及个体特征如何影响法官对AI建议的反应。
...
<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否与人类具有相似的创造力机制，以及相同的干预措施（跨领域映射）是否对两者都有效

Method: 人类参与者和LLMs在两种提示下为10种日常产品生成新特征：跨领域映射（从随机分配的远程源领域翻译属性）和用户需求（针对未满足需求提出创新）

Result: 人类可靠地从随机分配的跨领域映射中获益，而LLMs平均比人类产生更多原创想法，但跨领域映射对其影响不显著；两种系统中，当灵感源与目标语义距离更远时，跨领域映射的影响都会增加

Conclusion: 研究结果强调了远程关联在创意构思中的作用，以及人类和LLMs对相同创造力干预措施的系统性差异
...
TL;DR: LuMamba是一个自监督EEG基础模型框架，结合拓扑不变编码和线性复杂度状态空间建模，在21,000小时EEG数据上预训练，在多个下游任务上实现SOTA性能，计算效率显著提升。


<details>
  <summary>Details</summary>
Motivation: EEG基础模型面临两个主要挑战：1) 不同电极拓扑结构的差异导致模型难以统一处理；2) Transformer架构的二次序列复杂度限制了计算可扩展性。需要一种既能处理拓扑差异又具有计算效率的解决方案。

Method: 提出LuMamba框架，包含两个核心组件：1) 使用LUNA的learned-query cross-attention机制实现通道统一，创建拓扑不变的编码；2) 采用FEMBA的双向Mamba块进行高效的时间建模，具有线性复杂度。首次系统研究了LeJEPA（Latent-Euclidean Joint-Embedding Predictive Architecture）在生物信号学习中的应用。结合掩码重建和LeJEPA两个预训练目标。

Result: 在TUEG语料库21,000小时无标签EEG数据上预训练，在五个下游任务上评估：异常检测、伪影识别、精神状态分类（16-26通道配置）。仅4.6M参数，在TUAB上达到80.99%平衡准确率，阿尔茨海默症检测达到0.97 AUPR（SOTA）。相比SOTA模型，在相同序列长度下减少377倍FLOPS，在典型GPU内存限制下可处理12倍长的序列。

Conclusion: LuMamba通过结合拓扑不变编码和线性复杂度状态空间建模，成功解决了EEG基础模型的两个关键挑战。掩码重建和LeJEPA目标的结合产生了最鲁棒的表征。该框架在保持高性能的同时显著提升了计算效率，为EEG基础模型的发展提供了有效解决方案。
...
TL;DR: 论文研究推理语言模型在扩展思维链推理中的不确定性估计，发现并行采样结合口头置信度和自一致性信号能显著提升不确定性质量，尤其在数学领域效果最佳。


<details>
  <summary>Details</summary>
Motivation: 不确定性估计对于部署推理语言模型至关重要，但在扩展思维链推理中仍缺乏深入理解。需要研究完全黑盒方法（如并行采样）使用口头置信度和自一致性信号的效果。

Method: 采用并行采样作为完全黑盒方法，结合口头置信度和自一致性信号。在三个推理模型和17个任务（涵盖数学、STEM和人文学科）上进行实验，分析这些信号的扩展特性。

Result: 自一致性和口头置信度在推理模型中都能扩展，但自一致性初始区分度较低，在中等采样下落后于口头置信度。信号组合带来最大收益：仅用两个样本，混合估计器平均提升AUROC达+12分，即使扩展到更大预算也优于单独信号。数学领域（RLVR风格后训练的本土领域）不确定性质量最高，互补性最强且扩展最快。

Conclusion: 并行采样结合口头置信度和自一致性是有效的黑盒不确定性估计方法，信号组合能显著提升性能，尤其在数学领域效果突出。该方法为推理语言模型的不确定性估计提供了实用解决方案。
...
### [220] [Implicit Patterns in LLM-Based Binary Analysis](https://arxiv.org/abs/2603.19138)
*Qiang Li,XiangRui Zhang,Haining Wang*

Main category: cs.AI

TL;DR: 该论文首次对基于LLM的二进制漏洞分析中的大规模推理轨迹进行研究，发现了四种主导的隐式模式，这些模式构成了LLM推理的抽象结构，为构建更可靠的分析系统奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 目前基于LLM的二进制漏洞分析通常采用迭代、多轮次的推理方式，但这类系统如何在数百个推理步骤中组织探索过程仍不明确，主要受限于有限的上下文窗口和隐式的token级行为。

Method: 研究分析了521个二进制文件和99,563个推理步骤，通过大规模、轨迹级别的分析，识别了LLM推理中出现的四种主导隐式模式。

Result: 发现了四种主导的隐式模式：早期剪枝、路径依赖锁定、针对性回溯和知识引导优先级排序。这些模式形成了稳定、结构化的系统，具有不同的时间角色和可测量的特征。

Conclusion: 研究首次系统性地描述了LLM驱动的二进制分析，揭示了LLM推理中隐式决策如何组织探索过程，为构建更可靠的分析系统提供了理论基础。
...
### [221] [Box Maze: A Process-Control Architecture for Reliable LLM Reasoning](https://arxiv.org/abs/2603.19182)
*Zou Qiang*

Main category: cs.AI

TL;DR: 提出Box Maze框架，通过三层认知控制架构（记忆锚定、结构化推理、边界执行）提升LLM推理过程完整性，在对抗条件下将边界失效率从约40%降至1%以下。


<details>
  <summary>Details</summary>
Motivation: 当前LLM虽然生成能力强，但在对抗提示下容易产生幻觉和不可靠推理。现有安全方法（如RLHF和输出过滤）主要在行为层面操作，缺乏明确的架构机制来确保推理过程完整性。

Method: 提出Box Maze框架，将LLM推理分解为三个明确层次：记忆锚定层（确保信息基础）、结构化推理层（控制推理流程）、边界执行层（强制执行约束）。采用基于仿真的评估方法，在多个异构LLM系统（DeepSeek-V3、Doubao、Qwen）上进行渐进边界侵蚀场景测试。

Result: 在n=50个对抗场景中，明确的认知控制层提高了边界维护的一致性。架构约束将边界失效率从基线RLHF的约40%降至对抗条件下的1%以下。

Conclusion: 虽然当前验证基于仿真，但初步结果表明过程级控制为提高大型语言模型推理可靠性提供了有前景的方向。Box Maze框架展示了通过架构设计增强推理过程完整性的潜力。
...
### [222] [D5P4: Partition Determinantal Point Process for Diversity in Parallel Discrete Diffusion Decoding](https://arxiv.org/abs/2603.19146)
...
Method: 提出广义束搜索框架，支持并行生成候选序列和模块化束选择目标。具体实现D5P4，将选择步骤建模为确定性点过程的MAP推理，使用可扩展的贪心求解器，保持多GPU兼容性

Result: 在自由文本生成和问答任务上的实验表明，D5P4在保持竞争力的生成质量的同时，显著提升了多样性表现，优于现有基线方法

Conclusion: D5P4为离散扩散模型提供了有效的多样化解码框架，实现了模型概率与目标多样性之间的明确权衡，且计算开销几乎为零
...
### [223] [cuGenOpt: A GPU-Accelerated General-Purpose Metaheuristic Framework for Combinatorial Optimization](https://arxiv.org/abs/2603.19163)
*Yuyang Liu*

Main category: cs.AI

TL;DR: cuGenOpt是一个GPU加速的通用元启发式框架，通过CUDA架构、统一编码抽象和JIT编译等技术，在通用性、性能和易用性三个维度上取得突破。


<details>
  <summary>Details</summary>
Motivation: 解决组合优化问题中现有方法在通用性、性能和易用性之间的根本性权衡问题。物流、调度和资源分配等领域需要能够同时满足这三个维度的解决方案。

Method: 采用三层架构：1)引擎层："一个块演化一个解"的CUDA架构，包含统一编码抽象（排列、二进制、整数）、两级自适应算子选择机制和硬件感知资源管理；2)可扩展层：用户定义算子注册接口，允许注入特定问题的CUDA搜索算子；3)易用性层：JIT编译管道提供纯Python API，LLM建模助手将自然语言问题描述转换为可执行求解器代码。

Result: 在三个GPU架构（T4、V100、A800）上测试五个主题套件：1)比通用MIP求解器快几个数量级；2)在n=150实例上与专用求解器质量相当；3)TSP-442在30秒内达到4.73%间隙；4)十二种问题类型跨越五种编码变体达到最优解；5)框架级优化将pcb442间隙从36%降至4.73%，VRPTW吞吐量提升75-81%。

Conclusion: cuGenOpt成功解决了组合优化中通用性、性能和易用性的权衡问题，通过GPU加速元启发式框架实现了跨多个问题类型的高性能求解，为领域专家提供了易于使用的强大工具。
...
### [224] [OS-Themis: A Scalable Critic Framework for Generalist GUI Rewards](https://arxiv.org/abs/2603.19191)
...
TL;DR: OS-Themis是一个可扩展且准确的多智能体评论框架，通过将轨迹分解为可验证的里程碑来改进GUI智能体的强化学习奖励函数，在AndroidWorld实验中实现了10.3%的在线RL训练改进。


<details>
  <summary>Details</summary>
Motivation: 强化学习在随机环境中提升GUI智能体鲁棒性方面具有潜力，但训练对奖励函数质量高度敏感。现有奖励方法难以同时实现可扩展性和性能，需要一种更好的奖励评估框架。

Method: 提出OS-Themis多智能体评论框架，将轨迹分解为可验证的里程碑以隔离关键决策证据，并采用审查机制在做出最终裁决前严格审计证据链。同时引入OmniGUIRewardBench（OGRBench）作为跨平台GUI结果奖励的全面基准。

Result: 在AndroidWorld上的实验表明，OS-Themis用于支持在线RL训练时带来10.3%的改进，用于自训练循环中的轨迹验证和过滤时带来6.9%的增益。所有评估模型在OGRBench基准下使用OS-Themis都达到了最佳性能。

Conclusion: OS-Themis通过可验证的里程碑分解和严格的证据链审查，提供了一种可扩展且准确的奖励评估框架，能够有效驱动GUI智能体的进化，在强化学习训练中展现出显著性能提升。
